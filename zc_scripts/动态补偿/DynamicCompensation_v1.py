...
class DynamicCompensation(QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        header = ['勾选', '层名', "面铜", '最小线距(my)', '补偿值(my)']
        ...
        self.checkBox.clicked.connect(self.checkAll)
    ...
    def loadTable(self):
        ...
        for row, lay in enumerate(job.SignalLayers):
            ...
            self.tableWidget.setItem(row, 1, QTableWidgetItem(lay))
            edit = QLineEdit()
            edit.setValidator(QIntValidator())
            edit.rownum = row
            edit.setFont(QFont('微软雅黑', 10))
            edit.textChanged.connect(self.getCompensation)
            self.tableWidget.setCellWidget(row, 2, edit)

    def check(self):
        check_all = True
        for row in range(self.tableWidget.rowCount()):
            if not self.tableWidget.cellWidget(row, 0).isChecked():
                check_all = False
                break
        if check_all:
    ...
    def getCompensation(self):
        ...
        if cu_thickness < 12:
            ...
            min_spacing = 0
            compensation = 0
        self.tableWidget.setItem(rownum, 3, QTableWidgetItem(str(min_spacing)))
        self.tableWidget.setItem(rownum, 4, QTableWidgetItem(str(compensation)))

    def exec(self):
        flag = False
        for row in range(self.tableWidget.rowCount()):
            if self.tableWidget.cellWidget(row, 0).isChecked():
                if self.tableWidget.cellWidget(row, 2).text() == '':
                    QMessageBox.information(self, '提示', '请输入面铜')
                    return
                flag = True
                break
        if not flag:
        ...
        for row in range(self.tableWidget.rowCount()):
            if self.tableWidget.cellWidget(row, 0).isChecked():
                ...
                min_spacing = self.tableWidget.item(row, 3).text()
                compensation = self.tableWidget.item(row, 4).text()
                step.initStep()
                # 备份
                step.affect(layer)
                ...
                if lines_2:
                    ...
                    step.clearSel()
                step.unaffectAll()
        QMessageBox.information(self, 'tips', '已完成')
