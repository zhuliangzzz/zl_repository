...
class InputDxfTool(QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        self.setStyleSheet(
            '''QPushButton {background-color:rgb(49,194,124);color:white;} QPushButton:hover{background:#F7D674;color:black;}
            QListWidget{outline: 0px;border:0px;min-width: 200px;color:black;background:white;font:14px;}
            QComboBox{background:rgb(49,194,124);}
    ...
    def input(self):
        ...
        for input_file in input_files:
            ...
            if input_file.endswith('.dxf'):
                ...
                step.COM(f'input_manual_set,path={input_file},job={jobname},step={stepname},format=DXF,data_type=ascii,units=mm,coordinates=absolute,zeroes=none,nf1=2,nf2=4,decimal=no,separator=*,tool_units=inch,layer={layer},wheel=,wheel_template=,nf_comp=0,multiplier=1,text_line_width=0.06096,signed_coords=no,break_sr=yes,drill_only=no,merge_by_rule=no,threshold=200,resolution=3')
