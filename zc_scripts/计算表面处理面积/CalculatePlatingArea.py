...
def calculatePlatingArea():
    ...
    pnl_step.initStep()
    if job.SignalNum == 1:
        signal = job.SignalLayers[0]
        if signal in ('gtl', 'l1'):
            mix_top = 'mixtop'
        else:
            mix_top = 'mixbot'
        pnl_step.COM(
            f'exposed_area,layer1={signal},mask1={mix_top},layer2=,mask2=,mask_mode=or,drills=yes,consider_rout=no,ignore_pth_no_pad=no,drills_source=matrix,drills_list={";".join(drill_list)},thickness=0,resolution_value=25.4,x_boxes=3,y_boxes=3,area=no,dist_map=yes')
        area, percent = pnl_step.COMANS.split()
        text = f'AU:{"%.2f" % float(area)}SQ/MM({"%.2f" % float(percent)}%)'
        pnl_step.affect(signal)
