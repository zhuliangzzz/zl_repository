...
class Panel():
    ...
    def do_b2(self):
        if board.map_flag:
            ...
            for ab in self.abs:
                step.affect(ab)
            step.addPad(8, 10, 'r1505', 'negative')
            step.addPad(8, step.profile2.ymax - 10, 'r1505', 'negative')
            step.addPad(8, step.profile2.ymax - 16, 'r1505', 'negative')
            step.addPad(step.profile2.xmax - 8, step.profile2.ymax - 10, 'r1505', 'negative')
            step.addPad(step.profile2.xmax - 8, 10, 'r1505', 'negative')
            step.unaffectAll()
            step.affect(self.throughdrill)
            step.addPad(8, 10, 'r2000')
            step.addPad(8, step.profile2.ymax - 10, 'r2000')
            ...
            if self.self_drill:
                for drl in self.self_drill:
                    step.affect(drl)
                step.addPad(8, 10, 'r3800')
                step.addPad(8, step.profile2.ymax - 10, 'r3800')
                step.addPad(8, step.profile2.ymax - 16, 'r3800')
                step.addPad(step.profile2.xmax - 8, step.profile2.ymax - 10, 'r3800')
                step.addPad(step.profile2.xmax - 8, 10, 'r3800')
                step.unaffectAll()
            step.VON()
            step.affect('map')
            step.addPad(8, 10, 'sy-pnlb2')
    ...
    def do_x_ray(self):
        ...
        # subprocess.run(['python', 'D:/scripts/subScripts/update_symbol.py'])
        step.initStep()
    ...
    def do_wzmark(self):
        panelwzpx1 = 4
        panelwzpy1 = step.profile2.ymin + 7.5
        # panelwzpy1 = self.sr_ymin - 9 # 20221020
        panelwzpx2 = step.profile2.xmax - 4  # -9		#右下
        panelwzpy2 = step.profile2.ymin + 7.5
        # panelwzpy2 = self.sr_ymin - 9
        panelwzpx3 = 4  # 5		#左上
        panelwzpy3 = step.profile2.ymax - 28
        # panelwzpy3 = self.sr_ymax + 9
        panelwzpx4 = step.profile2.xmax - 4  # -9		#右上
