...
class Panel():
    ...
    def do_py(self):
        if board.map_flag:
            ...
            for signal in self.out_signals:
                step.affect(signal)
            step.addPad(6, step.profile2.ymin + 7, 'py', 'negative')
            step.addPad(6, step.profile2.ymax - 7, 'py', 'negative')
            step.addPad(step.profile2.xmax - 6, step.profile2.ymax - 10, 'py', 'negative')
            step.addPad(step.profile2.xmax - 6, step.profile2.ymin + 7, 'py', 'negative')
            step.unaffectAll()
            step.VOF()
            for cov in self.covs:
                step.affect(cov)
    ...
    def do_wzmark(self):
        ...
        v1 = 1000
        v2 = 550
        size = 550 * len(self.signalLays) + 800
        for layer in self.signalLays:
            step.affect(layer)
            symbol = 'donut_r%sx%s' % (v1, v2)
            v1 += 550
            v2 += 550
            if layer == 'l1':
                symbol = 'r1000'
            step.addPad(panelwzpx1, panelwzpy1, 'r%s' % size, 'negative')
            step.addPad(panelwzpx2, panelwzpy2, 'r%s' % size, 'negative')
            step.addPad(panelwzpx3, panelwzpy3, 'r%s' % size, 'negative')
            step.addPad(panelwzpx4, panelwzpy4, 'r%s' % size, 'negative')
