...
"""
   ...
   @software:PyCharm
   @desc:
   20240903 do_wzmark 按规则加层间对位pad
"""
import datetime
import os
import re
...
class MultiLayerBoard(QWidget, ui.Ui_Form):
    ...
    def load_position(self):
        ...
        if structure == '' or structure.endswith('+'):
            return
    ...
    def run(self):
        ...
        if self.structure == '' or self.structure.endswith('+'):
            QMessageBox.warning(self, '提示', '请检查叠构格式')
            return
...
class Panel():
    def __init__(self):
        ...
        self.abs = {}
        structure = board.comboBox_structure.currentText()
        struct_arr = [int(i) for i in structure.split('+')]
        i = 0
        layer_list = []
        ...
        # 外层线路
        self.out_signals = [self.signalLays[0], self.signalLays[-1]]
        self.throughdrill = 'f1' if step.isLayer('f1') else ''
        self.self_drill = [f'l{l}d' for l in filter(lambda i: step.isLayer(f'l{i}d'), range(1, len(self.signalLays) + 1))]
        self.blind_drill = list(filter(lambda i: re.match('via', i), self.drills))
        if layer_list:
            i = 0
            if len(board.signal_layers) % 2:
                for layer in layer_list:
                    top, bot = layer[0], layer[1]
                    if bot > len(self.signalLays) / 2 + 1:
                        name = 'bb' if i == 0 else f'bb{i}'
                        self.abs[name] = f'l{top}'
                    else:
                        name = 'aa' if i == 0 else f'aa{i}'
                        self.abs[name] = f'l{bot}'
            for layer in layer_list:
                drill = f'l{layer[0]}-{layer[1]}d'
                if step.isLayer(drill):
                    self.inner_drill[drill] = [f'l{layer[0]}', f'l{layer[1]}']
                else:
                    QMessageBox.warning(board, '提示', f'该叠构与钻孔不符')
                    sys.exit()
        # if not self.inner_drill:
        # 内层线路
        self.inner_signals = [self.signalLays[int(len(self.signalLays) / 2) - 1],
                                  self.signalLays[int(len(self.signalLays) / 2)]] if len(
        self.signalLays) % 2 == 0 else [self.signalLays[int(len(self.signalLays) / 2)]]
        # 次外层
        self.second_signals = [s for s in self.signalLays[1: -1] if s not in self.inner_signals]
        # print(self.inner_drill)
        # print(self.throughdrill)
        # print(self.self_drill)
        # print(self.blind_drill)
        # step.PAUSE('A')

    @classmethod
    def make_map_layer(cls):
        abs = {}
        structure = board.comboBox_structure.currentText()
        if structure == '' or structure.endswith('+'):
            QMessageBox.warning(board, '提示', '请检查叠构格式')
            return
        struct_arr = [int(i) for i in structure.split('+')]
        i = 0
        layer_list = []
        ...
        if layer_list:
            ...
            if len(board.signal_layers) % 2:
                for layer in layer_list:
                    top, bot = layer[0], layer[1]
                    if bot > len(board.signal_layers) / 2 + 1:
                        name = 'bb' if i == 0 else f'bb{i}'
                        abs[name] = f'l{top}'
                    else:
                        name = 'aa' if i == 0 else f'aa{i}'
        ...
        # ldi
        yt = step.profile2.ymax - 9
        yb = step.profile2.ymin + 7.5
        step.addPad(28.4, yb, 'ldi-mark-t')
        step.addPad(28.4, yt, 'ldi-mark-t')
        step.addPad(step.profile2.xmax - 31, yt, 'ldi-mark-t')
        step.addPad(step.profile2.xmax - 29, yb, 'ldi-mark-t')
        for signal in board.signal_layers:
            for ab, signal_ in abs.items():
                if signal == signal_:
                    yt -= 2.5
                    yb += 2.5
                    step.addPad(28.4, yb, 'nc-ldi-t', angle=180)
                    step.addPad(28.4, yt, 'nc-ldi-t')
                    step.addPad(step.profile2.xmax - 31, yt, 'nc-ldi-t')
                    step.addPad(step.profile2.xmax - 29, yb, 'nc-ldi-t', angle=180)
                    break
        # lpi
        step.addPad(36, 9, 'lpi-3.0')
        step.addPad(36, step.profile2.ymax - 9, 'lpi-2.5')
        step.addPad(step.profile2.xmax - 35.5, step.profile2.ymax - 9, 'lpi-2.5')
        step.addPad(step.profile2.xmax - 33, 9, 'lpi-2.5')
        # qp
        # 左右
        step.addPad(4.7, step.profile2.ymax - 75.5, 'qp', angle=90)
        step.addPad(4.7, step.profile2.ymin + 53, 'qp', angle=90)
        step.addPad(step.profile2.xmax - 4.7, step.profile2.ymax - 61, 'qp', angle=270)
        step.addPad(step.profile2.xmax - 4.7, 57, 'qp', angle=270)
        ...
        step.addPad(xb1, 6.5, 'donut_r2000x1100')
        step.addPad(xb1 + 3, 6.5, 'donut_r2000x1200')
        step.addPad(xb1 + 6, 6.5, 'donut_r2000x1300')
        step.addPad(xb3, 6.5, 'donut_r2000x1100')
        step.addPad(xb3 + 3, 6.5, 'donut_r2000x1200')
        step.addPad(xb3 + 6, 6.5, 'donut_r2000x1300')
        step.addPad(xt1, step.profile2.ymax - 6.5, 'donut_r2000x1100')
        step.addPad(xt1 + 3, step.profile2.ymax - 6.5, 'donut_r2000x1200')
        step.addPad(xt1 + 6, step.profile2.ymax - 6.5, 'donut_r2000x1300')
        step.addPad(xt3, step.profile2.ymax - 6.5, 'donut_r2000x1100')
        step.addPad(xt3 + 3, step.profile2.ymax - 6.5, 'donut_r2000x1200')
        step.addPad(xt3 + 6, step.profile2.ymax - 6.5, 'donut_r2000x1300')
        y = step.sr2.ymax + 2.5
        step.addPad(106.5, y, 'donut_s3000x2000', nx=3, dx=5000)
        step.addPad(step.profile2.xmax - 93.5, y, 'donut_s3000x2000', nx=3, dx=5000)
        # 20240912 板边下方一组
        y = step.sr2.ymin - 2.5
        step.addPad(106.5, y, 'donut_s3000x2000', nx=3, dx=5000)
        step.addPad(step.profile2.xmax - 93.5, y, 'donut_s3000x2000', nx=3, dx=5000)
        ...
        # py
        step.addPad(6, step.profile2.ymin + 7, 'py')
        step.addPad(6, step.profile2.ymax - 7, 'py')
        step.addPad(step.profile2.xmax - 6, step.profile2.ymax - 10, 'py')
        step.addPad(step.profile2.xmax - 6, step.profile2.ymin + 7, 'py')
        # 2d
        step.addPad(97.5, step.profile2.ymax - 8, '2d')
        step.addPad(step.profile2.xmax - 102.5, step.profile2.ymax - 8, '2d')
        # scale
        x, rectx = 89, -3.5
        step.addPad(x + rectx, step.sr2.ymax + 3.5, 'rect12400x1500')
        step.addPad(x, step.sr2.ymax + 3.5, 'x-scale', 'negative')
        step.addPad(x + rectx, step.sr2.ymax + 1.5, 'rect12400x1500')
        step.addPad(x, step.sr2.ymax + 1.5, 'y-scale', 'negative')
        # 子唛和lot
        ...
        step.addText(step.profile2.xcenter + 20, step.profile2.ymax - 7.7, f'{sign} F1', 3.6068, 4.2164,1.64041996, fontname='canned_57')
        step.addPad(135, step.profile2.ymin + 5.5, 'lot-drill')
        # b2
        step.addPad(9.5, 10, 'sy-pnlb2')
        step.addPad(9.5, step.profile2.ymax - 10, 'sy-pnlb2')
        step.addPad(9.5, step.profile2.ymax - 16, 'sy-pnlb2')
        step.addPad(step.profile2.xmax - 9.5, step.profile2.ymax - 10, 'sy-pnlb2')
        step.addPad(step.profile2.xmax - 9.5, 10, 'sy-pnlb2')
        # x-ray
        ...
        sym = 'fpjsk'
        step.addPad(31, step.profile2.ymax - 5, sym)
        step.addPad(54, step.profile2.ymin + 5, sym)
        step.addPad(step.profile2.xmax - 63, step.profile2.ymax - 5, sym)
        step.addPad(step.profile2.xmax - 46, step.profile2.ymin + 5, sym)
        step.unaffectAll()
            ...
            if len(self.signalLays) % 2:
                for layer in layer_list:
                    ...
                    if bot > len(self.signalLays)/2 + 1:
                        ...
                        name = 'aa' if i == 0 else f'aa{i}'
                        self.abs[name] = f'l{bot}'
                    if step.isLayer(name):
                        step.removeLayer(name)
                    step.createLayer(f'{name}', ins_layer=f'l{bot}')
                    i += 1
    # 铺铜
    def do_surface(self):
    ...
    def do_sy(self):
        if board.map_flag:
            ...
            if step.Selected_count():
                ...
                for signal in self.out_signals:
                ...
                step.copySel(self.throughdrill)
                step.unaffectAll()
                step.VON()
            step.unaffectAll()
            step.truncate(self.tmp_map)
    ...
    def do_component(self):
        if board.map_flag:
            ...
            if step.Selected_count():
                ...
                step.affect(self.tmp_map)
                for signal in self.signalLays:
                    if board.type_name == '软硬结合板':
                        if signal in self.out_signals:
                            step.copySel(signal)
                        else:
            ...
            for signal in self.signalLays + ['map']:
                if board.type_name == '软硬结合板' and signal in self.out_signals:
                    step.affect(signal)
                    step.addPad(25, 3.5, 'jsk')
                    step.addPad(50, step.profile2.ymax - 3.5, 'jsk')
                    step.addPad(step.profile2.xmax - 48, step.profile2.ymax - 3.5, 'jsk')
    ...
    def do_fxjt(self):
        for signal in self.signalLays:
            ...
            step.affect(signal)
            step.addPad(63, 3.5, 'fxjt', 'negative')
            step.addPad(74, 3.5, 'xlfp', 'negative', mirror=mir)
            step.unaffectAll()
            if signal == self.out_signals[0]:
                dk = 'dkt'
            elif signal == self.out_signals[-1]:
                dk = 'dkb'
            else:
                dk = None
            if dk and step.isLayer(dk):
                step.affect(dk)
                step.addPad(63, 3.5, 'fxjt', 'negative')
                step.addPad(74, 3.5, 'dkzp', 'negative', mirror=mir)
                step.unaffectAll()
        step.VOF()
        for wz in self.wzs:
            step.affect(wz)
        step.addPad(63, 3.5, 'fxjt')
        step.addPad(74, 3.5, 'zf')
        step.unaffectAll()
        for yy in self.yys:
            step.affect(yy)
            mir = 'no' if int(yy.replace('yy', '')) % 2 else 'yes'
            step.affect(yy)
            step.addPad(63, 3.5, 'fxjt')
            step.addPad(74, 2.5, 'zhyy', mirror=mir)
            step.unaffectAll()
        for zh in self.zhs:
            mir = 'yes' if 'b' in zh else 'no'
            step.affect(zh)
            step.addPad(63, 3.5, 'fxjt', 'negative')
            step.addPad(74, 2.5, 'zhfh', 'negative', mirror=mir)
            step.unaffectAll()
        step.VON()
    ...
    def do_film(self):
        if board.map_flag:
            ...
            if step.Selected_count():
                ...
                for signal in self.signalLays:
                    ...
                    if step.isLayer(dk):
                        step.copySel(dk, 'yes')
                for ab in self.abs:
                    signal = self.abs.get(ab)
                    step.selectChange('base_film_top') if board.signal_dict.get(signal) == 'b' else step.selectChange('base_film_bot')
                    step.copySel(ab)
                step.selectChange('r3000')
                for drl in self.inner_drill:
                    step.copySel(drl)
                step.copySel(self.throughdrill)
                step.unaffectAll()
            step.unaffectAll()
            ...
            step.affect(self.throughdrill)
            sym = 'r3000'
            step.addPad(step.profile2.xmin + 5, step.profile2.ymax - 88, sym)
            step.addPad(step.profile2.xmin + 5, step.profile2.ymin + 73, sym)
            step.addPad(step.profile2.xmax - 5, step.profile2.ymax - 72, sym, angle=180)
            ...
            for ab in self.abs:
                signal = self.abs.get(ab)
                step.affect(ab)
                sym = 'base_film_top' if board.signal_dict.get(signal) == 'b' else 'base_film_bot'
                step.addPad(step.profile2.xmin + 5, step.profile2.ymax - 89, sym)
                step.addPad(step.profile2.xmin + 5, step.profile2.ymin + 73, sym)
                step.addPad(step.profile2.xmax - 5, step.profile2.ymax - 73, sym, angle=180)
                step.addPad(step.profile2.xmax - 5, step.profile2.ymin + 73, sym, angle=180)
    ...
    def do_wz(self):
        ...
        for signal in self.signalLays:
            ...
            if board.fill_sur != 'surface':
                ...
                step.affect('zll')
                step.addPad(x1, 2.5, 'zc-job2', 'positive', mirror=mir)
                step.selectBreak()
                step.unaffectAll()
                # 加背景实铜
            ...
            if step.isLayer(dk):
                step.affect(dk)
            step.addPad(x1, 2.5, 'zc-job2', 'negative', mirror=mir)
        ...
        for wz in self.wzs:
            ...
            if 'b' in wz:
                mir = 'yes'
                x1, x2 = step.profile2.xmax - 50, step.profile2.xmax - 100
            step.addPad(x1, 2.5, 'zc-job2', mirror=mir)
            step.addText(x2, 2.5, text, 2, 2.3, 0.989993453, mir, fontname='simplex')
            step.unaffectAll()
        for zh in self.zhs:
            step.affect(zh)
    ...
    def do_py(self):
        if board.map_flag:
            ...
            if step.Selected_count():
                ...
                step.copySel(self.wz_map)
                for signal in self.out_signals:
                    step.copySel(signal) if board.type_name == '软硬结合板' else step.copySel(signal, 'yes')
            ...
            polarity = 'positive' if board.type_name == '软硬结合板' else 'negative'
            for signal in self.out_signals:
                step.affect(signal)
            step.addPad(6, step.profile2.ymin + 7, 'py', polarity)
            step.addPad(6, step.profile2.ymax - 7, 'py', polarity)
            step.addPad(step.profile2.xmax - 6, step.profile2.ymax - 10, 'py', polarity)
            step.addPad(step.profile2.xmax - 6, step.profile2.ymin + 7, 'py', polarity)
            step.unaffectAll()
            step.VOF()
            for cov in self.covs:
                step.affect(cov)
            step.addPad(6, step.profile2.ymin + 7, 'r2200')
            step.addPad(6, step.profile2.ymax - 7, 'r2200')
            step.addPad(step.profile2.xmax - 6, step.profile2.ymax - 10, 'r2200')
            step.addPad(step.profile2.xmax - 6, step.profile2.ymin + 7, 'r2200')
            step.unaffectAll()
            for dk in self.dks:
                step.affect(dk)
            step.addPad(6, step.profile2.ymin + 7, 'r2501', 'negative')
            step.addPad(6, step.profile2.ymax - 7, 'r2501', 'negative')
            step.addPad(step.profile2.xmax - 6, step.profile2.ymax - 10, 'r2501', 'negative')
            step.addPad(step.profile2.xmax - 6, step.profile2.ymin + 7, 'r2501', 'negative')
            step.unaffectAll()
            step.affect('map')
            step.affect(self.wz_map)
            step.addPad(6, step.profile2.ymin + 7, 'py')
            ...
            for wz in self.wzs:
                step.affect(wz)
            step.addAttr_zl('.out_flag,int=233')
            step.addPad(6, step.profile2.ymin + 7, 'r0', attributes='yes')
            step.addPad(6, step.profile2.ymax - 7, 'r0', attributes='yes')
            step.addPad(step.profile2.xmax - 6, step.profile2.ymax - 10, 'r0', attributes='yes')
            step.addPad(step.profile2.xmax - 6, step.profile2.ymin + 7, 'r0', attributes='yes')
            step.unaffectAll()
            step.VON()
    ...
    def do_ldi(self):
        if board.map_flag:
            ...
            step.truncate(self.tmp_map)
            step.affect('map')
            step.selectSymbol('nc-ldi-t')
            if step.Selected_count():
                step.copySel(self.tmp_map)
                step.unaffectAll()
                step.affect(self.tmp_map)
                for ab, signal_ in self.abs.items():
                    sym = 'nc-ldi-b' if board.signal_dict.get(signal_) == 't' else 'nc-ldi-t'
                    step.selectChange(sym)
                    step.copySel(ab, 'yes')
                    step.selectChange('nc-ldi-t') if board.signal_dict.get(signal_) == 't' else step.selectChange(
                        'nc-ldi-b')
                    step.copySel(signal_, 'yes')
                step.unaffectAll()
            step.unaffectAll()
            step.truncate(self.tmp_map)
        else:
            for signal in self.signalLays:
                if signal in self.out_signals:
                    ...
                    polarity = 'positive' if board.type_name == '软硬结合板' else 'negative'
                    step.addPad(28.4, yb, sym, polarity)
                    step.addPad(28.4, yt, sym, polarity)
                    step.addPad(step.profile2.xmax - 31, yt, sym, polarity)
                    step.addPad(step.profile2.xmax - 29, yb, sym, polarity)
                    step.unaffectAll()
                    dk = 'dkt' if signal == 'l1' else 'dkb'
                    if step.isLayer(dk):
                        step.affect(dk)
                    step.addPad(28.4, yb, sym, 'negative')
                    step.addPad(28.4, yt, sym, 'negative')
                    step.addPad(step.profile2.xmax - 31, yt, sym, 'negative')
                    step.addPad(step.profile2.xmax - 29, yb, sym, 'negative')
                    step.unaffectAll()
                    step.affect('map')
                    step.addPad(28.4, yb, 'ldi-mark-t')
                    step.addPad(28.4, yt, 'ldi-mark-t')
                    ...
                    for ab, signal_ in self.abs.items():
                        if signal == signal_:
                            ...
                            yb += 2.5
                            step.affect(ab)
                            sym = 'nc-ldi-b' if board.signal_dict.get(signal) == 't' else 'nc-ldi-t'
                            step.addPad(28.4, yb, sym, 'negative', angle=180)
                            step.addPad(28.4, yt, sym, 'negative')
                            step.addPad(step.profile2.xmax - 31, yt, sym, 'negative')
                            step.addPad(step.profile2.xmax - 29, yb, sym, 'negative', angle=180)
                            step.unaffectAll()
                            step.affect(signal)
                            sym = 'nc-ldi-t' if board.signal_dict.get(signal) == 't' else 'nc-ldi-b'
                            step.addPad(28.4, yb, sym, 'negative', angle=180)
                            step.addPad(28.4, yt, sym, 'negative')
                            step.addPad(step.profile2.xmax - 31, yt, sym, 'negative')
                            step.addPad(step.profile2.xmax - 29, yb, sym, 'negative', angle=180)
            ...
            for drill in self.drills:
                step.affect(drill)
            step.addPad(28.4, step.profile2.ymin + 7.5, 'r1000')
            step.addPad(28.4, step.profile2.ymax - 7.5, 'r1000')
            step.addPad(step.profile2.xmax - 31, step.profile2.ymax - 7.5, 'r1000')
            step.addPad(step.profile2.xmax - 29, step.profile2.ymin + 7.5, 'r1000')
            step.unaffectAll()
            step.VON()
    ...
    def do_lpi(self):
        if board.type_name == '软硬结合板':
            for signal in self.signalLays:
                step.affect(signal)
                if signal in self.out_signals:
                    step.addPad(36, 9, 'lpi-3.0', 'negative')
                    ...
                    if board.fill_sur != 'surface':
                        step.addPad(36, 9, 'r4502')
                        step.addPad(36, step.profile2.ymax - 9, 'r4502')
                        step.addPad(step.profile2.xmax - 35.5, step.profile2.ymax - 9, 'r4502')
                        step.addPad(step.profile2.xmax - 33, 9, 'r4502')
                step.unaffectAll()
        else:
            for signal in self.signalLays:
                step.affect(signal)
        ...
        for zh in self.zhs:
            step.affect(zh)
        step.addPad(36, 9, 'r4400')
        step.addPad(36, 9, 'donut_r5000x1000', 'negative')
        ...
        for dk in self.dks:
            step.affect(dk)
        step.addPad(36, 9, 'r4700', 'negative')
    ...
    def do_qp(self):
        if board.map_flag:
            ...
            for dk in self.dks:
                step.affect(dk)
            # 左右
            step.addPad(4.7, step.profile2.ymax - 75.5, 'qp-xl', angle=90)
            step.addPad(4.7, step.profile2.ymin + 53, 'qp-xl', angle=90)
            step.addPad(step.profile2.xmax - 4.7, step.profile2.ymax - 61, 'qp-xl', angle=270)
            step.addPad(step.profile2.xmax - 4.7, 57, 'qp-xl', angle=270)
            step.unaffectAll()
            step.affect('map')
            # 左右
            step.addPad(4.7, step.profile2.ymax - 75.5, 'qp', angle=90)
            ...
            for drill in self.drills:
                step.affect(drill)
            # 左右
            step.addPad(4.7, step.profile2.ymax - 75.5, 'qp-f', angle=90)
            step.addPad(4.7, step.profile2.ymin + 53, 'qp-f', angle=90)
            step.addPad(step.profile2.xmax - 4.7, step.profile2.ymax - 61, 'qp-f', angle=270)
            step.addPad(step.profile2.xmax - 4.7, 57, 'qp-f', angle=270)
            step.unaffectAll()

    def do_donut(self):
        if board.map_flag:
            step.affect('map')
            step.selectSymbol('donut_r*')
            if step.Selected_count():
                step.copySel(self.tmp_map)
                step.unaffectAll()
                step.affect(self.tmp_map)
            ...
            # donut_s
            step.affect('map')
            step.selectSymbol('donut_s*')
            if step.Selected_count():
                step.copySel(self.tmp_map)
                step.unaffectAll()
                step.affect(self.tmp_map)
                step.selectChange('s3200')
                for signal in self.signalLays:
                    step.copySel(signal)
                step.unaffectAll()
                step.resetFilter()
            ...
            for zh in self.zhs:
                step.affect(zh)
            step.addPad(xb1, 6.5, 'r1000', 'negative', nx=3, dx=3000)
            step.addPad(xb3, 6.5, 'r1000', 'negative', nx=3, dx=3000)
            step.addPad(xt1, step.profile2.ymax - 6.5, 'r1000', 'negative', nx=3, dx=3000)
            step.addPad(xt3, step.profile2.ymax - 6.5, 'r1000', 'negative', nx=3, dx=3000)
            step.unaffectAll()
            for cov in self.covs:
                step.affect(cov)
            ...
            if self.covs:
                for cov in self.covs:
                    if cov == 'c1':
                        ...
                        #20240912 板边下方第二组
                        y = step.sr2.ymin - 2.5 if board.signal_dict.get(self.signalLays[0]) == 't' else step.sr2.ymin - 6
                        step.addPad(106.5, y, 'r2500', nx=3, dx=5000)
                        step.addPad(step.profile2.xmax - 93.5, y, 'r2500', nx=3, dx=5000)
                        step.unaffectAll()
                    else:
                        ...
                        step.addPad(step.profile2.xmax - 93.5, y, 'r2500', nx=3, dx=5000)
                        # 20240912 板边下方第二组
                        y = step.sr2.ymin - 2.5 if board.signal_dict.get(self.signalLays[-1]) == 't' else step.sr2.ymin - 6
                        step.addPad(106.5, y, 'r2500', nx=3, dx=5000)
                        step.addPad(step.profile2.xmax - 93.5, y, 'r2500', nx=3, dx=5000)
                        step.unaffectAll()
            step.affect('map')
    ...
    def do_ws(self):
        if board.map_flag:
            ...
            if step.Selected_count():
                ...
                for signal in self.signalLays:
                    step.copySel(signal)
                step.selectChange('rect4200x12200')
                for dk in self.dks:
                    step.copySel(dk, 'yes')
                step.selectChange('r3000')
                for cov in self.covs:
                    step.copyToLayer(cov, dy=-3)
                    step.copyToLayer(cov, dy=1.7)
                step.selectChange('r4500')
                for yy in self.yys:
                    step.copyToLayer(yy, dy=-3)
                    step.copyToLayer(yy, dy=1.7)
                step.selectChange('r3500')
                for zh in self.zhs:
                    step.copyToLayer(zh, 'yes', dy=-3)
                    step.copyToLayer(zh, 'yes', dy=1.7)
                step.unaffectAll()
            step.unaffectAll()
            step.resetFilter()
            step.truncate(self.tmp_map)
            ...
            for zh in self.zhs:
                step.affect(zh)
            step.addPad(3.5, step.profile2.ymax - 65.7, 'r3500', 'negative')  # 线宽线距symbol
            step.addPad(3.5, step.profile2.ymax - 61, 'r3500', 'negative')  # 线宽线距symbol
            step.addPad(step.profile2.xmax - 3.5, step.profile2.ymin + 88.7, 'r3500', 'negative')  # 线宽线距symbol
            step.addPad(step.profile2.xmax - 3.5, step.profile2.ymin + 84, 'r3500', 'negative')  # 线宽线距symbol
            step.unaffectAll()
            for cov in self.covs:
                step.affect(cov)
            ...
            step.affect('map')
            step.addPad(3.5, step.profile2.ymax - 64, '1-ws')  # 线宽线距symbol
            step.addPad(step.profile2.xmax - 3.5, step.profile2.ymin + 87, '1-ws', angle=180)
            step.unaffectAll()
    ...
    def do_b2(self):
        if board.map_flag:
            ...
            if step.Selected_count():
                ...
                for signal in self.signalLays:
                    if signal in self.inner_signals:
                        ...
                        step.copySel(signal, 'yes')
                step.selectChange('r1505')
                for ab in self.abs:
                    step.copySel(ab, 'yes')
                step.selectChange('r2000')
                step.copySel(self.throughdrill)
                if self.self_drill:
                    step.selectChange('r3800')
                    for drl in self.self_drill:
                        step.copySel(drl)
                step.unaffectAll()
            step.unaffectAll()
            step.resetFilter()
            ...
            for signal in self.signalLays:
                ...
                if signal in self.inner_signals:
                    if signal == self.inner_signals[0]:
                        ...
                        step.addPad(9.5, 10, 'r2000')
                        step.addPad(9.5, 10, 'r1801', 'negative')
                        step.addPad(9.5, step.profile2.ymax - 10, 'r2000')
                        step.addPad(9.5, step.profile2.ymax - 10, 'r1801', 'negative')
                        step.addPad(9.5, step.profile2.ymax - 16, 'r2000')
                        step.addPad(9.5, step.profile2.ymax - 16, 'r1801', 'negative')
                        step.addPad(step.profile2.xmax - 9.5, step.profile2.ymax - 10, 'r2000')
                        step.addPad(step.profile2.xmax - 9.5, step.profile2.ymax - 10, 'r1801', 'negative')
                        step.addPad(step.profile2.xmax - 9.5, 10, 'r2000')
                        step.addPad(step.profile2.xmax - 9.5, 10, 'r1801', 'negative')
                else:
                    step.addPad(9.5, 10, 'wcb2', 'negative')
                    step.addPad(9.5, step.profile2.ymax - 10, 'wcb2', 'negative')
                    step.addPad(9.5, step.profile2.ymax - 16, 'wcb2', 'negative')
                    step.addPad(step.profile2.xmax - 9.5, step.profile2.ymax - 10, 'wcb2', 'negative')
                    step.addPad(step.profile2.xmax - 9.5, 10, 'wcb2', 'negative')
                step.unaffectAll()
            step.VOF()
            for ab in self.abs:
                step.affect(ab)
            step.addPad(9.5, 10, 'r1505', 'negative')
            step.addPad(9.5, step.profile2.ymax - 10, 'r1505', 'negative')
            step.addPad(9.5, step.profile2.ymax - 16, 'r1505', 'negative')
            step.addPad(step.profile2.xmax - 9.5, step.profile2.ymax - 10, 'r1505', 'negative')
            step.addPad(step.profile2.xmax - 9.5, 10, 'r1505', 'negative')
            step.unaffectAll()
            step.affect(self.throughdrill)
            step.addPad(9.5, 10, 'r2000')
            step.addPad(9.5, step.profile2.ymax - 10, 'r2000')
            ...
            if self.self_drill:
                for drl in self.self_drill:
                    step.affect(drl)
                step.addPad(9.5, 10, 'r3800')
                step.addPad(9.5, step.profile2.ymax - 10, 'r3800')
                step.addPad(9.5, step.profile2.ymax - 16, 'r3800')
                step.addPad(step.profile2.xmax - 9.5, step.profile2.ymax - 10, 'r3800')
                step.addPad(step.profile2.xmax - 9.5, 10, 'r3800')
                step.unaffectAll()
            step.VON()
            step.affect('map')
            step.addPad(9.5, 10, 'sy-pnlb2')
    ...
    def do_x_ray(self):
        if board.map_flag:
            ...
            step.unaffectAll()
        # 修改x-ray
        # os.system('D:/myfiles/genesisPro/genesisPro/zc_scripts/多层板/修改symbol/update_symbol.py')
        # subprocess.run(['python', 'D:/myfiles/genesisPro/genesisPro/zc_scripts/多层板/修改symbol/update_symbol.py'])
        subprocess.run(['python', '//172.16.0.106/scripts/subScripts/update_symbol.py'])
        step.initStep()

    def do_fpjsk(self):
        if board.map_flag:
            step.affect('map')
            step.selectSymbol('fpjsk')
            if step.Selected_count():
                step.copySel(self.tmp_map)
                step.unaffectAll()
                step.affect(self.tmp_map)
                for signal in self.signalLays:
                    sym = 'fpjsk' if signal in self.inner_signals else 'wcjsk'
                    step.selectChange(sym)
                    step.copySel(signal, 'yes')
                step.selectChange('wcjsk')
                for ab in self.abs:
                    step.copySel(ab, 'yes')
                if self.self_drill:
                    step.selectBreak()
            ...
            for lay in self.signalLays:
                sym = 'fpjsk' if lay in self.inner_signals else 'wcjsk'
                step.affect(lay)
                step.addPad(31, step.profile2.ymax - 5, sym, 'negative')
                step.addPad(54, step.profile2.ymin + 5, sym, 'negative')
                step.addPad(step.profile2.xmax - 63, step.profile2.ymax - 5, sym, 'negative')
                step.addPad(step.profile2.xmax - 46, step.profile2.ymin + 5, sym, 'negative')
                step.unaffectAll()
            sym = 'wcjsk'
            for ab in self.abs:
                step.affect(ab)
    ...
    # 层间对位pad
    # 从线路层最后一层往上 r800 -> donut_1300x900 -> donut_1800x1400 ...   背景为r6500+r6000（负性）
    def do_wzmark(self):
        panelwzpx1 = 9.5
        panelwzpy1 = step.profile2.ymin + 17.5
        # panelwzpy1 = self.sr_ymin - 9 # 20221020
        panelwzpx2 = step.profile2.xmax - 9.5  # -9		#右下
        panelwzpy2 = step.profile2.ymin + 17.5
        # panelwzpy2 = self.sr_ymin - 9
        panelwzpx3 = 9.5  # 5		#左上
        panelwzpy3 = step.profile2.ymax - 28
        # panelwzpy3 = self.sr_ymax + 9
        panelwzpx4 = step.profile2.xmax - 9.5  # -9		#右上
        panelwzpy4 = step.profile2.ymax - 23
        v1 = 6500
        v2 = 6000
        size = 800
        for i in range(len(self.signalLays)):
            layer = self.signalLays[len(self.signalLays) - 1 - i]
            symbol = f'donut_r{size + 500 * i}x{size + 100 + 500 * (i - 1)}'
            if i == 0:
                symbol = f'r{size}'
            step.affect(layer)
            step.addPad(panelwzpx1, panelwzpy1, f'r{v1}')
            step.addPad(panelwzpx1, panelwzpy1, f'r{v2}', 'negative')
            step.addPad(panelwzpx2, panelwzpy2, f'r{v1}')
            step.addPad(panelwzpx2, panelwzpy2, f'r{v2}', 'negative')
            step.addPad(panelwzpx3, panelwzpy3, f'r{v1}')
            step.addPad(panelwzpx3, panelwzpy3, f'r{v2}', 'negative')
            step.addPad(panelwzpx4, panelwzpy4, f'r{v1}')
            step.addPad(panelwzpx4, panelwzpy4, f'r{v2}', 'negative')

            step.addPad(panelwzpx1, panelwzpy1, symbol)
            step.addPad(panelwzpx2, panelwzpy2, symbol)
            step.addPad(panelwzpx3, panelwzpy3, symbol)
            step.addPad(panelwzpx4, panelwzpy4, symbol)
            step.unaffectAll()
    ...
    def do_cov(self):
        ...
        for cov in self.covs:
            mir = 'no'
            x = 70
            if board.signal_dict.get(cov.replace('c', 'l')) == 'b':
                mir = 'yes'
                x = 78
        ...
        for name, xmin, xmax, ymin, ymax in zip(step.info2.get('gREPEATstep'),
            ...
            repeat_xy.append([xmin, xmax, ymin, ymax])
            if 'set' in name:
                repeat_set_xy.append([xmin, xmax, ymin, ymax])
        # 20240723 排除set两边的coupon影响
        set_x_min, set_x_max = min(map(lambda x: x[0], repeat_set_xy)), max(map(lambda x: x[1], repeat_set_xy))
        actual_ty = []
        # 取第一列的set的y值
        for xy in repeat_xy:
            tx, ty_min, ty_max = xy[0], xy[2], xy[3]
            if set_x_min == tx:
                actual_ty.append(ty_min)
                actual_ty.append(ty_max)
        repeat_ty = sorted(actual_ty)
        repeat_set_y = list(map(lambda x: x[2], repeat_set_xy)) + list(map(lambda x: x[3], repeat_set_xy))
        # 覆盖膜加切线和序号
            ...
            if len(cut_ys):
                ...
                for cov in self.covs:
                    ...
                    for y in cut_ys:
                        ...
                        i -= 1
        for cov in self.covs:
            signal = cov.replace('c','l')
            tmp_cov = f'{cov}+++{step.pid}'
            step.affect(tmp_cov)
            # step.Contourize(0)
            # 把覆盖膜层名和数字分开转surface
            step.setFilterTypes('text')
            step.setTextFilter('C*')
            step.selectAll()
            step.resetFilter()
            step.Contourize(0)
            step.setFilterTypes('text')
            ...
            # 加覆盖膜对位孔
            step.affect(cov)
            for n, ty in enumerate(repeat_ty):
                if n % 2:
    ...
    def do_drill_letter(self):
        if board.map_flag:
            ...
            for drill in self.drills:
                step.affect(drill)
                step.setFilterTypes('text')
                step.setTextFilter(f'{sign}*')
                step.selectAll()
                step.resetFilter()
                if step.Selected_count():
                    step.changeText(f'{sign} {drill.upper()}')
                step.unaffectAll()
        else:
            sign = jobname[jobname.rfind("-") - 7:jobname.rfind("-")] if "-" in jobname else jobname[-7:-1]
            for drill in self.drills:
        ...
        for drill in self.drills:
            step.affect(drill)
        step.addPad(step.profile2.xmax - 19, step.profile2.ymax - 5, 'r1000', nx=5, dx=3000)
        step.unaffectAll()
        step.VON()
    ...
    def do_scale(self):
        if board.map_flag:
            step.affect('map')
            step.selectSymbol('rect12400x1500')
            step.resetFilter()
            if step.Selected_count():
                step.copySel(self.tmp_map)
                step.unaffectAll()
            ...
            step.truncate(self.tmp_map)
            step.affect('map')
            step.selectSymbol('x-scale;y-scale')
            step.resetFilter()
            if step.Selected_count():
                step.copySel(self.tmp_map)
                step.unaffectAll()
    ...
    def do_border(self):
        ...
        for dk in self.dks:
            ...
            step.unaffectAll()
            step.affect(tmp)
            signal = self.out_signals[0] if dk == 'dkt' else self.out_signals[-1]
            if board.signal_dict.get(signal) == 'b':
