...
class ScriptsSet(QMainWindow, ui.Ui_MainWindow):
    def __init__(self):
        ...
        self.config_file = 'cam_config.xml'
        self.setupUi(self)
        self.render()
    ...
    def get_script_hashmap(self):
        hashmap = {}
        with open('D:/scripts/cam_config.xml', encoding='utf-8') as r:
            content = r.read()
    ...
    def render(self):
        ...
        total = 0
        for module in self.script_map:
            count = len(self.script_map.get(module))
            ...
            item.flag = 'module'
            total += count
            for name, path in self.script_map.get(module):
