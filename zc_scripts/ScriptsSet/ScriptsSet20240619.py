...
class ScriptsSet(QMainWindow, ui.Ui_MainWindow):
    ...
    def getModules(self):
        ...
        sql = 'select a.id,module_name,count(1) from fpc_script_module a left join fpc_script_file b on a.id = b.module_id GROUP BY module_id'
    ...
    def run(self, item):
        stepname = self.stepSelect.currentText()
        if item.flag == 'module':
            return
