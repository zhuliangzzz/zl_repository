        ...
        if self.__type == '菲林Gbr':
            ...
            for row in range(self.tableWidget.rowCount()):
                ...
                if self.tableWidget.cellWidget(row,0).isChecked():
                    ...
                    step.layers.get(layer).setGenesisAttr('.out_y_scale', y_scale)
                    # 20240905 merge
                    step.COM(f'tools_merge_ex,layer={layer},mode=merge')
                    step.COM(f'ncset_cur,job={jobname},step={step.name},layer={layer},ncset={layer}')
                    ...
                    step.COM(f'ncd_ncf_export,stage=1,split=1,dir={path + "/DRILL"},name={prefix}{layer}.drl')
            ...
            for row in range(self.tableWidget.rowCount()):
                ...
                if self.tableWidget.cellWidget(row,0).isChecked():
                    ...
                    step.COM(f'tools_merge_ex,layer={layer},mode=merge')
                    nf1, nf2 = self.tableWidget.cellWidget(row,7).currentText(), self.tableWidget.cellWidget(row, 8).currentText()
                    step.COM(f'ncrset_cur,job={jobname},step={step.name},layer={layer},ncset={layer}')
                    step.COM('ncr_set_machine,machine=default_excellon,thickness=0')
                    step.COM(f'ncr_set_params,format=Excellon2,zeroes=none,units=mm,tool_units=mm,nf1={nf1},nf2={nf2},decimal=no,modal_coords=yes,single_sr=no,sr_zero_set=no,repetitions=sr,drill_layer=rt2drl,sr_zero_drill_layer=drill,break_sr=no,ccw=no,short_lines=none,press_down=no,last_z_up=16,max_arc_ang=180,sep_lyrs=no,allow_no_chain_f=yes,keep_table_order=no')
                    ...
                    step.COM(f'ncr_ncf_export,dir={path + "/ROU"},name={prefix}{layer}.rou')
