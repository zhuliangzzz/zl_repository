...
class AOI_LDI_Output(QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        self.radioButton_avi.clicked.connect(self.loadTable)
        # 类型
        self.checkBox_signal.clicked.connect(self.check_type)
        self.checkBox_sm.clicked.connect(self.check_type)
    ...
    def exec(self):
        ...
        if self.radioButton_aoi.isChecked():
            dest = f'{jobname}-aoi-{"-".join(layers)}'
            ...
            for step in job.steps:
                if step not in ('pcs', 'set'):
                    job.removeStep(step)
            job.matrix.refresh()
            job.save()
            job.close(1)
            if not os.path.exists(f'{path}/AVI'):
                os.makedirs(f'{path}/AVI')
            top.exportJob(jobname, f'{path}/AVI')
            top.renameJob(jobname, dest, 'genesis')
        # 把bak改回jobname
    ...
    def loadTable(self):
        ...
        tableData = []
        for name, context, type in zip(matrix_info.get('gROWname'), matrix_info.get('gROWcontext'), matrix_info.get('gROWlayer_type')):
            if context == 'board' and type in ('signal', 'drill', 'solder_mask'):
