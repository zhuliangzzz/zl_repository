...
class ScriptsSet(QMainWindow, ui.Ui_MainWindow):
    ...
    def getModules(self):
        ...
        sql = 'select a.id,module_name,count(1) from hdi_script_module a left join hdi_script_file b on a.id = b.module_id where module_id is not null GROUP BY module_id'
        self.cursor.execute(sql)
        results = self.cursor.fetchall()
        return results

    def getScripts(self, id):
        sql = "select script_name,script_path from hdi_script_file where module_id = %s order by CONVERT(script_name USING GBK);"
    ...
    def save_record(cls, name, path):
        ...
        sql = 'insert into hdi_script_record(script_name, script_path, host, user, jobname) values(%s, %s, %s, %s, %s)'
        try:
