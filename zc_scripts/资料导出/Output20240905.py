...
"""
   ...
   20240828 分段输出
   20240829 需要flatten，否则内部step的东西会导出
"""
import genClasses
import os
import re
        ...
        if self.__type == '菲林Gbr':
            ...
            for row in range(self.tableWidget.rowCount()):
                ...
                if self.tableWidget.cellWidget(row, 0).isChecked():
                    ...
                    if self.comboBox_split.currentIndex():
                        self.splitLayer(step, flat_layer, clip_layer)
                    step.DxfOut(flat_layer, path + '/DXF', mirror=mirror, x_scale=x_scale, y_scale=y_scale, prefix=prefix,x_anchor=x_anchor,y_anchor=y_anchor)
                    if scale_label:
