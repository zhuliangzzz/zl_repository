...
"""
   ...
   20240913 增加合并命名 菲林层合并 勾选层合并时，合并后的文件名默认是第一个层的命名，如果有输入合并层名，按输入的命名
"""
...
import sys
import qtawesome
sys.path.insert(0, '/incam/server/site_data/scripts/sh_script/zltest')
...
class Output(QWidget, ui.Ui_Form):
    ...
    def loadTable(self):
        ...
        if self.__type == '菲林Gbr':
            ...
            self.lineEdit_merge_name.setVisible(True)
    ...
    def run(self):
        ...
        if self.__type == '菲林Gbr':
            if self.checkBox_merge.isChecked():
                ...
                merge_name = self.lineEdit_merge_name.text().strip()
                merge_name = merge_name if merge_name else flats[0].replace('_flat', '')
                for flat in flats[1:]:
                    step.affect(flat)
                step.copyToLayer(flats[0])
                step.unaffectAll()
                step.Gerber274xOut(flats[0], path + '/FILM', mirror=mirror, x_scale=x_scale, y_scale=y_scale, line_units='mm', prefix=prefix, break_sr='no', units='mm', nf1=nf1, nf2=nf2,x_anchor=x_anchor,y_anchor=y_anchor)
                if scale_label:
                    shutil.move(f'{path}/FILM/{prefix}{flats[0]}.gbr', f'{path}/FILM/{prefix}{merge_name}({scale_label}).gbr')
                else:
                    shutil.move(f'{path}/FILM/{prefix}{flats[0]}.gbr', f'{path}/FILM/{prefix}{merge_name}.gbr')
                for flat in flats:
            ...
            for row in range(self.tableWidget.rowCount()):
                ...
                if self.tableWidget.cellWidget(row, 0).isChecked():
                    ...
                    step.COM('ncd_set_machine,machine=0.5ozda,thickness=0')
                    step.COM(f'ncd_set_params,format=Excellon2,zeroes=none,units=mm,tool_units=mm,nf1={nf1},nf2={nf2},decimal=no,modal_coords=no,single_sr=yes,sr_zero_set=no,repetitions=sr,incremental=no,optimize=yes,iterations=5,reduction_percent=1,cool_spread=0,break_sr=yes,sort_method=Standard,strip_width=0,xspeed=100,yspeed=100,rout_layer=,fixed_tools=yes,tools_assign_mode=increasing_size,comp_short_slot=no')
            ...
            if self.checkBox_merge.isChecked():
                ...
                if scale_label:
                    shutil.move(f'{path}/DXF/{prefix}{flats[0]}.dxf',
                                        f'{path}/DXF/{prefix}{merge_name}({scale_label}).dxf')
                else:
                    shutil.move(f'{path}/DXF/{prefix}{flats[0]}.dxf', f'{path}/DXF/{prefix}{merge_name}.dxf')
            else:
                for row in range(self.tableWidget.rowCount()):
                    scale_label = ''  # 系数标签，加在文件名中
                    if self.tableWidget.cellWidget(row, 0).isChecked():
            ...
            for row in range(self.tableWidget.rowCount()):
                if self.tableWidget.cellWidget(row, 0).isChecked():
                    ...
                    if not os.path.exists(path + '/CAM'):
                        os.makedirs(path + '/CAM')
                    gen.Top().exportJob(name, path + '/CAM')
