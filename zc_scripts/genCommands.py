...
class StepCmds:
    ...
    def setFilterTypes_pro(self,types="",polarity='positive|negative'):
        ...
        if filter_list:
            self.COM(f'set_filter_type, filter_name =, {",".join(filter_list)}')
        ...
        if polarity:
            ...
            self.COM(f'set_filter_polarity, filter_name=,{",".join(filter_list)}')
...
class LayerCmds:
    ...
    def featOut(self, fileName=''):
        ...
        STR = 'info,out_file=' + fileName + ',write_mode=replace,args=-t layer -e ' + self.job.name + '/' + self.step.name + '/' + self.name + ' -d FEATURES -o break_sr'
        self.job.COM(STR)
    ...
    def featSelOut(self, fileName=''):
        ...
        STR = 'info,out_file=' + fileName + ',write_mode=replace,args=-t layer -e ' + self.job.name + '/' + self.step.name + '/' + self.name + ' -d FEATURES -o select'
        self.job.COM(STR)
        lineList = open(fileName, 'r').readlines()
        os.unlink(fileName)
        dict = self.job.parseFeatureInfo(lineList)
