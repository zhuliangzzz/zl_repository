...
"""
   ...
   @software:PyCharm
   @desc: 修改component相关symbol位置，以profile为标准设置位置
   20240716 负性symbol和文字加底铜
   20240717 修改dk层的铺铜，只铺pnl板边
   20240718 加背景实铜 铺铜皮不加减少执行时间
   20240718 铺铜时避开阻抗条0.5，覆盖膜层加set切线考虑阻抗条的情况
   20240719 调整2d qp 选镀层wf c r-mark 的文字也避开0.2
   20240723 调整板边物件位置等
   20240729 增加do_hj
   20240911 按第一列的set的y坐标去加覆盖膜分割线 且文字放外面
   20240912 1.加线宽线距symbol 单双面都加
            2.板边下方一组 donut_s3000x2000
            3.覆盖膜上边加X+0Y+0文字 反面文字镜像
            4.阻焊/印油反面文字镜像
"""
import datetime
import os
...
class SingleDoubleBoard(QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        self.lineEdit_sr_margin_x.setValidator(QDoubleValidator())
        self.lineEdit_sr_margin_y.setValidator(QDoubleValidator())
        self.lineEdit_sr_margin_x.setText('0.2')
        self.lineEdit_sr_margin_y.setText('0.2')
        # 面向
        header = ['线路层名', '面向']
    ...
    def run(self):
        if self.lineEdit_sr_margin_x.text() == '' or self.lineEdit_sr_margin_y.text() == '':
            QMessageBox.warning(board, 'tips', 'sr-margin参数不能为空')
            return
        # 取到面向
...
class Panel():
    def __init__(self):
        ...
        # 阻焊层, 印油层
        self.zhs, self.dks, self.yys, self.ccds = [], [], [], []
        ...
        for row in job.matrix.returnRows():
            ...
            if re.match('yy(\d+)$', row):
                self.yys.append(row)
            if re.match('ccd(\d+)$', row):
                self.ccds.append(row)
        # 文字层
        self.wzs = []
    ...
    def make_map_layer(self):
        ...
        #
        sym = 'ldi-mark-t'
        step.addPad(19, step.profile2.ymin + 7.5, sym)
        step.addPad(19, step.profile2.ymax - 7.5, sym)
        step.addPad(step.profile2.xmax - 23, step.profile2.ymax - 7.5, sym)
        step.addPad(step.profile2.xmax - 19, step.profile2.ymin + 7.5, sym)
        # lpi
        step.addPad(33, 9, 'lpi-3.0')
        ...
        step.addPad(step.profile2.xmax - 93.5, y, 'donut_s3000x2000', nx=3, dx=5000)
        # 线宽线距symbol
        step.addPad(3.5, step.profile2.ymax - 64, '1-ws')
        ...
        # hj
        step.addPad(12, step.profile2.ymax - 9, '1-hj', 'negative')
        step.addPad(12, step.profile2.ymin + 9, '1-hj', 'negative')
        step.addPad(step.profile2.xmax - 12, step.profile2.ymax - 9, '1-hj', 'negative')
        step.addPad(step.profile2.xmax - 12, step.profile2.ymin + 9, '1-hj', 'negative')
        # scale
        x, rectx = 89, -3.5
        ...
        # 覆盖膜系数
        x, y = 70, step.sr2.ymax + 4.5
        step.addText(x, y, 'X+0Y+0', 3.5, 3.5, 1.64041996, fontname='simple')
        # 子唛和lot
        if self.drills:
            ...
            step.addText(step.profile2.xcenter + 20, step.profile2.ymax - 7.7, f'{sign} {self.drills[-1].upper()}', 3.6068, 4.2164,1.64041996, fontname='canned_57')
    ...
    def do_surface(self):
        ...
        sr_marigin_x, sr_marigin_y = board.lineEdit_sr_margin_x.text(), board.lineEdit_sr_margin_y.text()
        # 单面板上下0 左右1 双面板都为1
        step_margin_x, step_margin_y = 1, 0
        if len(self.signalLays) > 1:
            step_margin_x, step_margin_y = 1, 1
        i = 0
        for lay in self.signalLays:
            if i % 2:
                ...
                y_off = 0
            step.affect(lay)
            step.srFill_2(step_margin_x=step_margin_x, step_margin_y=step_margin_y, sr_margin_x=sr_marigin_x, sr_margin_y=sr_marigin_y, nest_sr='no')
            if imps:
    ...
    def do_fxk(self):
        if board.map_flag:
            ...
            step.addPad(step.profile2.xmin + 8.5, step.profile2.ymax - 2.5, 'fxk-wz')
            step.addPad(step.profile2.xmin + 14.5, step.profile2.ymax - 2.5, 'fxk-wz')
    ...
    def do_hj(self):
        ...
        if board.map_flag:
            ...
            for signal in self.signalLays:
                step.affect(signal)
                sym = '1-hj'
                step.addPad(12, step.profile2.ymax - 9, sym)
                step.addPad(12, step.profile2.ymin + 9, sym)
                step.addPad(step.profile2.xmax - 12, step.profile2.ymax - 9, sym)
                step.addPad(step.profile2.xmax - 12, step.profile2.ymin + 9, sym)
                step.unaffectAll()
            step.affect('map')
            step.addPad(12, step.profile2.ymax - 9, '1-hj', 'negative')
            step.addPad(12, step.profile2.ymin + 9, '1-hj', 'negative')
            ...
            for cov in self.covs:
                step.affect(cov)
            step.addPad(12, step.profile2.ymax - 9, 'r2500')
            step.addPad(12, step.profile2.ymin + 9, 'r2500')
            step.addPad(step.profile2.xmax - 12, step.profile2.ymax - 9, 'r2500')
            step.addPad(step.profile2.xmax - 12, step.profile2.ymin + 9, 'r2500')
            step.unaffectAll()
            for dk in self.dks:
                step.affect(dk)
            step.addPad(12, step.profile2.ymax - 9, 'r3400', 'negative')
            step.addPad(12, step.profile2.ymin + 9, 'r3400', 'negative')
            step.addPad(step.profile2.xmax - 12, step.profile2.ymax - 9, 'r3400', 'negative')
            step.addPad(step.profile2.xmax - 12, step.profile2.ymin + 9, 'r3400', 'negative')
            step.unaffectAll()
            if self.ccds:
                step.affect(self.ccds[0])
                step.addPad(12, step.profile2.ymax - 9, 'r2000')
                step.addPad(12, step.profile2.ymin + 9, 'r2000')
                step.addPad(step.profile2.xmax - 12, step.profile2.ymax - 9, 'r2000')
                step.addPad(step.profile2.xmax - 12, step.profile2.ymin + 9, 'r2000')
                step.unaffectAll()
            # 避字符
            # sym = 'sy-pin-t'
            # step.affect(self.wz_map)
    ...
    def do_fxjt(self):
        ...
        for wz in self.wzs:
            step.affect(wz)
            mir = 'yes' if 'b' in wz else 'no'
            step.addPad(57, 3.5, 'fxjt')
            step.addPad(68, 3.5, 'zf', mirror=mir)
            step.unaffectAll()
        for yy in self.yys:
            mir = 'no' if int(yy.replace('yy', '')) % 2 else 'yes'
            step.affect(yy)
            step.addPad(57, 3.5, 'fxjt')
            step.addPad(70, 2.5, 'zhyy', mirror=mir)
            step.unaffectAll()
        for zh in self.zhs:
            step.affect(zh)
            mir = 'yes' if 'b' in zh else 'no'
            step.addPad(57, 3.5, 'fxjt', 'negative')
            step.addPad(68, 2.5, 'zhfh', 'negative', mirror=mir)
            step.unaffectAll()
        step.VON()
    ...
    def do_ldi(self):
        if board.map_flag:
            ...
            for signal in self.signalLays:
                ...
                if step.isLayer(dk):
                    step.affect(dk)
                step.addPad(19, step.profile2.ymin + 7.5, sym, 'negative')
                step.addPad(19, step.profile2.ymax - 7.5, sym, 'negative')
                step.addPad(step.profile2.xmax - 23, step.profile2.ymax - 7.5, sym, 'negative')
                step.addPad(step.profile2.xmax - 19, step.profile2.ymin + 7.5, sym, 'negative')
                step.unaffectAll()
            step.affect('map')
            step.addPad(19, step.profile2.ymin + 7.5, 'ldi-mark-t')
            step.addPad(19, step.profile2.ymax - 7.5, 'ldi-mark-t')
            step.addPad(step.profile2.xmax - 23, step.profile2.ymax - 7.5, 'ldi-mark-t')
            step.addPad(step.profile2.xmax - 19, step.profile2.ymin + 7.5, 'ldi-mark-t')
            step.unaffectAll()
            step.VOF()
            for drill in self.drills:
                step.affect(drill)
            step.addPad(19, step.profile2.ymin + 7.5, 'r1000')
            step.addPad(19, step.profile2.ymax - 7.5, 'r1000')
            step.addPad(step.profile2.xmax - 23, step.profile2.ymax - 7.5, 'r1000')
            step.addPad(step.profile2.xmax - 19, step.profile2.ymin + 7.5, 'r1000')
            step.unaffectAll()
            step.VON()
    ...
    def do_lpi(self):
        if board.map_flag:
            step.affect('map')
            step.selectSymbol('lpi-3.0;lpi-2.5')
            if step.Selected_count():
                step.copySel(self.tmp_map)
                step.unaffectAll()
                step.affect(self.tmp_map)
                for signal in self.signalLays:
                    step.copySel(signal, 'yes')
                step.selectChange('donut_r5000x1000')
                for zh in self.zhs:
                    step.copySel(zh, 'yes')
                step.selectChange('r4000')
                for cov in self.covs:
                    step.copySel(cov, 'yes')
                step.selectChange('r4700')
                for dk in self.dks:
                    step.copySel(dk, 'yes')
                step.unaffectAll()
            step.unaffectAll()
    ...
    def do_cov(self):
        ...
        for cov in self.covs:
            ...
            step.affect(tmp_cov)
            step.addText(x, step.sr2.ymax + 0.5, cov.upper(), 3.5, 3.5, 1.64041996, mir, fontname='simple')
            step.unaffectAll()
        repeat_xy = []
        repeat_set_xy = []
        cut_ys = []
        ...
        if float(board.lineEdit_pnl_h.text()) > 300:
            ...
            if len(cut_ys):
                ...
                for cov in self.covs:
                    ...
                    for y in cut_ys:
                        mir = 'no'
                        x = step.profile2.xmin - 36
                        if cov == 'c2':
                            mir = 'yes'
                            x = step.profile2.xmin - 36 + 2.7
                        tmp_cov = f'{cov}+++{step.pid}'
                        step.affect(tmp_cov)
                        # 最下面一个加横线下方和上方各一个
                        if i == len(cut_ys) + 1:
                            step.addText(x, y - 26, i, 3.5, 3.5, 1.64041996, mir, fontname='simple')
                            i -= 1
                        step.addText(x, y + 26, i, 3.5, 3.5, 1.64041996, mir, fontname='simple')
                        step.unaffectAll()
                        i -= 1
    ...
    def do_drill_letter(self):
        ...
        if board.map_flag:
            step.affect('map')
            step.setFilterTypes('text')
            step.setAdv_fontname('canned_67;canned_57')
            step.selectAll()
            step.resetFilter()
    ...
    def do_scale(self):
        if board.map_flag:
            ...
            step.affect('map')
            step.setFilterTypes('text')
            step.setTextFilter('X+0Y+0')
            step.setAdv_fontname('simple')
            step.selectAll()
            step.resetFilter()
            if step.Selected_count():
                step.copySel(self.tmp_map)
                ...
                for cov in self.covs:
                    if cov == 'c2':
                        # 获取文字中心位置
                        info = step.DO_INFO(f'-t layer -e {jobname}/{step.name}/{self.tmp_map} -d LIMITS,units=mm')
                        x, y = info.get('gLIMITSxcenter'), info.get('gLIMITSycenter')
                        step.copySel_2(cov, x_anchor=x, y_anchor=y, mirror='horizontal')
                    else:
                        step.copySel(cov)
                step.unaffectAll()
            step.unaffectAll()
            ...
            # 20240912 X+0Y+0
            for cov in self.covs:
                x,y, mir = 70, step.sr2.ymax + 4.5, 'no'
                if cov == 'c2':
                    x, mir = 90, 'yes'
                step.affect(cov)
                step.addText(x, y, 'X+0Y+0', 3.5, 3.5, 1.64041996, fontname='simple', mirror=mir)
                step.unaffectAll()
            x, rectx = 89, -3.5
            step.affect('map')
            step.addPad(x + rectx, step.sr2.ymax + 3.5, 'rect12400x1500')
