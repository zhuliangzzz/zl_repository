...
class Panel():
    ...
    def do_cov(self):
        ...
        for cov in self.covs:
            ...
            step.affect(tmp_cov)
            # step.addText(x, step.sr2.ymax + 2, cov.upper(), 5, 5, 2.296587944, mir, fontname='simple')
            step.addText(x, step.sr2.ymax + 0.5, cov.upper(), 5, 5, 2.296587944, mir, fontname='simple')
            # step.addText(x, step.profile2.ymin + 7.5, f'{sign} {cov.upper()}', 5, 5, 2.296587944, mir, fontname='simple')
            step.unaffectAll()
        # 覆盖膜加切线和序号
