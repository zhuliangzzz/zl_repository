...
import os, sys, time, re, math, platform, string

# import package specific stuff
import genCommands_zl, genFeatures_zl

# Identify
print("Using genClasses.py Rev " + __version__)

from genCommands_zl import *
from genFeatures_zl import *
...
class Genesis:
    ...
    def normalize(self):
        ...
        if sys.platform == "win32":
            ...
            if not os.environ.get('GENESIS_DIR'):
                self.error('GENESIS_DIR not set', 1)
            self.gendir = os.environ['GENESIS_DIR']
            if not os.environ.get('GENESIS_EDIR'):
                self.error('GENESIS_EDIR not set', 1)
            self.edir = os.environ['GENESIS_EDIR']
            if not os.path.isdir(self.edir):
                self.edir = os.path.join(self.gendir, self.edir)
    ...
    def INFO(self, args):
        ...
        lineList = []
        lineList_tmp = open(self.tmpfile, 'r').readlines()
        for line_tmp in lineList_tmp:
            lineList.append(line_tmp)
        ...
        while piontList:  # 用列表做循环变量 列表数据被取完之后 就停止循环
            ...
            while piontList:  # 二次循环
                ...
                if term_1 or term_2 or term_3 or term_4:  # 如果找到任何一个坐标 把这两个坐标取出最大值 并从头开始循环
                    line = (min(piontList[count][0], line[0], line[1], piontList[count][1]),
                            max(piontList[count][0], line[0], line[1], piontList[count][1]))  # 取最小和小大值给line 用于和其他值再比较
        ...
        while piontListTemp:  # 用列表做循环变量 列表数据被取完之后 就停止循环
            ...
            while piontListTemp:  # 二次循环
                ...
                if piontListTemp[count][0] - line[1] <= tolerance:
                    line = (min(piontListTemp[count][0], line[0], line[1], piontListTemp[count][1]),
                            max(piontListTemp[count][0], line[0], line[1],
                                piontListTemp[count][1]))  # 取最小和小大值给line 用于和其他值再比较
...
class Top(Genesis):
    ...
    def copyJob(self, name, dbname, dbase='db253'):
        ...
        if name in joblist and dbname not in joblist:
            self.COM(
                'copy_entity,type=job,source_job=' + name + ',source_name=' + name + ',dest_job=' + dbname + ',dest_name=' + dbname + ',dest_database=' + dbase)
            return self.STATUS
        return -1
    ...
    def importJob(self, name, import_dir, db_name, analyze, verify):
        ...
        STR = 'import_job,db=%s,path=%s,name=%s,analyze_surfaces=%s,verify_tgz=%s' % (
        db_name, import_dir, name, analyze, verify)
        self.COM(STR)
...
class Job(Genesis, ExtraStuff, JobCmds):
    ...
    def __getattr__(self, name):
        ...
        if name == 'matrix':
            ...
            self.stepsList = self.getInfo()["gSTEPS_LIST"]
            self.stepsList = [str(step) for step in self.stepsList]
    ...
    # 网络分析  Per Layers
    def net_partial_compare(self, job2='', step1='', type1='cur', layer_list1='', step2='', type2='cur',
                            layer_list2=''):
        if not job2:
            job2 = self.name
        if not layer_list2:
            layer_list2 = layer_list1
        STR = 'netlist_partial_compare,job1=%s,step1=%s,type1=%s,layer_list1=%s\;,job2=%s,step2=%s,type2=%s,layer_list2=%s\;,batch_mode=yes,display=yes' % (
        self.name, step1, type1, layer_list1, job2, step2, type2, layer_list2)
        self.VOF()
        self.COM(STR)
        return self.STATUS
        self.VON()
...
class Step(Genesis, ExtraStuff, StepCmds):
    ...
    def isLayer(self, layer_name='misc'):
        ...
        d = self.DO_INFO(
            args='-t layer -e ' + self.job.name + '/' + str(self.name) + '/' + str(layer_name) + ' -d exists')
    ...
    def Features_INFO(self, Layer="", mode="", units="mm"):
        if mode:
            piont = self.INFO(
                ' -t layer -e ' + self.job.name + '/' + str(self.name) + '/%s -d FEATURES -o %s,units=%s' % (Layer, mode, units))
        else:
            piont = self.INFO(
                ' -t layer -e ' + self.job.name + '/' + str(self.name) + '/%s -d FEATURES,units=%s' % (Layer, units))
        del piont[0]
        coor = []
        for line in piont:
    ...
    def rotateStep(self, angle, rotated_step='', mode='datum', outputname=''):
        ...
        STR = 'rotate_step,job=%s,step=%s,rotated_step=%s,angle=%s,mode=%s,units=%s,anchor_x=%s,anchor_y=%s' % (
        self.job.name, self.name, rotated_step, angle, mode, units, anchor_x, anchor_y)
        self.COM(STR)
        if outputname:
...
class Form(Genesis):
    ...
    def color(self, element, color):
        ...
        STR = 'edit_form,job=' + self.job.name + ',form=' + str(
            self.name) + ',elem=' + element + ',color=' + color + ',opt_name=no'
        self.COM(STR)
        return self.COMANS
    ...
    def visibility(self, field, mode):
        ...
        STR = 'form_elem_visibility,job=' + self.job.name + ',form=' + str(
            self.name) + ',elem=' + field + ',mode=' + mode
        self.COM(STR)
        return self.COMANS
...
class Checklist(Genesis):
    ...
    def getInfo(self, action=1):
        ...
        if self.name in self.step.checks:
            self.VOF()
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d CHK_ATTR -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d DURATION -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d ERF -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO('-t check -e %s/%s/%s -d EXISTS' % (self.job.name, self.step.name, self.name))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d LAST_TIME -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d MEAS -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO('-t check -e %s/%s/%s -d MEAS_DISP_ID -o action=%s ' % (
            self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d NUM_ACT -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d REPOST -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d STATUS -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.tmp = self.DO_INFO(
                '-t check -e %s/%s/%s -d TITLE -o action=%s ' % (self.job.name, self.step.name, self.name, action))
            self.info.update(self.tmp)
            self.VON()
        return self.info
