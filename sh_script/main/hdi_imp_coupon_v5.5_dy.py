            ...
            if max_group == 1 or (max_group == 2 and (('特性' in mode_ip or '差动' in mode_ip) and ('特性共面' in mode_ip or '差动共面' in mode_ip))):
                ...
                for data in [zk_gm, zk_gm_n]:
                    if data:
                        ...
                        send_ipdata += get_ip
            sel_ipdata, v, k = self.deal_with_sqldata(send_ipdata)
            # 更改分组为1,2,3的顺序
        ...
        for group_x in [x_n,x_dy]:
            ...
            if group_x == x_dy:
                ...
                for key, val in self.coupon_area.items():
                    val_numlist = set(val['numlist']) & set(zu_imp)
                    if len(val_numlist) > 0:
                        val['numlist'] = list(val_numlist)
                        if val['h_num'] > 1:
                            val['h_num'] = len(val_numlist)
                        else:
                            val['v_num'] = len(val_numlist)
                    zu_areas[key] = val
                send_coupon_area = zu_areas
                send_coupon_num = len(self.teamc.keys())
