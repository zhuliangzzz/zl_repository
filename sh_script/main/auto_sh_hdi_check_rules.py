...
class auto_check_rule(object):
        ...
        if not data_info:
            ...
            if res:
                ...
                if dic_tgz_week_layers or dic_tgz_barcode_week_layers or dic_tgz_banci_layers or dic_tgz_picihao_layers:            
                    showMessageInfo(u"检测到外层资料已添加周期或批次号，但未登记公共程序内的 CAM资料数据记录程序，请登记ok后再输出资料！")
                else:
            ...
            if job_detail_info_for_output is None:
                ...
                for key, value in job_detail_info_for_output.iteritems():
                    if key in ["week_table", "silk_table"]:
                        for info in value:
                            if info[-1] != u"审核OK":
                                check_status = "error"
    ...
    def get_slot_rotation_angle(self, slot_size, slot_len):
        ...
        r = round((float(slot_len) *1000 +float(slot_size)) / float(new_slot_size), 1)
        if slot_size <= 809:
            if r >= 2:
                return 0
            if 1.8< r < 2:
