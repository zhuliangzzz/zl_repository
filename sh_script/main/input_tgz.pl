...
sub run_input_tgz {
	...
	# 20241118 zl
	my $idb_file = '/tmp/get_idb_path';
	# 判断idb文件
	system("python /incam/server/site_data/scripts/zl/check/GetIdbByOdbPath.py $input_path"); # 将结果写入临时文件
	# 获取文件内容
	if(-e $idb_file){
		open(my $fh, '<', $idb_file) or die "Could not open file '$idb_file' $!";
		while (my $line = <$fh>) {
