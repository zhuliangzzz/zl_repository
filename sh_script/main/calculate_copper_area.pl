...
sub appy {
	...
	if ($board_thick eq "" || $count_gold == 0 && $count_copper == 0 && $count_elec == 0 ) {
		...
		if ($count_gold == 1) {
			...
			# 加入表面积铜厚
			$f->COM("exposed_area,layer1=l1,mask1=$m1_value,layer2=l$signal_num,mask2=$m2_value,edges=yes,copper_thickness=$get_CN{l1},drills=yes,consider_rout=no,drills_source=matrix,drills_list=$exposed_drill,thickness=$board_thick1,resolution_value=25.4,x_boxes=3,y_boxes=3,area=no,dist_map=yes,out_file=$ENV{GENESIS_TMP}/copper_result,out_layer=first");
