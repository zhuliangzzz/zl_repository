...
class TemporaryLayerComparsion(QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        # 层类型
        self.checkBox_outer_signal.clicked.connect(self.check_type)
        self.checkBox_inner_signal.clicked.connect(self.check_type)
        self.checkBox_silk_screen.clicked.connect(self.check_type)
        self.checkBox_solder_mask.clicked.connect(self.check_type)
        self.checkBox_drill.clicked.connect(self.check_type)
        self.checkBox_board.clicked.connect(self.check_type)
        self.checkBox_all.clicked.connect(self.check_type)
        #
    ...
    def run(self):
        ...
        for layer in layers:
            ...
            step.compareLayers(tmp_layer, self.job.name, step.name, tmp_formal_layer, map_layer=map_layer,
                               map_layer_res=resolution)
