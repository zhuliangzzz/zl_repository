...
from open_job_lock import CheckUserOpenJob
...
class RemoveSlightSection(QtGui.QWidget, ui.Ui_Form):
    ...
    def update_removed_slight_section(self):
        # 更新最后一次记录
        sql = """select id from pdm_job_check_log_list where jobname = '{0}' and check_function_name = 'check_removed_slight_section' and check_process like '%异常%' order by check_time desc limit 1""".format(jobname)
        data_info = ikm_fun.PG.SELECT_DIC(ikm_fun.dbc_p, sql.format(jobname.lower()))
        if data_info:
            id = data_info[0]['id']
            sql = """update pdm_job_check_log_list set check_log = 'success', check_process = '检测完成' where id = '%s'""" % id
