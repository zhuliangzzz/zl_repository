...
class RemoveSlightSection(QtGui.QWidget, ui.Ui_Form):
    ...
    def run(self):
        ...
        for dict_ in add_outdata.get('list'):
                ...
                step.Contourize(0)
                step.COM('sel_decompose,overlap=no') # 把铺铜打散
                ...
                step.affect(layer)
                step.Contourize(0, break_to_islads='no')
                step.copySel(tmp_surface)
                step.unaffectAll()
                # 先将layer层清除
                step.truncate(layer)
                ...
                step.selectSymbol('sh+yh')
                step.resetFilter()
                if step.Selected_count():
                    step.copySel(tmp_symbol_)
                    step.unaffectAll()
                    step.affect(tmp_symbol_)
                    # step.selectChange('rect700x600')
                    step.selectChange('rect420x350')
                    step.copySel(layer)
                    step.unaffectAll()
                    step.affect(layer)
                    step.Contourize(0)
                    ...
                    step.unaffectAll()
                    step.affect(tmp_symbol_)
                    # step.selectChange('rect600x500')
                    step.selectChange('rect320x250')
                    step.copySel(layer)
                    step.unaffectAll()
                    step.affect(layer)
                    step.Contourize(0)
                    step.moveSel(tmp_surface, 'yes')
                step.unaffectAll()
                step.affect(tmp_symbol)
                ...
                if step.Selected_count():
                    ...
                    step.copySel(tmp_surface)
                step.unaffectAll()
                if step.isLayer(tmp_symbol_):
                    step.truncate(tmp_symbol_)
...
class Parm:
    def __init__(self):
        ...
        if xmin_list:
            self.sr_xmin = min(xmin_list)
            self.sr_ymin = min(ymin_list)
            self.sr_xmax = max(xmax_list)
            self.sr_ymax = max(ymax_list)
        else:
            for name, xmin, xmax, ymin, ymax in zip(step.info2.get('gREPEATstep'),
                                                    step.info2.get('gREPEATxmin'),
                                                    step.info2.get('gREPEATxmax'),
            ...
            if xmin_list:
                ...
                getData = GEN.DO_INFO('-t step -e %s/%s -d SR_LIMITS' % (jobname, 'panel'))
                self.sr_xmin = float(getData['gSR_LIMITSxmin'])
                self.sr_xmax = float(getData['gSR_LIMITSxmax'])
                self.sr_ymin = float(getData['gSR_LIMITSymin'])
                self.sr_ymax = float(getData['gSR_LIMITSymax'])
...
class Pnl_fill_copper:
    ...
    def put_copper(self):
        ...
        self.GEN.DELETE_LAYER('clip_profile_zltest+++')
        self.GEN.DELETE_LAYER('fill_profile_zltest+++')
    ...
    def create_ladder_sym(self):
        ...
        clip_profile_layer = 'clip_profile_zltest'
        flat_clip_profile = clip_profile_layer + '+++'
        ...
        for cur_lamin_num in range(0, len(lam_rout)):
            ...
            # 20241127 zl
            self.GEN.COM('clip_area_end,layers_mode=affected_layers,layer=,area=reference,area_type=rectangle,inout=inside,contour_cut=no,margin=250,ref_layer=%s,feat_types=line\;pad\;surface\;arc\;text' % flat_clip_profile)
            self.GEN.COM('affected_layer,mode=all,affected=no')
            self.GEN.CLEAR_LAYER()
            # --品字形排版sr范围内铺蜂窝铜
    ...
    def create_honeycomb_tmp(self):
        ...
        # 20241127 切掉panel内的sr step
        stepnames = get_panelset_sr_step(jobname, self.STEP)
        if 'set' in stepnames:
            stepnames.remove('set')
        if 'edit' in stepnames:
            stepnames.remove('edit')
        # 排除不掏铜
        exclude_stepnames = ['drl', '2nd']
        # stepnames.remove('set')
        # stepnames.remove('drl')
        # stepnames.remove('2nd')
        clip_profile_layer = 'clip_profile_zltest'   # 用来掏铜
        fill_profile_layer = 'fill_profile_zltest'   # 用来填铜
        flat_clip_profile = clip_profile_layer + '+++'
        flat_fill_profile = fill_profile_layer + '+++'
        self.GEN.DELETE_LAYER(clip_profile_layer)
        self.GEN.DELETE_LAYER(flat_clip_profile)
        self.GEN.DELETE_LAYER(fill_profile_layer)
        self.GEN.DELETE_LAYER(flat_fill_profile)
        self.GEN.CREATE_LAYER(flat_clip_profile)
        self.GEN.CREATE_LAYER(flat_fill_profile)
        for stepname in stepnames:
            if stepname in exclude_stepnames:
                stepObj = gen.Step(job, stepname)
                stepObj.open()
                stepObj.prof_to_rout(fill_profile_layer)
                stepObj.affect(fill_profile_layer)
                stepObj.selectCutData()
                stepObj.unaffectAll()
            else:
                stepObj = gen.Step(job, stepname)
                stepObj.open()
                stepObj.prof_to_rout(clip_profile_layer)
                stepObj.affect(clip_profile_layer)
                stepObj.selectCutData()
                stepObj.copySel(fill_profile_layer)
                stepObj.unaffectAll()
        self.GEN.OPEN_STEP(self.STEP)
        self.GEN.CLEAR_LAYER()
        self.GEN.COM('flatten_layer,source_layer=%s,target_layer=%s' % (clip_profile_layer, flat_clip_profile))
        self.GEN.COM('flatten_layer,source_layer=%s,target_layer=%s' % (fill_profile_layer, flat_fill_profile))
        self.GEN.COM("affected_layer,name=honeycomb-t,mode=single,affected=yes")
        self.GEN.COM("affected_layer,name=honeycomb-b,mode=single,affected=yes")
        self.GEN.COM('clip_area_end,layers_mode=affected_layers,layer=,area=reference,area_type=rectangle,inout=inside,contour_cut=no,margin=250,ref_layer=%s,feat_types=line\;pad\;surface\;arc\;text' % flat_clip_profile)
        self.GEN.COM('affected_layer,mode=all,affected=no')
        self.GEN.DELETE_LAYER(clip_profile_layer)
        self.GEN.DELETE_LAYER(fill_profile_layer)
        # self.GEN.DELETE_LAYER(flat_profile)
    ...
    def remove_small_surface(self, size=431, target_top='honeycomb-tok', target_bot='honeycomb-bok'):
        ...
        if size == 1000:
            ...
            self.GEN.COM("sel_resize,size=%s,corner_ctl=no" % size)
            self.GEN.COM('sel_contourize, accuracy=6.35, break_to_islands=no, clean_hole_size=76.2')
            # 20241128 zl 缩放回来后进入到coupon 重新clip
            self.GEN.COM('clip_area_end,layers_mode=affected_layers,layer=,area=reference,area_type=rectangle,inout=inside,contour_cut=no,margin=250,ref_layer=%s,feat_types=line\;pad\;surface\;arc\;text' % flat_clip_profile)
    ...
    def corner_copper(self, copper_dis=None, target_top='honeycomb-tok', target_bot='honeycomb-bok',
        ...
        #              "step_max_dist_y=2540,sr_margin_x=2.5,sr_margin_y=2.5,sr_max_dist_x=0,sr_max_dist_y=0,nest_sr=no,"
        #              "consider_feat=no,consider_drill=no,consider_rout=no,dest=affected_layers,attributes=no")
        # 20241128 zl 切之前先把coupon填上
        self.GEN.COM("affected_layer,mode=all,affected=no")
        fill_profile_layer = 'fill_profile_zltest'  # 用来填铜
        flat_fill_profile = fill_profile_layer + '+++'
        self.GEN.COM("affected_layer,name=%s,mode=single,affected=yes" % flat_fill_profile)
        self.GEN.SEL_COPY('honeycomb-tmp2', size=5002)
        self.GEN.COM("affected_layer,mode=all,affected=no")
        self.GEN.COM("affected_layer,name=honeycomb-tmp2,mode=single,affected=yes")
        self.GEN.SEL_CONTOURIZE(0)
        self.GEN.COM("clip_area_strt")
