...
# 20250624 zl run_genesis_make_press_rout_output 自动制作压合锣边资料工具输出
'''
...
class scheduleTask(QtCore.QObject):
                        ...
                        for info in sorted(data_info, key=lambda x: x["workLevel"] * -1):
                            ...
                            try:
                                ...
                                for key, value in dic_make_info.iteritems():
                                    if u"内层" in key:
                                        if isinstance(value, (str, unicode)) and value in [u"已退审", u"送审中", u"已解锁退审"]:
                                            finish_status = False
    ...
    def run_incam_ldi_modify_week_output_type(self, *args):
        """incam输出线路周期LDI 资料工具"""
        
        while True:
            try:             
                dic_condition = {"Facotry": "S01H1","type": u"临时","data_type": u"临时改LDI资料","dataName": u"外层线路LDI(周期批次)","isOutput": "Y",}
                param = []          
                for k, v in dic_condition.iteritems():
                    param.append(u"{0}='{1}'".format(k, v))        
                param.append("(OutputStatus is null or OutputStatus = '')")       
                ...
                for name in all_ouput_jobs:            
                    ...
                    if res:
                        ...
                        for info in sorted(data_info, key=lambda x: (x["workLevel"] * -1, x["appTime"])):
                            ...
                            dest_net_path = info["OutputPath"]
                            if u"LDI线路周期资料测试" not in dest_net_path:                                
                                dest_net_path = "\\192.168.2.57\临时文件夹\系列资料临时存放区\03.CAM制作资料暂放（勿删）\000勿删\LDI线路周期资料测试\{0}系列\{1}".format(
                                    jobname[1:4].upper(), jobname.upper()
                                )
                            dest_net_path = dest_net_path.replace("\\", "/")
                            dest_net_path = dest_net_path.replace(ur"//192.168.2.57/临时文件夹/系列资料临时存放区/", "/windows/33.file/")
                            ...
                            run_scripts = os.path.join(script_dir, "Auto_Engineer_Output_Tools.py")
                            data_type = "auto_output_LDI_signal_has_week"
                            ...
                            try:                                
                                ...
                                with open("/tmp/ldi_week_error.log", "w") as f:
                                    f.write(str(jobname) + str(e))                                
                            ...
                            if os.path.exists(error_file):
                                ...
                                if os.path.exists(success_file):
                                    ...
                                    log = u"INCAMPRO 输出程序异常中断，请手动输出线路LDI资料！<br>"                                
                                    self.update_mysql_data([u"OutputStatus = '{0}'".format(u"输出异常(推送消息)"),
                                                            u"Status = '{0}'".format(u"输出异常"),
                                                            u"AlarmLog = concat(AlarmLog,'{0}')".format(log)],
                                                           condition="id={0}".format(output_id))                                 
    ...
    def run_delete_scale_drill_output_type(self, *args):
        ...
        while True:            
            try:
                self.update_all_layer_target_info_to_mysql()
    ...
    # 20250624 zl
    def run_genesis_make_press_rout_output(self, *args):
        """自动制作压合锣边资料工具输出"""

        while True:
            try:
                dic_condition = {"Facotry": "S01H1", "type": u"临时", "data_type": u"压合锣边", "isOutput": "Y", }
                param = []
                for k, v in dic_condition.iteritems():
                    param.append(u"{0}='{1}'".format(k, v))
                param.append("(OutputStatus is null or OutputStatus = '')")
                ...
                for name in all_ouput_jobs:
                    ...
                    if "not_import_tgz" in args:
                        ...
                        if os.path.exists("{0}/outputing_{1}.log".format(tmp_dir, name)):
                            time.sleep(5)
                            continue
                        import_tgz_path = ur"//192.168.2.174/GCfiles/HDI全套tgz/{0}系列/{1}.tgz".format(name[1:4].upper(), name)
                        if not os.path.exists(import_tgz_path):
                            import_tgz_path = ur"//192.168.2.174/GCfiles/HDI全套tgz/{0}系列/{1}-inn.tgz".format(name[1:4].upper(), name)
                        res = self.import_tgz(name, dic_job_id[name], tgz_import_path=import_tgz_path)
                        # if res and os.path.exists("{0}/outputing_{1}.log".format(tmp_dir, name)):
                        # self.update_mysql_data([u"OutputStatus = ''",
                        # u"Status = ''",
                    ...
                    if res:
                        ...
                        for info in sorted(data_info, key=lambda x: x["workLevel"] * -1):
                            ...
                            dest_net_path = info["OutputPath"]
                            # if u"成型锣带资料" not in dest_net_path and info["dataName"] == u"锣带资料":
                            #     dest_net_path = ur"\\192.168.2.174\GCfiles\CNC\成型锣带资料\涨缩程式\{1}系列\{0}".format(
                            #         jobname.upper(), jobname[1:4].upper())
                            # dest_net_path = os.path.join(os.path.dirname(dest_net_path) ,zs_code+ "-"+jobname)

                            # if "192.168.19.243" in localip[2]:
                            #     dest_net_path = ur"\\192.168.2.57\临时文件夹\系列资料临时存放区\03.CAM制作资料暂放（勿删）\000勿删\临时锣带拉伸测试\{1}系列\{0}".format(
                            #         jobname.upper(), jobname[1:4].upper())
                            try:
                                os.makedirs(dest_net_path)
                            except Exception as e:
                            ...
                            run_scripts = os.path.join(script_dir, "Auto_Engineer_Output_Tools.py")
                            data_type = "output_make_press_rout_file"
                            ...
                            if "gui_run" in args:
                                ...
                                os.system("python %s/%s.py %s %s %s %s %s" %
                                          (script_dir, login_soft_script_name, run_scripts, data_type, jobname,
                                           output_id, "gui_run"))
                                ...
                                print('\n\n\n\n')
                                print(script_dir, login_soft_script_name, run_scripts, data_type, jobname,
                                           output_id, "no_gui")
                                # sys.exit()
                                os.system("python %s/%s.py %s %s %s %s %s" %
                                          (script_dir, login_soft_script_name, run_scripts, data_type, jobname,
                            ...
                            if os.path.exists(error_file):
                                ...
                                try:
                                    self.update_mysql_data([u"OutputStatus = '{0}'".format(u"输出异常(推送消息)"),
                                                            u"Status = '{0}'".format(u"输出异常"),
                                                            u"AlarmLog = concat(AlarmLog,'{0}')".format(
                                                                "<br>".join(log).decode("cp936"))
                                                            ],
                                                           condition="id={0}".format(output_id))
                                except Exception as e:
                                    print(e, 111111)
                                ...
                                if os.path.exists(success_file):
                                    ...
                                    #                                condition="id={0}".format(output_id))
                                    #     else:
                                    #         self.update_mysql_data(
                                    #             [u"OutputStatus = '{0}'".format(u"输出异常(推送消息)"),
                                    #              u"Status = '输出异常'",
                                    #              u"AlarmLog = concat(AlarmLog,'{0}')".format(responsere.json()['msg'])],
                                    #             condition="id={0}".format(output_id))
                                    self.update_mysql_data([u"OutputStatus = '{0}'".format(u"输出完成"),
                                    ...
                                    log = u"<br>genesis 输出程序异常中断，请手动输出压合锣边资料！<br>"
                                    self.update_mysql_data([u"OutputStatus = '{0}'".format(u"输出异常(推送消息)"),
                                                            u"Status = '{0}'".format(u"输出异常"),
                                                            u"AlarmLog = concat(AlarmLog,'{0}')".format(log)],
                                                           condition="id={0}".format(output_id))
            ...
            try:             
                ...
                for name in all_ouput_jobs:
                    ...
                    if "not_import_tgz" in args:
                        ...
                        print("{0}/outputing_{1}.log".format(tmp_dir, name), os.path.exists("{0}/outputing_{1}.log".format(tmp_dir, name)))
                        if os.path.exists("{0}/outputing_{1}.log".format(tmp_dir, name)):
                            time.sleep(5)
                            continue
                        ...
                        db_job_path = "/id/incamp_db1/jobs/{0}".format(jobname)
                        print(db_job_path, os.path.exists(db_job_path), check_status)
                ...
                if all_ouput_jobs:                    
                    ...
                    if "gui_run" not in args:
                        ...
                        if sys.platform <> "win32":
                            ...
                            if os.path.exists(db_job_path):
                                print("----------->", db_job_path)
                                incam_res = self.import_tgz(name, dic_job_id[name], "no_gui_login_incam", tgz_import_path=db_job_path)
                        ...
                        if os.path.exists(genesis_manual_tgz_path):
                            shutil.copy(genesis_manual_tgz_path, ur"/windows/33.file/03.CAM制作资料暂放（勿删）/000勿删/GB输出测试/")
                            genesis_res = self.import_tgz(name, dic_job_id[name], log_incam_win_soft="no_gui_login_genesis",
                                                  tgz_import_path=genesis_manual_tgz_path)                         
                        #else:
                            #res = self.import_tgz(name, dic_job_id[name], log_incam_win_soft="no_gui_login_genesis")
                        
                        if not genesis_res and not incam_res:
                            self.update_mysql_data([u"OutputStatus = '{0}'".format(u"输出异常(推送消息)"),
                                                    u"Status = '{0}'".format(u"输出异常"),
                                                    u"AlarmLog = concat(AlarmLog,'{0} {1}')".format(u'导入tgz失败', "not incam job")],
                                                   condition=u"Job='{0}' and id in ({1})".format(jobname, ",".join(dic_job_id[name])))
                        else:
                            res = 1
    ...
    def import_tgz(self, jobname, job_ids, log_incam_win_soft=None, tgz_import_path=None):
        ...
        run_scripts = "{0}/del_job.py no_gui".format(script_dir)
        if "192.168.19.243" not in localip[2]:
            os.system("/frontline/incampro_output/release/bin/dbutil check y")
        ...
        if sys.platform == "win32":
            ...
            if not os.path.exists(job_path):
                ...
                if not os.path.exists("c:/tmp/tgz"):
                    os.makedirs("c:/tmp/tgz")
                # os.system("rm -rf c:/tmp/tgz/{0}".format(jobname))
                if os.path.exists("c:/tmp/tgz/{0}".format(jobname)):
                    shutil.rmtree("c:/tmp/tgz/{0}".format(jobname))
                # shutil.copy(import_job_path, "c:/tmp/{0}.tgz".format(jobname))
                with tarfile.open(import_job_path, 'r:*') as tar:
                    for member in tar:
                        try:
                            tar.extract(member, path="c:/tmp/tgz")
                ...
                if os.path.exists("c:/tmp/tgz/{0}".format(jobname)):
                    ...
                    os.environ["TGZ_IMPORT_PATH"] = "c:/tmp/tgz/{0}".format(jobname)
                    os.system("python %s/%s.py %s %s %s %s %s %s" %
                              (script_dir, login_soft_script_name,
                               run_scripts, jobname, "hdi1", "genesis", "genesis", "no_gui"))
    ...
    def check_loss_test_pad_comp(self):
        """check loss test pad has comparated by zl 20250305"""
        run_scripts = "/frontline/incam/server/site_data/scripts/sh_script/zl/crontab/CheckLossTestPadComp-main.py"
        os.environ["INCAM_PRODUCT"] = "/frontline/incampro/release"
        os.environ["VNCSTARTPATH"] = "/usr/bin/"
        os.system("python %s/%s.py %s %s" %
                  (script_dir,login_soft_script_name, run_scripts, "no_gui"))
    ...
    def update_all_layer_target_info_to_mysql(self):
        """更新所有层的靶信息到数据库 20250320 by lyh"""
        dbc_p = conn.MYSQL_CONNECT(hostName='192.168.2.19',
                                   database='project_status', prod=3306,
                                   username='root', passwd='k06931!')
        dbc_m = conn.MYSQL_CONNECT(hostName='192.168.2.19', database='hdi_engineering', prod=3306,
                                   username='root', passwd='k06931!')
        sql = """
        select * from hdi_engineering.incam_job_layer_info a
        where a.panel_edge_symbol_coordinate is not null
        and datediff(now(),a.create_time) < 2
        and DATE_FORMAT(a.Create_Time, '%Y-%m-%d %H:%i:%s') not in 
        (select b.cam_notes from hdi_engineering.incam_process_target_info b where b.job_name = a.job_name)"""
        data_info = conn.SELECT_DIC(dbc_p, sql)
        
        #sql = """select job_name,create_time,layer_info,step_info,panel_edge_symbol_coordinate
        #from hdi_engineering.incam_job_layer_info a
        #where a.panel_edge_symbol_coordinate is not null"""
        
        #sql = """select DISTINCT a.job_name,a.create_time,a.layer_info,a.step_info,a.panel_edge_symbol_coordinate from hdi_engineering.incam_job_layer_info a,
        #hdi_engineering.incam_process_target_info b
        #where a.panel_edge_symbol_coordinate is not null
        #and  b.job_name = a.job_name
        #and b.create_time < '2025-05-13 10:00:00'"""
        # data_info = conn.SQL_EXECUTE(dbc_p, sql)
        
        for dic_info in data_info:
        #for array_info in data_info:
            #dic_info = {}
            #dic_info["job_name"] = array_info[0]
            #dic_info["create_time"] = array_info[1]
            #dic_info["layer_info"] = array_info[2]
            #dic_info["step_info"] = array_info[3]
            #dic_info["panel_edge_symbol_coordinate"] = array_info[4]
            
            jobname = dic_info["job_name"].lower()
            #if jobname != "s12602of147a1":
                #continue
            sql = "select * from hdi_engineering.incam_process_target_info  where job_name = '{0}' and cam_notes = '{1}'"
            exists_info = conn.SELECT_DIC(dbc_p, sql.format(jobname, str(dic_info["create_time"])))
            if not exists_info:
                
                sql = "select * from hdi_engineering.incam_process_target_info where job_name = '{0}'"
                exists_job_info = conn.SQL_EXECUTE(dbc_m, sql.format(jobname))
                if exists_job_info:
                    sql = "delete from hdi_engineering.incam_process_target_info where job_name = '{0}'"
                    conn.SQL_EXECUTE(dbc_m, sql.format(jobname))
                    
                try:                    
                    matrix_info = json.loads(dic_info["layer_info"], encoding='utf8')
                    step_info = json.loads(dic_info["step_info"], encoding='utf8')
                    symbol_info = json.loads(dic_info["panel_edge_symbol_coordinate"], encoding='utf8')
                except Exception as e:
                    print(e)
                    continue
                
                if not symbol_info or not step_info or not matrix_info:
                    continue
                
                solderMaskLayers = [lay for i, lay in enumerate(matrix_info["gROWname"])
                                    if matrix_info["gROWcontext"][i] == "board"
                                    and matrix_info["gROWlayer_type"][i] == "solder_mask"]
                
                mai_drill_layers = [lay for i, lay in enumerate(matrix_info["gROWname"])
                                    if matrix_info["gROWcontext"][i] == "board"
                                    and matrix_info["gROWlayer_type"][i] == "drill"
                                    and (re.match("^b\d{1,2}-\d{1,2}$", lay))]# 机械埋孔
                
                mai_man_drill_layers = [lay for i, lay in enumerate(matrix_info["gROWname"])
                                    if matrix_info["gROWcontext"][i] == "board"
                                    and matrix_info["gROWlayer_type"][i] == "drill"
                                    and (re.match("^m\d{1,2}-\d{1,2}$", lay))]  #埋盲孔
                
                tongkongDrillLayer = [lay for i, lay in enumerate(matrix_info["gROWname"])
                                      if matrix_info["gROWcontext"][i] == "board"
                                      and matrix_info["gROWlayer_type"][i] == "drill"
                                      and (re.match("^drl(_ccd)?$|^cdc(_ccd)?$|^cds(_ccd)?$", lay))]
                
                outsignalLayers = [lay for i, lay in enumerate(matrix_info["gROWname"])
                                   if matrix_info["gROWcontext"][i] == "board"
                                   and matrix_info["gROWlayer_type"][i] == "signal"
                                   and matrix_info["gROWside"][i] != "inner"]
                
                innersignalLayers = [lay for i, lay in enumerate(matrix_info["gROWname"])
                                     if matrix_info["gROWcontext"][i] == "board"
                                     and (matrix_info["gROWlayer_type"][i] == "signal"
                                          or matrix_info["gROWlayer_type"][i] == "power_ground")
                                     and matrix_info["gROWside"][i] == "inner"]                
                # 获取sr区域
                try:                    
                    gREPEAT_info = step_info
                ...
                all_jx_x = []
                all_jx_y = []
                for layer, info in symbol_info.iteritems():
                    if re.match("^l\d{1,2}$", layer):
                        for jx_symbol in ["sh-con2", "sh-con"]:
                            if jx_symbol in info:
                                all_jx_x.append(info[1])
                                all_jx_y.append(info[2])
                        if  all_jx_x and all_jx_y:
                            break                        
                
                set_rect_area = []
                for i, name in enumerate(gREPEATstep): 
                    if ("set" in name or "edit" in name or "icg" in name):
                ...
                sr_xmin, sr_ymin, sr_xmax, sr_ymax = 0, 0, 0, 0
                try:                
                    sr_xmin = min([min(x1, x2) for x1, y1, x2, y2 in set_rect_area]) *25.4    
                    sr_ymin = min([min(y1, y2) for x1, y1, x2, y2 in set_rect_area]) *25.4    
                    sr_xmax = max([max(x1, x2) for x1, y1, x2, y2 in set_rect_area]) *25.4      
                    sr_ymax = max([max(y1, y2) for x1, y1, x2, y2 in set_rect_area]) *25.4   
                except :
                    pass                
                
                dic_layer_by_mrpname = {}
                ...
                for dic_mrp_info in array_mrp_info:                    
                    if dic_mrp_info["FROMLAY"]:                    
                        ...
                        dic_layer_by_mrpname[from_lay] = dic_mrp_info["MRPNAME"]
                    if dic_mrp_info["TOLAY"]:                    
                        to_lay = dic_mrp_info["TOLAY"].lower()
                        check_layers.append(to_lay)
                        dic_layer_by_mrpname[to_lay] = dic_mrp_info["MRPNAME"]
                
                if not check_layers:
                    check_layers = outsignalLayers + innersignalLayers

                for worklayer in check_layers:
                    if symbol_info.get(worklayer, None):
                        # 获取备用靶的symbol名
                        byb_symbols = []
                        for layer, info in symbol_info.iteritems():
                            if layer == worklayer:                                                                      
                                byb_symbols += [x[0] for x in info
                                                if ("hdi1-by" in x[0] or "hdi1-byj" in x[0])
                                                and ((len(x) > 5 and "yh_py_pad" not in x[5].values()) or len(x) == 5)]
                                
                        byb_symbols = list(set(byb_symbols))
                        check_byb_symbols = []
                        if byb_symbols:
                            check_byb_symbols = [byb_symbols[0]]

                        for symbolname in check_byb_symbols + ["sh-ldi", "hdi-dwpad", "r0"]:
                            ccd_type_name = u"线路CCD"
                            if symbolname == "sh-ldi":
                                ccd_type_name = u"镭射内靶"
                            elif symbolname == "hdi-dwpad":
                                ccd_type_name = u"五代靶"
                            elif symbolname in byb_symbols:
                                ccd_type_name = u"备用靶"
                            res = self.uploading_target_distance_info(dbc_m, jobname, dic_layer_by_mrpname.get(worklayer, jobname.upper()), ccd_type_name,
                                                                      symbol_info, symbolname, worklayer,
                                                                      str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                      sr_xmax, sr_ymax, [], byb_symbols)
                            if symbolname == "hdi-dwpad" and res[0] == "success":
                                break
                                
                if solderMaskLayers:
                    for worklayer in solderMaskLayers:
                        if symbol_info.get(worklayer, None):
                            # for symbolname in ["r0", "sh-dw2014", "sh-dwsd2014"]:
                            res = self.uploading_target_distance_info(dbc_m, jobname, jobname.upper(), u"防焊CCD",
                                                                      symbol_info, "r0", worklayer,
                                                                      str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                      sr_xmax, sr_ymax, solderMaskLayers, [])
                            for symbolname in ["sh-dw2014", "sh-dwsd2014"]:
                                self.uploading_target_distance_info(dbc_m, jobname, jobname.upper(), u"防焊CCD",
                                                                    symbol_info, symbolname, worklayer,
                                                                    str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                    sr_xmax, sr_ymax, solderMaskLayers, [],
                                                                    exclude_coordinate_info=res[1])                                
                            
                if mai_drill_layers + mai_man_drill_layers:
                    for worklayer in mai_drill_layers + mai_man_drill_layers:
                        if symbol_info.get(worklayer, None):
                            for symbolname in ["r3175"]:
                                mai_sig_layer1 = worklayer.split("-")[0].replace("b", "l").replace("m", "l")
                                # mai_sig_layer2 = "l{0}".format(worklayer.split("-")[1])
                                # if dic_layer_by_mrpname.get(mai_sig_layer1):                                        
                                self.uploading_target_distance_info(dbc_m, jobname, dic_layer_by_mrpname.get(mai_sig_layer1, jobname.upper()), u"CCD埋孔",
                                                                    symbol_info, symbolname, worklayer,
                                                                    str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                    sr_xmax, sr_ymax, [], [])
                if tongkongDrillLayer:
                    for worklayer in tongkongDrillLayer:
                        if symbol_info.get(worklayer, None):
                            for symbolname in ["r3175"]:
                                res = self.uploading_target_distance_info(dbc_m, jobname, jobname.upper(), u"CCD通孔",
                                                                          symbol_info, symbolname, worklayer,
                                                                          str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                          sr_xmax, sr_ymax, [], [])
                                if res[0] != "success":
                                    if innersignalLayers:                                        
                                        res = self.uploading_target_distance_info(dbc_m, jobname, jobname.upper(), u"CCD通孔",
                                                                                  symbol_info, "sh-pindonut", worklayer,
                                                                                  str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                                  sr_xmax, sr_ymax, [], [], check_signal_layer=innersignalLayers[0])
                                    if res[0] != "success":
                                        res = [[], []]
                                        # if "l1" in outsignalLayers:
                                        for layer in outsignalLayers:  
                                            res = self.uploading_target_distance_info(dbc_m, jobname, jobname.upper(), u"线路CCD",
                                                                                      symbol_info, "r0", layer,
                                                                                      str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                                      sr_xmax, sr_ymax, [], [], check_signal_layer=layer)
                                            
                                        res = self.uploading_target_distance_info(dbc_m, jobname, jobname.upper(), u"CCD通孔",
                                                                                  symbol_info, symbolname, worklayer,
                                                                                  str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                                  sr_xmax, sr_ymax, [], [], check_signal_layer=worklayer,
                                                                                  exclude_coordinate_info=res[1])
                                # if "l1" in outsignalLayers:
                                for layer in outsignalLayers:
                                    self.uploading_target_distance_info(dbc_m, jobname, jobname.upper(), u"线路CCD",
                                                                        symbol_info, "sh-dwtop2013" if layer =="l1" else "sh-dwbot2013", layer,
                                                                        str(dic_info["create_time"]) ,sr_xmin, sr_ymin,
                                                                        sr_xmax, sr_ymax, [], [], check_signal_layer=layer,
                                                                        exclude_coordinate_info=res[1])
    
    def uploading_target_distance_info(self, dbc_m, jobname, mrp_name, ccd_type_name,
                                       symbol_info, symbolname, worklayer, cam_notes,
                                       sr_xmin, sr_ymin, sr_xmax, sr_ymax,
                                       solderMaskLayers=[],byb_symbols=[],
                                       check_signal_layer=None, exclude_coordinate_info=[]):
        """上传靶标距离数据"""
        uploading_data_info = [jobname, mrp_name, ccd_type_name, u"短边",
                               "", "", "", "", "", "", "", "", ""]
        arraylist_coordinate_info = []
        array_check_info = symbol_info.get(worklayer, [])
        if check_signal_layer:
            array_check_info = symbol_info.get(check_signal_layer, [])
            if u"线路CCD" in ccd_type_name and "r0" == symbolname:
                array_check_info = [info for info in array_check_info
                                    if "317.reg" in info[5].values()]
        for info in array_check_info:
            #if jobname == "da8622g8987c2":                
                #print ccd_type_name
                #print(worklayer, (info[1],info[2]), [(x[1], x[2]) for x in exclude_coordinate_info])
            if (info[1],info[2]) in [(x[1], x[2]) for x in exclude_coordinate_info]:
                continue
            if symbolname in byb_symbols:
                ...
                if symbolname in info:
                    ...
                    if len(info) > 5:
                        dic_attr = info[5]
                        if u"CCD通孔" in ccd_type_name and \
                           "drl_fxk" not in dic_attr.values() and \
                           not check_signal_layer:
                            del arraylist_coordinate_info[-1]
                        
                        if  symbolname == "hdi-dwpad" and \
                            "split_panel" in dic_attr.values():
                            uploading_data_info[2] = u"分割五代靶"
                            
                        if  symbolname == "sh-ldi" and \
                            "split_panel" in dic_attr.values():
                            uploading_data_info[2] = u"分割镭射内靶"                            
                            
                        if  symbolname in ["sh-dw2014", "sh-dwsd2014"] and \
                            "split_panel" in dic_attr.values():
                            uploading_data_info[2] = u"分割防焊CCD"
                            
        if symbolname == "r0":
            attrs = [" ".join(info[5].values()) for info in arraylist_coordinate_info if len(info) > 5]
            if "300.trmv" in "".join(attrs) and \
               "300.trmh" in "".join(attrs) and \
               "300.measure" in "".join(attrs):
                if worklayer in solderMaskLayers:
                    uploading_data_info[2] = u"分割防焊CCD"
                else:
                    uploading_data_info[2] = u"分割五代靶"
                 
        if u"CCD通孔" in ccd_type_name or u"CCD埋孔" in ccd_type_name:
            if len(arraylist_coordinate_info) > 5:
                if symbolname == "sh-pindonut":                    
                    array_check_info = symbol_info.get(worklayer, [])
                    arraylist_coordinate_info = [info for info in arraylist_coordinate_info
                                                 if (info[1],info[2]) in [(x[1], x[2]) for x in array_check_info]]
                else:
                    if u"CCD通孔" in ccd_type_name and symbolname == "r3175":
                        array_check_info = symbol_info.get("l1", [])
                        arraylist_coordinate_info = [info for info in arraylist_coordinate_info
                                                     if (info[1],info[2]) in [(x[1], x[2]) for x in array_check_info
                                                                              if x[0] in ("sh-dwtop2013", "sh-dwbot2013")]]
                    if u"CCD埋孔" in ccd_type_name and symbolname == "r3175":
                        sig_layer = "l{0}".format(worklayer.split("-")[1])
                        array_check_info = symbol_info.get(sig_layer, [])
                        pin_info = [(x[1], x[2]) for x in array_check_info if x[0] in ("sh-pindonut")]
                        if pin_info:                            
                            arraylist_coordinate_info = [info for info in arraylist_coordinate_info
                                                         if (info[1],info[2]) in pin_info]                        
                
        if not arraylist_coordinate_info:
            return "not_symbol_info", arraylist_coordinate_info 
        
        if len(arraylist_coordinate_info) == 4 or (len(arraylist_coordinate_info) == 5 and \
                                                   (u"CCD通孔" in ccd_type_name or \
                                                   u"CCD埋孔" in ccd_type_name)):
            ...
            try:                
                ...
                #print position
                #print sr_xmin, sr_ymin, sr_xmax, sr_ymax
                #print arraylist_xy
                uploading_data_info[3] = position
                uploading_data_info[4] = length_x1
                uploading_data_info[5] = length_x2
                uploading_data_info[6] = length_y1
                uploading_data_info[7] = length_y2
                uploading_data_info[8] = worklayer
                uploading_data_info[9] = symbolname if symbolname not in byb_symbols else ";".join(byb_symbols)
                uploading_data_info[10] = json.dumps(arraylist_coordinate_info)
                uploading_data_info[11] = "system"
                uploading_data_info[12] = cam_notes                
            except Exception as e:
                print(e)
                uploading_data_info[3] = u"多组靶点"
                uploading_data_info[8] = worklayer
                uploading_data_info[9] = symbolname if symbolname not in byb_symbols else ";".join(byb_symbols)
                uploading_data_info[10] = json.dumps(arraylist_coordinate_info)
                uploading_data_info[11] = "system"
        ...
        self.insert_into_database(jobname, dbc_m, uploading_data_info)
        if u"CCD通孔" in ccd_type_name or u"CCD埋孔" in ccd_type_name:
            if u"CCD埋孔" in ccd_type_name:                
                layer1 = worklayer.split("-")[0].replace("b", "l").replace("m", "l")
                layer2 = "l{0}".format(worklayer.split("-")[1])
            else:
                layer1 = "l1"
                try:                    
                    layer2 = "l{0}".format(int(jobname[4:6]))
                except:
                    layer2 = ""
            uploading_data_info[2] = u"线路CCD"
            for layer in [layer1, layer2]:
                if layer:                    
                    uploading_data_info[8] = layer
                    self.insert_into_database(jobname, dbc_m, uploading_data_info)
                
        return "success", arraylist_coordinate_info
            
    def insert_into_database(self, jobname, dbc_m, params):            
        sql = u"insert into hdi_engineering.incam_process_target_info\
        ...
        conn.SQL_EXECUTE(dbc_m, sql, params)        
    ...
    def get_short_long_position(self, arraylist_xy, sr_xmin, sr_ymin, sr_xmax, sr_ymax):
        ...
        # 左下角
        check_coordinates = [(x, y) for x, y in arraylist_xy
             if y < (sr_ymin + sr_ymax) * 0.5
             and x < (sr_xmin + sr_xmax) * 0.5]
        down_left_x, down_left_y = check_coordinates[0]
        if len(check_coordinates) > 1:
            if down_left_x == upper_left_x:
                down_left_x, down_left_y = check_coordinates[1]
    ...
    def find_slot_hole_info(self):
        ...
        for i, line in enumerate(joblists):
            ...
            run_scripts = "d:/genesis/sys/scripts/lyh/uploading_copper_usage-test.py"
            os.system(
                "python d:/genesis/sys/scripts/lyh/no_gui_login_genesis.py %s no_gui" % (run_scripts))            
            # os.system("pause")
            run_scripts = "d:/genesis/sys/scripts/lyh/del_job.py no_gui"
...
if __name__ == "__main__":
    ...
    # 测试    
    if func is None:
        func = "update_all_layer_target_info_to_mysql"
        # args = ["26953","gui_run"]
    ####
    if func is None:
        main.run()
