...
class AutoMakePressRout(object):
        ...
        for data in inplan_all_flow:
            if data['WORK_CENTER_CODE'] == '打靶' and data['PROCESS_DESCRIPTION'] == 'X-RAY打靶':
                ...
                if step.isLayer(pnl_rout):
                    ...
                    step.COM('stretch_feat,index=%s,x=%s,y=%s,xs=%s,ys=%s,xe=%s,ye=%s,tol=97.895' % (int(index) - 1, xe, stretch_y, xs, stretch_ys, xe, ymax - 1.2))
                    step.clearSel()
                    ...
                    step.COM('delete_feat,mode=intersect,index=%s,x=%s,y=%s,tol=76.645' % (int(index) - 1, xmax - 12.2, ymin + 1.2))
    ...
    def output(self, layer):
        ...
        if os.environ.get('INCAM_PRODUCT'):
            ...
            self.job.COM("ncr_ncf_export,dir=%s,name=%s.%s" % (self.output_path, jobname.upper(), layer.replace('+', '')))
