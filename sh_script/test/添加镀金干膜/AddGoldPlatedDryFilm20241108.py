...
class AddGoldPlatedDryFilm(QtGui.QWidget, ui.Ui_Form):
    ...
    def make(self):
        ...
        for xlym, signal in xlym_list.items():
            ...
            rects = []
            for d in info:
                x, y, symbol = float(d[1]), float(d[2]), d[3]
                rects.append([x, y, symbol])
            print(rects)
            ...
            for i in range(len(rects)-1):
                ...
                yts1, yts2 = cy1 + h1 / 2, cy2 + h2 / 2     # 上方y值
                yte1, yte2 = cy1 - h1 / 2, cy2 - h2 / 2     # 下方y值
                # 第一个位置
                if i == 0:
            ...
            print(lines)
            step.PAUSE('B')
            step.removeLayer('test_%s' % xlym)
            step.createLayer('test_%s' % xlym)
            step.affect('test_%s' % xlym)
            for line in lines:
