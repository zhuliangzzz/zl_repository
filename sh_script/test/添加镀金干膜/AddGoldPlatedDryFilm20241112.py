        ...
        if self.comboBox.currentText() == u'无蚀刻引线':
            ...
            for xlym, signal in xlym_list.items():
                ...
                if step.Selected_count():
                    ...
                    step.unaffectAll()
                    QtGui.QMessageBox.warning(self,u'提示', u'未找到手指引线\n请定义(.n_electric属性)')
                    sys.exit()
                step.unaffectAll()
                # 属性去找
                # step.affect(signal)
                ...
                step.affect(tmp_)
                step.refSelectFilter(test_xlym)
                if step.Selected_count():
                    step.selectReverse()
                    if step.Selected_count():
                        step.selectDelete()
                ...
                # step.unaffectAll()
                # 把金手指区域上方切掉
                infoDict = step.DO_INFO(' -t layer -e %s/%s/%s -d LIMITS,units=mm' % (job_name, step.name, test_xlym + '+++'))
