...
class IncrementBackup(object):
    def __init__(self):
        ...
        self.source_dir, self.backup_dir, self.backup_add_dir, self.diff_time = None, None, None, None
        self.num_threads = 10
        ...
        try:
            ...
            self.backup_dir = data.get('job_backup_path').encode('utf8')
            self.backup_add_dir = data.get('job_backup_addon_path').encode('utf8')
    ...
    def copy_dir(self, source):
        ...
        if os.path.exists(os.path.join(self.backup_dir, archive_name)):
            # add_dest = os.path.join(self.backup_dir + '_addon', archive_name)
            add_dest = self.backup_add_dir
            # os.system('rsync -avz --delete %s %s %s' % (source, dest, add_dest))  # 增量同步 提升备份效率
            # 定义命令和参数
            rsync_command = ["rsync", "-avz", "--delete", '--link-dest=%s' % dest, source, add_dest]
            # 创建子进程
            ...
            # os.system('rsync -avz  %s %s' % (source, dest))  # 第一次全量备份
            rsync_command = ["rsync", "-avz", source, dest]
            # 创建子进程
            process = subprocess.Popen(rsync_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            # 获取命令的输出和错误信息
            output, error = process.communicate()
