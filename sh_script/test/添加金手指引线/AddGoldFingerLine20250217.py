...
def AddGoldFingerLine():
    ...
    for outsignalLayer in outsignalLayers:
        ...
        step.COM('set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=no,attribute=.n_electric,min_int_val=0,max_int_val=0,min_float_val=0,max_float_val=0,option=,text=')
        step.COM('set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=no,attribute=.bit,min_int_val=0,max_int_val=0,min_float_val=0,max_float_val=0,option=,text=')
        ...
        if finger_group:
            ...
            step.unaffectAll()
        step.removeLayer(tmp)
    QtGui.QMessageBox.information(None, u'运行完成', u'请查看:\n%s' % u'\n'.join(['%s+++' % outsignalLayer for outsignalLayer in outsignalLayers]))
    exit()
