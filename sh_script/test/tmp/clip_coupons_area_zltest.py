...
# 20250621 zl 切铜前保留铜皮防止panel去细丝前切铜
# ---------------------------------------------------------#
import sys
import os
import platform
...
class MyApp(object):
    def __init__(self, step_name):
        ...
        self.fill_layer_pnl = 'sr_fill_pnl'
        self.flat_layer_pnl = 'sr_fill_flat_pnl'
        ...
        # 铜皮层
        self.surface_layer = 'inner_panel_surface'
        ...
        for step in SR:
            ...
            if re.match(r'^set\d?\d?$|^edit\d?\d?$', step):
                if step_now == 'panel':
                    ...
                    self.GEN.WORK_LAYER(self.fill_layer_pnl)
                    ...
                    self.GEN.COM(
                        ...
                        % ('positive', '-' + str(out_dist), '-' + str(out_dist), 0, 0, 0, 0, 0, 0))
                    self.GEN.COM('cur_atr_reset')
    ...
    def flatten_layer(self):
        ...
        self.GEN.COM('flatten_layer,source_layer=%s,target_layer=%s' % (self.fill_layer_pnl, self.flat_layer_pnl))
    ...
    # 20250620 zl
    def generate_innersurface(self):
        if self.step_name != 'panel':
            return
        job = genClasses_zl.Job(self.job_name)
        step = genClasses_zl.Step(job, self.step_name)
        step.initStep()
        ...
        for layer in layers:
            ...
            step.resetFilter()
            # 找铺铜
            if step.isLayer(self.surface_layer):
                ...
                if not step.Selected_count():
                    step.unaffectAll()
                    step.affect(self.surface_layer)
                    step.COM(
                        'set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=yes,attribute=.string,min_int_val=0,max_int_val=0,min_float_val=0,max_float_val=0,option=,text=%s' % layer)
                    step.COM('set_filter_and_or_logic,filter_name=popup,criteria=inc_attr,logic=or')
                    step.selectAll()
                ...
                if not step.Selected_count():
                    ...
                    step.copySel(self.surface_layer)
                    step.unaffectAll()
            else:
                step.affect(tmp_surface)
                step.addAttr('.string', attrVal='text=%s' % layer)
            ...
            step.removeLayer(tmp_signal)
            step.removeLayer(tmp_surface)
        CheckUserOpenJob(self.job_name).lock_panel_steps([self.surface_layer])
    def clip_panel_area(self):
        ...
        if 'panel' in self.step_name:
            # 线路层将单只内。parten_fill也切一下铜 20250612加入 by ynh
            self.GEN.COM('affected_filter,filter=(type=signal&context=board)')
            self.GEN.FILTER_RESET()
            self.GEN.FILTER_SET_FEAT_TYPES('surface')
            self.GEN.COM(
                "set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=no,attribute=.pattern_fill")
            self.GEN.FILTER_SELECT()
            self.GEN.COM("sel_reverse")
            self.GEN.COM('clip_area_strt')
            self.GEN.COM(
                'clip_area_end,layers_mode=affected_layers,layer=,area=reference,area_type=rectangle,inout=inside,'
                'contour_cut=no,margin=0,ref_layer=%s,feat_types=surface' % self.flat_layer_pnl)
            self.GEN.CLEAR_LAYER()
            self.GEN.FILTER_RESET()
