...
#		chdir "$do_path";
	...
	$windows_path = decode('utf8',$windows_path);
	&find_fileindir("$tgz_path");
	# 20250429 zl 判断是否有重复tgz
	my %hash;
	foreach my $input_path(@joblist_input_path){
	    my $tmp_input_path = $input_path;    # 20250430 zl 重新用变量，否则会改变joblist_input_path内的元素，导致inport_job内的路径错误
        my @sperators = split /\//,$tmp_input_path;
        my $tgzname = $sperators[-1];
        $tmp_input_path =~ s/$tgz_path/$windows_path/;
        push(@{$hash{$tgzname}}, $tmp_input_path);
	}
	my @info;
	foreach my $key(keys %hash){
	    if(scalar(@{$hash{$key}}) > 1){
            push(@info, join(decode('utf8','、'), @{$hash{$key}}));
#            push(@info, join('、', @{$hash{$key}}));
	    }
	}
	if(scalar(@info)>0){
#	    if(scalar(@pp_data) eq '1'){  # 只输入系列号，则不显示重复的tgz
#            my $ans1 = $mw9->messageBox(-icon => 'question',-message => decode('utf8',"存在重复tgz!请检查"),-title => 'Warning',-type => 'ok');
#	    }else{
#            my $infostr = join('\n', @info);
#            my $ans1 = $mw9->messageBox(-icon => 'question',-message => decode('utf8',"存在重复tgz!\n"). $infostr . decode('utf8', "\n请检查"),-title => 'Warning',-type => 'ok');
#	    }
        my $infostr = join('\n', @info);
        my $ans1 = $mw9->messageBox(-icon => 'question',-message => decode('utf8',"存在重复tgz!\n"). $infostr . decode('utf8', "\n请检查"),-title => 'Warning',-type => 'ok');
	}
...
sub OpenJob {
	...
	#检测层是否有损坏 20250210 by lyh
	$ENV{JOB} = $tJob;
	system("python /incam/server/site_data/scripts/sh_script/lyh/auto_sh_hdi_check_rules.py reread_all_step_layer");
...
sub run_message_job_yes {
	my $ans1 = $mw9->messageBox(-icon => 'info',-message => hanzi("料号已存在!导入动作终止"),-title => 'quit',-type => 'ok');
