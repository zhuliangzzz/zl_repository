            ...
            if top_v5_num > 0:
                ...
                if check_result == "1to2":                    
                    ...
                    if include_num != top_v5_num:
                        warn_text.append({
                        "content": u'层别:%s,五代靶应设计为1通2,但层:%s s5080的Symbol应选中数:%s,实际选中数:%s' % (cur_tlyr,next_one_layer,top_v5_num,include_num),
                        "type": "critical",
                    ...
                    if include_num != top_v5_num:
                        warn_text.append({
                        "content": u'层别:%s,五代靶应设计为1通3,但层:%s s5080的Symbol应选中数:%s,实际选中数:%s' % (cur_tlyr,next_one_layer,top_v5_num,include_num),
                        "type": "critical",
                        "layers":[v5_top_bar,next_one_layer]
                        })
                    self.GEN.FILTER_RESET()
                    ...
                    if include_num != top_v5_num:
                        warn_text.append({
                        "content": u'层别:%s,五代靶应设计为1通3,但层:%s s5080的Symbol应选中数:%s,实际选中数:%s' % (cur_tlyr,next_two_layer,top_v5_num,include_num),
                        "type": "critical",
            ...
            if bot_v5_num > 0:
                ...
                if check_result == "1to2":                    
                    ...
                    if include_num != bot_v5_num:
                        warn_text.append({
                        "content": u'层别:%s,五代靶应设计为1通2,但层:%s s5080的Symbol应选中数:%s,实际选中数:%s' % (cur_blyr,next_one_layer,bot_v5_num,include_num),
                        "type": "critical",
                    ...
                    if include_num != bot_v5_num:
                        warn_text.append({
                        "content": u'层别:%s,五代靶应设计为1通3,但层:%s s5080的Symbol应选中数:%s,实际选中数:%s' % (cur_blyr,next_one_layer,bot_v5_num,include_num),
                        "type": "critical",
                        "layers":[v5_bot_bar,next_one_layer]
                        })
                    self.GEN.FILTER_RESET()
                    ...
                    if include_num != bot_v5_num:
                        warn_text.append({
                        "content": u'层别:%s,五代靶应设计为1通3,但层:%s s5080的Symbol应选中数:%s,实际选中数:%s' % (cur_blyr,next_two_layer,bot_v5_num,include_num),
                        "type": "critical",
