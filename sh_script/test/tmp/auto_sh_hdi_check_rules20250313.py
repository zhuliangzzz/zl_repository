...
class auto_check_rule(object):
        ...
        for mrp_name in set(names):            
            ...
            if sz_process:                
                ...
                if array_sz_info:
                    ...
                    if not process_run:
                        return u"检测到此板有塞孔检验流程，但公共程序 制作树脂塞孔检测资料 程序未运行，请检查！", None
                
                jc_sz_layers = []
                for layer in matrixinfo["gROWname"]:
                    if layer.startswith("jc-"):
                ...
                for layer in jc_sz_layers:
                    if layer.replace("jc-", "") in "".join(arraylist_erp_sz_drill):
                ...
                for stepname in ["panel"]:
                    ...
                    for jc_layer in jc_sz_layers:
                        ...
                        for ref_layer in matrixinfo["gROWname"]:
                            if "...dq" in ref_layer:
                                continue
                            if jc_layer.replace("jc-", "") in ref_layer:
    ...
    def check_send_finish_status(self, *args, **kwargs):
        ...
        if self.dic_make_info :
            ...
            for key1, key2,key3 in [(u"(芯板)内层完成送审", u"(芯板)内层送审状态", u"(芯板)内层审核状态"),
                ...
                if self.dic_make_info.get(key3, None) in [u"已退审", u"送审中"]:
                    # print("-------------->2", key3.encode("utf8"), args[0], dic_type[args[0]].encode("utf8"))
                    if dic_type[args[0]] in key3:
                        showMessageInfo(u"检测到【{0}】为【{1}】，请通知CAM审核更新状态后再运行程序！".format(key3, self.dic_make_info[key3]))
                        if 6< h_time < 23:                            
    ...
    def check_output_layer_is_locked(self, *args, **kwargs):
        ...
        #多层事业部型号 不检测
        res = get_cam_maker_user_info(job.name)
        if not res:
            return 0
        ...
        for worklayer in args:            
            ...
            try:
                ...
                if worklayer.startswith("l"):
                    if worklayer.split("-")[0] in innersignalLayers or \
                       worklayer in innersignalLayers:
                        res = self.check_send_finish_status("inner", return_result="yes", **kwargs)
                        if res == 0:
                            res = self.check_send_finish_status("all", return_result="yes", **kwargs)
                            
                    elif worklayer.split("-")[0] in outsignalLayers or \
                       worklayer in outsignalLayers:
                        res = self.check_send_finish_status("outer", return_result="yes", **kwargs)
                        if res == 0:
                            res = self.check_send_finish_status("all", return_result="yes", **kwargs)                    
                    
                if "-ls" in worklayer:                    
                    res = self.check_send_finish_status("linshi", return_result="yes", **kwargs)
            ...
            for stepname in check_steps:
                ...
                #if user in ["84310"]:
                    #if not lock:                        
                        #job.PAUSE(str([lock, self.pre_lock]))                            
        ...
        if find_not_compare_layers:
            ...
            message_result = u"检测到值班流程中【临时层比对】程序未运行 【{0}】此层的比对，或运行结果异常，请处理正常后，才能输出！".format(
        ...
        if find_not_lock_layers:
            ...
            #showMessageInfo(u"检测到{0}...层未全部上锁，请通知审核是否已全部审核ok，<br>若已审核，请通知审核人手动上锁！".format(find_not_lock_layers),
                            #scoll_bar="yes", 
                            #scoll_widht_height=[800, 200])
            message_result = u"检测到{0}...层未全部上锁，请通知审核是否已全部审核ok，<br>若已审核，请通知审核人手动上锁！！".format(
                ",".join(find_not_lock_layers)
            )            
            result = send_message_to_director(message_result, job.name, message_result, password_approve=True)
            if not result:
                if kwargs.get("return_result", None) == "yes":
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            for stepname in all_steps:
                ...
                for worklayer in check_sig_layers:
                    ...
                    step.refSelectFilter("npth_tmp", mode="cover")
                    if step.featureSelected():
                        step.selectDelete()
    ...
    def check_set_to_set_connection_mask_open(self, *args):
        ...
        arraylist_log = []
        dic_zu_layer = {}
        if "auto_make_connection" not in args:
    ...
    def check_loss_icg_line_cover_by_reflayer(self, signal_layers, arraylist_imp_info):        
    ...
    def select_bga_smd_by_surface_around(self):
        ...
        for layer in matrixinfo["gROWname"]:            
            if "_surface" in layer or "_bga" in layer and "inner_panel_surface" not in layer:
                step.removeLayer(layer)
        ...
        for worklayer in outsignalLayers:
            ...
            step.COM("sel_resize,size=-{0}".format(20*25.4))
            step.selectAll()
            layer_cmd = gClasses.Layer(step, worklayer+"_surface")
            feat_indexes = layer_cmd.featSelIndex()
            
            for sur_index in feat_indexes:
                if int(sur_index) != 787:
                    continue
                step.clearAll()
                step.affect(worklayer+"_surface")                
                step.selectNone()
                step.selectFeatureIndex(worklayer+"_surface", sur_index)
                ...
                #layer_cmd = gClasses.Layer(step, sel_surface_layer)
                #feat_out = layer_cmd.featout_dic_Index(units="mm", options="feat_index")["lines"]
                #feat_out += layer_cmd.featout_dic_Index(units="mm", options="feat_index")["arcs"]
                step.filter_set(feat_types='line;arc', polarity='positive')
                step.selectRectangle(xmin+0.005, ymin+0.005, xmax-0.005, ymax-0.005)
                step.COM("sel_reverse")
                if step.featureSelected():
                    step.selectDelete()                
                else:
                    log = u"{0} 层surface：{1} 分析有异常，请参考层{2}手动处理此块surface区域对应的bga！".format(
                        worklayer, sur_index, worklayer+"_surface_{0}".format(sur_index)
                    )
                
                step.COM("sel_cut_data,det_tol=25.4,con_tol=25.4,rad_tol=2.54,"
                         "ignore_width=yes,filter_overlaps=no,delete_doubles=no,"
                         "use_order=yes,ignore_holes=none,start_positive=yes,"
                         "polarity_of_touching=same,contourize=yes,simplify=yes,"
                         "resize_thick_lines=no")
                step.resetFilter()
                step.filter_set(feat_types='surface', polarity='positive')
                step.selectAll()
                if step.featureSelected():
                    step.copyToLayer(sel_surface_layer+"_inner_area", size=10)
                    
                    step.clearAll()
                    step.affect(sel_surface_layer+"_line_bak")
                    step.resetFilter()
                    step.refSelectFilter(sel_surface_layer+"_inner_area", mode="cover")
                    if step.featureSelected():
                        step.selectDelete()
                        
                    step.clearAll()
                    step.affect(sel_surface_layer+"_inner_area")
                    step.COM("sel_resize,size=-10")
                    
                    
                
                step.PAUSE(str(sur_index))                
    ...
    def check_dynamic_barcode_output_odb_is_right(self, tgz_file_path=None):
        ...
        if os.path.exists(tgz_file_path):
            ...
            for stepname in matrix_info["gCOLstep_name"]:
                ...
                #if stepname == "set":
                    #step.PAUSE("check")
                for worklayer in outsignalLayers + solderMaskLayers + silkscreenLayers:
                    if step.isLayer(worklayer):
                        step.clearAll()
                        step.affect(worklayer)
                        ...
                        if step.featureSelected():
                            ...
                            for obj in feat_out:
                                ...
                                if symbolname in barcode_symbols:  
                                    ...
                                    step.addPad(obj.x, obj.y, symbolname)
                                    add_barcode = True
                        ...
                        if step.featureSelected():
                            ...
                            for obj in feat_out: 
                                ...
                                step.clearAll()
                                step.affect(worklayer+"_barcode_tmp")
                                step.addPad(obj.x, obj.y, obj.symbol, polarity="positive")
                                add_barcode = True
                    ...
                    if step.featureSelected():
                        ...
                        for index in indexes:
                            ...
                            if index not in touch_pad_indexes:
                                ...
                                if self.intersection_area(*area_xy1+area_xy2) and check_intersection:                                        
                                    #if obj.symbol == "rect15331x5000" and "-lyh" in job.name:
                                        #step.removeLayer("check_tmp")
                                        ...
                                        #step.addRectangle(*area_xy1)
                                        #step.addRectangle(*area_xy2)
                                        #step.PAUSE(str([worklayer, area_xy1, area_xy2, obj.symbol, touch_obj.symbol, obj.feat_index, touch_obj.feat_index]))
