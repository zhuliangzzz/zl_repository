...
class VIA_process(object):
    ...
    def inner_process(self):
        ...
        if self.layer not in outer:
            ...
            self.GEN.ADD_PAD(self.hole_inner_x, self.hole_inner_y, inner_pad, pol='positive', nx=1, ny=2, dx=0, dy=30)
...
class stub_process(object):
    ...
    def pad_process(self):
        ...
        if 'rect' in self.parm.testSize:
            ...
            testMask = 'r%s' % (self.parm.testSize + 3)
...
class line_process(object):
    ...
    def __init__(self, layer='l1', org_x=0, org_y=0, line_width=6.7, line_space=3.8, line_lenth=2.0, factor=1,
                 upper_length=0, lower_length=0, up_first='no'):
        self.org_x = org_x
        self.org_y = org_y
        self.layer = layer
...
class Loss(object):
    ...
    def loss_process(self, org_x, org_y, layer, refer1, refer2, line_width, line_space, org_width, org_space, ohms,
        ...
        if stub_side is None:
            # --按远的stub值
            stub_side = 'bottom' if layer_id < self.layer_number / 2 else 'top'
        ...
        for loss_len in loss_len_list:
            ...
            for direct in ['left', 'right']:
                if direct in ['left']:
                    ...
                    line = line_process(layer=line_layer, org_x=org_x, org_y=org_y, line_width=line_width,
                        line_space=line_space, line_lenth=loss_len, factor=factor)
                    line.all_process()
            ...
            if stub_side == 'top':
                ...
                if layer == next_layer:
                    # --同一层有两个走线
                    self.mark_top_x.append(org_x + mark_len + 0.1 + 0.25)
                    ...
                    # --内层走线
                    self.mark_top_x.append(org_x + 0.25)
                ...
                if layer not in outer:
                    ...
                    self.mark_bot_x.append(org_x + 2 * mark_len + 0.25575)
                    # --y坐标定义
