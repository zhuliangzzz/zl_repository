...
sub ULAction
{
    ...
    $f->COM("add_pad,attributes=no,x=$point[0],y=$point[1],symbol=$symbol,polarity=$polarity,angle=$rotate,mirror=$mirror,nx=1,ny=1,dx=0,dy=0,xscale=$ul_x_size,yscale=$ul_y_size");
    $mw->deiconify;    
    $mw->raise;
    return 1;
}
...
sub RohsAction
{
    ...
    $f->MOUSE("p select point");
    @point = split(' ',$f->{MOUSEANS});
    $f->COM("add_pad,attributes=no,x=$point[0],y=$point[1],symbol=$symbol,polarity=$polarity,angle=$rotate,mirror=$mirror,nx=1,ny=1,dx=0,dy=0,xscale=$rohs_x_size,yscale=$rohs_y_size");
    $mw->deiconify;    
    $mw->raise;
    return 1;
}
...
sub PbAction
{
    ...
    $f->MOUSE("p select point");
    @point = split(' ',$f->{MOUSEANS});
    $f->COM("add_pad,attributes=no,x=$point[0],y=$point[1],symbol=$symbol,polarity=$polarity,angle=$rotate,mirror=$mirror,nx=1,ny=1,dx=0,dy=0,xscale=$pb_x_size,yscale=$pb_y_size");
    $mw->deiconify;    
    $mw->raise;
    return 1;
}
