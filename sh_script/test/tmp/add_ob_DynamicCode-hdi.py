...
# ---------------------------------------------------------#
# 20250401 zl 周期未匹配到，设置周期可选，能匹配到的还是默认不可选
...
class MainWindowShow(QtGui.QMainWindow):
    ...
    def set_UI_by_SQL(self):
        ...
        week_code = re.search('([WY]+)', get_dc_type)
        if week_code:
            week_code = re.search('([WY]+)', get_dc_type).group(1)
        if week_code in ['WWYY', "YYWW"]:
            type_value = week_code.replace("WW", "$$WW").replace("YY", "$$YY")
            ...
            # self.ui.comboBox_bz_date.setEnabled(True)  # 20250401 zl 匹配不到的设置为可选
            self.ui.comboBox_bz_date.setCurrentIndex(2) # 其他
            # get_dc_type = self.dc_type['VALUE']
