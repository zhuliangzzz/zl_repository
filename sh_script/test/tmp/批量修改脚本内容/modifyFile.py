...
class ModifyFile():
    def __init__(self):
        ...
        self.search_word = '/workfile/hdi_film/'
    ...
    def modify(self, file):
        if os.path.isfile(file):
            ...
            if '.' in filename:
                back_file = os.path.join(dir_path, filename.replace('.', '_modify_back2.'))
                tmp_filename = os.path.join(dir_path, filename.replace('.', '_modify_tmp.'))
            else:
                back_file = os.path.join(dir_path, filename + '_modify_back2.')
