...
class MyApp(object):
    def __init__(self):
        ...
        if os.path.exists(self.scrDir + '/impCouponComp_Inner_v21.03.05.ini'):
            # f = open (r'impCouponComp_Inner.ini')
            f = open(self.scrDir + '/impCouponComp_Inner_v21.03.05.ini')
        ...
        if os.path.exists(self.scrDir + '/impCouponComp_Sec1_v21.03.05.ini'):
            f = open(self.scrDir + '/impCouponComp_Sec1_v21.03.05.ini')
        ...
        for sr_info in self.panelSRtable:
            if icg_reg.match(sr_info['OP_STEP']):
                ...
                if coupon_angle == 0:
                    ...
                    msg_box = msgBox()
                    msg_box.critical(self, '警告', u"阻抗条角度%s不在定义范围!" % (coupon_angle), QtGui.QMessageBox.Ok)
                    return False
    ...
    def check_touch(self, ip_coupon, exist_area):
        ...
        for c in exist_area:
            ...
            if len(x_sort_num) <= 3 and len(y_sort_num) <= 3:
                ...
                if (exist_area[c]['xmin'] - ip_coupon['dx']) <= ip_coupon['xmin'] < ip_coupon['xmax'] <= (
                        exist_area[c]['xmax'] + ip_coupon['dx']) and (exist_area[c]['ymin'] - ip_coupon['dy']
                ) <= ip_coupon['ymin'] < ip_coupon['ymax'] <= (exist_area[c]['ymax'] + ip_coupon['dy']):
                    ip_in_exist_yn = c
    ...
    def deal_with_sqldata(self, inpVal):
        ...
        for line in range(len(dataVal)):
            ...
            dataVal[line]['ORG_WIDTH'] = round(dataVal[line]['ORG_WIDTH'], 4)
            # 记录所有阻抗序号
            # dataVal[line]['ID'] = ind
...
class MainWindowShow(QtGui.QWidget, MyApp):
    ...
    def addUiDetail(self):
        ...
        """
        # === V5.1 暂时不勾选 添加孔
        self.ui.checkBox_innerpad.setChecked(False)
        ...
        tableRowWidth = [52, 40, 50, 50, 50, 52, 52, 50, 60, 55, 95, 60, 60, 60, 65, 50, 50, 50, 75]
        ...
        # 显示共面差分列 7 --> 8
        self.ui.tableWidget_LyrList.setColumnCount(9)
        self.ui.tableWidget_LyrList.setRowCount(len(self.sig_layers))
        tableLyrRowWidth = [52, 38, 45, 85, 65, 85, 85, 85, 75]
        ...
        if check_all_coupon_num == range(1, len(check_all_coupon_num) + 1, 1):
            ...
            showText = u"阻抗分组不连续，现在为%s!" % (check_all_coupon_num)
            msg_box = msgBox()
            msg_box.warning(self, '警告', showText, QtGui.QMessageBox.Ok)
        ...
        for ig in check_all_coupon_num:
            ...
            if len(check_current_mode) >= 3:
                ...
                check_result = True
            check_all_mode[ig] = check_result
            if not check_all_mode[ig]:
                self.ui.pushButton_3.hide()
        ...
        for x, line in enumerate(self.ip_data):
            ...
            for c_index, c_key in enumerate(column_keys):
                if c_key not in ['WGROUP1', 'WGROUP2']:
                ...
                if c_key == "SPC2CU" and float(line[c_key]) > 20:
                    self.table_item_set(x, c_index, bg='green', fg='white', tool_tip='底色绿色时为非常规设计,注意检查.')
                if c_key == "IMODEL" and not check_all_mode[line["IGROUP"]]:
                    self.table_item_set(x, c_index, bg='red')
            ...
            # 此MI序号基本与CAM序号不一致，CAM序号按层别分类，保证同一类型的线按一种形式分布，note by song 2020.05.20
            # if x + 1 != int(line["ID"]):
            #     self.ui.tableWidget.item (x, 16).setBackground (QtGui.QBrush (QtGui.QColor (255, 0, 0)))
        # === V4.0.1 增加分割阻抗，界面做变更 ===
    ...
    def table_item_set(self, row, colum, bg='', fg='', tool_tip=None, text=None):
        ...
        if text:
            if type(text) == int or type(text) == float:
                self.ui.tableWidget.setItem(row, colum, QtGui.QTableWidgetItem(str(text)))
            else:
    ...
    def MAIN_RUN(self):
        ...
        x = get_imp_data
        rever_imp_data = sorted(x, key=lambda x: x['id'], reverse=True)
        ...
        pass_list = [self.hole_wid, self.dingwei_wid, self.cedian_spc_x, self.cedian_spc_y, self.min_p2line,
                     self.job_layer_num,self.inner_test_add]
        CouponRun = ImpRun(rever_imp_data, self.icgCamData, self.teamc, self.coupon_area, self.coupon_num,
                           self.sig_layers, pass_list, self.tool_size_rule)
        ...
        if getwarn is not None:
            showText = u'%s 程序已运行完成，请检查.' % getwarn
            msg_box = msgBox()
            msg_box.warning(self, '警告', showText, QtGui.QMessageBox.Ok)
        else:
...
class ImpRun:
    ...
    def __init__(self, rever_imp_data, icgCamData, teamc, coupon_area, coupon_num, sig_layers, ui_info_list,tool_size_rule):
        ...
        self.job_name = os.environ.get('JOB', None)
        self.hole_wid, self.dingwei_wid, self.cedian_spc_x, self.cedian_spc_y, self.min_p2line, self.job_layer_num, self.inner_test_add = ui_info_list
        ...
        if self.inner_test_add:
            ...
            if len(self.drl_info['drl_layer']) > 1:
                ...
                burry_drl_list = [int(self.drl_info['min_list'][i]) for i, k in enumerate(self.drl_info['drl_layer']) if re.match('b[0-9]', k)]
    ...
    def icgCouponRun(self):
        ...
        for i in range(1, self.coupon_num + 1):
            ...
            coupon_length = self.icgCamData[i]['each_coupon_length']
            ...
            # === TODO 添加其他层的钻孔
            if self.inner_test_add is True and self.add_other_hole is True:
                oth_index = 1
                for h_index,hole_layer in enumerate(self.drl_info['drl_layer']):
                    if re.match('b[0-9]',hole_layer):
                        GEN.CLEAR_LAYER()
                        ...
                        for index, line in enumerate(coupon_data):
                            ...
                            self.Add_cedian_pad(*each_cor_list, atype='pad')
                    elif re.match('s[0-9]', hole_layer):
                        GEN.CLEAR_LAYER()
                        GEN.AFFECTED_LAYER(hole_layer, 'yes')
                        # 埋孔层加孔
                        drl_size = float(self.drl_info['min_list'][h_index])/25.4
                        # 四个位置循环加孔，左右下上
                        if oth_index % 4 == 1:
                            hc_keys = ['hx1', 'start_y0', 'add_nx', 'add_ny', 'add_dx', 'add_dy', 'mode', 'place']
                        elif oth_index % 4 == 2:
                            hc_keys = ['hx2', 'start_y0', 'add_nx', 'add_ny', 'add_dx', 'add_dy', 'mode', 'place']
                        elif oth_index % 4 == 3:
                            hc_keys = ['start_x0', 'hy1', 'add_nx', 'add_ny', 'add_dx', 'add_dy', 'mode', 'place']
                        else:  # oth_index % 4 == 0:
                            hc_keys = ['start_x0', 'hy2', 'add_nx', 'add_ny', 'add_dx', 'add_dy', 'mode', 'place']
                        for index, line in enumerate(coupon_data):
                            each_cor_list = [drl_size] + [line[ck] for ck in hc_keys] + ['positive']
                            self.Add_cedian_pad(*each_cor_list, atype='pad')
                        oth_index += 1
            ...
            if coupon_height > 450:
                chg_text_dy = 0.0075
            for line in coupon_data:
                # print json.dump(line, sort_keys=True)
                kk = line['igroup']
                # --开始添加阻抗线--
                ...
                if line['imodel'] == '特性' or line['imodel'] == '特性共面':
                    ...
                    if imp_line_length < 4500:
                        warn_text += u"检测到：Coupon %s 层 %s 线长小于4.5英寸.|\n" % (line['igroup'], line['worklay'])
                    ...
                    self.add_sig_imp_line(start_x0, start_y0, line1_wid, self.cedian_spc_x, self.cedian_spc_y,
                                          ...
                                          imp_line_length, spctocu=linetocu, add_locate=place, add_attr='yes')
                    ...
                    if imp_line_length < 4500:
                        warn_text += u"检测到：Coupon %s 层 %s 线长小于4.5英寸。|" % (line['igroup'], line['worklay'])
                    if line['wgroup'] == 1:
                        work_minp2line = self.min_p2line
                    elif line['wgroup'] == 2:
                        text_y1 = start_y0 - 0.015 + chg_text_dy + self.cedian_spc_y * 0.001
                    ...
                    if line['reftop'] == "" and line['refbot'] == "":
                        ...
                        if self.inner_test_add is False:
                            # === V5.1 所有层加pad
                            GEN.ADD_PAD(*pad_c1, nx=1, ny=1, dx=self.cedian_spc_x, dy=self.cedian_spc_y)
                            GEN.ADD_PAD(*pad_c2, nx=1, ny=1, dx=self.cedian_spc_x, dy=self.cedian_spc_y)
                        # 增加交叉线，接地pad连接
                ...
                if int(GEN.COMANS) != 0:
                    ...
                    GEN.GET_SELECT_COUNT()
                    if int(GEN.COMANS) != 0:
                        warn_text += u"检测到：coupon%s从右向左序号%s位置有相交字.|" % (i, line['gindex'])
                    GEN.CLEAR_LAYER()
                    GEN.AFFECTED_LAYER('imp_mark', 'yes')
                    GEN.SEL_REVERSE()
                    # 偷懒写法，直接移动到L1
    ...
    def add_sig_imp_line(self, start_x0, start_y0, line1_wid, cedian_spc_x, cedian_spc_y, cedian_wid, min_p2line,
                         imp_line_length, type='posline', spctocu=20, add_locate='up', add_attr='no'):
        """
        单线阻抗程序
        :param start_x0:
        :param start_y0:
    ...
    def split_combineCoupon(self):
        ...
        for coupon_tail in coupon_area:
            ...
            zkLength = coupon_area[coupon_tail]['each_coupon_length']
            ...
            for place_i in range(1, len(coupon_id_list) + 1):
                ...
                # couponName = icg_start[str(zkIndex)] + '_' + job_team[str(zkIndex)] + '_' + str(zkIndex)
