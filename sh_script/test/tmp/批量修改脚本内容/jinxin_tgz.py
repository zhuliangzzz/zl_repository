...
class Main:
    def __init__(self, debug=False):
        ...
        if self.GEN.LAYER_EXISTS('l1', job=self.job_name, step='panel') == 'yes':
            ...
            if not l1_lines and self.GEN.LAYER_EXISTS('l1-2', job=self.job_name, step='panel') == 'yes':
                l1_info = self.GEN.INFO("-t layer -e %s/%s/%s  -d FEATURES" % (self.job_name, 'panel', 'l1-2'),units='mm')
                l1_lines = [gFeatures.Pad(line) for line in l1_info if 'sh-con2' in line or 'sh-con' in line]
            if len(l1_lines) < 4:
        ...
        self.get_layers()
        # self.GEN.PAUSE(str(self.tgz_layers))
    ...
    def get_layers(self):
        ...
        for i, name in enumerate(matrix_info['gROWname']):
            ...
            if type == 'signal' and context == 'board' and name in ['l1-2', 'l{0}-2'.format(self.job_name[4:6])] and name not in self.tgz_layers:
                self.tgz_layers.append(name)
    ...
    def get_penyin_silk_layers(self):
                        ...
                        if i["WORK_DESCRIPTION"] and "使用曝光资料" in i["WORK_DESCRIPTION"]: 
                            if i['VALUE_AS_STRING'] is not None :
                                ...
                                for layer in self.silk_layers:
                                    if layer.split("-ls")[0] in value.split(","):
                                        if silk_type == "印刷":
        ...
        for layer in penyin_layers:
            if layer.split("-ls")[0] not in erp_penyin_layers:
                manual = True
                
        for layer in erp_penyin_layers:
            if layer not in [x.split("-ls")[0] for x in penyin_layers]:
                manual = True
        ...
        return list(set(select_layers)), list(set(yinshua_layers))
    ...
    def check_select_layer_is_right(self, select_output_silk_layers):
        """检测选择的喷印层是否正确 跟系统登记的比对"""
        JOB_SQL = self.job_name.split('-')[0] if '-' in self.job_name else self.job_name
        sql = '''
         SELECT job_detail_info_for_output FROM  hdi_engineering.incam_job_layer_info where job_name = '%s'
         ''' % JOB_SQL
        data = mysql.SELECT_DIC(dbc, sql)        
        if data and data[0].get('job_detail_info_for_output'):
            try:
                job_detail_info_for_output = json.loads(data[0]["job_detail_info_for_output"], encoding='utf8')
            except:
                job_detail_info_for_output = None

            if job_detail_info_for_output is None:
                return u"检测到资料未登记公共程序内的 CAM资料数据记录程序，请登记ok后再输出资料！", "error"
            else:                        
                for key, value in job_detail_info_for_output.iteritems():
                    if key == "silk_table":
                        for info in value:
                            if info[1] == u"文字喷印" and info[3] != u"审核OK":
                                return u"检测到资料 CAM资料数据记录程序 内的【文字喷印】没有审核，请通知审核ok后再输出资料！", "error"                        
                        
                        for layer in select_output_silk_layers:                            
                            for info in value:
                                if info[1] == u"文字喷印":
                                    if layer.split("-ls")[0] not in info[2].split():
                                        return u"检测到层{0}  CAM资料数据记录程序内登记【文字喷印】层{1}不一致，请检查选择的层是否正确！".format(
                                            layer.split("-ls")[0], info[2]
                                        ), "error"
                                    
                        for info in value:
                            if info[1] == u"文字喷印":
                                for layer in info[2].split():
                                    if layer not in [x.split("-ls")[0] for x in select_output_silk_layers]:                                        
                                        return u"检测到CAM资料数据记录程序内登记【文字喷印】层{0} 不在选择输出的层{1}内，请检查选择的层是否正确！".format(
                                            layer, select_output_silk_layers
                                        ), "error"
        else:
            return u"检测到资料未登记公共程序内的 CAM资料数据记录程序，请登记ok后再输出资料！", "error"
    ...
    def export_tgz(self):
        ...
        res = self.check_select_layer_is_right(select_output_silk_layers)
        if res[0] != "success":            
            self.Message('%s' % res[0])
            #夜班不拦截处理
            h_time = time.strftime("%H")
            if 6 < int(h_time) < 23:
                exit(0)
