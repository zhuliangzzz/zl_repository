...
sub get_pnl_rout_data
{
    ...
    for(my $i=1 ;$i <= $yh_num; $i++) {
		...
		if ($hdi_num >= 1) {
			...
			if ($c->LAYER_EXISTS($job_name, $panel, $pnl_rout_lyr) eq "yes") {
			    $c->COM("delete_layer,layer=$pnl_rout_lyr");
			}
            $c->AFFECTED_LAYER('pnl_rout', 'yes');
            $c->FILTER_RESET();
            $c->COM("set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=yes,attribute=.string,min_int_val=0,max_int_val=0,min_float_val=0,max_float_val=0,option=,text=$pnl_rout_lyr");
            $c->COM("set_filter_and_or_logic,filter_name=popup,criteria=inc_attr,logic=or");
            $c->FILTER_SELECT();
            $c->FILTER_RESET();
            if ($c->GET_SELECT_COUNT() > 0) {
                $c->COM("sel_copy_other,dest=layer_name,target_layer=$pnl_rout_lyr,invert=no,dx=0,dy=0,size=0");
            }
            $c->COM("affected_layer,name=,mode=all,affected=no");
		}
        if ($c->LAYER_EXISTS($job_name, $panel, $pnl_rout_lyr) eq "no") {
           #增加判断$inn_layer是否存在 2019.10.08
            $c->Messages("info","料号:$job_name 中缺少$pnl_rout_lyr 层或属性设计，无法输出裁磨数据，程序退出");
            exit 0;
       }       
		# 检测选中层别中的靶标数量是否正确=============================
		$c->CLEAR_LAYER();
