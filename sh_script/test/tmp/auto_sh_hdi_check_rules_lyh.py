...
'''
# 20250313 zl check_nv_hole_lost_add_pad_in_inner_layers 添加和检测只针对拼入set的 step。没有set时识别edit 提出人：翟鸣
'''
...
try:
    if "check_modify_signal_layer_detch_comp_info" in "".join(sys.argv[1:]):
...
from get_erp_job_info import get_face_tech_params, \
     ...
     get_drill_info_detail, get_core_inner_layers, \
     get_same_size_pnl_set_from_inplan, get_erp_cancel_jobs, \
     get_wip_jobs, get_cam_maker_user_info
...
if "coreinner" in os.environ.get("INNER_OUTER", ""):
    core_layers = get_core_inner_layers(jobname)
    if core_layers:
        new_layers = [x for x in innersignalLayers if x in core_layers]
        innersignalLayers = new_layers
        new_drills = []
...
class auto_check_rule(object):
    ...
    def view_note_detail(self, coordinate_info=None, show_units="mm"):
        """查看标记"""
        if coordinate_info is None:            
            ...
            function_name = dic_info[0]["check_function_name"]
        else:
            ...
            for worklayer, array_info in value.iteritems():                
                ...
                if step.isLayer(worklayer):                    
                    ...
                    for coordinate, note_info,display_layer in array_info:
                        ...
                        if function_name == "check_modify_signal_layer_detch_comp_info auto" or \
                           show_units == "inch":
            ...
            if mai_drill_layers:
                ...
                for stepname in all_steps:
                    ...
                    for drill_layer in mai_drill_layers:
                        ...
                        step.COM("profile_to_rout,layer=outline_tmp_check,width=1")
                        ...
                        if dic_drill_lp.get(drill_layer):
                            ...
                            for tmp_layer in dic_drill_lp[drill_layer]:
                                ...
                                step.affect(tmp_layer)
                                step.resetFilter()
                                if step.isLayer(lp_layer+"_autocheck_hole+++"):                                    
                        ...
                        if step.name == "panel":
                            ...
                            step.selectSymbol("r508")
                            if step.featureSelected() == 108:
                                step.selectDelete()
                                
                            lay_cmd_lp = gClasses.Layer(step, lp_layer+"_autocheck_hole+++")
                            feat_out_lp = lay_cmd_lp.featCurrent_LayerOut(units="mm")["pads"]
                            feat_out_lp += lay_cmd_lp.featCurrent_LayerOut(units="mm")["lines"]
                            big_hole_symbol = list(set([obj.symbol for obj in feat_out_lp
                                               if float(obj.symbol[1:]) >= 1000]))
                            step.resetFilter()
                            if big_hole_symbol:
                                step.selectSymbol(";".join(big_hole_symbol), 1, 1)
                                if step.featureSelected():
                                    step.selectDelete()                            
                                    
                        # if "-lyh" in job.name:                            
                        start_layer, end_layer = get_drill_start_end_layers(matrixinfo, drill_layer)
                        # 镭射层有做塞孔的考虑进来
                        for i in range(2, 10):
                            laser_drill_top = "s{1}-{0}".format(i, start_layer[1:])
                            laser_drill_bot = "s{0}-{1}".format(end_layer[1:], int(end_layer[1:])-i+1)
                            for laser_drill in [laser_drill_top, laser_drill_bot]:
                                if step.isLayer(laser_drill):
                                    step.clearAll()
                                    step.affect(laser_drill)
                                    step.resetFilter()
                                    step.refSelectFilter(lp_layer+"_autocheck_hole+++", mode="cover")
                                    if step.featureSelected():
                                        step.removeLayer(laser_drill+"_tmp_lp")
                                        step.copySel(laser_drill+"_tmp_lp")
                                        step.clearAll()
                                        step.affect(laser_drill+"_tmp_lp")
                                        step.refSelectFilter(drill_layer+"_autocheck_hole+++", mode="disjoint")
                                        if step.featureSelected():                                        
                                            step.copySel(drill_layer+"_autocheck_hole+++")
                        ...
                        step.resetFilter()
                        touch_profile_count = 0
                        if "coupon" in step.name:                            
                            if step.refSelectFilter("outline_tmp_check"):
                                # touch_profile_count = step.featureSelected()
                                step.selectDelete()
                        ...
                        if stepname == "panel":
                            ...
                            lay_cmd_lp = gClasses.Layer(step, drill_layer+"_autocheck_hole+++")
                            feat_out_lp = lay_cmd_lp.featCurrent_LayerOut(units="mm")["pads"]
                            feat_out_lp += lay_cmd_lp.featCurrent_LayerOut(units="mm")["lines"]
                            big_hole_symbol = list(set([obj.symbol for obj in feat_out_lp
                                               if float(obj.symbol[1:]) >= 1000]))
                        ...
                        step.removeLayer("outline_tmp_check")
                        # 切片孔模块不检测
                      
                        if "qie_hole_coupon_new_" in stepname:
                            step.resetFilter()
                            step.selectSymbol("r452", 1, 1)
                            if step.featureSelected():
                                step.selectDelete()
                            
                            step.resetFilter()
                            step.COM("sel_ref_feat,layers=l{0},use=filter,mode=cover,"
                                     "f_types=pad,polarity=positive,"
                                     "include_syms=,exclude_syms=".format(drill_layer.split("-")[1]))
                        ...
                        if not count2:
                            ...
                            if names:
                                ...
                                if not process2 and "icg" in step.name:
                                    # 无盖孔电镀且为阻抗条时 不塞孔
                                    update_log = False                                        
                            ...
                            if update_log:
                                ...
                                log = u"检测到{0}埋孔层{1} 有埋孔数量，但没做塞孔，请检查是否漏做塞孔！"
                                arraylist.append(log.format(step.name, drill_layer))
                        ...
                        if count1 - touch_profile_count != count2:
    ...
    def get_process_is_running(self, process_id, jobname, condition="status = 'Done'"):
        
        sql = """select a.* from pdm_job_workflow a,
        pdm_job b
        where a.process_id = {0}
        and a.job_id = b.id
        and b.jobname = lower('{1}')
        and {2}"""
        data_info = ikm_fun.PG.SELECT_DIC(ikm_fun.dbc_p, sql.format(process_id, jobname, condition))

        return data_info    
    
    def check_szsk_test_layers(self):
        """检测树脂塞孔检测资料是否有运行且存在"""
        ...
        dic_zu_layer = {}
        names = [dic_info["MRP_NAME"] for dic_info in data_info]
        for mrp_name in set(names):            
            arraylist_erp_sz_drill = []
            for dic_process_info in data_info:
                if dic_process_info["WORK_CENTER_CODE"] and \
                   u"树脂塞孔" in dic_process_info["WORK_CENTER_CODE"].decode("utf8") and \
                   mrp_name.upper() == dic_process_info["MRP_NAME"]:
                       ...
                       (u"树脂塞孔铝片" in dic_process_info["NOTE_STRING"].decode("utf8") or \
                        ...
                        if find_sting:
                            for x in find_sting:
                                ...
                                arraylist_erp_sz_drill += new_x.split(" ")
                            
                    if dic_process_info["WORK_DESCRIPTION"] and \
                       u"树脂塞孔铝片" in dic_process_info["WORK_DESCRIPTION"].decode("utf8"):
                        if dic_process_info["VALUE_AS_STRING"]:                                
                            arraylist_erp_sz_drill += [dic_process_info["VALUE_AS_STRING"]]
                            
            if not arraylist_erp_sz_drill:
                for layer in matrixinfo["gROWname"]:
                    if re.match("szsk", layer) or \
                       (re.match("sz", layer) and "lp" in layer):
                        arraylist_erp_sz_drill.append(layer)                        
            ...
            arraylist_erp_sz_drill = list(set(arraylist_erp_sz_drill))
            if sz_process:                
                sz_check_process = ['HDI19501',]
                array_sz_info = [dic_info for dic_info in data_info
                                  if (dic_info["OPERATION_CODE"] in sz_check_process or
                                      u"塞孔检验" in dic_info["WORK_CENTER_CODE"].decode("utf8"))
                                  and dic_info["MRP_NAME"] == mrp_name]
                if array_sz_info:
                    process_run = self.get_process_is_running(2096, job.name)
                    if not process_run:
                        return u"重要项目：检测到此板有塞孔检验流程，但公共程序 制作树脂塞孔检测资料 程序未运行，请检查！", None
                
                jc_sz_layers = []
                for layer in matrixinfo["gROWname"]:
                    if layer.startswith("jc-") and "-bf" not in layer:
                        jc_sz_layers.append(layer)
                
                if not jc_sz_layers:
                    return u"检测到此板有塞孔检验流程，未检测到有 jc- 开头的树脂检查层，请检查！", None                
                
                erp_jc_sz_layers = []
                for layer in jc_sz_layers:
                    if layer.replace("jc-", "").replace("fm", "lp") in "".join(arraylist_erp_sz_drill):
                        erp_jc_sz_layers.append(layer)
                        
                if not erp_jc_sz_layers:
                    return u"检测到{2}有塞孔检验流程及树脂铝片层{1}，但资料内的树脂检查层{0} 跟erp的树脂铝片层{1}不匹配，<br>请检查是否漏制作此铝片{1}的树脂检查资料！".format(
                        jc_sz_layers, arraylist_erp_sz_drill, mrp_name
                    ), None
                
                #all_steps = get_panelset_sr_step(job.name, "panel")
                get_sr_area_flatten("fill_surface_tmp")
                for stepname in ["panel"]:
                    step = gClasses.Step(job, stepname)
                    step.open()
                    
                    for jc_layer in jc_sz_layers:
                        step.clearAll()
                        step.affect(jc_layer)
                        step.resetFilter()
                        step.selectNone()
                        step.selectAll()
                        # step.refSelectFilter("fill_surface_tmp", mode="cover")
                        count1 = step.featureSelected()
                        if not count1:                            
                            return u"检测到此板有塞孔检验流程，检测到{0}树脂检查层无内容，请检查！".format(jc_layer), None
                        
                        ref_jc_layer = ""
                        for ref_layer in matrixinfo["gROWname"]:
                            if ".dq" in ref_layer:
                                continue
                            if jc_layer.replace("jc-", "").replace("fm", "lp") in ref_layer:
                                step.flatten_layer(ref_layer, ref_layer+"_check_sz_jc+++")
                                ref_jc_layer = ref_layer+"_check_sz_jc+++"
                                break
                            
                        if not ref_jc_layer:
                            arraylist.append(u"检测到此板有塞孔检验流程，未检测到{0}树脂检查层对应的树脂铝片层，请检查！".format(jc_layer))   
                        
                        step.selectNone()
                        step.COM("sel_ref_feat,layers={0},use=filter,mode=same_center,"
                                 "f_types=line\;pad\;surface\;arc\;text,polarity=positive\;negative,"
                                 "include_syms=,exclude_syms=,on_multiple=all".format(ref_jc_layer))                    

                        step.COM("sel_reverse")
                        if step.featureSelected():
                            step.removeLayer(jc_layer+"_find_hole_tmp+++")
                            step.copySel(jc_layer+"_find_hole_tmp+++")
                            step.clearAll()
                            step.resetFilter()
                            step.affect(jc_layer+"_find_hole_tmp+++")
                            step.filter_set(feat_types='pad;line;arc;surface')
                            step.refSelectFilter("fill_surface_tmp")
                            if step.featureSelected():
                                
                                log = u"检测{2}到树脂检查层{0}跟铝片层{1}孔位置不一致，请查看标记不一致的地方1！"
                                arraylist.append(log.format(jc_layer, jc_layer+"_find_hole_tmp+++", stepname))
                                
                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, jc_layer+"_find_hole_tmp+++",
                                                                                     ref_layer,
                                                                                     log.format(jc_layer, ref_layer, stepname),                                                                                     
                                                                                     dic_zu_layer, add_note_layer=jc_layer, )
                                # step.removeLayer(ref_jc_layer)
                                # continue
                            
                            # step.removeLayer(jc_layer+"_find_hole_tmp+++")
                        
                        #step.clearAll()
                        #step.affect(ref_jc_layer)
                        #step.resetFilter()
                        #step.refSelectFilter("fill_surface_tmp", mode="cover")
                        #count2 = step.featureSelected()
                        #if count1 != count2:
                            #arraylist.append(u"检测到此板有塞孔检验流程，{4}检测到{0}树脂检查层 数量{2}对应的树脂铝片层{1} 数量{3}不一致，请检查！".format(
                                #jc_layer, ref_jc_layer, count1, count2, step.name
                                #))
                        step.clearAll()
                        step.affect(ref_jc_layer)
                        step.resetFilter()
                        step.COM("sel_ref_feat,layers={0},use=filter,mode=same_center,"
                                 "f_types=line\;pad\;surface\;arc\;text,polarity=positive\;negative,"
                                 "include_syms=,exclude_syms=,on_multiple=all".format(jc_layer))                    
                        ...
                        if step.featureSelected():
                            ...
                            if step.featureSelected():
                                ...
                                log = u"检测{2}到树脂检查层{0}跟铝片层{1}孔位置不一致，请查看标记不一致的地方2！"
                                arraylist.append(log.format(jc_layer, jc_layer+"_find_hole_tmp+++", stepname))
                    ...
                    step.removeLayer("fill_surface_tmp")
    ...
    def check_send_finish_status(self, *args, **kwargs):
        """检测资料送审的状态"""
        
        if not args:
            return
        
        sql = "select * from  hdi_engineering.job_lock_data where job_name = '{0}'".format(job.name.split("-")[0])
        datainfo = conn.SELECT_DIC(dbc_m, sql)
        self.dic_make_info ={}
        if datainfo:
            try:                        
                self.dic_make_info = json.loads(datainfo[0]["job_make_status"], encoding='utf8')
            except:
                self.dic_make_info ={}
                
        if not self.dic_make_info:
        ...
        dic_type = {"inner": u"内层","outer": u"外层","all": u"全套","linshi": u"临时",}
        if self.dic_make_info :
            for key1, key2,key3 in [(u"(芯板)内层完成送审", u"(芯板)内层送审状态", u"(芯板)内层审核状态"),
                ...
                if self.dic_make_info.get(key3, None) in [u"已退审", u"送审中", u"已解锁退审"]:
                    if dic_type[args[0]] in key3:
                        
                        message_result = u"重要项目:检测到层别{2} 在【{0}】为【{1}】，{3} 请通知CAM审核更新状态后再运行程序！".format(
                            key3, self.dic_make_info[key3], kwargs.get("worklayer", None), kwargs.get("warning_text", "")
                        )
                        if kwargs.get("worklayer", None) is None:
                            message_result = u"重要项目:检测到【{0}】为【{1}】，{3} 请通知CAM审核更新状态后再运行程序！".format(
                                key3, self.dic_make_info[key3], kwargs.get("worklayer", None), kwargs.get("warning_text", "")
                            )
                            
                        result = send_message_to_director(message_result, job.name, message_result, password_approve=True)
                        if not result:
                            return 1
                        else:
                            return 2                        
                            
                if self.dic_make_info.get(key3, None) in [u"审核完成"]:
                    if dic_type[args[0]] in key3:
                        return 2

        return 0
    
    def check_output_layer_is_locked(self, *args, **kwargs):
        """检测输出是否有全部上锁"""
        #暂时屏蔽
        user = top.getUser()
        
        #多层事业部型号 不检测
        check_hdi_job = get_cam_maker_user_info(job.name)
        check_multi_job = get_cam_maker_user_info(job.name, org_code=u"多层事业部")
        if not check_hdi_job :
            if check_multi_job:
                return 0
            
        if user in "54902/69884/89646/69883":
            """NC组输出不检测"""
            return 0
        
        #if user not in ["83288", "84310"]:            
            #if kwargs.get("return_result", None) == "yes":
                #return 0
            #else:
                #exit(0)
        step_list, inner_layers, outer_layers, drill_layers,board_layers = self.get_job_matrix()
        
        # job.PAUSE(str([drill_layers, inner_layers, board_layers]))
        user_dir = os.environ["JOB_USER_DIR"]
        ls_layer_compare_log = os.path.join(user_dir, '%s_temp_layer_comparsion.log' % job.name)
        check_compare_log = []
        if os.path.exists(ls_layer_compare_log):
            check_compare_log = file(ls_layer_compare_log).readlines()

        h_time = time.strftime('%H', time.localtime(time.time()))        
        job_create_time = self.get_cam_director_finish_status(process_type="create_time")
        
        self.pre_lock = self.getJOB_LockInfo(dataType='locked_info')
        find_not_lock_layers = []
        find_not_compare_layers = []
        
        check_steps = matrixinfo["gCOLstep_name"]
        if "panel" in matrixinfo["gCOLstep_name"]:
            check_steps = get_panelset_sr_step(job.name, "panel")
            
        message_result = ""
        for worklayer in args:
            if worklayer in ["s1-2","s22-21"] and "SD1022E6261D2" in job.name.upper():
                message_result += u"重要项目：{0}型号 输出{1}层别，杨晓菲经理通知删外层镭射资料S1-2/S22-21，出资料要请示杨经理-伍青<br>".format(
                    job.name, worklayer
                )
        ...
        #镭射有新生成的S1-2-3 这种要单独优化下
        array_check_layers = []
        for layer in args:
            if layer not in job.matrix.getInfo()["gROWname"]:              
                continue            
            if re.match("s\d{1,2}-\d{1,2}-.*", layer.split("-ls")[0]):                
                start = layer.split("-")[0]
                for suffix in layer.split("-"):
                    new_layer = start + "-" + suffix
                    if new_layer in job.matrix.getInfo()["gROWname"]:
                        array_check_layers.append(new_layer)       
            else:
                array_check_layers.append(layer)            
        
        for worklayer in array_check_layers:                        
            if worklayer not in job.matrix.getInfo()["gROWname"]:              
                continue
            res = 0
            try:
                if worklayer in inner_layers + drill_layers:
                    res = self.check_send_finish_status("inner", worklayer=worklayer, **kwargs)
                    if lay_num <= 2 and res == 0:
                        # 双面板按外层来判断一次
                        res = self.check_send_finish_status("outer", worklayer=worklayer, **kwargs)
                        
                    if res == 0:
                        res = self.check_send_finish_status("all", worklayer=worklayer,**kwargs) 
                else :
                    res = self.check_send_finish_status("outer", worklayer=worklayer,**kwargs)
                    if res == 0:
                        res = self.check_send_finish_status("all", worklayer=worklayer,**kwargs)                    
                    
                if not res and "-ls" in worklayer:                    
                    res = self.check_send_finish_status("linshi", worklayer=worklayer,**kwargs)
            except Exception as e:                
                pass
                if user in ["84310"]:
                    job.PAUSE(str(e))
            
            if "-ls" in worklayer and \
               worklayer not in "".join(check_compare_log):
                find_not_compare_layers.append(worklayer) 
            ...
            if res == 1:
                ...
                # if job_create_time and str(job_create_time) > '2025-03-20 00:00:00':
                message_result = u"重要项目:检测到层别{0} 未审核完成，{1} 请通知CAM审核更新层别所在的【内/外/全套】中的状态后再运行程序！".format(
                            worklayer,  kwargs.get("warning_text", "")
                        )
                result = send_message_to_director(message_result, job.name, message_result, password_approve=True)
                if not result:                
                    if kwargs.get("return_result", None) == "yes":
            ...
            for stepname in check_steps:
                ...
                if not lock:
                    if [worklayer, stepname] not in find_not_lock_layers:                        
                        find_not_lock_layers.append("{0}->{1}".format(stepname, worklayer))
                        
        if find_not_compare_layers:
            #showMessageInfo(u"检测到值班流程中【临时层比对】程序未运行 {0}此层的比对，或运行结果异常，请处理正常后，才能输出！".format(
                #find_not_compare_layers
            #),
            #scoll_bar="yes", 
            #scoll_widht_height=[800, 200])
            message_result = u"重要项目:检测到值班流程中【临时层比对】程序未运行 【{0}】此层的比对，或运行结果异常，请处理正常后，才能输出！".format(
                ",".join(find_not_compare_layers)
            )
            result = send_message_to_director(message_result, job.name, message_result, password_approve=True)
            if not result:                
                if kwargs.get("return_result", None) == "yes":
        ...
        if find_not_lock_layers:
            # find_not_lock_layers = list(set(find_not_lock_layers))
            #showMessageInfo(u"检测到{0}...层未全部上锁，请通知审核是否已全部审核ok，<br>若已审核，请通知审核人手动上锁！".format(find_not_lock_layers),
                            #scoll_bar="yes", 
                            #scoll_widht_height=[800, 200])
            message_result = u"重要项目:检测到{0}...层未全部上锁，{1} 请通知审核是否已全部审核ok，<br>若已审核，请通知审核人点击审核完成！！".format(
                ",".join(find_not_lock_layers),  kwargs.get("warning_text", "")
            )            
            result = send_message_to_director(message_result, job.name, message_result, password_approve=True)
            if not result:
                if kwargs.get("return_result", None) == "yes":
    ...
    def get_job_matrix(self):
        ...
        m_info = job.matrix.getInfo()# GEN.DO_INFO("-t matrix -e %s/matrix" % self.job_name)
        ...
        for row in m_info['gROWrow']:
            ...
            if m_info['gROWcontext'][num] == 'board' and \
               m_info['gROWlayer_type'][num] == 'drill' and \
               m_info['gROWname'][num] not in other_layers:                
                drill_layers.append(m_info['gROWname'][num])
            
            #翟鸣通知增加铝片 导气板 树脂塞孔20241213 by lyh
            if m_info['gROWlayer_type'][num] == 'drill':
                if re.match("^sz.*\.lp\d?(-[c|s])?$|^sz.*\.dq\d?(-[c|s])?$", m_info['gROWname'][num]) or \
                   re.match("^dld\d{1,2}-\d{1,2}$", m_info['gROWname'][num]) or \
                   re.match("^bd.*", m_info['gROWname'][num]) or \
                   re.match(".*\.ykj(-ls*)$", m_info['gROWname'][num]) or \
                   re.match("^inn.*", m_info['gROWname'][num]) or \
                   re.match("^\dnd.*", m_info['gROWname'][num]) or \
                   re.match("^jc-sz.*", m_info['gROWname'][num]):
                    if m_info['gROWname'][num] not in other_layers:                        
                        drill_layers.append(m_info['gROWname'][num])
            
            if "ww" in str(m_info['gROWname'][num]):
                drill_layers.append(m_info['gROWname'][num])
                
            if "pnl_rout" in str(m_info['gROWname'][num]):
                drill_layers.append(m_info['gROWname'][num])
        ...
        drill_layers = [layer for layer in drill_layers
                        if layer not in ['lp']]
    ...
    def check_drl_hole_touch_smd_bga_info(self, autocheck=None):
        """1.通孔距bga间距小于6mil报警；
        2.通孔距smd（小于等于24mil）小于6mil报警；        
        3.脚本加在net分析步骤和钻孔制作步骤中，并设为必检项。异常情况报警，如此孔有树塞+盖孔电镀设计则允许。
        翟鸣沟通 钢网上只判断小于24mil的smd bga
        http://192.168.2.120:82/zentao/story-view-8707.html"""
        arraylist = []
        dic_zu_layer = {}
        dic_layer = {}
        dic_layer["l1"] = "p1"
        dic_layer["l{0}".format(lay_num)] = "p2"
        
        dic_make_info = self.getJOB_LockInfo(dataType='job_make_status')
        inner_finish = False
        ...
        for stepname in matrixinfo["gCOLstep_name"]:
            if "net" in stepname:
                ...
                if not drillLayers:
                    return u"未检测到drl层，请检查通孔命名是否异常！！", None                
                ...
                for worklayer in outsignalLayers:
                    ...
                    step.copySel(worklayer+"_smd_bga_tmp")
                    step.copyLayer(job.name, step.name,
                                   worklayer+"_smd_bga_tmp",
                                   worklayer+"_smd_bga_tmp_surface")
                    step.clearAll()
                    step.affect(worklayer+"_smd_bga_tmp_surface")
                    ...
                    step.selectAll()
                    if step.featureSelected():
                        # step.COM("sel_delete_atr,attributes=.smd")
                        step.selectDelete()
                    ...
                    step.selectAll()
                    if step.featureSelected():                        
                        step.COM("sel_resize,size=-{0}".format(24*25.4))
                        
                    step.COM("sel_resize,size={0}".format(25*25.4))
                    
                    step.clearAll()
                    step.affect(worklayer+"_smd_bga_tmp")
                    step.COM("sel_delete_atr,mode=list,attributes=.rout_chain")
                    step.resetFilter()
                    step.refSelectFilter(worklayer+"_smd_bga_tmp_surface", mode="multi_cover")
                    if step.featureSelected():
                        step.selectDelete()                    
                    
                    step.COM("sel_resize,size={0}".format(8*25.4))
                    step.resetFilter()
                    step.refSelectFilter(";".join(drillLayers))
                    if step.featureSelected():
                        step.copySel(worklayer+"_smd_bga_tmp_check")
                        step.clearAll()
                        step.affect(worklayer+"_smd_bga_tmp_check")
                        step.COM("sel_resize,size=-{0}".format(8*25.4))
                        step.resetFilter()
                        step.selectAll()
                        
                        log = u"检测到通孔层{0} 距{1}线路上的smd bga间距不够4mil，请将标记位置截图反馈主管确认！"
                        arraylist.append(log.format("drl", worklayer))                        
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, worklayer+"_smd_bga_tmp_check",
                                                                             drillLayers,
                                                                             log.format("drl", worklayer),                                                                             
                                                                             dic_zu_layer, add_note_layer=worklayer)
                    # 检测钢网
                    paste_layer = ""
                    if dic_layer.get(worklayer, None):
                        paste_layer = dic_layer[worklayer]
                        if step.isLayer(paste_layer):                            
                            step.copyLayer(job.name, step.name,paste_layer, paste_layer+"_smd_bga_tmp_surface")
                            step.clearAll()
                            step.affect(paste_layer+"_smd_bga_tmp_surface")
                            step.contourize(accuracy=0)
                            step.COM("sel_resize,size=-{0}".format(24*25.4))
                            step.clearAll()
                            step.affect(paste_layer)
                            step.resetFilter()
                            step.refSelectFilter(paste_layer+"_smd_bga_tmp_surface", mode="disjoint")
                            if step.featureSelected():
                                step.copySel(paste_layer+"_smd_bga_tmp")
                                
                    if paste_layer and step.isLayer(paste_layer+"_smd_bga_tmp"):
                        step.clearAll()
                        step.resetFilter()
                        step.affect(paste_layer+"_smd_bga_tmp")
                        step.contourize(accuracy=0)
                        ...
                        step.refSelectFilter(";".join(drillLayers))
                        if step.featureSelected():
                            step.copySel(paste_layer+"_smd_bga_tmp_check")
                            step.clearAll()
                            step.affect(paste_layer+"_smd_bga_tmp_check")
                            step.COM("sel_resize,size=-{0}".format(8*25.4))
                            step.resetFilter()
                            step.selectAll()
                            
                            log = u"检测到通孔层{0} 距{1} 钢网上的pad 间距不够4mil，请将标记位置截图反馈主管确认！"
                            arraylist.append(log.format("drl", worklayer))                        
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, paste_layer+"_smd_bga_tmp_check",
                                                                                 drillLayers+[paste_layer],
                                                                                 log.format("drl", worklayer),                                                                             
                                                                                 dic_zu_layer, add_note_layer=worklayer)
        ...
        if arraylist:
            if autocheck:
                ...
                self.view_note_detail(dic_zu_layer, show_units="inch")                
                if autocheck == "manual_run":
                    exit(1)
                
            return arraylist, dic_zu_layer
        
        if autocheck == "manual_run":
            showMessageInfo(u"检测成功，没有发现异常！")
            exit(0)    
        
        return "success", None
    
    def check_drl_hole_and_lp_is_right(self):
        """通孔钻孔与通孔塞孔铝片是否在同一位置，个数是否正确"""
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            for drill_layer in ["drl"]:
                ...
                sz_process = [dic_info for dic_info in data_info
                              ...
                              and u"树脂塞孔" in dic_info["WORK_CENTER_CODE"].decode("utf8")]
                if not sz_process:
                    log = u"success 通孔层{0} 不存在 树脂塞孔流程，无需检测".format(drill_layer)
                    arraylist.append(log)
                else:
                    break
            ...
            for stepname in all_steps:
                ...
                for drill_layer in ["drl"]:
                    ...
                    # 镭射层有做塞孔的考虑进来
                    for i in range(2, 10):
                        laser_drill_top = "s1-{0}".format(i)
                        laser_drill_bot = "s{0}-{1}".format(lay_num, lay_num-i+1)
                        for laser_drill in [laser_drill_top, laser_drill_bot]:
                            if step.isLayer(laser_drill):
                                step.clearAll()
                                step.affect(laser_drill)
                    ...
                    symbols = [obj.symbol for obj in feat_out if int(obj.symbol[1:]) > 510]
                    
                    #去掉大孔引孔
                    big_hole_symbols = list(set([obj.symbol for obj in feat_out if float(obj.symbol[1:]) >= 3500]))
                    step.selectSymbol(";".join(list(set(big_hole_symbols))))
                    if step.featureSelected():
                        step.resetFilter()
                        step.COM("sel_ref_feat,layers=,use=select,mode=touch,"
                                 "pads_as=shape,f_types=line\;pad\;surface\;arc\;text,"
                    ...
                    step.selectNone()
                    step.resetFilter()
                    step.selectSymbol(";".join(list(set(symbols))))
                    if step.featureSelected():
                        step.selectDelete()
                    ...
                    find_hole = False
                    if step.featureSelected():
                        log = u"检测到通孔层{0}跟铝片层{1}孔位置不一致，请到{4}中的{2}_autocheck_hole+++和{3}_autocheck_hole+++层查看不一致的地方！"
                        arraylist.append(log.format(drill_layer, lp_layer, drill_layer, lp_layer, stepname))
                    ...
                    count2 = step.featureSelected()
                    if not count2:
                        log = u"检测到{0}通孔层{1} 有通孔数量，但没做塞孔，请检查是否漏做塞孔！"
                        arraylist.append(log.format(step.name, drill_layer))
                        dic_zu_layer=self.get_view_dic_layers(job.name, stepname, step,
                                                              dic_layer_list={u"请查看异常层：":[lp_layer+"_autocheck_hole+++", drill_layer+"_autocheck_hole+++"], },
                                                              dic_zu_layer=dic_zu_layer)
                    ...
                    step.COM("sel_reverse")
                    if step.featureSelected() and not find_hole:
                        log = u"检测到铝片层{0}跟通孔层{1}孔位置不一致，请到{4}中的{2}_autocheck_hole+++和{3}_autocheck_hole+++层查看不一致的地方！"
                        arraylist.append(log.format(lp_layer, drill_layer, lp_layer, drill_layer, stepname))                        
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, lp_layer+"_autocheck_hole+++",
                                                                             drill_layer+"_autocheck_hole+++",
                                                                             log.format(lp_layer, drill_layer, lp_layer, drill_layer, stepname),
                    ...
                    #feat_out2 = layer_cmd.featCurrent_LayerOut(units="mm")["pads"]
                    if count1 - touch_profile_count != count2:
                        log = u"检测到铝片层{0}孔数{1}跟通孔层{2}孔数{3}不一致，请到{6}中的{4}_autocheck_hole+++和{5}_autocheck_hole+++层查看孔数！"
                        arraylist.append(log.format(lp_layer,count2, drill_layer, count1, lp_layer, drill_layer, stepname))
                        dic_zu_layer=self.get_view_dic_layers(job.name, stepname, step,
                                                              dic_layer_list={u"请查看异常层：":[lp_layer+"_autocheck_hole+++", drill_layer+"_autocheck_hole+++"], },
                                                              dic_zu_layer=dic_zu_layer)
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            if step.isLayer("drl") and step.isLayer("tk.ykj"):                
                for stepname in all_steps:
                    ...
                    # step.flatten_layer("drl", worklayer)
                    # step.flatten_layer("tk.ykj", reflayer)
                    step.copyLayer(job.name, step.name, "drl", worklayer)
                    step.copyLayer(job.name, step.name, "tk.ykj", reflayer)
                    ...
                    step.COM("compare_layers,layer1=%s,job2=%s,step2=%s,\
                        ...
                        worklayer, job.name, stepname, reflayer, tol, "drl"))
            ...
            step.removeLayer("drl_flt")
            step.removeLayer("tk.ykj_flt")
            if arraylist:
                if show_ui == "yes":
    ...
    def check_hole_plug_in_pth_holes(self):
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            for stepname in all_steps:
                ...
                step.open()
                step.COM("units,type=mm")
                #周涌通知 阻抗条上的要被大孔钻掉 不检测 20250408 by lyh
                if "icg" in stepname:
                    continue
                for worklayer in tong_lp_layers + other_lp_layers:
                    if step.isLayer(worklayer):
                        
                        mai_drill = worklayer.replace(".lp2", "").replace(".lp", "").replace("sz", "b")
                        man_drill = worklayer.replace(".lp2", "").replace(".lp", "").replace("sz", "m")
    ...
    def check_loss_pad_for_hole(self, *args):
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:            
            ...
            if check_type == "inner" or check_type == "coreinner":
        ...
        for worklayer in drill_layers:
            ...
            for siglayer in sig_layers:
                ...
                step.COM("sel_reverse")
                    ...
                    #step.PAUSE("ddd")                
                if step.featureSelected():
                    if "-floujie" not in step.name and "qie_hole_coupon_" not in step.name:                   
    ...
    def check_npth_hole_condition(self, auto_check=None):
        ...
        #update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                     #u"forbid_or_information='{0}'".format(u"提示"))
        ...
        # all_steps = get_panelset_sr_step(job.name, "panel")
        all_steps = [name for name in matrixinfo["gCOLstep_name"] if "orig" not in name and "net" not in name]            

        res = os.environ.get("INNER_OUTER", "inner")
        
        if res == "inner" or res == "coreinner":
        ...
        for stepname in all_steps:
            ...
            step.COM("units,type=mm")
            step.clearAll()
            if stepname == "set" or "panel" in stepname:
                get_sr_area_flatten(stepname+"_fill_tmp", stepname=stepname)    
                step.resetFilter()
                step.setAttrFilter(".drill,option=non_plated")
                step.affect(drill_layer)
                step.refSelectFilter(stepname+"_fill_tmp")
                if step.featureSelected():                    
                    step.flatten_layer(drill_layer, drill_layer+"_tmp")
                    step.unaffect(drill_layer)
                    step.affect(drill_layer+"_tmp")
                    # step.PAUSE(drill_layer)
                else:
                    step.affect(drill_layer) 
            ...
            step.COM("sel_delete_atr,mode=list,attributes=.rout_chain")
            step.VON()
            step.copyLayer(job.name, step.name, "npth_tmp", "npth_tmp_bak")
            step.COM("sel_resize,size={0}".format(12*25.4))
            
            step.copyLayer(job.name, step.name, "npth_tmp", "npth_tmp_surface")
            step.clearAll()
            step.affect("npth_tmp_surface")
            step.COM("sel_resize,size=-2")                
            
            #http://192.168.2.120:82/zentao/story-view-8608.html 比np孔单边小5mil的pad 
            step.clearAll()
            step.affect("npth_tmp_bak")
            step.COM("sel_resize,size=-{0}".format(9.9*25.4))
            ...
            for worklayer in check_sig_layers:
                ...
                step.refSelectFilter("npth_tmp_bak", mode="cover")
                if step.featureSelected():
                    step.selectDelete()
                ...
                step.affect("npth_tmp")
                step.resetFilter()
                step.refSelectFilter("npth_tmp_surface", mode="cover")
                if step.featureSelected():
                    # 去掉导引孔
                    step.selectDelete()
                ...
                #if step.name == "edit" and worklayer == "l2":
                    #step.PAUSE(worklayer)
                if step.featureSelected():
                    log = u"检测{0}中npth孔掏{1}层铜不够单边6mil，请到npth_clip_cu_not_enough层检查相应的孔位置查看！"
        ...
        step.removeLayer("npth_tmp")
        step.removeLayer("npth_tmp_bak")
        step.removeLayer("npth_tmp_surface")
        for layer in job.matrix.getInfo()["gROWname"]:
            if "_fill_tmp" in layer and ("set" in layer or "panel" in layer):
                step.removeLayer(layer)
    ...
    def check_aobao_danymic_text_info(self, *layers):
        ...
        check_layers = layers
        if not layers:
            if auto_check_type == "inner" or auto_check_type == "coreinner":
                           ...
                           "B2123123SSS" == str(obj.text).upper().replace("'", ""):
                            ...
                            if obj.shape == "Text":                                
                                ...
                                if auto_check_type and job.name.split("-")[0][-1] == "1":                                    
                                    if abs(xsize - ysize) > 0.1 or not (
                                        ...
                                        arraylist.append(log.format(worklayer, obj.text.replace("'", "").replace('"', ""), ("%0.1f" % xsize, "%0.1f" % ysize, "%0.1f" % (factor / 0.5 *6))))
                                        step.selectNone()
                                        ...
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(
                                            ...
                                            log.format(worklayer, obj.text.replace("'", "").replace('"', ""), ("%0.1f" % xsize, "%0.1f" % ysize, "%0.1f" % (factor / 0.5 *6))), dic_zu_layer)                                
                            ...
                            if "deferred" not in obj.attributes or \
                               "orbotech_plot_stamp" not in obj.attributes:
                                ...
                                arraylist.append(log.format(worklayer, obj.text.replace("'", "").replace('"', ""), obj.attributes))
                                step.selectNone()
                                step.selectFeatureIndex(worklayer, obj.feat_index)
                                
                                dic_zu_layer = self.get_selected_dic_note_coordinate(
                                    job.name, stepname, step, worklayer,worklayer,
                                    log.format(worklayer, obj.text.replace("'", "").replace('"', ""), obj.attributes), dic_zu_layer)                                 
                ...
                if find_indexes:
                    ...
                    for index in find_indexes:
                        ...
                        if step.featureSelected():
                            ...
                            for touch_index in indexes:
                                if float(touch_index) > float(index):
                                    log = u"检测到{0} 层明码或二维码被其他物件覆盖在上方，请检查标记处是否异常！"
                                    
                                    arraylist.append(log.format(worklayer))
                                    step.selectNone()
                                    step.selectFeatureIndex(worklayer, touch_index)                                    
                                    dic_zu_layer = self.get_selected_dic_note_coordinate(
                                        job.name, stepname, step, worklayer,worklayer,
                                        log.format(worklayer), dic_zu_layer)
    ...
    def auto_check_week_info(self):
        """全自动检测周期"""
        dic_info = getJobData(job.name.upper())
        arraylist = []
        dic_zu_layer = {}
        if dic_info:
            res = self.get_week_picihao_banci_info()
            if res:
                self.dic_tgz_week_layers, self.dic_tgz_barcode_week_layers, \
                    self.dic_tgz_banci_layers, self.dic_tgz_picihao_layers, self.dic_tgz_xinqi_layers = res
            else:
                return u"获取板内周期信息异常，请手动检查！！"
                
            dic_zu_layer_type = {}
            dic_zu_layer_type["signal"] = u"线路层"
            dic_zu_layer_type["solder_mask"] = u"阻焊层"
            dic_zu_layer_type["silk_screen"] = u"字符层"            
            ERPPeriod = dic_info["dc_format"].decode("utf8").lower()            
            ...
            ##周涌通知 以cam登记为准 暂时不跟inplan的周期信息比对 20241126 by lyh
            if not inplan_check_layer:
                arraylist.append(u"获取inplan周期面次【{0}】失败，请反馈程序工程师维护！".format(layer_side))
            ...
            日/周/年
            日/月/年"""
            if ERPPeriod == u"日/月/年":
                ERPPeriod = "d/m/y"
            if ERPPeriod == u"年/月/日":
                ERPPeriod = "y/m/d"
            if ERPPeriod == u"日/周/年":
                ERPPeriod = "d/ww/y"
            if ERPPeriod == u"先年后月":
                ERPPeriod = "y/m"
            if ERPPeriod == u"先月后日":
                ERPPeriod = "m/d"                  
            ywmd = re.findall("(yy|y)|(ww)|(mm|m)|(dd|d)", ERPPeriod)
            new_erp_period = ""
            if ywmd:
                new_erp_period = ["", "", "", ""]
                yy, ww, mm, dd = ywmd[0]
                ...
                if not yy:                      
                    arraylist.append(u"获取inplan周期【{0}】非常规周期，未检测到年yy|y，请手动检查周期类型跟MI是否一致！".format(ERPPeriod))

                if not (ww or mm or dd):
                    arraylist.append(u"获取inplan周期【{0}】非常规周期，未检测到ww或mm|m或dd|d，请手动检查周期类型跟MI是否一致！".format(ERPPeriod))
            else:
                arraylist.append(u"获取inplan周期【{0}】非常规周期2，未检测到yy|y或ww或mm|m或dd|d，请手动检查周期类型跟MI是否一致！".format(ERPPeriod))
            
            all_week_text = []
            manual_check_week_info = []
            for dic_week_info in [self.dic_tgz_week_layers]:                
                for key, values in dic_week_info.iteritems():
                    for obj, stepname in values:
                        res = re.findall("(\$\$yy)|(\$\$ww)|(\$\$mm)|(\$\$dd)|(\$\$year)|(\$\$month)|(\$\$day)", obj.text.lower().replace("{", "").replace("}", ""))
                        
                        new_text = ["", "", "", "", "", "", ""]
                        i = 0
                        for a, b, c, d,e,f,g in res:                    
                            new_text[i] += a.replace("$$", "")
                            new_text[i] += b.replace("$$", "")
                            new_text[i] += c.replace("$$", "")
                            new_text[i] += d.replace("$$", "")
                            new_text[i] += e.replace("$$", "")
                            new_text[i] += f.replace("$$", "")
                            new_text[i] += g.replace("$$", "")
                            i += 1
                            
                        all_week_text.append("".join([x.replace("year", "y").replace("month", "m").replace("day", "d")
                                                      for x in new_text if x ]))
                        
                        if len([x for x in new_text if x]) <= 1:
                            manual_check_week_info.append([stepname, key, obj])
                        
                        if "$$year" in obj.text.lower() or "$${year}" in obj.text.lower():
                            self.one_year = True
                        if "$$month" in obj.text.lower() or "$${month}" in obj.text.lower():
                            self.one_month = True
                        if "$$day" in obj.text.lower() or "$${day}" in obj.text.lower():
                            self.one_day = True
                        ...
                        for layer in inplan_check_layer:
                            ...
                            log = u"检测到{0}层内存在动态周期{1}，但跟inplan要求周期添加层次:{2}不一致，<br>\
                            请检查周期是否添加层次有误！".format(key, obj.text, layer_side)
                            if log not in arraylist:                                
                                arraylist.append(log)    
            ...
            for layer in inplan_check_layer:
                has_week = False
                for dic_week_info in [self.dic_tgz_week_layers,
                                      self.dic_tgz_barcode_week_layers]:                
                    for key, values in dic_week_info.iteritems():                
                        if layer in key:
                            has_week = True
                            
                if not has_week:                    
                    log = u"检测到inplan要求周期添加层次:{0}中的{1}内不存在动态周期，请检查周期是否添加层次有误，或周期是否打散！".format(layer_side, layer)
                    if log not in arraylist:                        
                        arraylist.append(log)
                            
            if len(set(all_week_text)) > 1:
                if manual_check_week_info:
                    stepnames = [x[0] for x in manual_check_week_info]
                    for stepname in set(stepnames):
                        step = gClasses.Step(job, stepname)
                        step.open()
                        step.COM("units,type=mm")
                        layers = [x[1] for x in manual_check_week_info]
                        for worklayer in layers:
                            step.clearAll()
                            step.affect(worklayer)
                            
                            text = []
                            for name, layer, obj in manual_check_week_info:
                                if name != stepname:
                                    continue
                                if layer != worklayer:
                                    continue
                                ...
                                step.selectFeatureIndex(worklayer, obj.feat_index)
                                text.append(obj.text.replace("'", ""))
                            
                            if step.featureSelected():
                                log = u"系统检测到{0} {1}层有多种形式的周期{2}，请检查周期是否分开添加，请确认标记位置周期是否异常！".format(
                                    step.name, worklayer, ",".join(text)
                                )
                                if log not in arraylist:
                                    arraylist.append(log)                            
                                    dic_zu_layer = self.get_selected_dic_note_coordinate(
                                        job.name, step.name, step, worklayer,worklayer,
                                        log, dic_zu_layer)                            
                        
                #else:
                    #arraylist.append(u"检测到资料内存在多种不同形式的周期格式{0}，请检查！".format(list(set(all_week_text))))
                    
            else:
                if all_week_text and all_week_text[0] != "".join([x for x in new_erp_period if x ]):
                    arraylist.append(u"检测到资料内周期形式{0}跟inplan周期形式{1}不一致，请检查！".format(
                        all_week_text[0].replace("-", ""), ERPPeriod))                

            self.tgz_week = []
            for week in all_week_text:
                if "y" in week:
                    self.tgz_week.append(week)
            
            if len(set(self.tgz_week)) > 1:
                arraylist.append(u"检测到资料内存在多种不同形式的周期格式{0}，请检查！".format(list(set(self.tgz_week))))
        else:
            arraylist.append(u"获取型号的周期信息异常，请检查型号命名是否正确！")
            
        check_type = ["signal", "solder_mask", "silk_screen"]
        log = u"检测到资料在数据库内没有记录信息，请运行公共程序内的 CAM资料数据记录程序!!！"
        res = self.check_week_picihao_banci_diff_mysql_record(
            {}, *check_type, warning_text=log,get_week_picihao_banci_info=[self.dic_tgz_week_layers, self.dic_tgz_barcode_week_layers, 
                                                                           self.dic_tgz_banci_layers, self.dic_tgz_picihao_layers,
                                                                           self.dic_tgz_xinqi_layers]
        )
        
        #if res[1] and log in res[1]:
            #update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                         #u"forbid_or_information='{0}'".format(u"拦截(不放行)")) 
        if res[0] != "success":
    ...
    def check_record_week_picihao_to_mysql(self, *args, **kwargs):
        ...
        if not data_info:
            ...
            if "SHOW_UI" in args:
                ...
                if kwargs.get("warning_text", None):
                    return "error",[kwargs["warning_text"]]
                else:                
                    return "error", [u"检测到资料未登记公共程序内的 CAM资料数据记录程序，请登记ok后再输出资料！"]
        else:
            try:
            ...
            if check_status == "error":                
                # if os.environ.get("SHOW_UI", "yes") == "yes":
                if "SHOW_UI" in args:
                    showMessageInfo(u"检测到资料 CAM资料数据记录程序 有数据未审核【{0}】，请通知CAM审核！".format(" ".join(not_director_info)))
                    exit(1)
                else:
                    return "error", [u"检测到资料 CAM资料数据记录程序 有数据未审核【{0}】，请通知CAM审核！".format(" ".join(not_director_info))]
                
            res = self.check_week_picihao_banci_diff_mysql_record(job_detail_info_for_output, *args, **kwargs)
            if res[0] == "error":                
                # if os.environ.get("SHOW_UI", "yes") == "yes":
                if "SHOW_UI" in args:
    ...
    def check_week_picihao_banci_diff_mysql_record(self, dic_job_detail_info_for_output, *args, **kwargs):
        """检测周期批次号班次信息跟mysql内登记的信息是否一致 20241023 by lyh"""
        global solderMaskLayers, silkscreenLayers
        
        if not solderMaskLayers:
            # 重新再获取下            
            solderMaskLayers = [lay for i, lay in enumerate(job.matrix.getInfo()["gROWname"])
                                if job.matrix.getInfo()["gROWcontext"][i] == "board"
                                and job.matrix.getInfo()["gROWlayer_type"][i] == "solder_mask"]
        if not silkscreenLayers:        
            silkscreenLayers = [lay for i, lay in enumerate(job.matrix.getInfo()["gROWname"])
                                if job.matrix.getInfo()["gROWcontext"][i] == "board"
                                and job.matrix.getInfo()["gROWlayer_type"][i] == "silk_screen"]
            
        #if job.name == "c18304oi868c1":
            #job.PAUSE(str(solderMaskLayers))        
        ...
        if not dic_job_detail_info_for_output:
            ...
            if not dic_job_detail_info_for_output:
                if kwargs.get("warning_text", None):
                    ...
                    return "error",[u"检测到资料在数据库内没有记录信息，请先运行公共程序内的 CAM资料数据记录程序，请登记ok后再输出tgz！"]
        
        if not kwargs.get("get_week_picihao_banci_info", None):
            res = self.get_week_picihao_banci_info(*args)
            if res:
                dic_tgz_week_layers, dic_tgz_barcode_week_layers, dic_tgz_banci_layers, dic_tgz_picihao_layers, dic_tgz_xinqi_layers = res
            else:
                return "error", [u"获取板内周期信息失败，请反馈程序组查看！"]
        else:
            dic_tgz_week_layers, dic_tgz_barcode_week_layers, dic_tgz_banci_layers, dic_tgz_picihao_layers, dic_tgz_xinqi_layers = kwargs["get_week_picihao_banci_info"]
        ...
        for key, value in dic_job_detail_info_for_output.iteritems():
            if key == "week_table":                
                for info in value:
                    ...
                    for check_type in ["signal", "solder_mask", "silk_screen"]:
                        if check_type in args:
                        
                            if check_type == "signal":
                                array_layers = layer1.split()
                                num = num1
                                linshi_layers = outsignalLayers
                            if check_type == "solder_mask":
                                array_layers = layer2.split()
                                num = num2
                                linshi_layers = solderMaskLayers
                            if check_type == "silk_screen":
                                array_layers = layer3.split()
                                num = num3
                                linshi_layers = silkscreenLayers
                                
                            calc_num = 0                        
                            for worklayer in array_layers:                                
                                if order == "1":  
                                    calc_num += len(self.get_value_contain_year(dic_tgz_week_layers.get(worklayer, [])))                                                
                                if order == "2":
                                    calc_num += len(self.get_value_contain_year(dic_tgz_barcode_week_layers.get(worklayer, [])))                                                
                                if order == "3":
                                    # job.PAUSE(str(dic_tgz_picihao_layers))
                                    calc_num += len(dic_tgz_picihao_layers.get(worklayer, []))                                                
                                if order == "4":
                                    calc_num += len(dic_tgz_banci_layers.get(worklayer, []))                                                
                                if order == "5":
                                    calc_num += len(dic_tgz_xinqi_layers.get(worklayer, []))                     
                            
                            try:
                                
                                if calc_num != int(num):
                                    log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                                    arraylist_log.append(log.format("".join(array_layers), calc_num, num, name))
                            except:
                                pass
                            
                            if kwargs.get("output_check_picihao", None):
                                if dic_tgz_picihao_layers and set(dic_tgz_picihao_layers.keys()) & set(array_layers):
                                    output_picihao, tgz_pichao = kwargs["output_check_picihao"]
                                    if "None" in output_picihao:
                                        log = u"检测到资料内有批次号，但申请周期未录入批次号，请检查！"
                                        if log not in arraylist_log:                                            
                                            arraylist_log.append(log)
                                    else:
                                        if len(str(tgz_pichao).lstrip("-")) != len(output_picihao):
                                            log = u"检测到修改批次号{0}跟资料内的批次号{1}长度不一致，请检查！".format(output_picihao, tgz_pichao.lstrip("-"))
                                            if log not in arraylist_log:                                            
                                                arraylist_log.append(log)
                                
                dic_check_type_layers = {"signal": outsignalLayers,
                                         "solder_mask": solderMaskLayers,
                                         "silk_screen": silkscreenLayers,}
                
                if dic_tgz_week_layers:
                    arraylist_log = self.get_week_check_log(value,
                                                            dic_tgz_week_layers,
                                                            dic_check_type_layers,
                                                            arraylist_log,
                                                            "1",
                                                            args)  
                ...
                if dic_tgz_banci_layers:
                    arraylist_log = self.get_week_check_log(value,
                                                            ...
                                                            "4",
                                                            args)                    
                if dic_tgz_xinqi_layers:
                    arraylist_log = self.get_week_check_log(value,
                                                            dic_tgz_xinqi_layers,
                                                            dic_check_type_layers,
                                                            arraylist_log,
                                                            "5",
                                                            args)                    
    ...
    def get_week_check_log(self, value, dic_week_layers_info,
                           dic_check_type_layers, arraylist_log,
                           row, args):
        """获取周期检测log"""
        for check_type in ["signal", "solder_mask", "silk_screen"]:
            if check_type in args:                     
                num = 0
                for key, values in dic_week_layers_info.iteritems():
                    if key not in dic_check_type_layers[check_type]:
                        continue
                    
                    if row in ["1", "2"]:
                        # 只检测周期跟二维码周期的是否包含年
                        values = self.get_value_contain_year(values)
                    ...
                    if check_type == "signal":                                    
                        calc_num = 0
                        for info in value:
                            if info[0] == row and info[2] == key.split("-ls")[0]:
                                calc_num = int(info[3])                                        
                    if check_type == "solder_mask":
                        calc_num = 0
                        for info in value:
                            if info[0] == row and info[4] == key.split("-ls")[0]:
                                calc_num = int(info[5])
                    if check_type == "silk_screen":
                        calc_num = 0
                        for info in value:
                            if info[0] == row and info[6] == key.split("-ls")[0]:
                                calc_num = int(info[7])
                    
                    if num != calc_num:
                        name = [info for info in value if info[0] == row][0][1]
                        log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                        if log.format(key, num, calc_num, name) not in arraylist_log:                                        
                            arraylist_log.append("2"+log.format(key, num, calc_num, name))
                            
        return arraylist_log
    
    def get_value_contain_year(self, values):
        """获取只含有年的value"""
        new_values = []
        for obj, name in values:
            for dynamic_text in ["$$yy", "$${yy}", "$$year", "$${year}", "$$mm", "$${mm}"]:                            
                if dynamic_text.lower() in str(obj.text).lower():
                    if [obj, name] not in new_values:                        
                        new_values.append([obj, name])
                    
        return new_values
    
    def check_set_to_set_connection_mask_open(self, *args):
        """检测连接位置是否开窗 20241014 by lyh 940*b77a1
        ...
        if "auto_check" in args:
            ...
            bad_mark_info = get_job_attr_info(job.name, "CI_CONNECTION_BAD_OPEN_WINDOW_")
            bad_mark_open = False
            if data_info:
                if data_info[0]["VALUE"].decode("utf8") != u"是":
                    return u"success inplan连接位是否开窗栏位：{0},无需检测！".format(data_info[0]["VALUE"].decode("utf8")), None
                
                if bad_mark_info and bad_mark_info[0]["VALUE"].decode("utf8") == u"是":
                    bad_mark_open = True
            else:
                return u"success 未获取到inplan连接位是否开窗栏位信息,无需检测！", None
        
        # job.PAUSE(str([bad_mark_open]))
        if "set" in matrixinfo["gCOLstep_name"]:            
                    ...
                    if step.featureSelected():
                        ...
                        if step.isLayer("find_connection_line"):
                            ...
                            for i in range(1, 1000):
                                ...
                                step.COM("filter_set,filter_name=popup,update_popup=yes,slot=line,slot_by=length,min_len=0.128,max_len=100")
                            ...
                            if step.isLayer("find_connection_outer_line"):
                                ...
                                step.copyLayer(job.name, step.name, "set_connection_surface", "set_connection_surface_bak_tmp")
                                ...
                                for mask_layer in ["m1", "m2"]:
                                    ...
                                    step.copyLayer(job.name, step.name, "set_connection_surface_bak_tmp", "set_connection_surface")
                                    ...
                                    if step.isLayer(mask_layer):
                                        ...
                                        if bad_mark_open:
                                            ...
                                            #step.COM("set_filter_attributes,filter_name=ref_select,"
                                                     #"exclude_attributes=no,condition=yes,attribute=.fiducial_name,"
                                                     #"min_int_val=0,max_int_val=0,min_float_val=0,"
                                                     #"max_float_val=0,option=,text=badmark")
                                            step.clearAll()
                                            step.affect(sig_layer)
                                            step.resetFilter()
                                            step.setAttrFilter(".fiducial_name,text=badmark")
                                            step.selectAll()
                                            if step.featureSelected():
                                                step.copySel(sig_layer+"_tmp_badmark")
                                                
                                                step.clearAll()
                                                step.affect("set_connection_surface")
                                                step.resetFilter()
                                                step.COM("reset_filter_criteria,filter_name=ref_select,criteria=inc_attr")
                                                step.COM("sel_ref_feat,layers={0},use=filter,mode=touch,"
                                                         "f_types=line\;pad\;surface\;arc\;text,polarity=positive,"
                                                         "include_syms=,exclude_syms=".format(sig_layer+"_tmp_badmark"))                                                
                                                if step.featureSelected():
                                                    # step.selectDelete()
                                                    step.moveSel(mask_layer+"_badmark_tmp", size=-20*25.4)
                                                    
                                            else:
                                                log = u"检测到{0} 内有坏板点开窗要求，但资料内{1}层未找到.fiducial_name,text=badmark的物件，请添加坏板点属性!".format(mask_layer, sig_layer)
                                                arraylist_log.append(log)                                        
                                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, mask_layer,
                                                                                                     [sig_layer], log,
                                                                                                     add_note_layer=mask_layer,
                                                                                                     dic_zu_layer=dic_zu_layer)
                                            step.removeLayer(sig_layer+"_tmp_badmark")
                                            
                                        step.clearAll()
                                        step.affect(mask_layer+"_tmp")                                        
                                        step.contourize()
                                        step.resetFilter()
                                        if step.isLayer(mask_layer+"_badmark_tmp"):
                                            step.refSelectFilter(mask_layer+"_badmark_tmp", mode='include')
                                            if step.featureSelected():
                                                log = u"检测到{0} 内有坏板点有开窗要求，但制作成了开通窗，请检查标记位置坏板点开窗是否异常!".format(mask_layer)
                                                arraylist_log.append(log)                                        
                                        ...
                                        #相切的情况会有误报 这里减一个5um 翟鸣通知增加到1mil范围
                                        step.COM("sel_resize,size=-25.4")
                                        step.refSelectFilter("set_fill_surface_bak")
                                        if step.featureSelected():
                                            step.copySel(mask_layer+"_tmp_touch_set")
                                            step.copyLayer(job.name, step.name, mask_layer+"_tmp_touch_set", mask_layer+"_tmp_touch_set_bak")
                                            step.clearAll()
                                            step.affect(mask_layer+"_tmp_touch_set")
                                            step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                                                     "area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                                                     "ref_layer={1},feat_types=line\;pad\;surface\;arc\;text".format(
                                                         mask_layer+"_tmp_touch_set_bak",
                                                         "set_fill_surface_bak", 0
                                                     ))
                                            
                                            step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                                                     "area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                                                     "ref_layer={1},feat_types=line\;pad\;surface\;arc\;text".format(
                                                         mask_layer+"_tmp_touch_set",
                                                         mask_layer+"_tmp_touch_set_bak", 1
                                                     ))                                            
                                            step.resetFilter()
                                            step.selectAll()
                                            if step.featureSelected():                                                
                                                ...
                                                arraylist_log.append(log)                                        
                                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, mask_layer+"_tmp_touch_set",
                                        ...
                                        if step.featureSelected():
                                            #if "-lyh" in job.name:
                                                #step.PAUSE("ddd")
                                            log = u"检测到{0} 内有连接位 未全部开窗，请检查标记位置是否异常!1".format(mask_layer)                                          
                                            arraylist_log.append(log)                                        
                                            ...
                                            if step.featureSelected():
                                                ...
                                                if step.featureSelected():
                                                    ...
                                                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, "find_connection_outer_touch",
                                                                                                         ...
                                                                                                         add_note_layer=mask_layer,
                                                                                                         dic_zu_layer=dic_zu_layer)
                                        if step.isLayer("find_connection_tmp"):
                                            # 翟鸣通知跟锣线中线线切去判断
                                            step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                                                     "area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                                                     "ref_layer={1},feat_types=line\;pad\;surface\;arc\;text,break_to_islands=yes".format(
                                                         "check_mask_open_1mil",
                                                         "find_connection_tmp", -127
                                                     ))
                                            
                                            if step.isLayer("find_connection_line_bak"):                                                
                                                step.clearAll()
                                                step.resetFilter()
                                                step.affect("check_mask_open_1mil")
                                                step.refSelectFilter("find_connection_line_bak", mode="disjoint")
                                ...
                                if step.isLayer("find_connection_line"):
                                    for mask_layer in ["m1", "m2"]:
                                        ...
                                        if step.isLayer(mask_layer):
                                            ...
                                            step.COM("sel_reverse")
                                            if step.featureSelected():
                                                log = u"检测到{0} 内有连接位 未全部开窗，请检查标记位置是否异常!2".format(mask_layer)                                          
                                                arraylist_log.append(log)                                        
                        ...
                        if "-lyh" not in job.name:
                            ...
                            for mask_layer in ["m1", "m2"]:
                                step.removeLayer(mask_layer+"_tmp")
                                step.removeLayer(mask_layer+"_tmp_touch_set")
                                step.removeLayer(mask_layer+"_tmp_touch_set_bak")
    ...
    def check_all_process_exists_layer(self):
        ...
        for info in mrp_info:
            ...
            if (inner_outer == "inner" or inner_outer == "coreinner")and "-" not in mrp_name:
                continue            
                        ...
                        if step.isLayer(worklayer):
                            ...
                            if big_hole_symbols:
                                ...
                                if step.featureSelected():
                                    ...
                                    step.refSelectFilter(worklayer, polarity="positive", exclude_syms=";".join(big_hole_symbols))                                   
                                    if step.featureSelected():                                        
                                        step.COM("sel_reverse")                                    
                                    if step.featureSelected():                                      
        ...
        if array_imp_check_info:
            ...
            for check_text in array_imp_check_info:
                for dic_imp_info in arraylist_imp_info:
                    ...
                    finish_lw_ls_text = str(QtCore.QString(str(round(finish_lw, 2)))).rstrip("0").rstrip(".")
                    if finish_ls:
                        finish_lw_ls_text = "{0}/{1}".format(("%.2f" % finish_lw).rstrip("0").rstrip("."),
                                                             ("%.2f" % finish_ls).rstrip("0").rstrip("."))
                        #因精度问题 要用qt转换一下 保持跟阻抗条程序一致
                        finish_lw_ls_text == "{0}/{1}".format(
                            str(QtCore.QString(str(round(finish_lw, 2)))).rstrip("0").rstrip("."),
                            str(QtCore.QString(str(round(finish_ls, 2)))).rstrip("0").rstrip("."),
                        )
                        
                    origin_lw_ls_text = ("%.2f" % origin_lw).rstrip("0").rstrip(".")
                    origin_lw_ls_text = str(QtCore.QString(str(round(origin_lw, 2)))).rstrip("0").rstrip(".")
                    if origin_ls:
                        origin_lw_ls_text = "{0}/{1}".format(("%.2f" % origin_lw).rstrip("0").rstrip("."),
                                                             ("%.2f" % origin_ls).rstrip("0").rstrip("."))
                        origin_lw_ls_text = "{0}/{1}".format(
                            str(QtCore.QString(str(round(origin_lw, 2)))).rstrip("0").rstrip("."),
                            str(QtCore.QString(str(round(origin_ls, 2)))).rstrip("0").rstrip("."),
                        )
                        
                    eq_lw_ls_text = ("%.2f" % eq_lw).rstrip("0").rstrip(".")
                    eq_lw_ls_text = str(QtCore.QString(str(round(eq_lw, 2)))).rstrip("0").rstrip(".")
                    if eq_ls:
                        eq_lw_ls_text = "{0}/{1}".format(("%.2f" % eq_lw).rstrip("0").rstrip("."),
                                                             ("%.2f" % eq_ls).rstrip("0").rstrip("."))
                        eq_lw_ls_text = "{0}/{1}".format(
                            str(QtCore.QString(str(round(eq_lw, 2)))).rstrip("0").rstrip("."),
                            str(QtCore.QString(str(round(eq_ls, 2)))).rstrip("0").rstrip("."),
                        )
                        
                    #if trace_layer == "L5" and user == "84310" and cusimp == "40":
                        #job.PAUSE(str([finish_lw_ls_text, origin_lw_ls_text, eq_lw_ls_text]))
    ...
    def check_hct_coupon_netlist_info(self):
        ...
        check_hct_steps = []
        for stepname in matrixinfo["gCOLstep_name"]:
            if "hct_coupon" in stepname or "hct-coupon" in stepname or "clip_line_coupon" in stepname:                
                ...
                for worklayer in outsignalLayers:
                    if step.isLayer(worklayer):                
                        ...
                        if "clip_line_coupon" in stepname:
                            step.resetFilter()
                        ...
                        if step.featureSelected():
                            ...
                            step.selectNone()
                            step.COM("sel_board_net_feat,operation=select,x={0},y={1},tol=170.4725,use_ffilter=no".format(pos_x-orignx, pos_y-origny))
                            #if "-lyh" in job.name and "clip_line_coupon" in stepname:
                                #step.PAUSE("c")                            
                            if step.featureSelected():
                                featout = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["pads"]
                                sel_feat_index = list(set([obj.feat_index for obj in featout if getattr(obj, "string", "") == "test_pad"]))
                                ...
                                if ("hct_coupon_new" in stepname and not symbols_rect) or \
                                   ("hct-coupon" in stepname and len(s1524_symbols) < 2) or \
                                   ("clip_line_coupon" in stepname and len(sel_feat_index) < 2):
    ...
    def check_pnl_stepname_info(self, auto_check=None):
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:            
            ...
            for name in all_steps:
                ...
                if re.match(".*[_-]d-coupon[_-]?.*|.*[_-]d_coupon[_-]?.*", name):
                    d_coupon_steps.append(name)
                    
                if "cu-coupon" in name:
                    cu_coupon_steps.append(name)
        ...
        if not d_coupon_steps and job.name[1:4] in ["a86", "d10"]:# and job.name[12:14] == "1":
            log= u"检测此nv型号 panel拼版内不存在d-coupon的step，请反馈主管，并检查是否有d-coupon的制作要求！"
            arraylist.append(log)
            
        if not cu_coupon_steps and job.name[1:4] in ["a86", "d10"]:
            log= u"检测此nv型号 panel拼版内不存在拉力测试 cu-coupon 的step，请反馈主管，并检查是否需求添加拉力测试coupon！"
            arraylist.append(log)
                
        if icg_step_check_erro:
            log= u"检测到存在icg step，panel拼版内未拼入{0}，请检查！"
            arraylist.append(log.format(";".join(icg_steps)))
                ...
                if step.featureSelected():
                    ...
                    for obj in feat_out:
                        ...
                        if "$$job" in getattr(obj, "text", "").lower() and "$$job_cut" not in getattr(obj, "text", "").lower():
                            if cust_no.upper() in "940/B69/676/975/C75/E33/B49/C18/E519/B27/473/A79/B78/E26":
                                if "$$jobn" not in obj.text.lower():
                                    log = u"940/B69/676/975/C75/E33/B49/C18/E519/B27/473/A79/B78/E26系列出货单元添加厂内料号时选择:$$jobn，检查到{0} 内添加的是$$job，请检查".format(stepname)
                                    arraylist.append(log)
                                    find_feat_index.append(obj.feat_index)
                            else:
                                if "$$jobn" in obj.text.lower():
                                    log = u"非940/B69/676/975/C75/E33/B49/C18/E519/B27/473/A79/B78/E26系列出货单元添加厂内料号时要用:$$job，检查到{0} 内添加的是$$jobn，请检查".format(stepname)
                                    arraylist.append(log)
                                    find_feat_index.append(obj.feat_index)                                
    ...
    def sort_points(self, points):
        """按中心点极角排序点，使四边形顶点顺序一致"""
        if not points:
            return []
    
        points = self.move_points_by_origin(points)
        # 计算中心点
        cx = sum(p[0] for p in points) / len(points)
        cy = sum(p[1] for p in points) / len(points)
        # 计算极角并排序
        points_with_angles = [
            (p, math.atan2(p[1] - cy, p[0] - cx))
            for p in points
        ]
        return [p for p, angle in sorted(points_with_angles, key=lambda x: x[1])]
    
    def move_points_by_origin(self, points):
        """以左下角靠近原点的相对原点来计算偏移后的位置"""
        all_len = [self.distance(p, (0, 0)) for p in points]
        find_point = []
        for i, length in enumerate(all_len):
            if length == min(all_len):
                find_point = points[i]
                break
    
        offset_x,offset_y = 0, 0
        if find_point:
            offset_x,offset_y = find_point
    
        return [(x-offset_x, y-offset_y) for x, y in points]    
    
    def has_displacement(self, group1, group2, threshold=1.0):
        """检查两组点是否存在错开超过阈值的情况"""
        sorted_group1 = self.sort_points(group1)
        sorted_group2 = self.sort_points(group2)
        print("--->", sorted_group1)
        print("--->", sorted_group2)
        for p1, p2 in zip(sorted_group1, sorted_group2):
            if self.distance(p1, p2) >= threshold:
                return p1, p2
            
        return False 
    
    def check_same_job_all_target_defender(self, *args):
        """检测当前型号同前版本所有型号各类靶点是否有防呆 20250321 by lyh"""
        panel_edge_symbol_info = self.get_panel_edge_symbol_coordinate_info()
        sql = "select * from hdi_engineering.incam_process_target_info where job_name like '%{0}%'"
        arraylist_target_info = conn.SELECT_DIC(dbc_m, sql.format(jobname[1:11]))
        arraylist_current_job_same_size_log = []        
        if arraylist_target_info:
            ...
            wip_jobs = [x["job"].upper() for x in get_wip_jobs()]
                
            array_cancel_jobs = [x["TC_AAC01"].upper() for x in get_erp_cancel_jobs()]
            array_cancel_jobs = [x for x in array_cancel_jobs if x not in wip_jobs]
            array_cancel_jobs = []
            
            # arraylist_target_info = self.get_current_job_all_target_info(job.name)
            #计算只保留最新版本
            # print(arraylist_target_info)
            new_arraylist_current_pre_jobs_info = [x for x in arraylist_target_info
                                                   if x["job_name"].upper() != job.name.split("-")[0].upper()
                                                   and x["job_name"].upper() not in array_cancel_jobs]
            
            # array_mrp_info = get_inplan_mrp_info(jobname.upper(), condtion="1=1")
            # print(new_arraylist_current_pre_jobs_info)
            symbol_info = panel_edge_symbol_info
            """job_name,mrp_name,bar_type,target_design_region,pnl_short_x1,pnl_short_x2,\
            pnl_long_y1,pnl_long_y2,layer,symbolname,coordinate_json,creator,cam_notes,create_time"""            
            for pre_job_dic_info in new_arraylist_current_pre_jobs_info:
                
                worklayer = pre_job_dic_info["layer"]
                symbolname = pre_job_dic_info["symbolname"]
                ccd_type_name = pre_job_dic_info["bar_type"]
                mrp_name = pre_job_dic_info["mrp_name"]
                pre_job_name = pre_job_dic_info["job_name"]
                pre_arraylist_coordinate_info = json.loads(pre_job_dic_info["coordinate_json"], encoding='utf8')
                pre_pnl_short_x1 = pre_job_dic_info["pnl_short_x1"]
                pre_pnl_short_x2 = pre_job_dic_info["pnl_short_x2"]
                pre_pnl_long_y1 = pre_job_dic_info["pnl_long_y1"]
                pre_pnl_long_y2 = pre_job_dic_info["pnl_long_y2"]
                pre_job_dic_info["X1"] = "%.1f" % pre_pnl_short_x1
                pre_job_dic_info["X2"] = "%.1f" % pre_pnl_short_x2
                pre_job_dic_info["Y1"] = "%.1f" % pre_pnl_long_y1
                pre_job_dic_info["Y2"] = "%.1f" % pre_pnl_long_y2
                if ccd_type_name == u"备用靶":
                    byb_symbols = symbolname.split(";")
                    symbolname = byb_symbols[0]
                
                current_mrp_name = mrp_name.replace(pre_job_name.upper(), jobname.split("-")[0].upper())
                current_job_dic_info = self.get_layer_target_distance_info(None, jobname, current_mrp_name, ccd_type_name,
                                                                           symbol_info, symbolname, worklayer,
                                                                           "" ,sr_xmin, sr_ymin,
                                                                           sr_xmax, sr_ymax, [], byb_symbols)
                if current_job_dic_info:
                    current_arraylist_coordinate_info = current_job_dic_info["coordinate_json"]
                    if current_job_dic_info.get("target_design_region", None) != u"多组靶点":
                        current_pnl_short_x1 = current_job_dic_info["pnl_short_x1"]
                        current_pnl_short_x2 = current_job_dic_info["pnl_short_x2"]
                        current_pnl_long_y1 = current_job_dic_info["pnl_long_y1"]
                        current_pnl_long_y2 = current_job_dic_info["pnl_long_y2"]
                        current_job_dic_info["X1"] = "%.1f" % current_pnl_short_x1
                        current_job_dic_info["X2"] = "%.1f" % current_pnl_short_x2
                        current_job_dic_info["Y1"] = "%.1f" % current_pnl_long_y1
                        current_job_dic_info["Y2"] = "%.1f" % current_pnl_long_y2                        
                        if round(abs(current_pnl_short_x1 - pre_pnl_short_x1), 1) < 1.1 and \
                           round(abs(current_pnl_short_x2 - pre_pnl_short_x2), 1) < 1.1 and \
                           round(abs(current_pnl_long_y1 - pre_pnl_long_y1), 1) < 1.1 and \
                           round(abs(current_pnl_long_y2 - pre_pnl_long_y2), 1) < 1.1 :
                            
                            same_size_log = u"当前版本{mrp_name} 层别{layer}: {bar_type} {target_design_region} X1：{X1} X2：{X2} Y1：{Y1} Y2：{Y2} "
                            arraylist_current_job_same_size_log.append(same_size_log.format(**current_job_dic_info))
                            same_size_log = u"其他版本{mrp_name} 层别{layer}: {bar_type} {target_design_region} X1：{X1} X2：{X2} Y1：{Y1} Y2：{Y2} (不防呆)"
                            arraylist_current_job_same_size_log.append(same_size_log.format(**pre_job_dic_info))
                    else:
                        group1 = [("%.1f" % float(x[1]), "%.1f" % float(x[2])) for x in pre_arraylist_coordinate_info]
                        group2 = [("%.1f" % float(x[1]), "%.1f" % float(x[2])) for x in current_arraylist_coordinate_info]
                        res = self.has_displacement([(float(x[0]), float(x[1])) for x in group1],
                                                    [(float(x[0]), float(x[1])) for x in group2])
                        if not res:
                            same_size_log = u"当前版本{mrp_name} 层别{layer}: {bar_type} {target_design_region}坐标：{0}"
                            arraylist_current_job_same_size_log.append(same_size_log.format(group2, **current_job_dic_info))
                            same_size_log = u"其他版本{mrp_name} 层别{layer}: {bar_type} {target_design_region}坐标：{0}(不防呆)"
                            arraylist_current_job_same_size_log.append(same_size_log.format(group1, **pre_job_dic_info))
            
        if arraylist_current_job_same_size_log:
            return arraylist_current_job_same_size_log, None
        
        return "success", None                    
                            
    def get_layer_target_distance_info(self, dbc_m, jobname, mrp_name, ccd_type_name,
                                       symbol_info, symbolname, worklayer, cam_notes,
                                       sr_xmin, sr_ymin, sr_xmax, sr_ymax,
                                       solderMaskLayers=[],byb_symbols=[]):
        """层靶标距离数据分类"""
        #uploading_data_info = [jobname, mrp_name, ccd_type_name, u"短边",
                               #"", "", "", "", "", "", "", "", ""]
        """job_name,mrp_name,bar_type,target_design_region,pnl_short_x1,pnl_short_x2,\
        pnl_long_y1,pnl_long_y2,layer,symbolname,coordinate_json,creator,cam_notes,create_time"""
        uploading_data_info = {"job_name": jobname,"mrp_name": mrp_name,"bar_type": ccd_type_name,
                               "target_design_region": u"短边","pnl_short_x1": "","pnl_short_x2": "",
                               "pnl_long_y1": "","pnl_long_y2": "","layer": "","symbolname": "",
                               "coordinate_json": "","creator": "","cam_notes": "",}
        arraylist_coordinate_info = []
        for info in symbol_info.get(worklayer, []):
            if symbolname in byb_symbols:
                for name in byb_symbols:
                    if name in info:
                        if len(info) > 5:
                            if "yh_py_pad" not in info[5].values():                            
                                arraylist_coordinate_info.append(info)
                        else:
                            arraylist_coordinate_info.append(info)
            else:
                if symbolname in info:
                    arraylist_coordinate_info.append(info)
                    if len(info) > 5:
                        dic_attr = info[5]                        
                        if  symbolname == "hdi-dwpad" and \
                            "split_panel" in dic_attr.values():
                            uploading_data_info["bar_type"] = u"分割五代靶"
                            
                        if  symbolname in ["sh-dw2014", "sh-dwsd2014"] and \
                            "split_panel" in dic_attr.values():
                            uploading_data_info["bar_type"] = u"分割防焊CCD"
                            
                        if symbolname == "r0" and \
                           "300.trmv" in dic_attr.values() and \
                            "300.trmh" in dic_attr.values() and \
                            "300.measure" in dic_attr.values():
                            if worklayer in solderMaskLayers:
                                uploading_data_info["bar_type"] = u"分割防焊CCD"
                            else:
                                uploading_data_info["bar_type"] = u"分割五代靶"
                
        if not arraylist_coordinate_info:
            return {}
        
        if len(arraylist_coordinate_info) == 4:
            arraylist_xy = [(float(info[1]), float(info[2])) for info in arraylist_coordinate_info]
            position, length_x1, length_x2, length_y1, length_y2 = \
                self.get_short_long_position(arraylist_xy, sr_xmin, sr_ymin, sr_xmax, sr_ymax)
            uploading_data_info["target_design_region"] = position
            uploading_data_info["pnl_short_x1"] = length_x1
            uploading_data_info["pnl_short_x2"] = length_x2
            uploading_data_info["pnl_long_y1"] = length_y1
            uploading_data_info["pnl_long_y2"] = length_y2
            uploading_data_info["layer"] = worklayer
            uploading_data_info["symbolname"] = symbolname if symbolname not in byb_symbols else ";".join(byb_symbols)
            uploading_data_info["coordinate_json"] = arraylist_coordinate_info# json.dumps(arraylist_coordinate_info)
            uploading_data_info["creator"] = "system"
            uploading_data_info["cam_notes"] = cam_notes
        else:
            uploading_data_info["target_design_region"] = u"多组靶点"
            uploading_data_info["layer"] = worklayer
            uploading_data_info["symbolname"] = symbolname if symbolname not in byb_symbols else ";".join(byb_symbols)
            uploading_data_info["coordinate_json"] = arraylist_coordinate_info# json.dumps(arraylist_coordinate_info)
            uploading_data_info["creator"] = "system"
        ...
        return uploading_data_info
    
    def get_short_long_position(self, arraylist_xy, sr_xmin, sr_ymin, sr_xmax, sr_ymax):
        """获取靶在长短边方向"""        
        # 左上角
        upper_left_x, upper_left_y = \
            [(x, y) for x, y in arraylist_xy
             if y > (sr_ymin + sr_ymax) * 0.5
             and x < (sr_xmin + sr_xmax) * 0.5][0]
        ...
        length_x1 = upper_right_x - upper_left_x
        length_x2 = down_right_x - down_left_x
        length_y1 = upper_left_y - down_left_y
        length_y2 = upper_right_y - down_right_y
        
        for x, y in arraylist_xy:
            if sr_xmin< x < sr_xmax and 0< y < sr_ymin + 50:
                return u"短边", length_x1, length_x2, length_y1, length_y2
            
            if 0 < x < sr_xmin + 50 and sr_ymin< y < sr_ymax:
                return u"长边", length_x1, length_x2, length_y1, length_y2
    ...
    def check_target_distance_uploading_to_erp(self, uploading_to_erp="NO"):
        ...
        all_mrp_info = get_inplan_mrp_info(job.name.upper(), condtion=" 1=1 ")
        ...
        ##获取流程内的备注的打靶信息 用于比对检测打靶是否一致 20250210 by lyh
        xray_data_info = get_inplan_all_flow(job.name, select_dic=True)
        target_xray_process = ['HDI10601',]
        array_xray_info = [dic_info for dic_info in xray_data_info
                          if dic_info["OPERATION_CODE"] in target_xray_process]
                
        # arraylist = []
        dic_zu_layer = {}
        arraylist = [u"检测到靶距有以下问题，请检查是否异常！"]
        arraylist_same_size_log = [u"检测当前型号靶距跟ERP内相同panel跟set尺寸(尺寸小于1mm以内算相同)靶距之差小于1mm为不防呆，请手动调整！"] # 记录比对相同尺寸 靶距不防呆的信息
        ...
        if uploading_to_erp == "check_same_size_defender":
            ...
            array_cancel_jobs = [x for x in array_cancel_jobs if x not in wip_jobs]
            # 相同panel跟set尺寸信息型号
            job_param_info = getJobData(job.name)
            set_size_x = job_param_info["setX"]
            set_size_y = job_param_info["setY"]            
            get_same_pnl_set_size_jobs = [x["JOB_NAME"] for x in get_same_size_pnl_set_from_inplan(f_xmax, f_ymax, set_size_x, set_size_y)]
            #if "-lyh" in job.name:
                #job.PAUSE(str(get_same_pnl_set_size_jobs))            
            #相同尺寸的型号靶距信息
            arraylist_same_size_info = self.get_same_size_jobs_info(job.name, f_xmax, f_ymax)
            for dic_info in arraylist_same_size_info:
                for key in dic_info.keys():
                    value = dic_info[key]
                    if key.upper() in ["PNLXINCH", "PNLYINCH"]:
                        if "X" in key.upper():                            
                            dic_info["panel_x"] = value * 25.4
                        else:
                            dic_info["panel_y"] = value * 25.4
                    elif key.upper() == "PROCESS_NUM":
                        dic_info["PRESS_NUM"] = value
                    else:
                        dic_info[key.upper()] = value
            #本料号其他版本去掉 这个检测在同型号不同版本内有检测逻辑
            arraylist_same_size_info = [x for x in arraylist_same_size_info
                                        if x["JOB_NAME"][1:11].lower() != job.name[1:11]
                                        and x["JOB_NAME"].upper() in get_same_pnl_set_size_jobs
                                        and x["JOB_NAME"].upper() not in array_cancel_jobs]
            #计算只保留最新版本
            new_arraylist_same_size_info = arraylist_same_size_info
            ...
            #if "-lyh" in job.name:
                #job.PAUSE(str([x["JOB_NAME"] for x in new_arraylist_same_size_info]))
                
        arraylist_current_job_same_size_log = [u"检测当前型号靶距跟前版本靶距之差小于1mm为不防呆，请手动调整！"]
        new_arraylist_current_pre_jobs_info = []
        if uploading_to_erp == "check_current_job_same_size_defender":
            # 检测当前型号跟前版本是否靶距防呆
            #dic_job_date = {}
            #for dic_info in get_all_job_creation_date():       
                #dic_job_date[dic_info["JOB_NAME"]] = dic_info["CREATION_DATE"]
            ...
            new_arraylist_current_pre_jobs_info = [x for x in arraylist_target_info
                                                   if x["JOB_NAME"].upper() != job.name.split("-")[0].upper()
                                                   and x["JOB_NAME"].upper() not in array_cancel_jobs]
            #names = [x['JOB_NAME'] for x in new_arraylist_current_pre_jobs_info]
            #if user == "84310":                
                #job.PAUSE(str([new_arraylist_current_pre_jobs_info,  arraylist_target_info]))
            compare_jobs_log_file = os.path.join(userDir, 'check_current_job_same_size_defender_jobs.json')
            try:
                with open(compare_jobs_log_file, "w") as f:
                    f.write(str(new_arraylist_current_pre_jobs_info))
            except Exception, e:
                print e
                
        # 检测靶被移动后所有层是否有对齐20250407 by lyh
        # if "-lyh" in job.name:            
        panel_edge_symbol_info = self.get_panel_edge_symbol_coordinate_info()
        dic_check_target_center_zu = {}
        for dic_info in sorted(all_mrp_info, key=lambda x: x["PROCESS_NUM"] * -1):
            from_lay = dic_info["FROMLAY"].lower()
            to_lay = dic_info["TOLAY"].lower()
            press_num = dic_info["PROCESS_NUM"]
            if not dic_check_target_center_zu.get(press_num, None):
                dic_check_target_center_zu[press_num] = []
                
            if from_lay in matrixinfo["gROWname"]:
                dic_check_target_center_zu[press_num].append(from_lay)
                
            if to_lay in matrixinfo["gROWname"]:
                dic_check_target_center_zu[press_num].append(to_lay)
        
        for key, checklayers in dic_check_target_center_zu.iteritems():
            res = self.check_contact_layer_target_is_in_center(step, panel_edge_symbol_info, checklayers, dic_zu_layer)
            if res:
                if uploading_to_erp == "manual":
                    showMessageInfo(res)
                    # return
        ...
        for dic_info in sorted(mrp_info, key=lambda x: x["PROCESS_NUM"] * -1):
            ...
            #if "-lyh" in job.name:
                #job.PAUSE(str([dic_info,lb_y, f_ymax, rout_y]))
            ...
            if from_lay in signalLayers:
                index = signalLayers.index(from_lay)
                check_layer = signalLayers[index+1]
            else:
                # 临时层的情况
                for index, siglayer in enumerate(signalLayers):
                    if from_lay + "-ls" in siglayer:
                        break
                check_layer = signalLayers[index+1]
            ...
            #特殊料号
            if mrp_name == "EB1308GHB39A1-10104":
                press_num = 1
                step.selectSymbol("hdi1-baj{0}".format(press_num), 1, 1)
                layer_cmd = gClasses.Layer(step, check_layer)
                feat_out1 = layer_cmd.featSelOut(units="mm")["pads"]
            ...
            #特殊料号
            if mrp_name == "EB1308GHB39A1-10508":
                press_num = 2
                step.selectSymbol("hdi1-baj{0}".format(press_num), 1, 1)
                layer_cmd = gClasses.Layer(step, check_layer)
                feat_out1 = layer_cmd.featSelOut(units="mm")["pads"]
            ...
            if len(feat_out1) ==2 and len(feat_out2) == 5:
                ...
                #if "-lyh" in job.name:
                    #job.PAUSE(str([all_x, sr_xmin, sr_xmax, min(all_x), max(all_x)]))
                    ...
                    if (len(core_mrp_info) >= 2 or (f_xmax <= 622.3 and f_ymax <= 622.3)):
                        ...
                        if f_ymax - lb_y - upper_right_y > 70 or \
                           f_ymax - lb_y - upper_right_y < 20:
                            log =u"检测四靶在长边，但{1}层四靶顶部靶距裁边超工艺范围20-70，实际值{0}，请检查！{2}".format(abs(f_ymax - lb_y - upper_right_y), check_layer, lb_y)
                            arraylist.append(log)
                            show_msg = True                        
                    ...
                    if (len(core_mrp_info) >= 2 or (f_xmax <= 622.3 and f_ymax <= 622.3)):
                        ...
                        if abs(f_ymin + lb_y - down_left_y) > 70 or \
                           abs(f_ymin + lb_y - down_left_y) < 20:
                            log =u"检测四靶在长边，但{1}层四靶底部靶距裁边超工艺范围20-70，实际值{0}，请检查！{2}".format(abs(f_ymin + lb_y - down_left_y), check_layer, lb_y)
                            arraylist.append(log)
                            show_msg = True 
                ...
                if job.name.upper() == "HC1116PH155A1":
                    if abs((upper_right_x - f_xmin) - (f_xmax - upper_left_x)) > 0.1:
                        log =u"检测到第{1}次四靶靶距X方向不居中，相差{0}".format(round(abs((upper_right_x - f_xmin) - (f_xmax - upper_left_x)), 2), press_num)
                        arraylist.append(log)
                        show_msg = True
                        update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                                     u"forbid_or_information='{0}'".format(u"拦截(不放行)"))
                        
                    # job.PAUSE(str([down_left_y, upper_left_y]))
                    if abs((down_right_y - f_ymin) - (f_ymax - upper_right_y)) > 0.1:
                        log =u"检测到第{1}次四靶靶距Y方向不居中，相差{0}".format(round(abs((down_left_y - f_ymin) - (f_ymax - upper_left_y)), 2), press_num)
                        arraylist.append(log)
                        show_msg = True
                        update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                                     u"forbid_or_information='{0}'".format(u"拦截(不放行)"))                     
                        ...
                        if (len(core_mrp_info) >= 2 or (f_xmax <= 622.3 and f_ymax <= 622.3)):
                            ...
                            if abs(f_ymin + lb_y - min(left_all_y)) > 70 or \
                               abs(f_ymin + lb_y - min(left_all_y)) < 20:
                                log =u"检测外四靶在长边，但{1}层外四靶底部靶距裁边超工艺范围20-70，实际值{0}，请检查！{2}".format(abs(f_ymin + lb_y - min(left_all_y)), check_layer, lb_y)
                                arraylist.append(log)
                                show_msg = True                             
                ...
                # 检测外四靶跟inplan备注是否一致 20250210 by lyh
                # if "-lyh" in job.name:                    
                if "-" in mrp_name:
                    ba_num = press_num
                else:
                    ba_num = "T"                
                if has_waisiba:
                    res = self.check_has_target_waisiba(mrp_name, array_xray_info, "{0}{0}".format(ba_num))                    
                    if not res:                        
                        log = u"检测到{2} 流程中xray打靶 未备注使用{1}{1}靶，但资料内存在{1}{1}靶不一致，请检查是否异常！"
                        arraylist.append(log.format("", ba_num,mrp_name))
                        #跟翟鸣沟通 此种情况可以不拦截 仅提示 20250212 by lyh
                        show_msg = False
                else:
                    res = self.check_has_target_waisiba(mrp_name, array_xray_info, "{0}{0}".format(ba_num))
                    if res:                        
                        log = u"检测到{2} 流程中xray打靶 备注：{0}，但资料内未添加{1}{1}靶标，请检查是否异常！"
                        arraylist.append(log.format(res.decode("utf8"), ba_num,mrp_name))
                        show_msg = True
                ...
                if uploading_to_erp == "check_same_size_defender":
                    for same_size_dic_info in new_arraylist_same_size_info:                        
                        ...
                        if "-" in same_size_dic_info["MRPNAME"]:
                            ba_num1 = same_size_dic_info["PRESS_NUM"] - 1
                        else:
                            ba_num1 = "T"                            
                        
                        bar4_x1 = same_size_dic_info["BAR4X1"]
                        bar4_y1 = same_size_dic_info["BAR4Y1"]
                        bar4_x2 = same_size_dic_info["BAR4X2"]
                        bar4_y2 = same_size_dic_info["BAR4Y2"]
                        w_x1 = same_size_dic_info["SIG4X1"]
                        w_y1 = same_size_dic_info["SIG4Y1"]
                        w_x2 = same_size_dic_info["SIG4X2"]
                        w_y2 = same_size_dic_info["SIG4Y2"]
                        #if same_size_dic_info["MRPNAME"][1:11] == mrp_name[1:11]:                            
                            #job.PAUSE(str([same_size_dic_info, dic_calc_target_info, mrp_name, same_size_dic_info["MRPNAME"]]))
                        if round(abs(bar4_x1 - dic_calc_target_info["X1"]), 1) < 1.1 and \
                           round(abs(bar4_y1 - dic_calc_target_info["Y1"]), 1) < 1.1 and \
                           round(abs(bar4_x2 - dic_calc_target_info["X2"]), 1) < 1.1 and \
                           round(abs(bar4_y2 - dic_calc_target_info["Y2"]), 1) < 1.1 :
                            same_size_log = u"当前型号{MRPNAME}: {ba_num}靶 X1：{X1} X2：{X2} Y1：{Y1} Y2：{Y2} panel_x:{panel_x} panel_y:{panel_y} "
                            arraylist_same_size_log.append(same_size_log.format(JOB_NAME=job.name,panel_x=f_xmax, panel_y=f_ymax, MRPNAME=mrp_name,ba_num=ba_num, **dic_calc_target_info))
                            same_size_log = u"其他型号{MRPNAME}: {ba_num}靶 X1：{BAR4X1} X2：{BAR4X2} Y1：{BAR4Y1} Y2：{BAR4Y2}  panel_x:{panel_x} panel_y:{panel_y} (四靶不防呆)"
                            arraylist_same_size_log.append(same_size_log.format(ba_num=ba_num1, **same_size_dic_info))
                        
                        if not (w_x1 == 0 and w_y1 == 0 and w_x2 == 0 and w_y2 == 0):                            
                            if round(abs(w_x1 - dic_calc_target_info.get("W_X1", 0)), 1) < 1.1 and \
                               round(abs(w_y1 - dic_calc_target_info.get("W_Y1", 0)), 1) < 1.1 and \
                               round(abs(w_x2 - dic_calc_target_info.get("W_X2", 0)), 1) < 1.1 and \
                               round(abs(w_y2 - dic_calc_target_info.get("W_Y2", 0)), 1) < 1.1 :
                                same_size_log = u"当前型号{MRPNAME}: {ba_num}/{ba_num}外四靶 X1：{W_X1} X2：{W_X2} Y1：{W_Y1} Y2：{W_Y2} panel_x:{panel_x} panel_y:{panel_y} "
                                arraylist_same_size_log.append(same_size_log.format(JOB_NAME=job.name,panel_x=f_xmax, panel_y=f_ymax, MRPNAME=mrp_name, ba_num=ba_num,**dic_calc_target_info))
                                same_size_log = u"其他型号{MRPNAME}: {ba_num}/{ba_num}外四靶 X1：{SIG4X1} X2：{SIG4X2} Y1：{SIG4Y1} Y2：{SIG4Y2}  panel_x:{panel_x} panel_y:{panel_y} (外四靶不防呆)"
                                arraylist_same_size_log.append(same_size_log.format(ba_num=ba_num1,**same_size_dic_info))                            
                       
                        bar3_x = same_size_dic_info["BAR3X"]
                        bar3_y = same_size_dic_info["BAR3Y"]
                        if abs(bar3_x - dic_calc_target_info["L_X"]) < 1.1 and abs(bar3_y - dic_calc_target_info["L_Y"]) < 1.1:
                            same_size_log = u"当前型号{MRPNAME}:{ba_num}L靶 防呆靶：{L_X} 箭靶：{L_Y} panel_x:{panel_x} panel_y:{panel_y} "
                            arraylist_same_size_log.append(same_size_log.format(JOB_NAME=job.name,panel_x=f_xmax, panel_y=f_ymax, MRPNAME=mrp_name, ba_num=ba_num,**dic_calc_target_info))                            
                            same_size_log = u"其他型号{MRPNAME}:{ba_num}L靶 防呆靶：{BAR3X} 箭靶：{BAR3Y} panel_x:{panel_x} panel_y:{panel_y} (L靶不防呆)"
                            arraylist_same_size_log.append(same_size_log.format(ba_num=ba_num1,**same_size_dic_info))
                ...
                if uploading_to_erp == "check_current_job_same_size_defender":                                                           
                    for pre_job_dic_info in new_arraylist_current_pre_jobs_info:                        
                        ...
                        # 王总指示要跟其他不同层次比对
                        #if "-" not in mrp_name and "-" in pre_job_dic_info["MRPNAME"]:
                            #continue
                        
                        #if "-" in mrp_name and "-" not in pre_job_dic_info["MRPNAME"]:
                            #continue                        
                        
                        #if "-" in mrp_name and "-" in pre_job_dic_info["MRPNAME"]:                        
                            #if pre_job_dic_info["MRPNAME"].split("-")[1] != mrp_name.split("-")[1]:
                                #continue
                        ...
                        if "-" in pre_job_dic_info["MRPNAME"]:
                            ba_num1 = pre_job_dic_info["PRESS_NUM"] - 1
                        else:
                            ba_num1 = "T"                            
                        
                        bar4_x1 = pre_job_dic_info["BAR4X1"]
                        bar4_y1 = pre_job_dic_info["BAR4Y1"]
                        bar4_x2 = pre_job_dic_info["BAR4X2"]
                        bar4_y2 = pre_job_dic_info["BAR4Y2"]
                        w_x1 = pre_job_dic_info["W_X1"]
                        w_y1 = pre_job_dic_info["W_Y1"]
                        w_x2 = pre_job_dic_info["W_X2"]
                        w_y2 = pre_job_dic_info["W_Y2"]                        
                        # job.PAUSE(str([pre_job_dic_info, dic_calc_target_info, mrp_name]))
                        if round(abs(bar4_x1 - dic_calc_target_info["X1"]), 1) < 1.1 and \
                           round(abs(bar4_y1 - dic_calc_target_info["Y1"]), 1) < 1.1 and \
                           round(abs(bar4_x2 - dic_calc_target_info["X2"]), 1) < 1.1 and \
                           round(abs(bar4_y2 - dic_calc_target_info["Y2"]), 1) < 1.1 :
                            same_size_log = u"当前版本{MRPNAME}: {ba_num}靶 X1：{X1} X2：{X2} Y1：{Y1} Y2：{Y2} panel_x:{panel_x} panel_y:{panel_y} "
                            arraylist_current_job_same_size_log.append(same_size_log.format(JOB_NAME=job.name,panel_x=f_xmax, panel_y=f_ymax, MRPNAME=mrp_name,ba_num=ba_num, **dic_calc_target_info))
                            same_size_log = u"其他版本{MRPNAME}: {ba_num}靶 X1：{BAR4X1} X2：{BAR4X2} Y1：{BAR4Y1} Y2：{BAR4Y2}  panel_x:{panel_x} panel_y:{panel_y} (四靶不防呆)"
                            arraylist_current_job_same_size_log.append(same_size_log.format(ba_num=ba_num1, **pre_job_dic_info))
                        ...
                        if not (w_x1 == 0 and w_y1 == 0 and w_x2 == 0 and w_y2 == 0):                            
                            if round(abs(w_x1 - dic_calc_target_info.get("W_X1", 0)), 1) < 1.1 and \
                               ...
                               round(abs(w_x2 - dic_calc_target_info.get("W_X2", 0)), 1) < 1.1 and \
                               round(abs(w_y2 - dic_calc_target_info.get("W_Y2", 0)), 1) < 1.1 :
                                same_size_log = u"当前版本{MRPNAME}: {ba_num}/{ba_num}外四靶 X1：{W_X1} X2：{W_X2} Y1：{W_Y1} Y2：{W_Y2} panel_x:{panel_x} panel_y:{panel_y} "
                                arraylist_current_job_same_size_log.append(same_size_log.format(JOB_NAME=job.name,panel_x=f_xmax, panel_y=f_ymax, MRPNAME=mrp_name, ba_num=ba_num,**dic_calc_target_info))
                                same_size_log = u"其他版本{MRPNAME}: {ba_num}/{ba_num}外四靶 X1：{W_X1} X2：{W_X2} Y1：{W_Y1} Y2：{W_Y2}  panel_x:{panel_x} panel_y:{panel_y} (外四靶不防呆)"
                                arraylist_current_job_same_size_log.append(same_size_log.format(ba_num=ba_num1,**pre_job_dic_info))                            
                       
                        bar3_x = pre_job_dic_info["BAR3X"]
                        bar3_y = pre_job_dic_info["BAR3Y"]
                        if abs(bar3_x - dic_calc_target_info["L_X"]) < 1.1 and abs(bar3_y - dic_calc_target_info["L_Y"]) < 1.1:
                            same_size_log = u"当前版本{MRPNAME}:{ba_num}L靶 防呆靶：{L_X} 箭靶：{L_Y} panel_x:{panel_x} panel_y:{panel_y} "
                            arraylist_current_job_same_size_log.append(same_size_log.format(JOB_NAME=job.name,panel_x=f_xmax, panel_y=f_ymax, MRPNAME=mrp_name, ba_num=ba_num,**dic_calc_target_info))                            
                            same_size_log = u"其他版本{MRPNAME}:{ba_num}L靶 防呆靶：{BAR3X} 箭靶：{BAR3Y} panel_x:{panel_x} panel_y:{panel_y} (L靶不防呆)"
                            arraylist_current_job_same_size_log.append(same_size_log.format(ba_num=ba_num1,**pre_job_dic_info))
                ...
                if matrixinfo["gROWlayer_type"][i] == "drill":
                    ...
                    if re.match("[bc]d[cs]?.*", ks_drill_lay) and \
                       step.isLayer(ks_drill_lay+".inn"):
                        ...
                        # step.PAUSE(str([len(feat_out), attrs, is_continue]))
                        if is_continue:
                            continue
        ...
        if uploading_to_erp == "manual":
            ...
            for dic_info in mrp_info:
                mrp_name = dic_info["MRPNAME"]
                # 20241228 mrp_name存在时 执行append
                if mrp_name in dic_uploading_info:
        ...
        if uploading_to_erp == "check_current_job_same_size_defender":
            if len(arraylist_current_job_same_size_log) > 2:
                # showMessageInfo(*arraylist_current_job_same_size_log)
                # 跟伍青沟通 增加判断外层审核时间 若已审核则为改临时资料 此处只做警告 可以审批放行 20241226 by lyh
                res = self.get_cam_director_finish_status()
                if res:
                    update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
    ...
    def check_contact_layer_target_is_in_center(self, step, panel_edge_symbol_info, checklayers, dic_zu_layer):
        """检测相关的叠构层靶标 symbol是否有对齐"""
        dic_zu = {}
        for worklayer in checklayers:
            groups = []
            if panel_edge_symbol_info.get(worklayer):
                for info in panel_edge_symbol_info.get(worklayer, []):
                    for symbolname in ["hdi1-ba", "hdi1-bs", "hdi1-by"]:
                        if symbolname in info[0]:
                            groups.append((float(info[1]), float(info[2])))            
            dic_zu[worklayer] = groups
        
        # step = gClasses.Step(job, step.name)
        compare_group = dic_zu[checklayers[0]]        
        for worklayer in checklayers[1:]:
            res = self.has_displacement(compare_group, dic_zu[worklayer], threshold=0.1)
            #if "l2" in checklayers and "-lyh" in job.name:
                #step.PAUSE(str([compare_group, dic_zu[worklayer], checklayers, res]))
                
            if res:
                for layer in [checklayers[0], worklayer]:
                    step.clearAll()
                    step.affect(layer)
                    step.resetFilter()
                    step.selectSymbol("hdi1-ba*;hdi1-bs*;hdi1-by*", 1, 1)
                    if step.featureSelected():
                        step.removeLayer(layer+"_babiao_error++++")
                        step.copySel(layer+"_babiao_error++++")
                
                step.clearAll()
                step.resetFilter()
                
                log = u"检测到{0} 层跟{1}层靶标位置有偏移，请检查参考层{0}_babiao_error++++跟{1}_babiao_error++++位置是否异常！！" .format(checklayers[0], worklayer)
                dic_zu_layer = self.get_view_dic_layers(job.name, step.name, step,
                                                        worklayer="view_layer_" + worklayer+"_babiao_error++++",
                                                        dic_layer_list={
                                                            log: [checklayers[0] + "_babiao_error++++"], },
                                                        dic_zu_layer=dic_zu_layer)
                
                return log
    
    def check_has_target_waisiba(self, mrpname, array_xray_info, ba_index):
        """检测是否有外四靶"""
        for dic_info in array_xray_info:
            if dic_info["MRP_NAME"] == mrpname:
                if dic_info["NOTE_STRING"] and ba_index in dic_info["NOTE_STRING"].upper():
                    return dic_info["NOTE_STRING"]
    ...
    def get_same_size_jobs_info(self,jobname, panel_x, panel_y):
        ...
        where ABS(a.pnlxinch*25.4-{0}) < 1
        and ABS(a.pnlyinch*25.4-{1}) < 1
    ...
    def get_current_job_all_target_info(self, jobname):
        """获取当前型号所有靶距的信息"""
        index1 = jobname[1:4]
        index2 = jobname[8:11]
        ...
        a.sig4x1 W_X1,a.sig4y1 W_Y1,a.sig4x2 W_X2,a.sig4y2 W_Y2,
        a.job_name JOB_NAME,round(a.pnlxinch*25.4,3) panel_x,
        a.mrpname MRPNAME,
        a.process_num PRESS_NUM,
        round(a.pnlyinch*25.4,3) panel_y from hdi_engineering.incam_process_info a
        where a.job_name like upper('%{0}%{1}%')"""
        data_info = conn.SELECT_DIC(dbc_m, sql.format(index1, index2))
    ...
    def reread_all_step_layer(self):
        """导出tgz时检测层是否有损坏"""
        arraylist = []
        ...
        for stepname in matrixinfo["gCOLstep_name"]:
            ...
            for layer in matrixinfo["gROWname"]:
                ...
                if step.STATUS > 0:
                    try:
                        float(layer)
                        step.display(str(layer) + "0")
                        if not (step.STATUS == 0):
                            log = u"检测到{0} {1}层打不开，有可能损坏，请到资料内点开此层查看是否异常！！"
                            arraylist.append(log.format(stepname, layer))
                    except Exception as e:
                        log = u"检测到{0} {1}层打不开，有可能损坏，请到资料内点开此层查看是否异常！！"
                        arraylist.append(log.format(stepname, layer))                        
                step.VON()
    ...
    def get_cam_director_finish_status(self, process_type="ocheckfinish_time"):
        """获取审核的完成状态"""
        sql = """select psj.*
        from project_status_jobmanage psj
        WHERE job = '{0}'
        ...
        if job_process_info and job_process_info[0].get(process_type, ""):
            return job_process_info[0].get(process_type, "")
    ...
    def misc_rout_layer(self):
        """把锣带层的属性misc掉"""
        for i, layer in enumerate(matrixinfo["gROWname"]):
            if matrixinfo["gROWcontext"][i] == "board" and \
               matrixinfo["gROWlayer_type"][i] == "rout":
                if "pnl_rout" not in layer and "prof_rout" not in layer:                    
    ...
    def check_set_panel_conductor_width(self):
        ...
        翟鸣通知卡10mil 20250408 by lyh"""
        #if job.name == "s88810pbe20d1":
            #return "success", None
        arraylist = []
        dic_zu_layer = {}        
        if "set" in matrixinfo["gCOLstep_name"]:
            ...
            if flip_steps:                  
                ...
                for layer in outsignalLayers + innersignalLayers:
                    self.create_linshi_layer_for_checklist(layer)                
            else:                
                step.clearAll()
                for layer in outsignalLayers + innersignalLayers:
                    self.create_linshi_layer_for_checklist(layer)                                                
            ...
            step.clearAll()
            for layer in outsignalLayers + innersignalLayers:
                if step.isLayer("{0}_run_checklist_tmp".format(layer)):
                    step.affect("{0}_run_checklist_tmp".format(layer))                   
            #step.resetFilter()
            #step.filter_set(feat_types='surface', polarity='positive')
            #step.selectAll()
            step.contourize(accuracy=0)
            ...
            step.clearAll()
            for layer in outsignalLayers + innersignalLayers:
                ms_layer = "ms_1_{0}_conductor".format("{0}_run_checklist_tmp".format(layer))
                if step.isLayer(ms_layer):
                    step.clearAll()
                    step.affect(ms_layer)
                    step.resetFilter()
                    step.setAttrFilter(".string,text=conductor_width")
                    step.refSelectFilter(layer, mode='touch', f_types="text")
                    if step.featureSelected():
                        step.selectDelete()
                    step.refSelectFilter(layer, mode='touch', include_syms="wzfd")
                    if step.featureSelected():
                        step.selectDelete()
                    
                    step.refSelectFilter(layer, mode='disjoint', f_types="surface")
                    if step.featureSelected():
                        step.selectDelete()
                    ...
                    layer_cmd = gClasses.Layer(step, ms_layer)
                    feat_out = layer_cmd.featout_dic_Index(options="select+feat_index")["lines"]
                    step.selectNone()
                    
                    for obj in feat_out:
                        if obj.len > 9.95 * 0.001:
                            step.selectFeatureIndex(ms_layer, obj.feat_index)
                    ...
                    step.selectAll()
                    if step.featureSelected():
                        log= u"检测到{0}层折断边铜条宽度不够10 Mil，请检查异常位置是否正常！".format(layer)
                        arraylist.append(log)                        
            ...
            if flip_steps:                  
                ...
                flipstep.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=yes' % job.name)
                for layer in outsignalLayers + innersignalLayers:
                    flipstep.removeLayer("ms_1_{0}_conductor".format("{0}_run_checklist_tmp".format(layer)))
                    flipstep.removeLayer("mk_1_{0}_conductor".format("{0}_run_checklist_tmp".format(layer)))
                    flipstep.removeLayer("{0}_run_checklist_tmp".format(layer))
            else:
                for layer in outsignalLayers + innersignalLayers:
                    step.removeLayer("ms_1_{0}_conductor".format("{0}_run_checklist_tmp".format(layer)))
                    step.removeLayer("mk_1_{0}_conductor".format("{0}_run_checklist_tmp".format(layer)))
                    step.removeLayer("{0}_run_checklist_tmp".format(layer))
    ...
    def check_pth_hole_ring(self, autocheck=None):
        ...
        if 'DA8624G7T75A1'.lower() in jobname:
            return u"测试板孔环全不够，跳过检测", None        
        ...
        for stepname in all_steps:
            # 切片孔不检测
            if "qie_hole_coupon_" in stepname:
                continue
            ...
            for drill_layer in arraylist_drill:
                ...
                for signal_layer, index, ring in info:
                    ...
                    if not dic_zero_ring.has_key(signal_layer):
                        dic_zero_ring[signal_layer] = []
                    if ring < 1.98 * 0.0254:
                        if ring == 0:
                            dic_zero_ring[signal_layer].append(index)
                ...
                for signal_layer, indexes in dic_find_feat_index.iteritems():
                    # 20250313 把铜皮挑出
                    signal_surface = '%s+++surface' % signal_layer
                    step.resetFilter()
                    ...
                    step.resetFilter()
                    if step.featureSelected():
                        step.copySel(signal_surface)
                    step.clearAll()
                    # 钻孔层复制一层用来找ring环为0的pth
                    tmp_drill = '%s+++ring' % drill_layer
                    tmp_drill_ = '%s_+++ring' % drill_layer
                    step.affect(drill_layer)
                    step.copySel(tmp_drill)
                    step.clearAll()
                    ...
                    if indexes:
                        step.clearAll()
                        single_feat_layer = 'single_feat_index'
                        exclude_indexes = []
                        # 20250313 zl 可能有slot被铜皮cover但是线路分析显示ring环为0 这里处理一下防止误报
                        for index in indexes:
                            if index in dic_zero_ring.get(signal_layer):
                                step.affect(signal_layer + "_pth_ring")
                                step.selectNone()
                                step.selectFeatureIndex(signal_layer + "_pth_ring", index)
                                step.copySel(single_feat_layer)
                                step.clearAll()
                                step.affect(tmp_drill)
                                step.refSelectFilter(single_feat_layer)
                                if step.featureSelected():
                                    step.copySel(tmp_drill_)
                                    step.clearAll()
                                    step.affect(tmp_drill_)
                                    step.COM('sel_resize,size=1')
                                    if step.isLayer(signal_surface):
                                        step.refSelectFilter(signal_surface, mode='cover')
                                        if step.featureSelected():
                                            exclude_indexes.append(index)
                                    step.clearAll()
                                    step.COM('truncate_layer,layer=%s' % tmp_drill_)
                                step.clearAll()
                                step.COM('truncate_layer,layer=%s' % single_feat_layer)
                        for index in indexes:
                        ...
                        if step.featureSelected():
                            log = u"检测到{2} {0}层 对应线路{1} 的pth孔ring环小于2mil，请检查".format(drill_layer,
                                                                                                     signal_layer, stepname)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                 signal_layer + "_pth_ring",
                                                                                 drill_layer, log, dic_zu_layer,
                                                                                 add_note_layer=signal_layer)
                    step.removeLayer(tmp_drill_)
                    step.removeLayer(tmp_drill)
                    step.removeLayer(signal_surface)
                for sig_layer in signalLayers:
                    if step.isLayer(sig_layer + "_miss_pad"):
                        log = u"检测到{2} {0}层 对应线路{1} 的有丢失pad，请参考{3}层进行比对检查是否异常！".format(
                            drill_layer, signal_layer, stepname, sig_layer + "_miss_pad")
                        arraylist.append(log)
                        dic_zu_layer = self.get_view_dic_layers(job.name, stepname, step,
                                                                worklayer="view_layer_" + sig_layer,
                                                                dic_layer_list={
                                                                    log: [sig_layer + "_miss_pad", drill_layer], },
                                                                dic_zu_layer=dic_zu_layer)
    ...
    def check_modify_signal_layer_base_comp_info(self, autocheck=None):
        ...
        if res == "inner" or res == "coreinner":            
            #if "-lyh" in job.name:
                #os.system("python /incam/server/site_data/scripts/hdi_scr/Etch/innerLayerComp/innComp_win_20250428.py autocheck")
            #else:
            os.system("python /incam/server/site_data/scripts/hdi_scr/Etch/innerLayerComp/innComp_win.py autocheck")
            ...
            #if "-lyh" in job.name:
                #os.system("python /incam/server/site_data/scripts/hdi_scr/Etch/outerBaseComp/out_comp_20250428.py autocheck")
            #else:            
            os.system("python /incam/server/site_data/scripts/hdi_scr/Etch/outerBaseComp/out_comp.py autocheck")
    ...
    def check_modify_signal_layer_detch_comp_info(self, autocheck=None):
        """检测修改线路后是否有漏动补 以及基础补偿是否漏补20240924"""
        userDir = os.environ.get('JOB_USER_DIR', None)
        res = os.environ.get("INNER_OUTER", None)
        if res == "inner" or res == "coreinner":
            check_signal_layers = innersignalLayers
        ...
        #if "-lyh" in job.name:
            #if "l8" in signalLayers:                
                #check_signal_layers = ["l8"]
            #if "l4" in signalLayers:                
                #check_signal_layers = ["l4"]                
        ...
        # if led_type:
            ...
            #翟鸣通知 灯板无需检测动态补偿 及基础补偿20250311 by lyh
            # return u"success灯板无需检测动态补偿", None            
        ...
        edits = [name for name in get_panelset_sr_step(job.name, "panel")
                 if re.match("^edit", name) and "loss" not in name
                 and "coupon" not in name]
        # if autocheck == "auto":            
            # dic_check_step["edit"] = check_signal_layers
        for name in edits:
            dic_check_step[name] = check_signal_layers
        #else:
        ...
        if dic_check_step:                
            for stepname, check_layers in dic_check_step.iteritems():
                ...
                if stepname not in matrixinfo["gCOLstep_name"]:
                    continue
                if "loss" in stepname or "coupon" in stepname:
                    continue
                step = gClasses.Step(job, stepname)
                step.open()
                step.COM("units,type=mm")               
                
                for layer_tmp in matrixinfo["gROWname"]:
                    if "yibanline_check_" in layer_tmp or \
                       "impline_check_" in layer_tmp or \
                       "check_orig_size_line" in layer_tmp:
                        step.removeLayer(layer_tmp)
                
                for worklayer in list(set(check_layers)):
                    if led_type:
                        #翟鸣通知 灯板无需检测动态补偿 20250311 by lyh
                        continue
                    ...
                    if step.isLayer(worklayer+"_check_detch_line+++") and \
                       not step.isLayer(worklayer+"_surface"):
                        log = u"{1}检测到层{0} 有线未做动补，请到{0}_check_detch_line+++层检查是否异常！！".format(worklayer, step.name)
                        arraylist.append(log)                            
                        dic_zu_layer=self.get_view_dic_layers(job.name, stepname, step,
                    ...
                    if step.featureSelected():
                        ...
                        if log not in arraylist:                            
                            arraylist.append(log)                            
                            dic_zu_layer=self.get_view_dic_layers(job.name, stepname, step,
                                                                  worklayer="view_layer_"+worklayer+"_check_detch_line+++", 
                                                                  dic_layer_list={log:[worklayer], },
                                                                  dic_zu_layer=dic_zu_layer)
                    else:
                        step.removeLayer(worklayer+"_check_detch_line+++")
                ...
                #以及基础补偿是否漏补
                os.environ["STEP"] = step.name
                result = self.check_modify_signal_layer_base_comp_info()
                if result[0] != "success":
                    for layer_tmp in job.matrix.getInfo()["gROWname"]:
                        if "_base_compensate_error_+++" in layer_tmp:
                            new_layer = layer_tmp.split("_")[0] + "_base_compensate_error_{0}_+++".format(step.name)
                            if step.isLayer(new_layer):
                                step.removeLayer(new_layer)
                            step.COM("matrix_rename_layer,job={0},matrix=matrix,layer={1},new_name={2}".format(job.name, layer_tmp, new_layer))
                            log = u"{2}检测到{0}层线路基础补偿跟程序自动补偿有差异，请到{1}层比对检查异常位置(每根线中的属性orig_size为原稿大小，bc_vaule为自动补偿值)！！".format(
                                layer_tmp.split("_")[0], new_layer, step.name)
                            arraylist.append(log)                            
                            dic_zu_layer=self.get_view_dic_layers(job.name, stepname, step,
                                                                  worklayer="view_layer_"+new_layer, 
                                                                  dic_layer_list={log:[layer_tmp.split("_")[0]], },
                                                                  dic_zu_layer=dic_zu_layer)
                            
                if "-lyh" in job.name:
                    self.get_orig_size_line(step.name)                
                    self.check_has_not_orig_size_line(step.name, arraylist, dic_zu_layer)
                
                if "-lyh" not in job.name:
                    for layer in job.matrix.getInfo()["gROWname"]:
                        if "yibanline_check_" in layer or \
                           "impline_check_" in layer or \
                           "check_orig_size_line" in layer:
                            step.removeLayer(layer)
        ...
        # if "-lyh" in job.name:
        self.check_loss_icg_base_compensate(arraylist, dic_zu_layer)
    ...
    def check_loss_icg_base_compensate(self, arraylist, dic_zu_layer):
        """检测loss 跟icg的基础补偿是否异常"""
        array_imp_info = icg_coupon_compensate.get_dic_compensate_imp_info()
        layerInfo = icg_coupon_compensate.layerInfo
        arraylist += icg_coupon_compensate.warn_list
        dic_bc = {}
        for index in range(len(layerInfo)):
            layName = layerInfo[index]['LAYER_NAME'].lower()
            dic_bc[layName] = layerInfo[index]['compData'].impBaseComp
        
        all_steps = [name for name in get_panelset_sr_step(job.name, "panel")
                     if  "loss" in name or "icg"  in name]
        for stepname in all_steps:
            step = gClasses.Step(job, stepname)
            step.open()
            step.COM("units,type=inch")
            for signal_layer in signalLayers:
                step.clearAll()
                step.copyLayer(job.name, step.name, signal_layer, signal_layer+"_check_imp_tmp")
                step.affect(signal_layer+"_check_imp_tmp")                
                step.resetFilter()
                step.filter_set(feat_types='line;arc;', polarity='positive')                
                step.selectAll()
                if step.featureSelected():
                    layer_cmd = gClasses.Layer(step, signal_layer+"_check_imp_tmp")
                    feat_out = layer_cmd.featout_dic_Index(options="select+feat_index")["lines"]
                    feat_out += layer_cmd.featout_dic_Index(options="select+feat_index")["arcs"]
                    base_compensate_not_same_indexes = {}
                    bc_value = dic_bc[signal_layer]
                    for obj in feat_out:
                        imp_info = getattr(obj, "imp_lw_ref_info", None) or \
                           getattr(obj, "string", None)
                        if imp_info and imp_info.find("_") >= 3:
                            finish_lw = float(imp_info.split("_")[0])
                            work_lw = float(obj.symbol[1:])
                            
                            if abs(work_lw - finish_lw - dic_bc[signal_layer]) > 0.05 and \
                               work_lw < finish_lw + dic_bc[signal_layer]:
                                if base_compensate_not_same_indexes.has_key(obj.symbol):
                                    base_compensate_not_same_indexes[obj.symbol].append(int(obj.feat_index))
                                else:
                                    base_compensate_not_same_indexes[obj.symbol] = [int(obj.feat_index)]
                    ...
                    if base_compensate_not_same_indexes:
                        ...
                        for orig_size, indexes in base_compensate_not_same_indexes.iteritems():
                            ...
                            for group in group_indexes:
                                ...
                                step.selectAll()
                                if step.featureSelected():                       
                                    step.copyToLayer(signal_layer+"_base_compensate_error_+++")
                        
                        log = u"{2}检测到{0}层线路基础补偿跟程序自动补偿有差异，请到{1}层比对检查异常位置(每根线中的属性orig_size为原稿大小，bc_vaule为自动补偿值)！！".format(
                            signal_layer, signal_layer+"_base_compensate_error_+++", step.name)
                        if log not in "".join(arraylist):
                            arraylist.append(log)
                            dic_zu_layer=self.get_view_dic_layers(job.name, stepname, step,
                                                                  worklayer="view_layer_"+signal_layer+"_base_compensate_error_+++", 
                                                                  dic_layer_list={log +"3":[signal_layer], },
                                                                  dic_zu_layer=dic_zu_layer)                    
    
                step.removeLayer(signal_layer+"_check_imp_tmp")
                
    def check_has_not_orig_size_line(self, stepname, arraylist, dic_zu_layer):
        """检测没有原稿属性的线是否有基础补偿"""
       
        step = gClasses.Step(job, stepname)
        step.open()
        step.COM("units,type=inch")
        all_layers = job.matrix.getInfo()["gROWname"]
        for worklayer in all_layers:
            if "_impline_check_" in worklayer or "_yibanline_check_" in worklayer:
                signal_layer = worklayer.split("_")[0]

                if not step.isLayer(signal_layer+"_check_orig_size_line"):                
                    continue
                
                step.clearAll()
                step.affect(signal_layer+"_check_orig_size_line")
                step.resetAttr()
                step.addAttr(".string", attrVal='check', valType='text', change_attr="yes")
                step.resetAttr()
                ...
                step.addAttr(".area", valType='', change_attr="yes")
                step.resetAttr()
                step.copyLayer(job.name, step.name, signal_layer+"_check_orig_size_line",
                               worklayer, mode='append')
                step.resetFilter()
                step.setAttrFilter(".string,text=check")
                step.selectAll()
                step.resetFilter()
                step.COM("sel_ref_feat,layers=,use=select,mode=include,\
                pads_as=shape,f_types=line\;pad\;surface\;arc\;text,\
                polarity=positive\;negative,include_syms=,exclude_syms=")
                cover_select_feat = layer_cmd.featout_dic_Index(units="inch", options="feat_index+select")
                cover_feat_out = cover_select_feat["lines"]
                cover_feat_out += cover_select_feat["arcs"]
                cover_feat_indexes = [cover_obj.feat_index for cover_obj in cover_feat_out]
                
                not_cover_feat_out = [obj for obj in feat_out
                                      if obj.feat_index not in cover_feat_indexes]
                
                bc_value = float(worklayer.split("_")[-1])
                base_compensate_not_same_indexes = {}
                
                for feat_out_zu in [cover_feat_out, not_cover_feat_out]:
                    
                    dic_zu = {}
                    for obj in feat_out_zu:                   
                        if dic_zu.has_key(obj.symbol):
                            dic_zu[obj.symbol].append(obj)
                        else:
                            dic_zu[obj.symbol]=[obj]
                                
                    for symbol, group_obj in dic_zu.iteritems():
                        if feat_out_zu == cover_feat_out:
                            step.selectNone()
                            step.resetFilter()
                            step.selectSymbol(symbol, 1, 1)
                            #if step.featureSelected():
                                #step.copySel(worklayer+"_cover")
                            res = self.check_base_compensate_not_same_indexes(step, group_obj[0], worklayer, bc_value,
                                                                              layer_cmd, base_compensate_not_same_indexes, "cover")
                            if res:
                            ...
                            group_indexes = self.group_and_limit_numbers([int(obj.feat_index) for obj in group_obj])
                            for group in group_indexes:
                                step.selectNone()
                                step.resetFilter()
                                step.COM("adv_filter_reset")
                                ...
                                step.selectAll()
                                #if step.featureSelected():
                                    #step.copySel(worklayer+"_not_cover")                                
                                res = self.check_base_compensate_not_same_indexes(step, group_obj[0], worklayer, bc_value,
                                                                                  layer_cmd, base_compensate_not_same_indexes, "total")
                                if res:
                                    for obj in group_obj:
                                        step.selectNone()
                                        step.selectFeatureIndex(worklayer, obj.feat_index)
                                        self.check_base_compensate_not_same_indexes(step, obj, worklayer, bc_value,
                                                                                    layer_cmd, base_compensate_not_same_indexes, "single")                                       
                ...
                if base_compensate_not_same_indexes:
                    ...
                    if log not in "".join(arraylist):
                        ...
                        dic_zu_layer=self.get_view_dic_layers(job.name, stepname, step,
                                                              worklayer="view_layer_"+signal_layer+"_base_compensate_error_+++", 
                                                              dic_layer_list={log +"2":[signal_layer], },
                                                              dic_zu_layer=dic_zu_layer)
                        
    def check_base_compensate_not_same_indexes(self, step, obj,
                                               worklayer, bc_value,
                                               layer_cmd, 
                                               base_compensate_not_same_indexes,
                                               line_model):
        if step.featureSelected():
            step.resetFilter()
            step.setAttrFilter(".string,text=check")
            step.filter_set(feat_types='line;arc;', include_syms=obj.symbol[0]+"*", polarity='positive')
            if line_model == "single":
                step.COM("sel_ref_feat,layers=,use=select,mode=cover,\
                pads_as=shape,f_types=line\;pad\;surface\;arc\;text,\
                polarity=positive\;negative,include_syms=,exclude_syms=")
                if not step.featureSelected():
                    step.selectFeatureIndex(worklayer, obj.feat_index)
            ...
            if step.featureSelected():
                select_feat = layer_cmd.featout_dic_Index(units="inch", options="feat_index+select")
                orig_feat = select_feat["lines"]
                orig_feat += select_feat["arcs"]
                orig_symbols = sorted([o_obj.symbol for o_obj in orig_feat], key=lambda x: float(x[1:]))
                if line_model == "single":                  
                    for orig_size in orig_symbols:
                        if abs(float(obj.symbol[1:]) - float(orig_size[1:]) - float(bc_value)) <= 0.05 or \
                           float(obj.symbol[1:]) >= float(orig_size[1:]) + float(bc_value):
                            return 0                        
                                   
                orig_size = orig_symbols[-1]                    
                if abs(float(obj.symbol[1:]) - float(orig_size[1:]) - float(bc_value)) > 0.05 and \
                   float(obj.symbol[1:]) < float(orig_size[1:]) + float(bc_value):
    ...
    def get_orig_size_line(self, stepname):
        """从net中获取原稿线宽"""                
        # stepname = "edit"
        net_stepname = stepname.replace("edit", "net")
        if  net_stepname not in matrixinfo["gCOLstep_name"]:
            step = gClasses.Step(job, "net")
        else:
            step = gClasses.Step(job, net_stepname)
        ...
        for layer in all_layers:
            if "_impline_check_" in layer:
                dic_check_layer[layer.split("_")[0]] = layer
        ...
        for worklayer in check_signal_layers:
            
            for dic_imp_info in arraylist_imp_info:                
                trace_layer = dic_imp_info["TRACE_LAYER_"]
                orig_width = dic_imp_info["ORG_WIDTH"]
                finish_width = dic_imp_info["FINISH_LW_"]
                ...
                if step.featureSelected():
                    if dic_check_layer.get(worklayer):
                        step.copySel(dic_check_layer[worklayer]+"_orig")
                        
                        step.clearAll()
                        step.affect(dic_check_layer[worklayer]+"_orig")
                        step.selectSymbol("r{0}".format(orig_width), 1, 1)
                        if step.featureSelected():
                            step.changeSymbol("r{0}".format(finish_width))    
                
            step.clearAll()
            step.affect(worklayer)
            step.resetFilter()
            step.filter_set(feat_types='line;arc', polarity='positive')
            if dic_check_layer.get(worklayer) and step.isLayer(dic_check_layer[worklayer]+"_orig"):
                step.selectAll()
                step.copySel(worklayer+"_line_tmp")
                step.clearAll()
                step.affect(worklayer+"_line_tmp")
                step.refSelectFilter(dic_check_layer[worklayer]+"_orig", mode='include')
                step.COM("sel_reverse")
            else:
                step.selectAll()
                
            if step.featureSelected():
                step.copySel(worklayer+"_check_orig_size_line")
                
                if dic_check_layer.get(worklayer) and step.isLayer(dic_check_layer[worklayer]+"_orig"):
                    step.copyLayer(job.name, step.name, dic_check_layer[worklayer]+"_orig",
                                   worklayer+"_check_orig_size_line", mode='append')
            
            step.removeLayer(worklayer+"_line_tmp")
            if dic_check_layer.get(worklayer) and step.isLayer(dic_check_layer[worklayer]+"_orig"):
                step.removeLayer(dic_check_layer[worklayer]+"_orig")
                
        # stepname = "edit"
        step = gClasses.Step(job, stepname)
        step.open()
        for layer in job.matrix.getInfo()["gROWname"]:
            if "check_orig_size_line" in layer:
    ...
    def check_zk_line_cover_by_reflayer(self, *args):
        ...
        #if job.name.upper() in ["FA8622O6784H2"]:
            #signal_layers = ["l17"]
            
        #if job.name.upper() in ["FA8622O6784H1"]:
            #signal_layers = ["l6"]               
        ...
        for worklayer in signal_layers:
            ...
            for dic_imp_info in arraylist_imp_info:
                ...
                for i, layer in enumerate(ref_layers):
                    if layer not in signalLayers:
                        new_layer = [x for x in signalLayers if layer + "-ls" in x]
                        if new_layer:
                            ref_layers[i] = new_layer[0]
            ...
            for orig_width, array_ref_layers in dic_ref_layer.iteritems():
                ...
                step.removeLayer(worklayer+"_{0}".format(orig_width))
                for layer in job.matrix.getInfo()["gROWname"]:
                    if "_surface" in layer and \
                       "_imp_orig_surface" not in layer and \
                       "inner_panel_surface" not in layer:                        
                        step.removeLayer(layer)
                step.removeLayer("find_ref_line")
                step.removeLayer("find_ref_line_tmp")
                step.removeLayer("find_ref_line_surface_tmp")
        ...
        for check_layer in check_imp_layers:
            ...
            edit_check_line_layer = check_layer.replace("orig", stepname)
            ...
            step.clearAll()            
            if abs(int(worklayer.split("-ls")[0][1:]) - int(ref_layer.split("-ls")[0][1:])) > 1:
                if int(worklayer.split("-ls")[0][1:]) > int(ref_layer.split("-ls")[0][1:]):
                    flag = -1
                else:
                    flag = 1
                
                step.affect(edit_check_line_layer)
                for i in range(1, abs(int(worklayer.split("-ls")[0][1:]) - int(ref_layer.split("-ls")[0][1:]))):                    
                    center_layer = "l{0}".format(int(worklayer.split("-ls")[0][1:]) + i*flag)                    
                    step.resetFilter()
                    if center_layer in signalLayers:                        
                        step.refSelectFilter(center_layer, f_types='line;arc;text;surface')
                    else:
                        for layer in signalLayers:
                            if center_layer + "-ls" in layer:
                                step.refSelectFilter(layer)
                                break
                    if step.featureSelected():
                        # step.PAUSE(str([worklayer, ref_layer, center_layer]))
                        log = u"重要项目：检测到{0} 参考{1}层 中间隔层{2} 铜皮未全部掏空，请检查标识位置是否异常！".format(
                            worklayer, ref_layer, center_layer
                        )
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, edit_check_line_layer,
        ...
        if "cam_director" in args:
            showMessageInfo(u"检测成功，未发现异常！")
        
        return "success", None
    
    def check_loss_icg_line_cover_by_reflayer(self):        
        """检测loss条及icg阻抗条的阻抗线是否被屏蔽层覆盖 20251011 by lyh"""
        
        arraylist_imp_info = get_inplan_imp(job.name.upper())
        
        dic_ref_layer = {}
        dic_not_touch_feature_layer = {}
        for worklayer in signalLayers:
            dic_ref_layer[worklayer] = []
            dic_not_touch_feature_layer[worklayer] = []
            for dic_imp_info in arraylist_imp_info:
                ref_layers = dic_imp_info["REF_LAYER_"]
                if ref_layers is None:
                    ref_layers = ""
                ...
                trace_layer = dic_imp_info["TRACE_LAYER_"]
                if trace_layer.lower() == worklayer:
                    dic_ref_layer[worklayer] += ref_layers
                    
                    if worklayer in innersignalLayers:
                        for ref_layer in ref_layers:
                            num = abs(int(ref_layer[1:]) - int(worklayer[1:]))
                            if  num > 1:
                                if int(ref_layer[1:]) < int(worklayer[1:]):                                    
                                    dic_not_touch_feature_layer[worklayer] += ["l{0}".format(int(worklayer[1:]) - 1 - i) for i in range(num)]
                                else:
                                    dic_not_touch_feature_layer[worklayer] += ["l{0}".format(int(worklayer[1:]) + 1 + i) for i in range(num)]
                    ...
                    if len(ref_layers) == 1:
                        if worklayer in outsignalLayers:
                            if worklayer == "l1":
                                dic_not_touch_feature_layer[worklayer] += [layer for layer in solderMaskLayers if "m1" in layer]
                            else:
                                dic_not_touch_feature_layer[worklayer] += [layer for layer in solderMaskLayers if "m2" in layer]
                        else:
                            if int(ref_layers[0][1:]) < int(worklayer[1:]):
                                dic_not_touch_feature_layer[worklayer] += [layer for layer in signalLayers if int(worklayer[1:]) < int(layer[1:])]
                                dic_not_touch_feature_layer[worklayer] += [layer for layer in solderMaskLayers if "m2" in layer]
                            else:
                                dic_not_touch_feature_layer[worklayer] += [layer for layer in signalLayers if int(worklayer[1:]) > int(layer[1:])]
                                dic_not_touch_feature_layer[worklayer] += [layer for layer in solderMaskLayers if "m1" in layer]                                
        ...
        setsteps = get_panelset_sr_step(jobname, "panel", is_group_by=True, is_all=True)
        arraylist_log = []
        dic_zu_layer = {}
        is_check = False
        for stepname in list(set(setsteps)):
            if "loss" not in stepname and "icg" not in stepname:
                continue
            is_check = True
            step = gClasses.Step(job, stepname)
            step.open()
            ...
            for worklayer, ref_layers in dic_ref_layer.iteritems():
                step.clearAll()
                step.affect(worklayer)
                step.resetFilter()
                
                step.filter_set(feat_types='line;arc', polarity='positive')
                step.setAttrFilter(".string,text=loss_line")
                step.selectAll()
                delta_v4 = False
                if step.featureSelected():
                    delta_v4 = True
                
                if not delta_v4 and "loss" in step.name:
                    x1, y1, x2, y2 = get_profile_limits(step)
                    if x2 - x1 > 200:
                        delta_v4 = True
                ...
                step.selectAll()
                if step.featureSelected():
                    # 翟鸣通知 屏蔽层安全距离为10mil范围不能有负片
                    step.copyToLayer(worklayer+"_check_imp_line_cover", size=20*25.4)
                    step.clearAll()
                    step.affect(worklayer+"_check_imp_line_cover")
                    step.resetFilter()                    
                    if "icg" in step.name:
                        step.setAttrFilter(".imp_line")
                        step.selectAll()
                        ...
                        step.resetFilter()
                        step.setAttrFilter(".string,text=jx_line")
                        step.selectAll()
                        if step.featureSelected():
                            step.selectDelete()
                    ...
                    step.copyLayer(job.name, step.name, worklayer+"_check_imp_line_cover", worklayer+"_check_imp_line_cover_bak")
                    ...
                    polarity=positive\;negative,include_syms=,exclude_syms=")
                    if step.featureSelected():                        
                        step.copyToLayer(worklayer+"_check_imp_line_touch_pad")
                    
                        step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                 "area=reference,area_type=rectangle,inout=inside,"
                                 "contour_cut=yes,margin={2},ref_layer={1},"
                                 "feat_types=line\;pad\;surface\;arc\;text,".format(worklayer+"_check_imp_line_cover",
                                                                                    worklayer+"_check_imp_line_touch_pad",
                                                                                    20*25.4))
                    #检测是否被参考层完全屏蔽
                    #if worklayer == "l2":
                        #step.PAUSE(str([worklayer, ref_layers]))
                    #for num, ref_layer in enumerate(sorted(set(ref_layers), key=lambda x: abs(int(x[1:]) - int(worklayer[1:])))):
                    for flag in [1, -1]:                        
                        for num in range(len(signalLayers) + 1):
                            linshi_layer = "l{0}".format(int(worklayer[1:]) - 1*flag - num*flag)
                            if not step.isLayer(linshi_layer):
                                continue
                            step.copyLayer(job.name, step.name, linshi_layer, linshi_layer+"_imp_line_cover")
                            step.clearAll()
                            step.affect(linshi_layer+"_imp_line_cover")
                            step.contourize()
                            ...
                            if num == 0:
                                ...
                                if step.isLayer(worklayer+"_check_imp_line_cover_disjoint"):
                                    step.affect(worklayer+"_check_imp_line_cover_disjoint")
                                else:
                                    continue
                                
                            step.resetFilter()
                            step.refSelectFilter(linshi_layer+"_imp_line_cover", mode="touch")
                            if step.featureSelected():
                                step.removeLayer(worklayer+"_check_imp_line_cover_touch")
                                step.copySel(worklayer+"_check_imp_line_cover_touch")
                            
                            if num == 0:
                                step.refSelectFilter(linshi_layer+"_imp_line_cover", mode="disjoint")
                                if step.featureSelected():
                                    step.removeLayer(worklayer+"_check_imp_line_cover_disjoint")
                                    step.copySel(worklayer+"_check_imp_line_cover_disjoint")
                                
                            # count = abs(int(linshi_layer[1:]) - int(worklayer[1:]))
                            
                            if step.isLayer(worklayer+"_check_imp_line_cover_touch") and linshi_layer in ref_layers:
                                step.clearAll()
                                step.affect(worklayer+"_check_imp_line_cover_touch")                            
                                step.refSelectFilter(linshi_layer+"_imp_line_cover", mode="multi_cover")
                                step.COM("sel_reverse")
                                if step.featureSelected():                                    
                                    step.clearAll()
                                    step.affect(linshi_layer)
                                    step.filter_set(polarity='negative')
                                    step.refSelectFilter(worklayer+"_check_imp_line_cover_touch")
                                    if step.featureSelected():
                                        log = u"检测到{2}  {0} 参考{1}层 阻抗线单边加大10mil未全部被铜皮屏蔽，请检查标识位置是否异常！".format(worklayer, linshi_layer, step.name)
                                        arraylist_log.append(log)                    
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, linshi_layer,
                                                                                             worklayer, log, dic_zu_layer, add_note_layer=linshi_layer)
                                    else:
                                        if delta_v4:
                                            # delta按实际大小来检测
                                            step.clearAll()
                                        ...
                                        if step.featureSelected():                                            
                                            log = u"检测到{2}  {0} 参考{1}层 阻抗线单边加大10mil未全部被铜皮屏蔽，请检查标识位置是否异常！".format(worklayer, linshi_layer, step.name)
                                            arraylist_log.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, worklayer+"_check_imp_line_cover_touch",
                                                                                                 linshi_layer, log, dic_zu_layer, add_note_layer=worklayer)                                        
                                        

                    #检测隔层是否套开 逻辑还有问题 需跟cam协商处理
                    if "-lyh" in job.name and False:
                        for not_touch_layer in dic_not_touch_feature_layer[worklayer]:
                            if step.isLayer(not_touch_layer):
                                step.copyLayer(job.name, step.name, not_touch_layer, not_touch_layer+"_imp_line_cover")
                                step.clearAll()
                                step.affect(not_touch_layer+"_imp_line_cover")
                                step.contourize()
                                
                                step.clearAll()
                                step.affect(worklayer+"_check_imp_line_cover")
                                step.resetFilter()                        
                                step.refSelectFilter(not_touch_layer+"_imp_line_cover", mode="touch")
                                if step.featureSelected():
                                    if not_touch_layer in solderMaskLayers:
                                        log = u"检测到{2}  {0} 参考{1}层 阻抗线防焊有开窗，请检查标识位置是否异常！".format(worklayer, not_touch_layer, step.name)
                                    else:
                                        log = u"检测到{2}  {0} 参考{1}层 阻抗线隔层未套开，请检查标识位置是否异常！".format(worklayer, not_touch_layer, step.name)
                                    arraylist_log.append(log)                    
                                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, worklayer+"_check_imp_line_cover",
                                                                                         not_touch_layer, log, dic_zu_layer, add_note_layer=worklayer)
                                
                    #检测阻抗线是否接触其他物件
                    if "-lyh" in job.name:
                        
                        step.clearAll()
                        step.copyLayer(job.name, step.name, worklayer, worklayer+"imp_line_touch_feature")
                        step.affect(worklayer+"imp_line_touch_feature")
                        step.resetFilter()
                        step.filter_set(feat_types='line;arc', polarity='positive')
                        step.selectAll()
                        step.resetFilter()
                        ...
                        polarity=positive\;negative,include_syms=,exclude_syms=")
                        if step.featureSelected():
                            log = u"检测到{2}  {0} 阻抗线有接触到其他text物件，请检查标识位置是否异常！".format(worklayer, ref_layer, step.name)
                            arraylist_log.append(log)                    
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, worklayer+"imp_line_touch_feature",
                                                                                 worklayer, log, dic_zu_layer, add_note_layer=worklayer)
                            
                        if step.isLayer(worklayer+"_check_imp_line_touch_pad"):
                            step.copyLayer(job.name, step.name, worklayer+"_check_imp_line_touch_pad",
                                           worklayer+"_check_imp_line_cover", mode='append')
                            step.clearAll()
                            step.affect(worklayer+"_check_imp_line_cover")
                            step.resetFilter()
                            step.filter_set(feat_types='pad', polarity='positive')
                            ...
                            layer_cmd = gClasses.Layer(step, worklayer+"_check_imp_line_cover")
                            feat_out = layer_cmd.featout_dic_Index(units="mm", options="feat_index")["pads"]
                            for obj in feat_out:
                                step.selectNone()
                                step.selectFeatureIndex(worklayer+"_check_imp_line_cover", obj.feat_index)
                                if step.featureSelected():
                                    step.resetFilter()
                                    step.COM("sel_ref_feat,layers=,use=select,mode=touch,\
                                    pads_as=shape,f_types=line\;pad\;surface\;arc\;text,\
                                    ...
                                    if step.featureSelected() > 1:
                                        step.selectNone()
                                        step.selectFeatureIndex(worklayer+"_check_imp_line_cover", obj.feat_index)
                                        log = u"检测到{2}  {0} 阻抗线有接触到其他pad物件，请检查标识位置是否异常！".format(worklayer, ref_layer, step.name)
                                        arraylist_log.append(log)                    
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, worklayer+"_check_imp_line_cover",
                                                                                             worklayer, log, dic_zu_layer, add_note_layer=worklayer)                                    
                        
                        while True:                        
                            layer_cmd = gClasses.Layer(step, worklayer+"_check_imp_line_cover_bak")
                            feat_out = layer_cmd.featCurrent_LayerOut(units="mm")["lines"]
                            feat_out += layer_cmd.featCurrent_LayerOut(units="mm")["arcs"]
                            if "icg" in step.name:
                                feat_out = [obj for obj in feat_out if "jx_line" not in obj.attrs.values()]
                            
                            step.clearAll()
                            step.affect(worklayer+"_check_imp_line_cover_bak")
                            if feat_out:
                                for obj in feat_out:                                
                                    x, y = obj.xe, obj.ye
                                    step.COM("sel_polyline_feat,operation=select,x={0},y={1},tol=6.7946850394".format(x, y))
                                    if step.featureSelected():
                                        step.resetFilter()
                                        step.filter_set(feat_types='line;arc', polarity='positive')
                                        step.COM("sel_ref_feat,layers=,use=select,mode=touch,\
                                        ...
                                        polarity=positive\;negative,include_syms=,exclude_syms=")
                                        if step.featureSelected():
                                            log = u"检测到{2}  {0} 阻抗线有接触到其他线或有重复的线，请检查标识位置是否异常！".format(worklayer, ref_layer, step.name)
                                            arraylist_log.append(log)                    
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, worklayer+"_check_imp_line_cover_bak",
                                                                                                 worklayer, log, dic_zu_layer, add_note_layer=worklayer)
        ...
        if is_check:
            if "-lyh" not in job.name:                
                for layer in job.matrix.getInfo()["gROWname"]:
                    if "imp_line_cover" in layer or \
                       "imp_line_touch_pad" in layer or \
                       "imp_line_touch_feature" in layer or \
                       "_check_imp_line_cover_bak" in layer:
                        step.removeLayer(layer)
    ...
    def create_mask_layer_condition(self, step, is_flip):
        ...
        # 不存在osp层 都要把防焊临时层建好 后面要处理
        for layer in ["sgt-c", "sgt-s"]:
            layer_cmd = gClasses.Layer(step, layer)
            if not (step.isLayer(layer) and getattr(layer_cmd.cell, "context", None) == "board"):
                if step.name == "panel":  
                    ...
                    step.resetFilter()
                    step.filter_set(feat_types='pad;line;arc;surface')
                    step.refSelectFilter(dic_mask_layer[dic_zu[layer]], mode="cover")
                    if step.featureSelected():
                        step.selectDelete()      
    ...
    def re_calc_expose_area_auto(self, sig_layer):
        ...
        for stepname in list(set(get_pnl_steps)) + ["panel"]:
            ...
            if user not in ["70928", "50313", "68233", "72616", "72451", "69615", "system"]:
                self.create_mask_layer_condition(step, is_flip=flip_steps)
        ...
        if "l1" == sig_layer.split("-")[0]:
            ...
            if step.isLayer(mask_layer+"_calc_expose_tmp"):
                ...
                step.selectAll()
                if step.featureSelected():
                    step.copyToLayer(mask_layer, invert="yes")
            ...
            if face_tech and (u"化金" in face_tech.decode("utf8") or \
                ...
                if not expose_area_info:
                    ...
                    for info in expose_area_info:
                        if (info["TOP_AREA"] is  None and info["BOT_AREA"] is None) or \
                           (info["TOP_AREA"] == 0 and info["BOT_AREA"]  == 0):
                            update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                                         u"forbid_or_information='{0}'".format(u"拦截(不放行)"))                             
                            return u"检查到此化金板未上传金面积到ERP，请重新运行金面积计算程序！", None
                        ...
                        for key, value in info.iteritems():
                            if key in ["TOP_AREA", "BOT_AREA"]:
                                if info[key] is not None :
                                    ...
                                    if abs(float(info[key]) - dic_zu[key]) > 300:
    ...
    def check_only_skip_via_laser_target_is_right(self, auto_check=None):
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:            
            ...
            # job = gClasses.Job(jobname)
            if stepname not in matrixinfo["gCOLstep_name"]:
                stepname = [name for name in matrixinfo["gCOLstep_name"] if re.match("^edit", name)]
            
                if not stepname:
                    return "success无edit", None
                stepname = stepname[0]
    ...
    def check_skip_via_center_layer_is_right(self):
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            for layer in job.matrix.getInfo()["gROWname"]:                
                if "_run_checklist_tmp" in layer:
                    step.removeLayer(layer)
    ...
    def check_skipvia_isolatepad_info(self, step, drill_layer, start_index, end_index, flag):
        ...
        for index in range(start_index+1*flag, end_index, flag):
            ...
            sig_layer = "l{0}".format(index)
            if not step.isLayer(sig_layer+"_run_checklist_tmp"):                
                sig_layer = self.create_linshi_layer_for_checklist(sig_layer)
            else:
                sig_layer = sig_layer+"_run_checklist_tmp"
            ...
            #if "_run_checklist_tmp" in sig_layer and "-lyh" in job.name:
                #step.removeLayer(sig_layer)
    ...
    def check_nv_close_hole_is_resize(self):
        """检测NV密集孔是否按最新分刀 跳刀"""
        ...
        WHERE job = '{0}'
        order by id desc"""
        job_process_info = conn.SELECT_DIC(dbc_p, sql.format(job.name.split("-")[0]))
        if not job_process_info:
            return u"success工程任务计划内无此型号，无需检测。", None
            
        ##已审核完的不检测 
        if job_process_info and job_process_info[0].get("ocheckfinish_time", "2025-12-30 00:00") < '2025-01-13 00:00':
            update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                         u"forbid_or_information='{0}'".format(u"提示")) 
            #return u"success旧资料时间{0}，无需检测。".format(job_process_info[0].get("ocheckfinish_time", "")), None        
        
        setsteps = get_panelset_sr_step(job.name, "panel", is_group_by=True, is_all=True)
        arraylist_log = []
        dic_zu_layer = {}
        for stepname in list(set(setsteps)):
            if stepname == "set":
            ...
            for drill_layer in laser_drill_layers:
                ...
                step.affect(drill_layer)
                step.resetFilter()
                step.setAttrFilter(".string,text=nv_close_hole_new")
                step.selectAll()
                if not step.featureSelected():
                    check_layers.append(drill_layer)
            ...
            if check_layers:
                ...
                step.COM("chklist_show,chklist=hdi-drc,nact=1,pinned=no,pinned_enabled=yes")
                
                step.COM("""chklist_cupd,chklist=hdi-drc,nact=1,params=((pp_drill_layer=.affected)
                (pp_rout_distance=5080)(pp_tests=Hole Size\;Hole Separation\;Missing Holes\;Extra Holes\;NPTH to Rout)
                (pp_extra_hole_type=Pth\;Via)(pp_use_compensated_rout=Skeleton)),mode=regular""")
                
                step.COM("chklist_run,chklist=hdi-drc,nact=1,area=profile")
                step.COM("chklist_pclear")
                step.COM("chklist_pcopy,chklist=hdi-drc,nact=1")
                ...
                # find_close_hole_info = []
                # find_close_hole_layers = []
                for worklayer in check_layers:
                    for layer in matrixInfo["gROWname"]:
                        if "ms_1" in layer and worklayer in layer:
                            ...
                            step.affect(layer)
                            step.resetFilter()
                            step.COM("filter_atr_set,filter_name=popup,condition=yes,attribute=.string,text=closeh")                            
                            step.COM("filter_set,filter_name=popup,update_popup=yes,slot=line,slot_by=length,min_len=0,max_len=0.301")            
                            step.selectAll()
                            if step.featureSelected():
                                step.copySel(worklayer+"_find_close_tmp+++")
                                step.clearAll()
                                step.affect(worklayer+"_find_close_tmp+++")
                                step.COM("sel_extend_slots,mode=ext_to,size=301,from=center")
                                step.copyLayer(job.name, step.name, worklayer, worklayer+"_hole_tmp")
                                step.clearAll()
                                step.affect(worklayer+"_hole_tmp")
                                step.changeSymbol("r1")
                                step.resetFilter()
                                step.refSelectFilter(worklayer+"_find_close_tmp+++")
                                #if worklayer == "s6-8":
                                    #step.PAUSE(worklayer)                                
                                if step.featureSelected():
                                    step.copySel(worklayer+"_find_close_hole+++")
                                    step.clearAll()
                                    step.affect(worklayer)
                                    step.resetFilter()
                                    step.refSelectFilter(worklayer+"_find_close_hole+++")
                                    if step.featureSelected():
                                        step.copySel(worklayer+"_find_close_hole+++")
                                    
                                    step.clearAll()
                                    # find_close_hole_layers.append(worklayer)
                                    log = u"{0} {1}层发现孔中心间距有小于等于0.3mm镭射孔未进行新密集孔分刀，请参考挑选出来的近孔层{2}进行比对！".format(
                                        stepname, worklayer, worklayer+"_find_close_hole+++"
                                    )
                                    arraylist_log.append(log)                                    
                                    dic_zu_layer=self.get_view_dic_layers(job.name, stepname, step,
                                                                          worklayer="view_layer_"+worklayer, 
                                                                          dic_layer_list={log:[worklayer+"_find_close_hole+++"], },
                                                                          dic_zu_layer=dic_zu_layer)
                                    
                                step.removeLayer(worklayer+"_find_close_tmp+++")
                                step.removeLayer(worklayer+"_hole_tmp")
        ...
        if arraylist_log:
            # self.view_note_detail(dic_zu_layer)
            if job.name.upper() in ["BA8622O6784T2"]:
                update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                             u"forbid_or_information='{0}'".format(u"拦截")) 
            return arraylist_log, dic_zu_layer
    ...
    def check_panel_info(self):
        ...
        if array_data_info and "panel" in matrixinfo["gCOLstep_name"]:
            ...
            for worklayer in signalLayers:
                ...
                step.affect(worklayer)
                step.resetFilter()
                step.selectSymbol("sh-con2;sh-con", 1, 1)
                if step.featureSelected():                    
                    layer_cmd = gClasses.Layer(step, worklayer)
                    feat_out = layer_cmd.featCurrent_LayerOut(units="mm")["pads"]
                    jx_symbol_obj = [obj for obj in feat_out if obj.symbol in ("sh-con2", "sh-con")]            
                    ...
                    for i, name in enumerate(gREPEATstep):
                        ...
                        if gREPEATxmin[i] > min_jx_x and gREPEATymin[i] > min_jx_y and \
                           gREPEATxmax[i] < max_jx_x and gREPEATymax[i] < max_jx_y and \
                           ("set" in name or "edit" in name or "icg" in name or "loss" in name):
                            set_rect_area.append(
                                [gREPEATxmin[i], gREPEATymin[i], gREPEATxmax[i], gREPEATymax[i]])
            ...
            if all_sr_x and all_sr_y:                
                ...
                return u"获取有效单元区域失败，请检查线路层的角线是否被打撒或转铜皮！！", None                
    ...
    def check_gold_finger_connection_info(self):
        ...
        if not has_jsz_process and "-lyh" not in job.name:
        ...
        if job.name.upper().split("-")[0] in ["H02720GBB55A1"]:
            outsignalLayers = ["l6", "l20"]
        ...
        # misc掉背钻的board属性 20250305 by lyh
        for i, layer in enumerate(job.matrix.getInfo()["gROWname"]):
            if job.matrix.getInfo()["gROWcontext"][i] == "board" and \
               job.matrix.getInfo()["gROWlayer_type"][i] == "drill":
                if "bd" in layer:
                    job.COM("matrix_layer_context,job={0},matrix=matrix,layer={1},context=misc".format(job.name, layer))        
        ...
        for layer in outsignalLayers:
            ...
            if job.name.upper().split("-")[0] in ["H02720GBB55A1", "H02720GBB55A1_FLT"]:
                if layer == "l6":         
        ...
        if jobname.upper() == "HA7712PH457F2":
            # 此型号会卡死 这里将外层多余的元素都转铜皮
            edit_step.clearAll()
            for layer in outsignalLayers:                
                edit_step.affect(layer)
        ...
        for i, layer in enumerate(matrixinfo["gROWname"]):
            if layer not in check_layers and \
               matrixinfo["gROWcontext"][i] != "board":
                step.removeLayer(layer)
    ...
    def check_gold_finger_elic_line_info(self):
        """检测金手指引线开窗区域是否异常问题 20241024 by lyh"""
        arraylist_log = []
        dic_zu_layer = {}        
        if "panel" in matrixinfo["gCOLstep_name"]:
            data_info = get_inplan_all_flow(job.name.upper(), True)       
            suanshi_process = [dic_info for dic_info in data_info
                            if u"显影/蚀刻/去膜" in dic_info["PROCESS_DESCRIPTION"].decode("utf8") and 
                            u"金手指去镀金导线" in dic_info["WORK_CENTER_CODE"].decode("utf8") ]
            jianshi_process = [dic_info for dic_info in data_info
                            if u"碱性蚀刻" in dic_info["PROCESS_DESCRIPTION"].decode("utf8") and
                            u"金手指去导线" in dic_info["WORK_CENTER_CODE"].decode("utf8")]
            #if "-lyh" in job.name:
                #job.PAUSE(str([jianshi_process]))
            ganmo_process = [dic_info for dic_info in data_info
                            if u"选化干膜" in dic_info["WORK_CENTER_CODE"].decode("utf8") or \
                            u"选镀干膜" in dic_info["WORK_CENTER_CODE"].decode("utf8") ]
            shimo_process = [dic_info for dic_info in data_info
            ...
            jsz_order_nums = [x["ORDER_NUM"] for x in has_jsz_process]
            jsz_travel_nums = [x["TRAVELER_ORDERING_INDEX"] for x in has_jsz_process]
            ksz_process = [dic_info for dic_info in data_info
                           if u"控深钻" in dic_info["WORK_CENTER_CODE"].decode("utf8")
                           and dic_info["ORDER_NUM"] == max(jsz_order_nums)
                           and dic_info > max(jsz_travel_nums)]
            ...
            step.COM("units,type=mm")
            for layer in matrixinfo["gROWname"]:
                if "_flt" in layer or "_finger" in layer or "_auto" in layer or "check_4mil" in layer:
                    step.removeLayer(layer)

            step.removeLayer("surface_fill_outline")
            step.removeLayer("surface_fill")
            dic_xlym = {}
            for worklayer in outsignalLayers:
                if worklayer == 'l1':
                    xlym = ''
                    if 'xlym-c' in boardLayers:
                        xlym = 'xlym-c'
                    if 'gold-c' in boardLayers:
                        xlym = 'gold-c'
                    dic_xlym[worklayer] = xlym
                else:
                    xlym = ''
                    if 'xlym-s' in boardLayers:
                        xlym = 'xlym-s'
                    if 'gold-s' in boardLayers:
                        xlym = 'gold-s'
                    dic_xlym[worklayer] = xlym
                    
            for key, value in dic_xlym.iteritems():
                if not value:
                    dic_xlym = self.select_xlym_layer_ui(boardLayers, outsignalLayers)
                    break
            
            get_sr_area_flatten("surface_fill", include_sr_step="edit")
            step.clearAll()
            step.affect("surface_fill")
            step.selectFeatureIndex("surface_fill", "1")
            if step.featureSelected():
                step.COM("sel_reverse")
                if step.featureSelected():
                    step.selectDelete()
                
            step.copyLayer(job.name, step.name, "surface_fill", "surface_fill_outline")
            step.clearAll()
            step.affect("surface_fill_outline")
            step.COM("sel_surf2outline,width=1")
            
            for sig_layer, xlym_layer in dic_xlym.iteritems():
                if not xlym_layer:
                    continue
                if sig_layer == "l1":
                    mask_layer = "m1"
                ...
                step.clearAll()
                step.flatten_layer(sig_layer, sig_layer+"_flt")
                step.flatten_layer(xlym_layer, xlym_layer+"_flt")
                step.flatten_layer(mask_layer, mask_layer+"_flt")
                drl = "drl"
                for i, drl_layer in enumerate(matrixinfo["gROWname"]):
                    # if step.isLayer(drl):
                    if matrixinfo["gROWcontext"][i] == "board" and \
                       matrixinfo["gROWlayer_type"][i] == "drill":
                        if not (drl_layer.startswith("s") or drl_layer.startswith("b") or drl_layer.startswith("m")):
                            step.flatten_layer(drl_layer, drl_layer+"_flt_tmp")
                            step.clearAll()
                            step.affect(drl_layer+"_flt_tmp")
                            step.copySel(drl+"_flt")
                    
                if step.isLayer(etch_layer):
                    step.flatten_layer(etch_layer, etch_layer+"_flt")
                    step.affect(etch_layer+"_flt")
                
                step.affect(sig_layer+"_flt")
                step.affect(xlym_layer+"_flt")
                step.affect(mask_layer+"_flt")                    
                step.resetFilter()
                step.refSelectFilter("surface_fill", mode="disjoint")
                if step.featureSelected():
                    step.selectDelete()                    
                ...
                step.clearAll()
                if step.isLayer("vcut"):
                    step.flatten_layer("vcut", "vcut"+"_flt")
                    step.affect("vcut_flt")
                    step.changeSymbol("r500")
                
                step.clearAll()
                step.affect("surface_fill_outline")
                step.changeSymbol("r1000")
                ...
                step.resetFilter()
                step.filter_set(feat_types="line;arc", polarity='positive')
                step.refSelectFilter("surface_fill_outline", mode="cover")
                if step.featureSelected():
                    step.selectDelete()
                # step.PAUSE("ddd")
                step.contourize(accuracy=0)
                ...
                if step.isLayer("vcut_flt"):
                    step.clearAll()
                    step.affect(mask_layer+"_flt")
                    step.COM("sel_resize,size=-500")
                    step.COM("sel_resize,size=500")
                
                if step.isLayer(drl+"_flt"):
                    step.clearAll()
                    step.affect(drl+"_flt")
                    step.resetFilter()
                ...
                step.selectAll()
                if step.featureSelected():
                    step.copySel(sig_layer+"_finger")
                    step.clearAll()
                    step.affect(mask_layer+"_flt")
                    step.resetFilter()
                    step.refSelectFilter(sig_layer+"_finger")
                    if step.featureSelected():
                        step.copySel(mask_layer+"_finger")
                    ...
                    step.refSelectFilter(sig_layer+"_finger")
                    if step.featureSelected():
                        step.copySel(xlym_layer+"_finger")                    
                
                    step.clearAll()
                    step.affect(sig_layer+"_flt")
                    step.resetFilter()
                    step.filter_set(feat_types='line', polarity='positive')
                    step.refSelectFilter("surface_fill_outline")
                    if step.featureSelected():
                        step.moveSel(sig_layer+"_finger_line")
                    
                    step.copyLayer(job.name, step.name, sig_layer+"_flt", sig_layer+"_flt_contourize")
                    step.clearAll()
                    step.affect(sig_layer+"_flt_contourize")
                    step.contourize(accuracy=0)
                    
                else:
                    log = u"检测到此板为金手指板，但{0}层内未发现定义属性.string=gf 的金手指pad".format(sig_layer)
                    arraylist_log.append(log)
                    continue
                        
                if step.isLayer(sig_layer+"_finger_line"):
                    step.clearAll()
                    step.affect(sig_layer+"_finger_line")
                    step.copyLayer(job.name, step.name, sig_layer+"_finger_line", sig_layer+"_finger_line_orig")
                    step.resetFilter()
                    if jianshi_process:
                        # step.COM("sel_resize,size=-1")
                        process_type = u"碱蚀"
                        ...
                        # step.COM("sel_resize,size=-{0}".format(3.05*25.4*2))
                        process_type = u"酸蚀"
                        re_size = 3
                    ...
                    step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                             "area=reference,area_type=rectangle,inout=inside,"
                             "contour_cut=yes,margin={2},ref_layer={1},"
                             "feat_types=line\;pad\;surface\;arc\;text,".format(sig_layer+"_finger_line",
                                                                                sig_layer+"_flt_contourize",
                                                                                re_size*25.4))                        
                        
                    step.refSelectFilter(xlym_layer+"_finger")
                    if step.featureSelected():
                        step.copyLayer(job.name, step.name, sig_layer+"_finger_line", sig_layer+"_finger_line_bak")
                        step.clearAll()
                        step.affect(sig_layer+"_finger_line_bak")
                        step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                 "area=reference,area_type=rectangle,inout=inside,"
                                 "contour_cut=yes,margin=0,ref_layer={1},"
                                 "feat_types=line\;pad\;surface\;arc\;text,".format(sig_layer+"_finger_line_bak",
                                                                                    xlym_layer+"_finger"))
                        step.resetFilter()
                        step.selectAll()
                        if step.featureSelected():                            
                            ...
                            step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                     ...
                                     "contour_cut=yes,margin=0,ref_layer={1},"
                                     "feat_types=line\;pad\;surface\;arc\;text,".format(sig_layer+"_finger_line",
                                                                                        sig_layer+"_finger_line_bak"))
                        else:
                            step.clearAll()
                            step.affect(sig_layer+"_finger_line")
                            
                        step.COM("sel_resize,size=-{0}".format(0.2*25.4))
                        step.resetFilter()
                        step.selectAll()
                        if step.featureSelected():
                            
                            log = u"检测到此型号为{2}流程，{0}层金手指引线跟手指镀金区域{1}加大{3}mil有接触，请检查标记位置处是否异常，请检查！".format(
                                sig_layer, xlym_layer, process_type, re_size
                            )
                            arraylist_log.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, sig_layer+"_finger_line",
                                                                                 [sig_layer, xlym_layer],log, dic_zu_layer,
                                                                                 add_note_layer=sig_layer)
                    #蚀刻引线层检测
                    if step.isLayer(etch_layer):
                        step.clearAll()
                        ...
                        step.clearAll()
                        step.affect(sig_layer+"_finger_line_orig")
                        step.COM("sel_extend_slots,mode=ext_by,size=500,from=center")
                        step.resetFilter()
                        if jianshi_process:
                            process_type = u"碱蚀"
                            re_size = 0
                        ...
                        step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                 ...
                                 "break_to_islands=yes".format(sig_layer+"_finger_line_orig",
                                                               sig_layer+"_flt_contourize",
                                                               0))                            
                        step.copyLayer(job.name, step.name, sig_layer+"_finger_line_orig", sig_layer+"_finger_line_orig_clip")
                        
                        step.clearAll()
                        step.affect(sig_layer+"_finger_line_orig_clip")                        
                        step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                 "area=reference,area_type=rectangle,inout=inside,"
                                 "contour_cut=yes,margin={2},ref_layer={1},"
                                 "feat_types=line\;pad\;surface\;arc\;text,"
                                 "break_to_islands=yes".format(sig_layer+"_finger_line_orig_clip",
                                                               sig_layer+"_flt_contourize",
                                                               re_size*25.4-2))
                        ...
                        step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                 ...
                                 "feat_types=line\;pad\;surface\;arc\;text,"
                                 "break_to_islands=yes".format(sig_layer+"_finger_line_orig",
                                                               sig_layer+"_finger_line_orig_clip",
                                                               0))
                        ...
                        step.selectNone()
                        step.resetFilter()
                        step.refSelectFilter(etch_layer+"_flt", mode="cover")
                        step.COM("sel_reverse")
                        #if step.featureSelected():
                            #step.selectDelete()
                        ...
                        #step.selectAll()
                        if step.featureSelected():
                            log = u"检测到此型号为{2}流程，{0}层金手指引线被smd切完加大{3}mil后 不能被蚀刻引线层{1}包含，请检查标记位置处是否异常，请检查！".format(
                                sig_layer, etch_layer, process_type, re_size
                            )
                            arraylist_log.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, sig_layer+"_finger_line_orig",
                                                                                 [sig_layer, etch_layer, sig_layer+"check_4mil_etch_line"],log, dic_zu_layer,
                                                                                 add_note_layer=sig_layer)
                            step.copySel(sig_layer+"check_4mil_etch_line")
                else:
                    # 检测是否内拉手指引线
                    if ksz_process:
                        step.clearAll()
                    ...
                    if step.isLayer(sig_layer+"_finger_line"):
                        ...
                        step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                 ...
                                 "contour_cut=yes,margin=0,ref_layer={1},"
                                 "feat_types=line\;pad\;surface\;arc\;text,"
                                 "break_to_islands=yes".format(sig_layer+"_finger_line",
                                                               sig_layer+"_flt"))
                        if sig_layer == "l1":
                            ksz = "bdc"
                        else:
                        ...
                        ksz_layers = []
                        for drill_layer in job.matrix.getInfo()["gROWname"]:
                            if ksz in drill_layer and ".inn" not in drill_layer:
                                ksz_layers.append(drill_layer)
                                step.flatten_layer(drill_layer, drill_layer+"_flt")
                                step.refSelectFilter(drill_layer+"_flt", mode="cover")
                                if step.featureSelected():
                                    step.selectDelete()
                        ...
                        step.selectAll()
                        if step.featureSelected():
                            log = u"检测到此型号为内拉手指引线，有控深钻引线，但{0}层内有部分引线未被控深孔钻掉，请检查标记位置是否异常".format(
                                sig_layer
                            )
                            arraylist_log.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, sig_layer+"_finger_line",
                                                                                 [sig_layer]+ksz_layers,log, dic_zu_layer,
                                                                                 add_note_layer=sig_layer)
                    else:
                        log = u"检测到此型号镀金手指板，但{0}层内未检测到金手指引线，请检查是否异常".format(
                            sig_layer
                        )
                        arraylist_log.append(log)
                        
                if step.isLayer(mask_layer+"_finger") and step.isLayer(xlym_layer+"_finger"):
                    step.clearAll()
                    step.affect(mask_layer+"_finger")
                    step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                             "area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                             "ref_layer={1},feat_types=line\;pad\;surface\;arc\;text,"
                             "break_to_islands=yes".format(
                                 mask_layer+"_finger",
                                 "surface_fill_outline", 1
                             ))                       
                    step.resetFilter()
                    step.refSelectFilter("surface_fill", mode="disjoint")
                    if step.featureSelected():
                    ...
                    #阻焊加大4.9mil 变成outline 取靠内线看是否被cover
                    step.copyLayer(job.name, step.name, mask_layer+"_finger", mask_layer+"_finger_resize")
                    step.clearAll()
                    step.affect(mask_layer+"_finger_resize")
                    step.COM("sel_resize,size={0}".format(4.9*25.4*2))
                    step.copyLayer(job.name, step.name, "surface_fill", "surface_fill_bak")
                    step.clearAll()
                    step.affect("surface_fill_bak")
                    step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                             "area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                             "ref_layer={1},feat_types=line\;pad\;surface\;arc\;text,"
                             "break_to_islands=yes".format(
                                 "surface_fill_bak",
                                 mask_layer+"_finger_resize", 0
                             ))
                    step.resetFilter()
                    step.selectAll()
                    layer_cmd = gClasses.Layer(step, "surface_fill_bak")
                    surface_indexes = layer_cmd.featSelIndex()
                    dic_length_surface = {}
                    for index in surface_indexes:
                        step.selectNone()
                        step.selectFeatureIndex("surface_fill_bak", index)
                        step.COM("sel_surf2outline,width=1")
                        feat_out_outline = layer_cmd.featCurrent_LayerOut(units="mm")["lines"]
                        feat_out_outline += layer_cmd.featCurrent_LayerOut(units="mm")["arcs"]
                        dic_length_surface[index] = sum([obj.len for obj in feat_out_outline])
                        step.COM("undo,depth=1")
                    
                    for index, value in dic_length_surface.iteritems():
                        if value == max(dic_length_surface.values()):
                            step.selectNone()
                            step.selectFeatureIndex("surface_fill_bak", index)
                            step.COM("sel_reverse")
                            if step.featureSelected():
                                step.selectDelete()
                    #step.refSelectFilter(mask_layer+"_finger")
                    #if step.featureSelected():
                        #step.selectDelete()
                    step.COM("sel_resize,size=10")
                    step.copyLayer(job.name, step.name, mask_layer+"_finger_resize", mask_layer+"_auto_resize")
                    ...
                    step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                             ...
                             "ref_layer={1},feat_types=line\;pad\;surface\;arc\;text,"
                             "break_to_islands=yes".format(
                                 mask_layer+"_finger_resize",
                                 xlym_layer+"_finger", 0
                             ))
                    if step.isLayer(drl+"_flt"):
                        step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                                 "area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                    ...
                    step.resetFilter()
                    step.refSelectFilter("surface_fill_bak", mode="disjoint")
                    if step.featureSelected():
                        step.selectDelete()
                        
                    step.selectAll()
                    if step.featureSelected():
                        log = u"检测到{0}手指镀金区域靠内距阻焊开窗{1}距离不足5mil，<br>{2}为阻焊手指位置开窗整体加大10mil效果，请参考比对，请检查标记位置是否异常！".format(
                            xlym_layer, mask_layer, mask_layer+"_auto_resize"
                        )
                        arraylist_log.append(log)                        
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, mask_layer+"_finger_resize",
                                                                             [xlym_layer, mask_layer, mask_layer+"_auto_resize"],
                                                                             log, dic_zu_layer,
                                                                             add_note_layer=xlym_layer)                        
                    step.clearAll()
                    step.resetFilter()
                    step.affect(sig_layer+"_flt")
                    # step.setAttrFilter(".smd")
                    step.refSelectFilter(mask_layer+"_finger")
                    if step.featureSelected():
                        step.removeLayer(sig_layer+"_finger_smd")
                        step.copySel(sig_layer+"_finger_smd")
                        
                        #有负片削smd的情况
                        step.resetFilter()
                        step.filter_set(polarity='negative')
                        step.refSelectFilter(sig_layer+"_finger_smd")
                        if step.featureSelected():                            
                            step.copyLayer(job.name, step.name, sig_layer+"_finger_smd", sig_layer+"_finger_smd_bak")
                            step.clearAll()
                            step.affect(sig_layer+"_finger_smd_bak")
                            step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                                     "area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                                     "ref_layer={1},feat_types=line\;pad\;surface\;arc\;text,"
                                     "break_to_islands=yes".format(
                                         sig_layer+"_finger_smd_bak",
                                         sig_layer+"_flt_contourize", 0
                                     ))
                            step.resetFilter()
                            step.COM("sel_resize,size=-10")
                            step.contourize(accuracy=0)
                            ...
                            if step.featureSelected():                                
                                step.clearAll()
                                step.affect(sig_layer+"_finger_smd")
                                step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                                         "area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                                         "ref_layer={1},feat_types=line\;pad\;surface\;arc\;text,"
                                         "break_to_islands=yes".format(
                                             sig_layer+"_finger_smd",
                                             sig_layer+"_finger_smd_bak", 0
                                         ))
                                
                        step.resetFilter()
                        step.clearAll()
                        step.affect("surface_fill_bak")
                        step.COM("sel_resize,size={0}".format(4.9*25.4*2-10))
                        ...
                        step.COM("sel_resize,size=-{0}".format(0.2*25.4))
                        step.resetFilter()
                        step.refSelectFilter(xlym_layer+"_flt", mode="cover")
                        if step.featureSelected():
                            step.selectDelete()
                        # step.COM("sel_reverse")
                        step.selectAll()
                        if step.featureSelected():
                            log = u"检测到{0}层手指开窗区域有smd 未被手指镀金区域{1}完全覆盖，会存在漏镀金的情况，请检查标记位置是否异常！".format(
                                sig_layer, xlym_layer
                            )
                            arraylist_log.append(log)                        
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, sig_layer+"_finger_smd",
                                                                                 [sig_layer, xlym_layer],log, dic_zu_layer,
                                                                                 add_note_layer=sig_layer)                              
                        
                    
                    #step.clearAll()
                    #step.affect(xlym_layer+"_finger")
                    #step.COM("clip_area_end,layers_mode=layer_name,layer={0},area=reference,"
                             #"area_type=rectangle,inout=inside,contour_cut=yes,margin={2},"
                             #"ref_layer={1},feat_types=line\;pad\;surface\;arc\;text,"
                             #"break_to_islands=yes".format(
                    ...
                    #step.COM("chklist_single,action=valor_analysis_signal,show=yes")
                    #step.COM("""chklist_cupd,chklist=valor_analysis_signal,nact=1,
                    #params=((pp_layer=.affected)(pp_spacing=500)(pp_r2c=254)
                    #(pp_d2c=254)(pp_sliver=101.6)(pp_min_pad_overlap=127)
                    #(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)
                    #(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular""")
                    ...
                    #step.COM("chklist_close,chklist=checklist_space,mode=hide")
                    #step.resetFilter()
                    #ms_1_layers = [x for x in job.matrix.getInfo()["gROWname"] if "ms_1" in x and xlym_layer+"_finger" in x]
                    ## mk_1_layers = [x for x in job.matrix.getInfo()["gROWname"] if "mk_1" in x]
                    #if not ms_1_layers:
                        #continue
                    
                    #step.clearAll()
                    #step.affect(ms_1_layers[0])
                    #step.resetFilter()
                    #step.setAttrFilter(".string,text=c2c")
                    #step.selectAll()
                    #step.resetFilter()
                    #step.setAttrFilter(".string,text=l2l")
                    #step.selectAll()
                    #if step.featureSelected():
                        #ms_line_lay_cmd = gClasses.Layer(step, ms_1_layers[0])
                        #ms_line_feat_out = ms_line_lay_cmd.featout_dic_Index(units="inch", options="select+feat_index")["lines"]
                        ## ms_line_len = [obj.len *1000 for obj in ms_line_feat_out]
                        #for obj in ms_line_feat_out:
                            #if obj.len *1000 + 1 /25.4 < 4.9:
                                #log = u"检测到{0}手指选化区域靠内距阻焊开窗{1}距离不足5mil，实际间距为:{2}mil,请检查标记位置是否异常！".format(
                                    #xlym_layer, mask_layer, round(obj.len *1000 + 1 /25.4, 2)
                                #)
                                #arraylist_log.append(log)
                                #step.selectNone()
                                #step.selectFeatureIndex(ms_1_layers[0], obj.feat_index)
                                
                                #dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, ms_1_layers[0],
                                                                                     #[xlym_layer, mask_layer],log, dic_zu_layer,
                                                                                     #add_note_layer=xlym_layer)
            if "-lyh" not in job.name:                
                for layer in job.matrix.getInfo()["gROWname"]:
                    if "_flt" in layer or "_finger" in layer:
                        step.removeLayer(layer)
                    
        else:
            return "success不存在panel，无需检测", None
    ...
    def select_xlym_layer_ui(self, boardlayers, outsignalLayers):
        """手动选择镀金区域层"""
        dialog = QtGui.QDialog()
        dialog.resize(200, 200)
        dialog.setObjectName("mainWindow")
        dialog.setWindowTitle(u"镀金区域层选择")        
        label1= QtGui.QLabel(u"TOP面:")
        toplayer_combox = QtGui.QComboBox()
        label2= QtGui.QLabel(u"BOT面:")
        botlayer_combox = QtGui.QComboBox()        
        
        for name in [""] + boardlayers:
            toplayer_combox.addItem(name)
            botlayer_combox.addItem(name)
        ...
        gridlayout.addWidget(toplayer_combox, 1, 1, 1, 1)
        
        gridlayout.addWidget(label2, 2, 0, 1, 1)
        gridlayout.addWidget(botlayer_combox, 2, 1, 1, 1)
        gridlayout.addWidget(start_btn, 20, 1, 1, 1)
        # gridlayout.addWidget(end_btn, 20, 2, 1, 1)
        
        dialog.setLayout(gridlayout)
        dialog.exec_()
        top_layer = str(toplayer_combox.currentText())
        bot_layer = str(botlayer_combox.currentText())
        dic_layer = {}
        for layer in outsignalLayers:
            if layer == "l1":
                dic_layer[layer] = top_layer
            else:
                dic_layer[layer] = bot_layer
        
        return dic_layer
    
    def get_trace_barcode_number_to_mysql(self):
        """上传追溯码数量到mysql数据库"""
        if "panel" in matrixinfo["gCOLstep_name"]:            
            setsteps = get_panelset_sr_step(jobname, "panel", is_group_by=False)         
            ...
            for stepname in list(set(setsteps)):
                step = gClasses.Step(job, stepname)
                step.open()
                panel_contain_step = [name for name in setsteps if name == stepname]
                dic_step_nums[stepname] = len(panel_contain_step)                
                for worklayer in checkLayers:                    
                    step.clearAll()
                    step.affect(worklayer)                    
                    layer_cmd = gClasses.Layer(step, worklayer)                    
                    ...
                    if worklayer.startswith("c1"):
                        mianci = "TOP"
                    else:
                        mianci = "BOT"
                    for obj in pad_obj:
                        if (re.match("barcode\d+x\d+|barcode\d+\.?\d+?x\d+\.?\d+?", obj.symbol) and \
                            getattr(obj, "id", None) is not None):
                            if dic_barcode_info.has_key(stepname):
                                dic_barcode_info[stepname][mianci].append((obj.symbol, "%.1f" % obj.x, "%.1f" % obj.y,
                                                                           getattr(obj, "id", None), worklayer))
                            else:
                                dic_barcode_info[stepname] = {"TOP": [],"BOT": [],}
                                dic_barcode_info[stepname][mianci].append((obj.symbol, "%.1f" % obj.x, "%.1f" % obj.y,
                                                                           getattr(obj, "id", None), worklayer))                              
       
            return dic_barcode_info, dic_step_nums
        
        return None
    
    def get_panel_edge_symbol_coordinate_info(self):
        """获取板边所有symbol的坐标信息"""
        step = gClasses.Step(job, "panel")
        step.open()
        
        all_drills = []
        for i, worklayer in enumerate(matrixinfo["gROWname"]):
            if ".ykj" in worklayer:
                continue
            if matrixinfo["gROWcontext"][i] == "board":
                if matrixinfo["gROWlayer_type"][i] == "drill":
                    if not re.match("s\d{1,2}-\d{1,2}", worklayer):
                        all_drills.append(worklayer)
                        continue
                    
            if matrixinfo["gROWlayer_type"][i] == "drill" and \
               re.match("^bd.*", worklayer):
                all_drills.append(worklayer)
        
        dic_coordinate_info = {}
        for worklayer in silkscreenLayers + solderMaskLayers + signalLayers + all_drills + ["pnl-fg"]:
            if step.isLayer(worklayer):
                if "-ls" in worklayer:
                    worklayer = worklayer.split("-ls")[0]
                    if not step.isLayer(worklayer):
                        continue
                ...
                if feat_out:
                    if worklayer == "pnl-fg":
                        symbol_info = [(obj.symbol, ("%.3f" % obj.xs, "%.3f" % obj.ys,),
                                        ("%.3f" % obj.xe, "%.3f" % obj.ye),
                                        obj.polarity, obj.angle, obj.attrs) for obj in feat_out]
                    else:
                        symbol_info = [(obj.symbol, "%.3f" % obj.x, "%.3f" % obj.y,
                                        obj.polarity, obj.rotation, obj.attrs) for obj in feat_out]
                    dic_coordinate_info[worklayer] = symbol_info
        
        return dic_coordinate_info                
    
    def check_nv_hole_lost_add_pad_in_inner_layers(self):
        """检测大于1mm的孔未在线路上加比孔小12mil的pad 20250116 by lyh"""
        if job.name[1:4] not in ["a86", "d10"]:
            return u"success非NV型号，无需检测", None
        ...
        if drl_drills:        
            ...
            if not all_steps:
                return u"success set或edit没有拼版,无需检测", None
            for stepname in all_steps:
                step = gClasses.Step(job, stepname)
                step.open()
                step.COM("units,type=mm")
                ...
                if step.isLayer("pad_tmp"):            
                    for sig_layer in innersignalLayers:
                        ...
                        step.clearAll()
                        step.affect("pad_tmp")
                        step.COM("sel_resize,size=-{0}".format(12*25.4+10))
                        step.refSelectFilter(sig_layer+"_check_pad", mode="cover")
                        step.COM("sel_reverse")
                        # step.PAUSE(sig_layer)
                        if step.featureSelected():
                            log = u"检测到{0} {1}层有大于1mm的孔未在线路上加比孔小12mil的pad，请检查标记位置是否异常！！".format(
                                stepname, sig_layer
                            )
                            arraylist_log.append(log)                            
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, "pad_tmp",
                                                                                 [sig_layer],log, dic_zu_layer,
                                                                                 add_note_layer=sig_layer)
                step.clearAll()
    ...
    def select_bga_smd_by_surface_around(self):
        ...
        for layer in matrixinfo["gROWname"]:            
            if ("_surface" in layer or "_bga" in layer or "_smd" in layer )\
               and "inner_panel_surface" not in layer:
                step.removeLayer(layer)
        ...
        for worklayer in outsignalLayers:
            ...
            if step.featureSelected():
                ...
                step.createLayer(worklayer+"_bga")
            ...
            if step.featureSelected():
                ...
                step.createLayer(worklayer+"_smd")
            
            step.clearAll()
            step.affect(worklayer+"_surface")            
            for attr in [".tear_drop", ".detch_comp"]:                
                step.resetFilter()
                step.setAttrFilter(attr)
                step.selectAll()
                if step.featureSelected():
                    step.selectDelete()
            ...
            step.copyLayer(job.name, step.name, worklayer+"_surface", worklayer+"_surface_resize")
            step.clearAll()
            step.affect(worklayer+"_surface_resize")
            step.COM("sel_clean_holes,max_size={0},clean_mode=x_or_y".format(400*25.4))
            
            step.COM("sel_resize,size={0}".format(44*25.4))
            step.COM("sel_resize,size=-{0},corner_ctl=yes".format(44*25.4))
            step.COM("sel_resize,size={0}".format(10*25.4))
            
            step.clearAll()
            step.affect(worklayer+"_bga")
            step.resetFilter()
            step.refSelectFilter(worklayer+"_surface_resize", mode="cover")
            if step.featureSelected():
                step.moveSel(worklayer+"_bga_around_by_sur")
            ...
            step.refSelectFilter(worklayer+"_surface_resize", mode="cover")
            if step.featureSelected():
                step.moveSel(worklayer+"_smd_around_by_sur")
    ...
    def get_week_picihao_banci_info(self, *check_types):
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:            
                ...
                for output_type in check_types:
                    ...
                    for worklayer in checkLayers:
                        ...
                        feat_out = layer_cmd.featout_dic_Index(
                            units="mm", options="feat_index+select")["text"]
                        
                        for obj in feat_out:
                            for wwyy in ["$$yy", "$$ww", "$$mm", "$$dd","$$year", "$$month", "$$day"]:
                                if wwyy in str(obj.text).replace("{", "").replace("}", "").lower():                                
                                    if dic_tgz_week_layers.has_key(worklayer):
                                        if (obj, step.name) not in dic_tgz_week_layers[worklayer]:                                        
                                            dic_tgz_week_layers[worklayer] += [(obj, step.name)]
                                            week_font.append(obj.font)
                                    else:
                                        dic_tgz_week_layers[worklayer] = [(obj, step.name)]
                                        week_font.append(obj.font)
                                        
                            for banci_pici in ["$$banci", "$$picihao", "$$xingqi"]:
                                if banci_pici in str(obj.text).replace("{", "").replace("}", "").lower():
                                    if banci_pici == "$$banci":                                    
                                        if dic_tgz_banci_layers.has_key(worklayer):
                                            if (obj, step.name) not in dic_tgz_banci_layers[worklayer]:                                            
                                                dic_tgz_banci_layers[worklayer] += [(obj, step.name)]
                                                week_font.append(obj.font)
                                        else:
                                            dic_tgz_banci_layers[worklayer] = [(obj, step.name)]
                                            week_font.append(obj.font)
                                    ...
                                    if banci_pici == "$$picihao":
                                        if dic_tgz_picihao_layers.has_key(worklayer):
                                            if (obj, step.name) not in dic_tgz_picihao_layers[worklayer]:                                            
                                                dic_tgz_picihao_layers[worklayer] += [(obj, step.name)]
                                                week_font.append(obj.font)
                                        else:
                                            dic_tgz_picihao_layers[worklayer] = [(obj, step.name)]
                                            week_font.append(obj.font)
                                            
                                    if banci_pici == "$$xingqi":
                                        if dic_tgz_xinqi_layers.has_key(worklayer):
                                            if (obj, step.name) not in dic_tgz_xinqi_layers[worklayer]:                                            
                                                dic_tgz_xinqi_layers[worklayer] += [(obj, step.name)]
                                                week_font.append(obj.font)
                                        else:
                                            dic_tgz_xinqi_layers[worklayer] = [(obj, step.name)]
                                            week_font.append(obj.font)                                            
                        ...
                        feat_out = layer_cmd.featout_dic_Index(
                            units="mm", options="feat_index+select")["barcodes"]
                        for obj in feat_out:
                            for wwyy in ["$$yy", "$$ww", "$$mm", "$$dd", "$$year", "$$month", "$$day"]:                        
                                if wwyy in str(obj.text).replace("{", "").replace("}", "").lower():                                    
                                    if dic_tgz_barcode_week_layers.has_key(worklayer):
                                        if (obj, step.name) not in dic_tgz_barcode_week_layers[worklayer]:                                        
                                            dic_tgz_barcode_week_layers[worklayer] += [(obj, step.name)]
                                            week_font.append(obj.font)
                                    else:
                                        dic_tgz_barcode_week_layers[worklayer] = [(obj, step.name)]
                                        week_font.append(obj.font)
                        ...
                        if step.featureSelected():
                            feat_out = layer_cmd.featout_dic_Index(
                                units="mm",options="feat_index+select") ["pads"]
                            if dic_tgz_week_layers.has_key(worklayer):
                                if (obj, step.name) not in dic_tgz_week_layers[worklayer]:                                
                                    dic_tgz_week_layers[worklayer] += [(obj, step.name) for obj in feat_out]
                            else:
                                dic_tgz_week_layers[worklayer] = [(obj, step.name) for obj in feat_out]                   
            ...
            return dic_tgz_week_layers, dic_tgz_barcode_week_layers, dic_tgz_banci_layers, dic_tgz_picihao_layers, dic_tgz_xinqi_layers
            ...
            if dic_tgz_week_layers:            
                for layer in inplan_check_layer:
                    if layer not in dic_tgz_week_layers.keys():
                        ...
                        show_warning_log.append(log.format(layer, layer_side, inplan_check_layer))
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:            
            ...
            for stepname in list(set(setsteps)):
                ...
                for worklayer in checkLayers:
                    check_mirror = "no"
                    if worklayer in outsignalLayers:
                        if worklayer == "l1" or worklayer.split("-")[0] == "l1":
                            check_mirror = "no"
                        else:
                            check_mirror = "yes"
                    ...
                    step.affect(worklayer)
                    step.resetFilter()
                    step.filter_set(feat_types='text;pad')
                    step.selectAll()
    ...
    def auto_check_pnl_rout_result(self):
        """检测裁膜输出异常内容20250306 by lyh"""
        os.system("perl /incam/server/site_data/scripts/sh_script/auto_output_pnlrout/output_pnlrout_auto_check.pl {0} {1} {2}".format(
            "auto_check", job.name, job.name+"_pnl_rout"
        ))
        error_log = "/tmp/success_{0}_check_error.log".format(job.name+"_pnl_rout")
        success_log = "/tmp/success_{0}.log".format(job.name+"_pnl_rout")
        if os.path.exists(error_log):
            lines = file(error_log).readlines()
            os.unlink(error_log)
            return [line.strip().decode("utf8")+"<br>" for line in lines if line.strip()], None
        
        if os.path.exists(success_log):
            os.unlink(success_log)
            return "success", None
        else:
            return u"裁膜资料检测程序运行异常，请手动检查！", None        
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:            
            ...
            for stepname in setsteps:
                ...
                for mai_drill in drill_layers:
                    ...
                    if mai_drill not in ["drl", "cdc", "cds"]:
                        ...
                        for dic_mrp in get_mrp_names:
                            ...
                            if "-" in dic_mrp["MRP_NAME"]:
                                #要去除光板 类似D51910PBCV6A1-光板1  by ynh 20250319
                                if not re.match("\d{5,6}$", dic_mrp["MRP_NAME"].split("-")[1]):
                                    continue
                    ...
                    laser_drill_contain_mai_tong_layers = [] # 镭射跟埋孔完全被通孔包含的层 不含外层镭射及埋孔层关联的镭射
                    if result:
                        a, b = result[0]
                        a = int(a)
                        b = int(b)
                        if a < b:                            
                            ...
                            if "-lyh" in job.name:                                
                                if u"通孔" in drill_name:
                                    laser_drill_contain_mai_tong_layers += mai_drill_layers
                                    laser_drill_contain_mai_tong_layers += mai_man_drill_layers
                                    
                                for layer in laser_drill_layers:
                                    start = int(layer.split("-")[0][1:])
                                    end = int(layer.split("-")[1])
                                    if a< start < b and a< end < b:                                    
                                        laser_drill_contain_mai_tong_layers.append(layer)
                            ...
                            if "-lyh" in job.name:         
                                ...
                                for layer in laser_drill_layers:
                                    ...
                                    if b< start < a and b< end < a:                                    
                                        laser_drill_contain_mai_tong_layers.append(layer)                                    
                    #if "-lyh" in job.name and u"通孔" in drill_name:                
                        #job.PAUSE(str([laser_drill_contain_mai_tong_layers]))                                
                    if laser_drill_inner_mai_layers or laser_drill_outer_mai_layers or laser_drill_contain_mai_tong_layers:
                        for laser_drill in laser_drill_inner_mai_layers + laser_drill_outer_mai_layers + laser_drill_contain_mai_tong_layers:
                            
                            if laser_drill in laser_drill_layers:                                
                                if laser_drill not in erp_laser_drills:
                                        continue
                            ...
                            if laser_drill in laser_drill_contain_mai_tong_layers:
                                if "icg" not in stepname: 
                                    if step.featureSelected():
                                        log = u"检测到{3} {0}钻孔被{1}{2}钻到，请检查是否异常，并反馈给MI问客是否删除！"
                                        if laser_drill in mai_drill_layers + mai_man_drill_layers:
                                            log = u"检测到{3} {0}钻孔跟{1}{2}钻到，请检查是否异常！"
                            ...
                            if step.featureSelected():                                    
                                # step.PAUSE("dd")
                                log = u"检测到{3} {0}钻孔跟{1}{2}孔间距小于4mil，请检查是否异常，并反馈给MI问客是否删除！"
                                if laser_drill in mai_drill_layers + mai_man_drill_layers:
                                    log = u"检测到{3} {0}钻孔跟{1}{2}孔间距小于4mil，请检查是否异常！"
            ...
            if step.featureSelected():
                ...
                for obj in featout:
                    ...
                    if obj.x + 6.5 * 0.5 > sr_xmin - 1:
                        if obj.x + 6.5 * 0.5 >= sr_xmin:
                            log = u"重要项目：检测到此型号{0}有添加全流程二维码坐标左侧进单元内，请反馈主管确认！".format(jobname)
                            if log not in arraylist:                        
                                arraylist.append(log)
                        else:
                        ...
                        if f_xmax - obj.x - + 6.5 * 0.5 <= sr_xmax:
                            log = u"重要项目：检测到此型号{0}有添加全流程二维码坐标镜像或旋转右侧进单元内，请反馈主管确认！".format(jobname)
                            if log not in arraylist:                        
                                arraylist.append(log)                                
                ...
                if exists_info:
                    ...
                    res = self.get_cam_director_finish_status(process_type="icheckfinish_time")
                    if not res:                        
                        # 工艺要求所有系统都要加 20250204 by lyh 翟鸣通知暂时针对新单 旧单按以前的逻辑检测
                        log = u"{0}要求添加板边全流程追溯码，此板未添加，请重新运行添加程序！".format(job.name)
                        arraylist.append(log)
                    else:
    ...
    def check_trace_barcode_coordinate_info_new(self):
        """检测新的全流程追溯二维码坐标信息是否上传 20250402 by lyh"""
        stepname = "panel"
        arraylist = []
        exists_info = self.get_exists_coordinate_info(job.name, select_dic=True)
        dic_exists_info = {}
        ...
        if stepname in matrixinfo["gCOLstep_name"]:            
            ...
            step.resetFilter()
            step.setAttrFilter(".bit,text=trace_barcode_new")
            step.selectAll()
            if not step.featureSelected():
                log = u"未检测到新的追溯二维码添加标记，请重新运行添加程序！"
                arraylist.append(log)
                
                showMessageInfo(*arraylist)
                return arraylist, None
            
            array_mrp_info = get_inplan_mrp_info(job.name.replace("_lyh", ""), condtion="1=1")            
            dic_attr = {"bar_code_innerl2l3": "innerl2l3",
                        "bar_code_sec": "inner",
                        "bar_code_out": "out",}
            ...
            for dic_info in sorted(array_mrp_info, key=lambda x: x["PROCESS_NUM"]):
                ...
                if process_num == 1: 
                    ...
                    if "-" in mrp_name:
                        step.resetFilter()
                        step.setAttrFilter(".string,text=bar_code_sec1_"+mrp_name.lower())
                        step.selectAll()
                    else:
                        step.resetFilter()       
                        step.setAttrFilter(".string,text=bar_code_out")
                ...
                if step.featureSelected():
                    ...
                    for obj in featout:
                        ...
                        if "bar_code_sec1_" in obj.string:
                            dic_attr[obj.string] = "out"
                            
                        if dic_attr.get(obj.string, None):
                            if obj.string == "bar_code_out" or "bar_code_sec1_" in obj.string:
                                # 二位码坐标定义：压合裁膜后的这个机械钻码机是以二维码的左上角来定义二维码位置坐标的
                                # 因外层进板方向跟芯板不一样 外层 x 跟y方向要对调
                                # x方向要从原点开始计算 而不是板中心                    
                                dic_zu[dic_attr[obj.string]+"_x"] = round(obj.y - 6.5 * 0.5 - out_lb_y, 2)
                ...
                if dic_zu:                    
                    dic_mrp_name_trace_info[mrp_name] = dic_zu
                ...
                for key, value in dic_exists_info.iteritems():
                    if mrp_name == key:
                        if not dic_zu:
                            log = u"检测到此型号{0} MES系统有上传坐标信息，但板边未加全流程追溯二维码位置属性\n(.string,text=bar_code_innerl2l3 <br>或 .string,text=bar_code_sec 或 .string,text=bar_code_out)，请重新运行添加程序！".format(mrp_name)
                            arraylist.append(log)
                            
                        for calc_key, calc_value in dic_zu.iteritems():                            
                            if abs(float(value.get(calc_key, 0)) - float(calc_value)) > 0:
                                add_log = True
                                log += u"<br>MES坐标信息{0}:{1}".format(calc_key, value.get(calc_key, 0))
                                log += u"<br>TGZ坐标信息{0}:{1}".format(calc_key, calc_value)
                        break
                else:
                    if dic_zu:
                        log = u"检测到此型号{0}对应的层有加全流程追溯二维码，但未上传坐标信息到MES系统，请运行上传程序！".format(mrp_name)
                        arraylist.append(log)                        
            ...
            res = self.get_cam_director_finish_status(process_type="icheckfinish_time")
            if not dic_mrp_name_trace_info:                
                if not res:                        
                    # 工艺要求所有系统都要加 20250204 by lyh 翟鸣通知暂时针对新单 旧单按以前的逻辑检测
                    log = u"{0}要求添加板边全流程追溯码，此板未添加，请重新运行添加程序！".format(job.name)
                    arraylist.append(log)
    ...
    def check_barcode_error(self, auto_check="auto"):
        """检测动态二维码是否为动态TEXT"""
        
        # print("------------------------------>start", )        
        symbol_path = os.path.join("/id/incamp_db1/jobs", jobname, "symbols")
        if "172.28.30" in localip[2][0]:
            symbol_path = os.path.join("/id/incamp_db3/jobs", jobname, "symbols")
            
        symbols = os.listdir(symbol_path)
        is_check = False
        for name in symbols:            
            if name.startswith("barcode+") or name == "barcode":
                is_check = True
                break
        ...
        get_all_steps = get_panelset_sr_step(job.name, "panel")
        
        dic_symbolname = {}
        arraylist = []
        dic_zu_layer = {}
        for stepname in get_all_steps + ["panel"]:            
            step = gClasses.Step(job, stepname)
            step.open()
            for layer in silkscreenLayers + solderMaskLayers + outsignalLayers:
                step.clearAll()
                ...
                if step.featureSelected():
                    layer_cmd = gClasses.Layer(step, layer)
                    feat_out = layer_cmd.featout_dic_Index(options="feat_index+select")["pads"]
                    step.selectNone()
                    for obj in feat_out:
                        barcode_names.append(obj.symbol)                          
                        dic_symbolname[obj.symbol] = layer
                        if "barcode" in getattr(obj, "genesis_sym_name", ""):                            
                            step.selectFeatureIndex(layer, obj.feat_index)                            
                
                if step.featureSelected():
                    log = u"检测到{0} {1}的二维码symbol{2}不是动态text类型，请调整修改此二维码！！".format(stepname, layer, barcode_names)
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, layer,
                                                                         layer,log, dic_zu_layer) 
            step.clearAll()
    ...
    def check_barcode_error_manual(self, auto_check="auto"):
        """手动 检测动态二维码是否为动态TEXT"""
    ...
    def check_inner_barcode_is_right(self, tgz_file_path=None):
        ...
        if tgz_file_path is not None:
            ...
            try:
                ...
                for member in members:
                    # 检查是否是文件
                    if member.isfile():
                        tgz_contain_jobname.append(member.name.split("/")[0])
                ...
                if tgz_contain_jobname and list(set(tgz_contain_jobname))[0].lower() != job.name and "-inn" not in list(set(tgz_contain_jobname))[0].lower():
                # if "-lyh" in job.name:                    
                    log = u"检测到输出的tgz压包内的资料文件名{0}跟当前型号名{1}不一致，程序将自动修复，请在windows电脑将tgz文件打开检测是否已修复OK！"
                    arraylist1.append(log.format(list(set(tgz_contain_jobname))[0].lower(), job.name))
                    
                    old_folder_name = list(set(tgz_contain_jobname))[0].lower()
                    new_folder_name = job.name                    
                    extracted_path = "/tmp/tgz_exact_file_{0}".format(job.name)
                    if os.path.exists(extracted_path):
                        shutil.rmtree(extracted_path)
                    os.makedirs(extracted_path)
                    tgz.extractall(path=extracted_path)
                    
                    old_folder_path = os.path.join(extracted_path, old_folder_name)
                    new_folder_path = os.path.join(extracted_path, new_folder_name)
                    shutil.move(old_folder_path, new_folder_path)
                    
                    new_tar_path = "/tmp/{0}.tgz".format(job.name)
                ...
                for key, value in dic_layer_symbol.iteritems():                
                    ...
                    for key1, value1 in dic_layer_symbol.iteritems():
                        if key != key1:            
                            ...
                            if same_symbol:
                                arraylist2.append(u"%s检测到tgz内%s层跟%s层二维码一样:%s，会造成产线抓取此二维码预叠防呆机制失效，<br>\
                                程序将删除已输出tgz，请将此两层二维码内的.genesis_sym_name属性删除或检查层内板边二维码是否为动态后再重新输出tgz！"%(jobname, key.upper(), key1.upper(), same_symbol))
                                exists_layers.append(key)
                                exists_layers.append(key1)
                            
            if arraylist1:                
                return arraylist1, None
            
            if arraylist2:
                os.unlink(tgz_file_path)
                return arraylist2, None            
        ...
        if len(check_layers) > 2 and "panel" in matrixinfo["gCOLstep_name"]:
            ...
            step.clearAll()
            #翟鸣通知 所有genesis_sym_name属性输出tgz时自动清除
            delete_attr = True
            
            #if job.name.upper() == "HC1116PH155A1":
                #job.PAUSE(str(check_layers))
            
            if arraylist: # and "-lyh" in job.name:
                # 开始自动修改异常二维码symbol  20241106 by lyh
                arraylist.append(u"程序将开始自动修改，修复后请对比检查是否异常！")
                showMessageInfo(*arraylist)
                self.modify_panel_edge_barcode_symbol_to_text()
                #检测是否已修改ok
                res = self.modify_panel_edge_barcode_symbol_to_text()
                if res[0] == "success":
                    # job.save()
                    arraylist = []
    ...
    def modify_panel_edge_barcode_symbol_to_text(self):
        
        if "panel" not in matrixinfo["gCOLstep_name"]:
            showMessageInfo(u"panel不存在，请检查！")
            return "success", None
        ...
        for layer in innersignalLayers:
            step.clearAll()
            step.removeLayer(layer+"_barcode_bak")
            step.affect(layer)
            step.resetFilter()
            step.setAttrFilter(".nomenclature")
            step.selectSymbol("barcode*")
            if step.featureSelected():
                layer_cmd = gClasses.Layer(step, layer)
                feat_out = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["pads"]
                for obj in feat_out:
                    if getattr(obj, "genesis_sym_name", None) == obj.symbol:
                        step.selectNone()
                        step.selectFeatureIndex(layer, obj.feat_index)
                        if step.featureSelected():
                            step.moveSel(layer+"_barcode_bak")
                        
                        step.COM("add_text,type=barcode,polarity=positive,\
                        x={0},y={1},text={2},\
                        mirror=no,angle=0,direction=cw,bar_type=ecc-200,matrix=minimal,\
                        bar_background=no,bar_add_string=no,bar_marks=no,bar_width=0.2032,\
                        bar_height=7".format(obj.x, obj.y, "$$job $$layer"))                      
                        
            if step.isLayer(layer+"_barcode_bak"):                
                log = u"{0}层修改板边防叠反二维码symbol成功，请比对原来备份symbol是否位置层{1}有异常！"
                arraylist.append(log.format(layer, layer+"_barcode_bak"))
                step.clearAll()
                step.affect(layer+"_barcode_bak")
                step.resetFilter()
                step.selectAll()
                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, layer+"_barcode_bak",
                                                                     layer+"_barcode_bak",log.format(layer, layer+"_barcode_bak"),
    ...
    def check_dynamic_barcode_output_odb_is_right(self, tgz_file_path=None, check_layers=None):
        """检测动态二维码输出odb++时内容是否有异常"""
        if os.path.exists(tgz_file_path):
            ...
            #if job.name.upper() == "MC1808OB146F5":
                #return
            ...
            for worklayer in check_layers:
                if step.isLayer(worklayer):
                    ...
                    if step.featureSelected():
                        layer_cmd = gClasses.Layer(step, worklayer)
                        feat_out = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["barcodes"]
                        if feat_out:
                            for obj in feat_out:
                                text = obj.text.lower()
                                if "$$jobn" in text.lower() or \
                                    "$$job_cut" in text.lower():                            
                                    check_barcode = True
            ...
            showMessageInfo(u"程序将开始回读tgz，并检测比对动态二维码内容是否异常！！")            
            
            new_jobname = job.name + "_check_barcode"
            self.delJob(new_jobname)
            top.importJob(new_jobname, tgz_file_path, "db1")
            new_job = gClasses.Job(new_jobname)
            new_job.open(0)
            matrix_info = new_job.matrix.getInfo()
            
            symbol_path = os.path.join("/id/incamp_db1/jobs", new_jobname, "symbols")                
            barcode_symbols = os.listdir(symbol_path)
            arraylist = []
            dic_zu_layer = {}
            for stepname in matrix_info["gCOLstep_name"]:
                step = gClasses.Step(new_job, stepname)
                step.open()
                step.COM("units,type=mm")
                #if stepname == "set":
                    #step.PAUSE("check")
                for worklayer in check_layers:
                    if step.isLayer(worklayer):
                        step.clearAll()
                        step.affect(worklayer)
                        step.copyLayer(new_job.name, step.name, worklayer, worklayer+"_bak")
                        step.resetFilter()                        
                        step.filter_set(feat_types='text')
                        step.selectAll()
                        add_barcode = False
                        if step.featureSelected():
                            ...
                            for obj in feat_out:
                                symbolname = getattr(obj, "genesis_sym_name", "none")
                                if symbolname in barcode_symbols and \
                                   ("$$jobn" in obj.text.lower() or \
                                    "$$job_cut" in obj.text.lower() or job.name == "s94010pbb77b1"):  
                                    if not step.isLayer(worklayer+"_barcode_tmp"):                                
                                        step.createLayer(worklayer+"_barcode_tmp")
                                    if not step.isLayer(worklayer+"_barcode_bak"):                                
                                        step.createLayer(worklayer+"_barcode_bak")
                                    step.clearAll()    
                                    step.affect(worklayer+"_barcode_tmp")
                                    polarity = "positive"
                                    if getattr(obj, "invert_bg", None) == 1:
                                        polarity = "negative"
                                    step.addPad(obj.x, obj.y, symbolname, polarity=polarity, mirror=obj.mirror)
                                    add_barcode = True
                                    ...
                                    step.resetFilter()
                                    step.filter_set(feat_types='pad;line;arc;surface', polarity='positive')
                                    step.refSelectFilter(worklayer+"_barcode_tmp")
                                    if step.featureSelected():
                                        step.copySel(worklayer+"_barcode_bak")
                                    else:
                                        step.clearAll()
                                        step.affect(worklayer+"_barcode_bak")
                                        step.reset_fill_params()
                                        if obj.mirror == "no":      
                                            step.addRectangle(obj.x-0.35, obj.y-0.35, obj.x+4.45, obj.y+4.45)
                                        else:
                                            step.addRectangle(obj.x+0.35, obj.y-0.35, obj.x-4.45, obj.y+4.45)
                        ...
                        if step.featureSelected():
                            ...
                            for obj in feat_out: 
                                ...
                                step.clearAll()
                                step.affect(worklayer+"_barcode_tmp")
                                step.addPad(obj.x, obj.y, obj.symbol, polarity="positive", mirror=obj.mirror)
                                add_barcode = True
                            ...
                            # step.PAUSE("check1")
                            step.copyLayer(job.name, step.name, worklayer, worklayer)
                            step.copyLayer(job.name, step.name, worklayer, worklayer+"_bak")
                            # step.PAUSE("check2")
                            
                        if add_barcode:
                            step.clearAll()
                            step.affect(worklayer+"_bak")
                            step.contourize(accuracy=0)
                            step.resetFilter()
                            step.refSelectFilter(worklayer+"_barcode_tmp")
                            #因二维码surface有空洞 这里要处理下
                            if step.featureSelected():                                
                                step.COM("sel_resize,size=2")
                                step.refSelectFilter(worklayer+"_barcode_tmp")
                                step.COM("sel_resize,size=-10")
                            ...
                            step.COM("sel_resize,size=-10")
                            step.resetFilter()
                            step.refSelectFilter(worklayer+"_bak", mode="disjoint")
                            if step.featureSelected():
                                step.selectDelete()
                            
                            step.selectAll()
                            step.removeLayer(worklayer+"_bak")
                            if step.featureSelected():
                                step.COM("sel_resize,size=10")
                                step.copyToLayer(worklayer+"_barcode_bak", invert="yes")
                                ...
                                feat_out = layer_cmd.featout_dic_Index(options="feat_index+select")["barcodes"]
                                feat_out += layer_cmd.featout_dic_Index(options="feat_index+select")["text"]
                                all_texts = [obj.text.replace("'", "") for obj in feat_out]
                                
                                log = u"检测到tgz回读型号{4}  {3} {0}层动态二位码{2}输出ODB++转换异常，<br>程序将删除已输出的tgz，请比对层{1} ODB++内的symbol图形检查是否异常！".format(
                                    worklayer, worklayer+"_barcode_bak", all_texts, step.name, new_job.name
                                )
                                arraylist.append(log)
                                dic_zu_layer = self.get_selected_dic_note_coordinate(new_job.name, step.name, step, worklayer,
                                                                                     worklayer+"_barcode_bak",log,
                                                                                     dic_zu_layer, add_note_layer=worklayer) 
                        step.removeLayer(worklayer+"_bak")
                        step.removeLayer(worklayer+"_barcode_tmp")
    ...
    def check_Features_in_profile(self):
        ...
        for stepname in steps:
            ...
            step.open()
            step.COM("units,type=mm")
            if step.isLayer("patternfill"):                
                step.COM("truncate_layer,layer=patternfill")
            if step.isLayer("patternfill_flatten"):    
                step.COM("truncate_layer,layer=patternfill_flatten")
            if step.isLayer("outline_tmp"):    
                step.COM("truncate_layer,layer=outline_tmp")
            ...
            for editname in set(all_steps):
                ...
                if stepname == "set":
                    if "edit" not in editname:
                        ...
                        editstep.clearAll()
                        editstep.affect("patternfill")                        
                        editstep.COM("sel_resize,size=-{0}".format(16*25.4))                        
            
            if not step.isLayer("patternfill"):
                continue        
    
            if res == "outer":
                check_layers = [x for x in checklayers if x not in innersignalLayers + drill_layers]
            else:
    ...
    def check_Features_Outside_profile(self):
        ...
        if res == "outer":
            check_layers = [x for x in boardlayers if x not in innersignalLayers + drill_layers	]# boardlayers + solderMaskLayers + silkscreenLayers + other_layers
        else:
            check_layers = innersignalLayers + drill_layers	
    ...
    def check_slot_rotation_angle_condition(self):
        ...
        for stepname in ["edit", "set"]:            
            if stepname in matrixinfo["gCOLstep_name"]:
                ...
                if step.isLayer(worklayer):
                    ...
                    coordinate_info1 = [(obj, math.hypot((obj.xs+obj.xe) *0.5, (obj.ys+obj.ye) *0.5),
                                        obj.xs, obj.ys, obj.xe, obj.ye,obj.angle) for obj in feat_out1
                                        if self.get_short_slot_status(float(obj.symbol[1:]), obj.len) == "short"]
                else:
                    continue
                        ...
                        for suffix_info in coordinate_info2:
                            ...
                            if pre_info[1] - suffix_info[1] < 0.1 and \
                               ...
                               abs((pre_info[3] + pre_info[5]) * 0.5 - (suffix_info[3] + suffix_info[5]) * 0.5) < 0.1:
                                ...
                                #if "da8614pc982a2" in job.name:
                                    #step.PAUSE(str([calc_angle, slot_size, diff_angle]))
                                ...
                                if round(diff_angle, 1) != calc_angle:
                                    step.selectNone()
        ...
        if ykj_has_slot:
            if "yes" not in ykj_has_slot.values():
                return u"set跟edit的 {0}层内都不存在槽孔，请重新运行槽孔旋转及添加导引程序!!".format(reflayer), None
    ...
    def get_short_slot_status(self, slot_size, slot_len):
        """判断是否短槽"""
        ...
        r = round((float(slot_len) *1000 +float(slot_size)) / float(new_slot_size), 3)
    ...
    def check_target_symbol_touch_other_features(self):
        ...
        新增融合块检测 http://192.168.2.120:82/zentao/story-view-7684.html 20241203 by lyh"""
        ...
        for worklayer in outsignalLayers + innersignalLayers:
            ...
            if step.featureSelected():                
                ...
                feat_out = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["pads"]
                #if "-lyh" in job.name:
                    #step.PAUSE(str(set([obj.symbol for obj in feat_out])))                
                for obj in feat_out:
                    step.selectNone()
                    step.clearAll()
                    step.affect(worklayer)
                    ...
                    if "et_rj_pad" in obj.symbol:
                        step.filter_set(exclude_syms='hdi1-mdk*;sh-pindonut')                         
                        ...
                        for index in indexes:
                            if float(index) > float(obj.feat_index):
                                if index not in touch_indexes:
                                    if obj.symbol in ["et_rj_pad_p", "et_rj_pad"]:
                                        res = self.check_et_rj_pad_touch_features(step, obj, index, worklayer)
                                        if res:
                                    ...
                                    #if "hdi1-b" in obj.symbol and "-lyh" in job.name:
                                        #step.PAUSE(str([obj.feat_index, index]))                                    
                                    # if "-lyh" in job.name:                                        
                                    resize = 0.3
                                    if obj.symbol in ["hdi-dwpad"]:
                                        resize = 0.9
                                    ...
                                    if obj.symbol in ["et_rj_pad_p", "et_rj_pad"]:
                                        resize = -1
                                        obj.symbol = "check_touch_" + obj.symbol                                    
                                    ...
                                    check_intersection = True # 是否相交检测
                                    # 靶加了一圈外框 这里在判断一下
                                    if "donut_s" in touch_obj.symbol and "hdi1-b" in obj.symbol:
                                        if abs(obj.x - touch_obj.x) < 0.1 and abs(obj.y - touch_obj.y) < 0.1:
                                            check_intersection = False
                                    
                                    # 中心光点加补偿的排除掉
                                    if "sh-bb" in obj.symbol and re.match("r\d.*", touch_obj.symbol):
                                        if abs(obj.x - touch_obj.x) < 0.1 and abs(obj.y - touch_obj.y) < 0.1:
                                            check_intersection = False                                        
                                        
                                    if self.intersection_area(*area_xy1+area_xy2) and check_intersection:                                        
                                        #if "hdi1-b" in obj.symbol and "-lyh" in job.name:
                                            #step.removeLayer("check_tmp")
                                            #step.clearAll()
                                            #step.affect("check_tmp")
                                            #step.addRectangle(*area_xy1)
                                            #step.addRectangle(*area_xy2)
                                            #step.PAUSE(str([area_xy1, area_xy2, obj.feat_index, index]))
            ...
            # if "-lyh" in job.name:
            # 融合块不跟钻孔比对
            symbols = [x for x in symbols if "et_rj_pad" not in x]
                ...
                for i, layer in enumerate(matrixinfo["gROWname"]):
                    if matrixinfo["gROWcontext"][i] == "board" and \
                       drill_layer in layer:
                        ...
                        step.refSelectFilter(layer)
                        if step.featureSelected():
                            step.removeLayer(worklayer+"_touch_hole_tmp")
                            step.copySel(worklayer+"_touch_hole_tmp")
                            step.clearAll()
                            step.affect(worklayer+"_touch_hole_tmp")
                            step.resetFilter()
                            #跟孔中心一致的排除掉 
                            step.COM("sel_ref_feat,layers={0},use=filter,mode=same_center,"
                                     "f_types=line\;pad\;surface\;arc\;text,polarity=positive\;negative,"
                                     "include_syms=,exclude_syms=,on_multiple=smallest".format(
                                         layer))
                            if step.featureSelected():
                                step.selectDelete()
                            ...
                            step.selectAll()
                            if step.featureSelected():
                                layer_cmd = gClasses.Layer(step, worklayer+"_touch_hole_tmp")
                                feat_out_touch = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["pads"]
                                find_symbols += [touch_obj.symbol for touch_obj in feat_out_touch]
                                touch_hole_symbols = [touch_obj.symbol for touch_obj in feat_out_touch]
                                log = u"检测到{0} 层靶标{1} 上有被钻孔层{2}接触，请检查靶标是否完整！".format(
                                    worklayer,";".join(list(set(touch_hole_symbols))), layer)
                                arraylist.append(log)
                                
                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, worklayer+"_touch_hole_tmp",
                                                                                     layer, log, dic_zu_layer, add_note_layer=worklayer)
                step.resetFilter()
            ...
            if find_indexes:
                ...
                log = " "
                if find_symbols:
                    if [x for x in find_symbols if "et_rj_pad" not in x ]:                        
                        log = u"检测到{0} 层靶标{1} 上有被其他元素覆盖接触，请移开接触的靶标的元素！".format(
                            worklayer,";".join(list(set([x for x in find_symbols if "et_rj_pad" not in x ]))))
                        update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                                     u"forbid_or_information='{0}'".format(u"拦截(不放行)"))
                    if "et_rj_pad" in "".join(find_symbols):
                        log += u"<br>检测到融合块{0}被其他元素接触，请检查接触位置是否跟大铜皮导通，若有导通请移开处理！！".format(
                            ";".join(list(set([x.replace("check_touch_", "") for x in find_symbols if "et_rj_pad" in x ])))
                        )
                    arraylist.append(log)
    ...
    def check_all_symbol_touch_other_features(self):
        """检测板边所有的symbol是否被其他surface的元素有覆盖 20250310 by lyh
        """
        check_symbol = """hdi-dwpad
        hdi-dwtop-t
        hdi-dwtop-b
        ...
        for worklayer in outsignalLayers + innersignalLayers:
            ...
            step.resetFilter()
            step.affect(worklayer)
            step.filter_set(feat_types='pad', exclude_syms="r0;sh_lengnew;gold_sq")
            step.selectAll()
            if step.featureSelected():
                # select_feat_indexes = layer_cmd.featSelIndex()
                #feat_out = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["pads"]
                #select_feat_symbols = list(set([obj.symbol for obj in feat_out]))
                pass
            else:
                continue
            
            #if worklayer == "l3":
                #job.PAUSE(str(check_symbols))
            
            #for sel_feat_index in select_feat_indexes:
                #step.selectNone()
                #step.VOF()
                #step.selectFeatureIndex(worklayer, sel_feat_index)
                #step.VON()
                # x1, y1, x2, y2 = get_layer_selected_limits(step, worklayer)
                
            exists_check_indexes = []
            if step.featureSelected():                
                
                feat_out = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["pads"]
                positive_feat = [obj for obj in feat_out if obj.polarity == "positive"]
                negative_feat = [obj for obj in feat_out if obj.polarity == "negative"]                
                
                for obj in positive_feat + negative_feat:
                    ...
                    if obj.symbol in ["rect11312.5x11312.5"]:
                        step.filter_set(feat_types='pad;line;arc;surface', polarity='positive')
                    ...
                    polarity=positive\;negative,include_syms=,exclude_syms=")
                    log = ""
                    #if worklayer == "l3" and obj.symbol == 'sh-pindonut':                        
                        #step.PAUSE(str([worklayer, obj.symbol]))
                    #if "et_rj_pad" in obj.symbol and "-lyh" in job.name:
                        #step.PAUSE(str([worklayer]))
                        
                    if step.featureSelected():
                        indexes = layer_cmd.featSelIndex()
                        feat_out_pad_touch = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["pads"]
                        touch_pad_indexes = [touch_obj.feat_index for touch_obj in feat_out_pad_touch]
                        
                        # if "SD1024E6T26A1".lower() in job.name:                            
                        for touch_obj in layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["lines"]:
                            # if touch_obj.shape == "Line":
                            if abs(touch_obj.angle) in [0, 90, 180, 270]:
                                touch_obj.x = (touch_obj.xe + touch_obj.xs) * 0.5
                                touch_obj.y = (touch_obj.ye + touch_obj.ys) * 0.5
                                touch_obj.rotation = abs(touch_obj.angle)
                                touch_obj.mirror = "no"
                                touch_obj.orig_symbol = touch_obj.symbol
                                if abs(touch_obj.angle) in [0, 180]:
                                    touch_obj.symbol = "rect%sx%s" % (
                                        touch_obj.len * 1000 + float(touch_obj.symbol[1:]), touch_obj.symbol[1:])
                                else:
                                    touch_obj.symbol = "rect%sx%s" % (
                                        touch_obj.symbol[1:], touch_obj.len * 1000 + float(touch_obj.symbol[1:]))
                                    
                                touch_obj.angle = 0
                                touch_obj.rotation = 0
                                
                                touch_pad_indexes.append(touch_obj.feat_index)
                                feat_out_pad_touch.append(touch_obj)
                        
                        all_touch_feat_out = feat_out_pad_touch
                        # all_touch_feat_out += layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["lines"]
                        all_touch_feat_out += layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["arcs"]
                        all_touch_feat_out += layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["text"]
                        all_touch_feat_out += layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["barcodes"]
                        all_touch_feat_indexes = [touch_obj.feat_index for touch_obj in all_touch_feat_out]
                        
                        touch_surface_indexes = [index for index in indexes if index not in all_touch_feat_indexes]                        
                        
                        for index in indexes:
                            if index in exists_check_indexes:
                                continue                            
                            
                            # 同一类型的物件不检测
                            symbol_attr = [getattr(o, "string", "") for o in all_touch_feat_out
                                           if o.feat_index == index]
                            if getattr(obj, "string", None) in symbol_attr:
                                continue
                            
                            # 同心圆
                            if hasattr(obj, "out_scale") and re.match("r\d+.*|donut_r.*", obj.symbol):
                                symbol_attr = [o for o in all_touch_feat_out
                                               if o.feat_index == index and hasattr(o, "out_scale")]
                                if symbol_attr:
                                    continue
                            
                            symbol_attr = [getattr(o, "bit", "") for o in all_touch_feat_out
                                           if o.feat_index == index]
                            if getattr(obj, "bit", None) == "scale_yes" and getattr(obj, "bit", None) in symbol_attr:
                                continue
                            
                            # 追溯码同类不检测
                            if getattr(obj, "bit", None) == "trace_barcode":
                                symbol_attr = [o for o in all_touch_feat_out
                                               if o.feat_index == index
                                               and getattr(o, "bit", "") == "trace_barcode"]
                                if symbol_attr:
                                    continue
                                
                            if getattr(obj, "bit", None) == "trace_barcode_new":
                                symbol_attr = [o for o in all_touch_feat_out
                                               if o.feat_index == index
                                               and getattr(o, "bit", "") == "trace_barcode_new"]
                                if symbol_attr:
                                    continue                                
                            
                            # 背钻定位孔靶上有字符c跟s
                            if obj.symbol in ["hdi1-bmd", "hdi1-bmdj",]:
                                symbol_text = [o for o in all_touch_feat_out
                                               if o.feat_index == index
                                               and o.shape == "Text"
                                               and o.text.replace("'", "").lower() in ["c", "s"]]
                                if symbol_text:
                                    continue
                            
                            # 旧型号层数字排列
                            if obj.symbol == "s2540" and obj.polarity == "negative":
                                symbol_text = [o for o in all_touch_feat_out
                                               if o.feat_index == index
                                               and o.shape == "Text"
                                               and o.text.replace("'", "").lower() in [str(x) for x in range(50)]]
                                if symbol_text:
                                    continue
                            
                            if float(index) < float(obj.feat_index):
                                # job.PAUSE(str([worklayer, obj.symbol, index, obj.feat_index, touch_surface_indexes]))
                                if index in touch_surface_indexes:
                                    # surface在底下的情况跳过
                                    continue
                                
                                find_negative_pad = [touch_obj.symbol for touch_obj in feat_out_pad_touch
                                                     if touch_obj.feat_index == index
                                                     and touch_obj.polarity == "negative"
                                                     and touch_obj.symbol == "rect15331x5000"]                                 
                                if re.match("r\d+.*", obj.symbol) and find_negative_pad:
                                    # 切片孔pad的情况跳过
                                    continue
                                
                                if index in touch_pad_indexes:
                                    find_negative_pad = [touch_obj.symbol for touch_obj in feat_out_pad_touch
                                                         if touch_obj.feat_index == index
                                                         and touch_obj.polarity == "negative"
                                                         and (re.match("s\d+.*", touch_obj.symbol)
                                                              or re.match("rect\d+.*", touch_obj.symbol))]                                    
                                    if find_negative_pad:
                                        # 负片在底下的情况跳过
                                        continue
                                    
                                find_negative_r1_line = [touch_obj.symbol for touch_obj in all_touch_feat_out
                                                        if touch_obj.feat_index == index
                                                        and touch_obj.polarity == "negative"
                                                        and touch_obj.shape == "Line"
                                                        and touch_obj.symbol=="r1"]
                                if find_negative_r1_line:
                                    continue
                                    
                            # if float(index) > float(obj.feat_index):
                            if index not in touch_pad_indexes:
                                if obj.symbol in ["et_rj_pad_p", "et_rj_pad"]:
                                    res = self.check_et_rj_pad_touch_features(step, obj, index, worklayer)
                                    if res:
                                        find_symbols.append(obj.symbol)
                                        log = u"检测到{0} 层图形{1} 上有被其他元素[索引:{2}]覆盖接触，请检查两个图形是否完整！".format(worklayer, obj.symbol, index)
                                        find_touch_logs.append(log)
                                        find_touch_indexes.append(index)
                                else:
                                    find_symbols.append(obj.symbol)
                                    log = u"检测到{0} 层图形{1} 上有被其他元素[索引:{2}]覆盖接触，请检查两个图形是否完整！".format(worklayer, obj.symbol, index)
                                    find_touch_logs.append(log)
                                ...
                                touch_obj = [o for o in feat_out_pad_touch if o.feat_index == index][0]                                   
                                
                                check_intersection = True # 是否相交检测
                                
                                # 本身为负片的情况 且在被touch元素的下方
                                #if (re.match("s\d+.*", obj.symbol)
                                    #or re.match("rect\d+.*", obj.symbol)) and \
                                   #obj.polarity == "negative" and \
                                   #float(touch_obj.feat_index) > float(obj.feat_index):
                                    #if abs(obj.x - touch_obj.x) < 0.1 and abs(obj.y - touch_obj.y) < 0.1:
                                        #check_intersection = False
                                
                                if obj.symbol == "rect15331x5000" and \
                                   obj.polarity == "negative" and \
                                   float(touch_obj.feat_index) > float(obj.feat_index):
                                    check_intersection = False
                                ...
                                for symbolname in ["donut_s", "sh-dwtop2023", "sh-dwbot2023",
                                                   "sh-dwtop", "sh-dwbot", "hdi1-ba", "hdi1-by",
                                                   "hdi1-bs","s5080", "s5082"]:                                    
                                    if (symbolname in touch_obj.symbol and "s5082" in obj.symbol) or \
                                       ("s5082" in touch_obj.symbol and symbolname in obj.symbol) :
                                        if abs(obj.x - touch_obj.x) < 0.1 and abs(obj.y - touch_obj.y) < 0.1:
                                            check_intersection = False                                            
                                    if (symbolname in touch_obj.symbol and "s5080" in obj.symbol) or \
                                       ("s5080" in touch_obj.symbol and symbolname in obj.symbol) :
                                        if abs(obj.x - touch_obj.x) < 0.1 and abs(obj.y - touch_obj.y) < 0.1:
                                            check_intersection = False                                               
                                ...
                                if self.intersection_area(*area_xy1+area_xy2) and check_intersection:                                        
                                    #if obj.symbol == "hdi1-ba3" and "-lyh" in job.name:                                    
                                        #step.removeLayer("check_tmp")
                                        #step.createLayer("check_tmp")
                                        #step.clearAll()
                                        #step.affect("check_tmp")
                                        #step.addRectangle(*area_xy1)
                                        #step.addRectangle(*area_xy2)
                                        #step.PAUSE(str([worklayer, area_xy1, area_xy2, obj.symbol, touch_obj.symbol, touch_obj.rotation, obj.feat_index, touch_obj.feat_index]))
                    ...
                    exists_check_indexes.append(obj.feat_index)                    
                    if log:
                        find_indexes.append([obj.feat_index, find_touch_indexes])
                     
                    if find_indexes:
                        #if "-lyh" in job.name:                    
                            #log = "<br>".join(list(set(find_touch_logs)))
                        #else:
                        #log = " "
                        #if find_symbols:
                            #if [x for x in find_symbols if "et_rj_pad" not in x ]:                        
                                #log += u"<br>检测到{0} 层图形{1} 上有接触到其他物件，请移开接触的物件！".format(
                                    #worklayer,";".join(list(set([x for x in find_symbols if "et_rj_pad" not in x ]))))
                                #update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                                             #u"forbid_or_information='{0}'".format(u"拦截(不放行)"))
                            #if "et_rj_pad" in "".join(find_symbols):
                        ...
                        step.selectNone()
                        for feat_index,touch_indexes in find_indexes:
                            step.VOF()
                            step.COM("sel_layer_feat,operation=select,layer={0},index={1}".format(worklayer, feat_index))
                            step.VON()
                            for new_feat_index in touch_indexes:
                                step.VOF()
                                step.COM("sel_layer_feat,operation=select,layer={0},index={1}".format(worklayer, new_feat_index))
                                step.VON()                        
                            log = u"<br>检测到{0} 层图形{1} 上有接触到其他物件，接触物件的索引({2})，请移开接触的物件！".format(
                                worklayer,obj.symbol, ",".join(touch_indexes))
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, worklayer,
                                                                                 worklayer, log, dic_zu_layer)            
        step.clearAll()        
    ...
    def check_et_rj_pad_touch_features(self, step, check_obj, touch_obj_index, worklayer):
        """融合块的单独在检测中间区域是否被接触"""
        # step = gClasses.Step(job, step.name)
        step.removeLayer(worklayer+"_check_touch")
        step.selectNone()
        step.COM("sel_layer_feat,operation=select,layer={0},index={1}".format(worklayer, touch_obj_index))
        if step.featureSelected():            
            step.copySel(worklayer+"_check_touch")
        step.COM("sel_layer_feat,operation=select,layer={0},index={1}".format(worklayer, check_obj.feat_index))
        if step.featureSelected():
            step.copySel(worklayer+"_check_touch")
        step.clearAll()
        step.affect(worklayer+"_check_touch")
        step.resetFilter()
        step.selectSymbol(check_obj.symbol, 1, 1)
        step.changeSymbol("check_touch_"+check_obj.symbol)
        step.selectSymbol("check_touch_"+check_obj.symbol, 1, 1)
        step.resetFilter()
        step.COM("sel_ref_feat,layers=,use=select,mode=touch,\
        pads_as=shape,f_types=line\;pad\;surface\;arc\;text,\
        polarity=positive\;negative,include_syms=,exclude_syms=")
