...
my $org_code = &get_org_code;
if($org_code){
	$c->Messages("info", "多层型号请使用12.31输出");
    exit(0);
}
...
for (my $i = 0; $i <= $#gROWname; $i++) {
    if (($gROWlayer_type[$i] eq 'drill') && ($gROWcontext[$i] eq 'board') && $gROWname[$i] =~ /^s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$/) {
		# 特殊型号不合并  樊后星通知  by ynh 20250519 
		
		if ($JOB =~ /sd1024e6t96a1|ba8622o6t97a1/){
			push(@rlayers, $gROWname[$i]);
			next;
		}		
        ...
        for(my $h=0; $h<=$#gROWname; $h++){            
			if ($gROWcontext[$h] eq 'board' and $gROWname[$h] =~ /^s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$/ and $gROWname[$i] ne $gROWname[$h]) {
        ...
        push(@all_signal_array, $gROWname[$i]);
    }elsif ($JOB =~ /sd1024e6t96a1/ &&  ($gROWlayer_type[$i] eq 'drill') && ($gROWcontext[$i] eq 'board') && $gROWname[$i] eq 's18-17-2'){
		push(@rlayers, $gROWname[$i]);
	}
}
...
for (my $i = 0; $i <= $#rlayers; $i++) {
	...
	my $five_8mil = 'no';
	my $exist_8mil = 'no';
	my $show_rlyaer = $rlayers[$i];
	if ($rlayers[$i] =~ /^(s\d+-\d+)(-ls\d{6}\.\d{4})?$/){
		$show_rlyaer = $1;
	}			
        ...
        if ($_ eq $pnlspecial[0] || $_ eq $pnlspecial[1] || $_ eq $pnlspecial[2] || $_ eq $pnlspecial[3] || $_ eq $pnlspecial[4]) {
			...
			if ( ($drillSize - 203 <=5.01 and $drillSize - 202 > 0) || ($drillSize - 254 <=5.01 and $drillSize - 253 > 0) || ($drillSize - 233 <=5.01 and $drillSize - 228 > 0) || ($drillSize - 245 <=5.01 and $drillSize - 240 > 0) || ($drillSize - 220 <=5.01 and $drillSize - 215 > 0)){		
				...
				# {		
					...
					#判断是否打5遍					
					my @rlayer_split =  split /-/, substr($show_rlyaer,1);			
    ...
    if ($r3175num ne '4') {		
		$c->OPEN_STEP("$JOB", "$panel_step");	
		$c->CLEAR_LAYER();
		$c->FILTER_RESET;
		$c->AFFECTED_LAYER("$rlayers[$i]", "yes");
		$c->COM("filter_atr_set,filter_name=popup,condition=yes,attribute=.string,text=3175");
		$c->FILTER_SELECT;
		my $sel_count = $c->GET_SELECT_COUNT;
		$c->CLEAR_LAYER();
		$c->FILTER_RESET;
		if ($sel_count != 4) {
			# $c->Messages("info", "$rlayers[$i] 3.175mmm 钻孔数量不是4个，不能正确生成G82及G83指令");
			$c->Messages("info", "$rlayers[$i] 3.175mmm 钻孔数量不是4个，不能正确生成G82及G83指令;\n如果使用其它孔径需要加上.string=3175的属性而且同样只能存在4个");
			exit;
		}		
    }
    if ($r45num ne '0') {
...
my $output_path_des = "/id/workfile/hdi_film/$JOB/";
...
sub show_table {
    for (my $i = 0; $i <= $#rlayers; $i++) {
        ...
        $conlyer[$i] = $table->Label(
            -text         => \${$hash{$show_rlyaer}}[8],
            -textvariable => \${$hash{$show_rlyaer}}[8],
            -background   => "$bg_color",
            -font         => [ -size => 12 ],
            -relief       => 'groove',
            -width        => 70)
...
sub apply_ {
    ...
    for (my $a = 0; $a <= $#rlayers; $a++) {
		...
		push @check_layers,$rlayers[$a];
	}

	my $get_result = system("python /incam/server/site_data/scripts/lyh/auto_sh_hdi_check_rules.py check_output_layer_is_locked @check_layers");
	if ($get_result != 0) {
		exit 0;
	}
    ...
    for (my $a = 0; $a <= $#rlayers; $a++) {
		...
		#伍青通知 这里做防呆拦截 改测试程序输出 20241101 by lyh
		if ($JOB =~ /na8612pi770.*/){
			if ($rlayers[$a] =~ /s1-2-3/ || $rlayers[$a] =~ /s12-11-10/){
				$c->Messages("info", "输出层包含 $rlayers[$a] 工艺提单 $JOB 镭射L1-2-3/L12-11-10两层的镭射对位选用L3层/L10的对位靶点进行对位。请使用测试中的程序输出！！");
				exit();
			}
		}	
    ...
    for (my $i = 0; $i <= $#gSRstep; $i++) {
        if ($gSRstep[$i] =~ /^set/ || $gSRstep[$i] =~ /^edit/) {
			# 特殊料号已经出了内层，0.501的点加在里面需要排除掉edit-字符的step  周涌 提出 by ynh
			if ($JOB =~/^h00008cld11a1/ && $gSRstep[$i] =~ /^edit-.*/){
				next;
			}
    ...
    for (my $i = 0; $i <= $#rlayers; $i++) {
        ...
        if ($rlayers[$i] =~ /^s(\d+)-(\d+)-?(\d+)?-?(\d+)?-?(\d+)?-?(\d+)?-?(\d+)?-?(\d+)?$/ or $rlayers[$i] =~ /^s(\d+)-(\d+).*-ls\d{6}\.\d{4}$/) {
            #if ($rlayers[$i] =~ /^s(\d+)-(\d+)-?(\d+)$/) {
				my $check_signal = "";
				if ($rlayers[$i] =~ /^s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$/){
					$dld_lyr = "dld$1-$2";
				...
				if ($JOB =~ /sd1024e6t96a1/ && $rlayers[$i] =~ /s18-17-2/){
						$dld_lyr = "dld18-17-2";
						$check_signal = "l17";						
					}
			...
			# 特殊料号检测线路对应的关系变更 by 20250509
			
			if ($JOB =~ /^na8612pb770a5.*/){
				if ($dld_lyr eq "dld1-2"){
					$check_signal = "l3";
				}elsif($dld_lyr eq "dld12-11"){
					$check_signal = "l10";
				}
			}
        ...
        if (${$hash{$rlayers[$i]}}[0] == 1) {
            ...
            for (my $n = 1; $n <= $nLoop; $n++) {
				...
				# 资料中如果有.string=3175属性的先改成3.175再输出
				my $string_3175;
				...
				if ($sel_count ) {
					...
					$f->COM("info, out_file=$csh_file, units=mm,args= -t layer -e $JOB/$STEP/$rlayers[$i] -d FEATURES -o select");
				...
				$c->COM("cur_atr_reset");
				$c->COM("cur_atr_set,attribute=.orig_features,text=$org_size");
				$c->COM("sel_change_atr,mode=add");			
			}
			...
			my $sel_count = $c->GET_SELECT_COUNT;
				if ($sel_count ) {
					$c->COM("units,type=mm");
					$c->COM("sel_change_sym,symbol=r3175,reset_angle=no");
				}
				$c->CLEAR_LAYER();
				$c->FILTER_RESET;
				...
				my $rlayer_work = $rlayers[$i];
				if ($rlayers[$i] =~ /^(s.*-\d+)(-ls\d{6}\.\d{4})?$/){
					$rlayer_work = $1;
				}			
                $f->COM("nc_set_file_params,output_path=$output_path,output_name=$JOB.$rlayer_work,zeroes=trailing,units=mm,tool_units=mm,nf1=3,nf2=3,decimal=no,modal_coords=no,single_sr=no,sr_zero_set=no,repetitions=sr,incremental=no,ext_layer=drl2rt,sr_zero_layer=,ccw=no,short_lines=none,allow_no_chain_f=no,no_chain_as_slot=no,keep_table_order=no,max_arc_angle=0,comp_short_slot=no,gscale_file_name=,layer_datum=bottom_left,gscle_align_angle=0,gscl_offset_x=0,gscl_offset_y=0");
                ...
                ####################################################导出分割资料####################################################
                # --输出备份分割层数据
                if ($clip_mode ne "不分割" and $rlayers[$i] =~ /^s(\d+)-(\d+)-?(\d+)?-?(\d+)?-?(\d+)?-?(\d+)?-?(\d+)?-?(\d+)?(-ls\d{6}\.\d{4})?$/) {
                    # my $clip_layer = $rlayers[$i] . "-fg";
                    my $clip_layer = $rlayer_work . "-fg";
				...
				my $head_log = "/Date:$local_time User:$incam_user Host:$ipname Job:$JOB Step:$step_name Layer:$rlayer_work Scale:X=$scale_x Y=$scale_y Anchor=$scale_position X/2=$scale_center_x  Y/2=$scale_center_y	Mirror_ver:$ver";
				...
				my $res = system("python $cur_script_dir/reWriteLaserFiles_new.py $output_path $JOB.$rlayer_work.write $send_five_size");				
				if ($res != 0){
					exit;
				}
				
				if ($string_3175){
					# 还原原始钻带孔径并将文件中的3.175大小改为org的大小
					$c->CLEAR_LAYER();
					$c->FILTER_RESET;
					$c->AFFECTED_LAYER("$rlayers[$i]", "yes");
					$c->COM("filter_atr_set,filter_name=popup,condition=yes,attribute=.string,text=3175");
					...
					if ($sel_count ) { 
						$c->COM("units,type=mm");
						$c->COM("sel_change_sym,symbol=$string_3175,reset_angle=no");
						$c->CLEAR_LAYER();
						$c->FILTER_RESET;
					}	
					
					system("python $cur_script_dir/resize_3175_drill.py $output_path $JOB.$rlayer_work.write $string_3175");	
										
				}
				
				
				# 镭射钻带回读自动比对 芯板镭射第2次输出不回读
				if ($nLoop == 1) {					
				$f->COM("set_subsystem,name=1-Up-Edit");
				# my $res = system("python $cur_script_dir/compare_laser_output_drill.py $JOB $step_name $rlayer_work $output_path $JOB.$rlayer_work.write");
				my $res = system("python $cur_script_dir/compare_laser_output_drill.py $JOB $step_name $rlayers[$i] $output_path $JOB.$rlayer_work.write");
				if ($res != 0){
					exit;
				  }
				}
				...
				# === 在此处增加钻带文件比对ERP
				if ($erp_laser =~ /^(s.*)-ls\d{6}\.\d{4}$/){
					$erp_laser = $1;
				}				
				
				# === 在此处增加钻带文件比对ERP
			    my $get_result = system("python $cur_script_dir/compare_drl_data-zltest.py $JOB $erp_laser $output_path $lr_laser $UserName $rlayer_work");
				if ($get_result != 0) {
...
sub clip_laser_layer{
	...
	# 单元内小于0.076mm  ,修改为审批放行  http://192.168.2.120:82/zentao/story-view-8934.html
	my @ps_laser_drill;
	my $min_laser = 74;
	foreach my $t(@symbol_lsit){
        $t =~ s/^r//;
        if ($t != 45 and $t < 74) {
			push (@ps_laser_drill, "r".$t);
			if ($t <  $min_laser){
				$min_laser = $t;
			}
        }
	...
	if (@ps_laser_drill) {
		my $mgr_symbol = join(",", @ps_laser_drill);
		my $show_mgs = $laser_layer."单元内存在下列孔径:".$mgr_symbol.",超出工艺能力,继续输出需要审批";	
		my $res = system("python /incam/server/site_data/scripts/ynh/no_gui/send_mgs_approval_information.py $JOB $show_mgs");	
		if ($res != 0) {
			exit 0;
		}
	}	
    ...
    foreach my $t(@symbol_lsit){
        $t =~ s/^r//;
        # --清理不需要孔(单元内0.074-0.203mm之间,超出不算入镭射钻咀  来源工艺规范周涌提供  2021.11.24)		
        if ($t != 520 and ($t < $min_laser or $t > 385)) {
...
sub chg_hole {
    ...
    #print $head_log;
    my @file_list = ();
	my $rlayer_work = $laser_layer;
	if ($laser_layer =~ /^(s.*-\d+)(-ls\d{6}\.\d{4})?$/){
		$rlayer_work = $1;
	}			
    my $source_file = "$output_path/$JOB.$rlayer_work";
    if (-e $source_file) {
	...
	our $des_file = "$output_path/$JOB.$rlayer_work.write";
        ...
        foreach $tmp (@t_list) {
            ...
            if (exists $t_chg{"$tmp"}) {
                ...
                #}elsif($real_t ne "T500" and $real_t ne "T03" and $real_t ne "T23") {
					...
					my $clip_layer_file = "$output_path/$JOB.$rlayer_work"."-fg";
					...
					system("python $cur_script_dir/calc_laser_fg_coordinate_new.py $des_file $clip_layer_file $five_times $laser_symbol_size_num{$laser_layer}");					
					close(SESAME);
	...
	if ( $clip_mode eq "不分割"){
		...
		system("python $cur_script_dir/calc_laser_only_five_times_coordinate_new.py $des_file $output_path/$JOB.$rlayer_work $five_times_str");				
	}
    
    if ($clip_mode ne "不分割") {
        # --设定分割资料回读文件  AresHe 2021.10.11
        # our $reread_file = "$output_path/$JOB.$laser_layer.write.reread";
		our $reread_file = "$output_path/$JOB.$rlayer_work.write.reread";      
...
sub getInplanData {
    ...
    while (my @recs = $sth->fetchrow_array) {
		...
		foreach my $sLy (keys %hash){
			if ($sLy =~/^(s\d+-\d+).*/){				
				if ($tmp_name eq $1 && $hash{$1}) {
				$hash{$sLy}[8] = $tmp_array[0];
				$hash{$sLy}[10] = $tmp_array[1];
				$hash{$sLy}[11] = $tmp_array[2];
				}
...
sub get_org_code{
	my $drc_job = uc(substr($JOB, 0,13));
	my $sql = "SELECT job,Org_Code FROM project_status.project_status_jobmanage WHERE job='$drc_job' and Org_code = '多层事业部'";
	my $sql_hdi = "SELECT job,Org_Code FROM project_status.project_status_jobmanage WHERE job='$drc_job' and Org_code = 'HDI事业部'";
	my $sth = $dbc_m->prepare($sql);#结果保存在$sth中
    $sth->execute() or die "无法执行SQL语句:$dbc_m->errstr";
	
	my $sth_hdi = $dbc_m->prepare($sql_hdi);#结果保存在$sth中
    $sth_hdi->execute() or die "无法执行SQL语句:$dbc_m->errstr";
	my @recs=$sth->fetchrow_array;
	my @recs_hdi=$sth_hdi->fetchrow_array;
...
2025.01.13  
作者：杨年虎   
1.加入匹配临时层的输出，临时层命名( 例如：s1-2-ls.250113.1625)；  http://192.168.2.120:82/zentao/story-view-8169.html
2.打5遍抓取MI排刀序；
3.5Mil以上孔径不打5遍都需要抓取MI选项是否添加绕烧指令。  
4.旧版模式输出启用 