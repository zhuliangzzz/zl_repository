...
sub RunScriptAct {
	...
	my @tParams = @_;
	our $tThread;
	system("python /incam/server/site_data/scripts/lyh/messageBox.py $MainConfig{GetDisplay}   "."%${CURRENTPIDNUM}\@hdilinux55.hdilinux55:30");
	$tThread = threads->create( sub {
		...
		system($MainConfig{GatewayPath},"%${CURRENTPIDNUM}@hdilinux55.hdilinux55:30","COM script_run,name=$ScripFile,env1=JOB=$ShareJob,env2=STEP=$ShareStep,params=@tParams");
	} );
