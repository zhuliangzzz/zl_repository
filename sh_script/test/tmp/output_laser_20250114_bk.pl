...
for (my $i = 0; $i <= $#rlayers; $i++) {
        ...
        if ($_ eq $pnlspecial[0] || $_ eq $pnlspecial[1] || $_ eq $pnlspecial[2] || $_ eq $pnlspecial[3] || $_ eq $pnlspecial[4]) {
			...
			if ( ($drillSize - 203 <=5.01 and $drillSize - 202 > 0) || ($drillSize - 254 <=5.01 and $drillSize - 253 > 0) || ($drillSize - 233 <=5.01 and $drillSize - 228 > 0) || ($drillSize - 245 <=5.01 and $drillSize - 240 > 0) || ($drillSize - 220 <=5.01 and $drillSize - 215 > 0)){		
				if ( substr($JOB,1,3) eq 'a86' || substr($JOB,1,3) eq 'd10'){					
					...
					if ((substr($JOB,1,10) eq 'd1024e7213' && $rlayers[$i] =~ /s7-8|s18-17/ && (($drillSize - 203 <=5.01 and $drillSize - 202 > 0) || ($drillSize - 254 <=5.01 and $drillSize - 253 > 0)))
					  || (substr($JOB,1,10) eq 'd1022e6261' && $rlayers[$i] =~ /s6-7-8|s17-16-15/ && ($drillSize - 254 <=5.01 and $drillSize - 253 > 0))
					){						
						# d1024e7213 按王总指示特定料号8mil孔不打5遍，2024-12-20
					if (substr($JOB,1,10) eq 'd1024e7213'){
						$c->Messages("info", "$rlayers[$i] 层按规定8mil不打5遍也不绕烧！");
					}else{
						$c->Messages("info", "$rlayers[$i] 层按规定10mil不打5遍要绕烧！");
					}
						
					}else{
						#$layer_cool_speed_info{"$rlayers[$i]"}{$drillSize} = "yes";					
	...
	if ($exist_8mil eq 'yes' && $laser_drill_five_times_info{$rlayers[$i]} ne "yes") {
		# 获取承接层铜厚
...
sub apply_ {
    ...
    for (my $a = 0; $a <= $#rlayers; $a++) {
		...
		if ($layer_8mil_info{$rlayers[$a]}{'mgs'} eq 'mgs1' ){
			my $mw1 = MainWindow->new(
								-title      => "镭射输出",
								-background => '#9BDBDB');
			my $ans = $mw1->messageBox(-icon => 'question',-message => "$rlayers[$a] 8-9mil孔承接层铜厚小于1OZ，按工艺规范需要打5遍，继续输出必须确认绕烧参数是否添加T508,不添加T508需要在文件里面手动删除!，点击NO取消输出",-title => 'quit',-type => 'YesNo');
			if ($ans ne "Yes"){
			return;
			}
		}
		# elsif($layer_8mil_info{$rlayers[$a]}{'mgs'} eq 'mgs2'){
			# $c->Messages("info", "$rlayers[$i] 8-9mil孔承接层铜厚大于1OZ，工艺规范需要绕烧，MI打5遍,请和MI确认！");
			# return;
		# }
    ...
    for (my $i = 0; $i <= $#gSRstep; $i++) {
        if ($gSRstep[$i] =~ /^set/ || $gSRstep[$i] =~ /^edit/ || $gSRstep[$i] =~ /pnl-set/) {
...
sub chg_hole {
	...
	if ( $laser_drill_five_times_info{$laser_layer} eq "yes" && $clip_mode eq "不分割"){
		
		#if ( $#hole_8mil_suffix <= 1 and $#hole_10mil_suffix <= 1)
		#{
				#&modify_laser_five_times($laser_layer, $head_log, $meter_data, $des_file);
				system("python $cur_script_dir/calc_laser_only_five_times_coordinate.py $des_file $output_path/$JOB.$laser_layer yes none");
		#}		
	}
    ...
    while (my @recs = $sth->fetchrow_array) {
        ...
        if (abs($recs[3] - $recs[4]) == 3) {
			...
			if ($recs[3] > $recs[4]) {
				...
				push @couple_layers_one, "s" . "$recs[3]" . "-" . "$middle_num";
				push @couple_layers_one1, "s" . "$recs[3]" . "-" . "$middle_num1";
