...
class auto_check_rule(object):
    ...
    def check_record_week_picihao_to_mysql(self, *args):
        ...
        if not data_info: 
            ...
            if check_status == "error" and "-lyh" not in job.name:                
                # if os.environ.get("SHOW_UI", "yes") == "yes":
                if "SHOW_UI" in args:
                    showMessageInfo(u"检测到资料 CAM资料数据记录程序 有数据未审核，请通知CAM审核后再输出资料！")
                    exit(1)
                else:
                    return "error", [u"检测到资料 CAM资料数据记录程序 有数据未审核，请通知CAM审核后再输出资料！"]
                        ...
                        if check_type in args:                    
                            ...
                            for key, vaules in dic_tgz_week_layers.iteritems():
                                ...
                                if check_type == "signal":                                    
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "1" and info[2] == key:
                                            calc_num = int(info[3])                                       
                                if check_type == "solder_mask":
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "1" and info[4] == key:
                                            calc_num = int(info[5])
                                if check_type == "silk_screen":
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "1" and info[6] == key:
                                            calc_num = int(info[7])
                                
                                if num != calc_num:
                                    name = [info for info in value if info[0] == "1"][0][1]
                                    log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                        ...
                        if check_type in args:                     
                            ...
                            for key, vaules in dic_tgz_barcode_week_layers.iteritems():
                                ...
                                if check_type == "signal":                                    
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "2" and info[2] == key:
                                            calc_num = int(info[3])                                        
                                if check_type == "solder_mask":
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "2" and info[4] == key:
                                            calc_num = int(info[5])
                                if check_type == "silk_screen":
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "2" and info[6] == key:
                                            calc_num = int(info[7])
                                
                                if num != calc_num:
                                    name = [info for info in value if info[0] == "2"][0][1]
                                    log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                                    if log.format(key, num, calc_num, name) not in arraylist_log:                                        
                                        arraylist_log.append("2"+log.format(key, num, calc_num, name))                   
                        ...
                        if check_type in args:                    
                            ...
                            for key, vaules in dic_tgz_picihao_layers.iteritems():
                                ...
                                if check_type == "signal":                                    
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "3" and info[2] == key:
                                            calc_num = int(info[3])                                        
                                if check_type == "solder_mask":
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "3" and info[4] == key:
                                            calc_num = int(info[5])
                                if check_type == "silk_screen":
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "3" and info[6] == key:
                                            calc_num = int(info[7])
                                
                                if num != calc_num:
                                    name = [info for info in value if info[0] == "3"][0][1]
                                    log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                                    if log.format(key, num, calc_num, name) not in arraylist_log:                                        
                                        arraylist_log.append("2"+log.format(key, num, calc_num, name))                          
                        ...
                        if check_type in args:                     
                            ...
                            for key, vaules in dic_tgz_banci_layers.iteritems():
                                ...
                                if check_type == "signal":                                    
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "4" and info[2] == key:
                                            calc_num = int(info[3])                                        
                                if check_type == "solder_mask":
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "4" and info[4] == key:
                                            calc_num = int(info[5])
                                if check_type == "silk_screen":
                                    calc_num = 0
                                    for info in value:
                                        if info[0] == "4" and info[6] == key:
                                            calc_num = int(info[7])
                                
                                if num != calc_num:
                                    name = [info for info in value if info[0] == "4"][0][1]
                                    log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                                    if log.format(key, num, calc_num, name) not in arraylist_log:                                        
                                        arraylist_log.append("2"+log.format(key, num, calc_num, name))                          
    ...
    def check_gold_finger_elic_line_info(self):
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            step.COM("units,type=mm")
            for layer in matrixinfo["gROWname"]:
                if "_flt" in layer or "_finger" in layer or "_auto" in layer:
                    step.removeLayer(layer)
                    ...
                    if step.isLayer(etch_layer):
                        ...
                        step.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                 ...
                                 "break_to_islands=yes".format(sig_layer+"_finger_line_orig",
                                                               ...
                                                               re_size*25.4*-1))
                        step.resetFilter()
                        step.refSelectFilter(etch_layer+"_flt", mode="disjoint")
                        if step.featureSelected():
                            step.selectDelete()
                        ...
                        if step.featureSelected():
                            ...
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, sig_layer+"_finger_line_orig",
                                                                                 [sig_layer, etch_layer],log, dic_zu_layer,
                                                                                 add_note_layer=sig_layer)                        
                else:
                    # 检测是否内拉手指引线
                    if ksz_process:
            ...
            for stepname in list(set(setsteps)):
                ...
                for worklayer in checkLayers:                    
                    ...
                    if worklayer.startswith("c1"):
                        ...
                        mianci = "BOT"
                    for obj in pad_obj:
                        if (re.match("barcode\d+x\d+", obj.symbol) and \
    ...
    def check_inner_barcode_is_right(self, tgz_file_path=None):
        ...
        if tgz_file_path is not None:
            ...
            try:
                ...
                if tgz_contain_jobname and list(set(tgz_contain_jobname))[0].lower() != job.name:
                # if "-lyh" in job.name:                    
                    log = u"检测到输出的tgz压包内的资料文件名{0}跟当前型号名{1}不一致，程序将自动修复，请在windows电脑将tgz文件打开检测是否已修复OK！"
                    arraylist.append(log.format(list(set(tgz_contain_jobname))[0].lower(), job.name))
