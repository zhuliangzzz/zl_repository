...
class DrillReadBackComparison(QWidget, ui.Ui_Form):
    ...
    def set_path(self):
        ...
        if sys.platform == "win32":
            self.get_para_path = ur"\\192.168.2.174\GCfiles\Program\工程辅助资料\{0}系列\{1}".format(self.jobname[1:4],self.jobname.split('-')[0])
        else:
            self.get_para_path = u"/windows/174.file/Program/工程辅助资料/{0}系列/{1}".format(self.jobname[1:4],self.jobname.split('-')[0])
