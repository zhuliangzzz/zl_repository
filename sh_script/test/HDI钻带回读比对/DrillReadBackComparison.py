...
class DrillReadBackComparison(QWidget, ui.Ui_Form):
    ...
    def run(self):
        ...
        if self.err_info:
            ...
            QMessageBox.information(self, u'提示', u'比对通过!', QMessageBox.Ok)
        sys.exit(0)
    ...
    def check_file(self, file_path, type=1):
        if type:
            ...
            if os.path.exists(file_path):
                g82_g83 = re.compile('(G82|G83)(X(-?)(\d+)Y(-?)(\d+))')
                delete_tool = re.compile('^T(\d+)')
                delete_line = re.compile('(M97|M98),\$')
                change_tool = re.compile('\w')
                orig_tool = re.compile('T21')
                per_symbol = re.compile('%')
        ...
        if self.radioButton_machinery.isChecked():
            ...
            if layer_name[0]:
                if self.step.isLayer(layer_name[0]) and self.step.isLayer(self.file_bak):
                    layer_bak = 'new_bak_' + layer_name[0]
                    # 1.合并dld 2.r520->r102
                    dld = 'dld%s' % layer_name[0].replace('s', '')
                    dld_tmp = '%s+++%s' % (dld, self.step.pid)
                    if self.step.isLayer(dld):
                        self.step.affect(dld)
                        self.step.copySel(dld_tmp)
                        self.step.unaffectAll()
                        self.step.affect(dld_tmp)
                        self.step.selectChange('r45')
                        self.step.unaffectAll()
                    map_layer = layer_name[0] + '_reread_compare'
                    self.step.COM('flatten_layer,source_layer=%s,target_layer=%s' % (layer_name[0], layer_bak))
                    self.step.affect(dld_tmp)
                    self.step.copySel(layer_bak)
                    self.step.unaffectAll()
                    self.step.affect(layer_bak)
                    self.step.selectSymbol('r520')
                    self.step.resetFilter()
                    if self.step.Selected_count():
                        self.step.selectChange('r102')
                    self.step.unaffectAll()
                    self.step.removeLayer(dld_tmp)
                    self.step.COM(
