...
class WINDOWS():
    ...
    def selete_file(self):
        self.find_file = QtGui.QFileDialog.getOpenFileNames(self.Form, QtCore.QString(u"选择镭射分割文件"),QtCore.QString(self.ui.lineEdit.text()),QtCore.QString(u"All(*.*);;分割镭射文件(*.write);;Images(*.png *.jpg);;txt文件(*.txt)"))
        # --系统自行过滤分割镭射资料
        file = re.compile('.write$')
    ...
    def input_file(self):
        ...
        if "Mirror_ver:7" in "".join(lines):
            ...
            if sys.platform == "win32":                
                ...
                self.GEN.COM("sel_transform,oper=rotate\;mirror,x_anchor=0,y_anchor=0,angle=180,\
    ...
    def auto_compare(self):
        ...
        if layer_name[0]:
            if self.GEN.LAYER_EXISTS(layer_name[0]) == 'yes' and self.GEN.LAYER_EXISTS(self.file_bak) == 'yes':
                ...
                self.GEN.COM('compare_layers,layer1=%s,job2=%s,step2=%s,layer2=%s,layer2_ext=,tol=1,area=global,consider_sr=yes,ignore_attr=,map_layer=%s,map_layer_res=200'%(layer_bak,self.JOB,self.STEP,self.file_bak,map_layer))
