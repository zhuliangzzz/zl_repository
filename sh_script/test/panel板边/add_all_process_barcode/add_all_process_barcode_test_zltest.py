...
"""
# 20250121 zl 外层线路二维码.bit=trace_barcode位置加一块负性surface http://192.168.2.120:82/zentao/story-view-8140.html
"""
...
from genesisPackages import get_profile_limits,get_sr_area_for_step_include, \
     ...
     innersignalLayers, solderMaskLayers, bd_drillLayer, tongkongDrillLayer, mai_drill_layers,mai_man_drill_layers
...
def check_exists_barcode_symbol(step):
    ...
    if step.featureSelected():
        ...
        step.clearAll()
        for layer in signalLayers + solderMaskLayers + ["md1", "md2"] + ['sgt-c', 'sgt-s'] :
...
def add_barcode_area(jobname, stepname):
    ...
    for dic_info in sorted(array_mrp_info, key=lambda x: x["PROCESS_NUM"]):
        ...
        if top_layer is None or bot_layer is None:
            continue
    ...
    if dic_sec_zu:
        ...
        dic_position[next_position_num] = ("s9000", symbol_width / 2.0, symbol_length - 10.5 * 0.5, "bar_code_sec1")
    ...
    if dic_outer_zu:
        ...
        # 20250121 zl 外层二维码位置加一块负性surface 距二维码顶部11.7mm
        if re.search('.+a86.{4}(826|784|804|694|791|798).+|.+d10.{4}213.+' ,jobname):
            next_position_num = next_position_num + 1
            dic_position[next_position_num] = ("s10500", symbol_width / 2.0, symbol_length - 11.7, "")
            # dic_position[next_position_num] = ("s10500", symbol_width / 2.0, symbol_length/2.0 + 2.5, "bar_code_out")
            dic_negative_layer[next_position_num] = outsignalLayers
            for key in sorted(dic_outer_zu.keys()):
                dic_outer_zu[key][0][5] = next_position_num
    ...
    checklayer_list = signalLayers + tongkongDrillLayer + mai_drill_layers + bd_drillLayer + mai_man_drill_layers
    if step.isLayer('2nd'):
        checklayer_list.append('2nd')
    if step.isLayer('3nd'):
        checklayer_list.append('3nd')
    area_x = [sr_xmin - 1 if sr_xmin - 1 <= 25 else 25, f_xmin + out_lb_x, -0.25]
        ...
        for layer in dic_negative_layer[key]:
            if layer not in core_bot_layers:
                if value[-1]:
                    ...
                    dic_zu_layername[layer].append([(add_x - 10.5 * 0.5, add_y - symbol_length / 2.0),
                                                    (add_x + 10.5 * 0.5, add_y - symbol_length / 2.0 + value[2]),
                                                    "s10510",  # 负片设计多加10um
                                                    {"polarity": "negative",
                                                     "addRectangle": ".bit,text=trace_barcode"}])
                if coordinate_mode == u"斜对称转换": 
                    dic_zu_layername[layer].append([convert_x1,
                                                    convert_y1,
                                                    value[0],
    ...
    #彭课通知 供应商说全流程追溯码要档油和开窗   帮忙处理下 20231201 by lyh
    worklayer = outsignalLayers[0]
    step.clearAll()
    step.affect(worklayer)
    step.resetFilter()
    ...
    step.selectAll()    
    if step.featureSelected():
        step.removeLayer("barcode_open")
        step.copyToLayer("barcode_open")
        step.clearAll()
        step.affect("barcode_open")
        step.COM("sel_resize,size=500,corner_ctl=no")
        # for layer in ["md1", "md2"]:
        #     step.copySel(layer)
        # 周涌通知阻焊层档点网选化层要套开 20240516 ynh http://192.168.2.120:82/zentao/story-view-6873.html
        for slayer in solderMaskLayers + ['sgt-c', 'sgt-s'] + ["md1", "md2"]:
            if step.isLayer(slayer):
                step.copySel(slayer, invert='yes')
...
def uploading_barcode_coordinate_info(jobname,step, x_center, y_center, out_lb_x, out_lb_y, out_lb_sec_x, out_lb_sec_y):
    ...
    if step.featureSelected():
        ...
        for obj in featout:
            if dic_attr.get(obj.string, None):                
                if obj.string == "bar_code_out":
                    ...
                    # 20230911 跟宇之光厂家沟通 内层要用两个坐标 计算逻辑一样 外层次外层公共最后一个坐标
        ...
        # showMessageInfo(u"添加完成111，上传MES坐标数据如下:", **dic_zu)
