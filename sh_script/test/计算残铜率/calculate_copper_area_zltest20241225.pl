...
sub appy {
	...
	if ($board_thick eq "" || $count_gold == 0 && $count_copper == 0 && $count_elec == 0 ) {
		...
		system("python /incam/server/site_data/scripts/sh_script/uploading_copper_usage/uploading_copper_usage_zltest.py");
		$f->DO_INFO("-t layer -e $job_name/panel/m1_calc_expose_tmp -d EXISTS");
		if($f->{doinfo}{gEXISTS} eq "yes"){
		...
		if ($count_gold == 1) {
			...
			$f->DO_INFO("-t layer -e $job_name/panel/$exposed_drill -d EXISTS");
