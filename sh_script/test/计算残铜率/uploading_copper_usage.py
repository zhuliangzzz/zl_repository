...
def flipStepName( step, stepname, flipList = []):
    """
    ...
    for (name, flip_yn) in step_flip_tuple:
        if flip_yn == 'yes':
            ...
            flipList = flipStepName(step, name, flipList)
    # --返回
    return flipList
    
def create_mask_layer_condition(step, is_flip):
...
if "panel" in matrixInfo["gCOLstep_name"]:
    ...
    step.open()
    flip_steps = []
    flip_steps = flipStepName(step, "panel", flip_steps)
    if flip_steps:
        get_pnl_steps = []
    ...
    for stepname in list(set(get_pnl_steps)) + ["panel"]:
        ...
        if user not in ["70928", "50313", "68233", "72616", "72451", "69615", "system"]:
            create_mask_layer_condition(step, flip_steps)    
