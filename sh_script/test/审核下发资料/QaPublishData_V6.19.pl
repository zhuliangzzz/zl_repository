...
sub getHandOutPath { #获取通用路径及变量 # my %tPath = &getHandOutPath("$tJob"); 
    ...
    if ( defined $tJob ){
        ...
        # 大族镭射资料 dazuLaser
        $tPath{'Laser'}                       = $tPath{'Program'}                    . '/' . 'Laser';
        $tPath{'dazuLaser'}                   = $tPath{'Laser'}                 . '/' . '大族UV laser';
        $tPath{'dazuLaserCustomer'}           = $tPath{'dazuLaser'}                 . '/' . $tPath{'vCustomerCode'}     . '系列';
        $tPath{'dazuLaserJob'}                = $tPath{'dazuLaserCustomer'}         . '/' . $tPath{'vuJob'};
