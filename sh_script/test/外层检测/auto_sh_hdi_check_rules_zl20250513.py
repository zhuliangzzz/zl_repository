...
class auto_check_rule(object):
    ...
    def check_serial_number(self, autocheck=None):
        ...
        data_info = conn.select_dict(sql.format(jobname.split("-")[0].upper()), is_arraysize=False)
        if not data_info:
            arraylist.append(u"success 没有添加序号的数据")
        data = data_info[0]
        if data['CI_ADD_POSITION_CODE_'] == 0:
            arraylist.append(u"success 不加序列号，不检测")
        step = gen.Step(job, stepname)
        # set数量
        # set_repeat = step.info.get('gREPEATstep').count('set')
        set_list = list(filter(lambda x: 'set' in x, step.info.get('gREPEATstep')))
        set_repeat = len(set_list)
        set_list = set(set_list)
        for setname in set_list:
            get_sr_area_flatten("surface_fill_edit", setname, include_sr_step='edit')
        step.PAUSE(set_list.__str__())
        check_layers = outsignalLayers + silkscreenLayers + solderMaskLayers
        step.initStep()
        for check_layer in check_layers:
        ...
        if data['CI_ADD_POSITION_CODE_'] == 3:
            get_sr_area_flatten("surface_fill", include_sr_step=set_list)
            # 检测对应是否有加pcs序列号
            pcs_serial_layers = []
            for setname in set_list:
            ...
            if pcs_serial_layers:
                serial_layers = []
                step.initStep()
                for check_layer in set(pcs_serial_layers):
                    serial_layer = 'serial_%s+++' % check_layer
                    step.affect(check_layer)
                    ...
                    if step.Selected_count():
                        ...
                        if nums:
                            ...
                            for n in nums:
                                if n not in range(1, set_repeat + 1):
                                    log = u"检测到%s中%s超出set序列号范围" % (check_layer, n)
                                    arraylist.append(log)
                                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
