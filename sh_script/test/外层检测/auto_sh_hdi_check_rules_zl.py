...
class auto_check_rule(object):
                                    ...
                                    for i in range(1, pcs_repeat + 1):
                                        if nums.count(i) > 1:
                                            ...
                                            for idex, num in enumerate(nums):
                                                if num == i:
                                                    feat_indexs.append(idex)
                                            for idex in feat_indexs:
                                                set_step.selectByIndex(check_layer, indexs[idex])
                                            log = u"检测到%s多加pcs序列号%s" % (check_layer, i)
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, set_step.name, set_step,
                                            ...
                                            if nums.count(i) > pcs_repeat:
                                                ...
                                                for idex, num in enumerate(nums):
                                                    if num == i:
                                                        feat_indexs.append(idex)
                                                for idex in feat_indexs:
                                                    step.selectByIndex(check_layer, indexs[idex])
    ...
    def check_selective_window(self, autocheck=None):
        ...
        if not sgt_lays:
            return u'success 没有层sgt-c/sgt-s', None
        profile_fill = 'profile_fill+++'
        step.removeLayer(profile_fill)
        step.prof_to_rout(profile_fill)
        step.affect(profile_fill)
        step.selectCutData()
        step.unaffectAll()
        step.removeLayer('%s+++' % profile_fill)
        for sgt_lay in sgt_lays:
            res_lays, rm_lays = [], []
            tmp = '%s+++%s' % (sgt_lay, step.pid)
            step.affect(profile_fill)
            step.copySel(tmp)
            step.unaffectAll()
            step.affect(sgt_lay)
            step.copyToLayer(tmp, 'yes')
            step.unaffectAll()
            step.affect(tmp)
            step.Contourize()
            step.unaffectAll()
            if step.isLayer('hj-pad'):
                tmp_hj = 'hj-pad+++%s' % sgt_lay
                step.removeLayer(tmp_hj)
                step.affect('hj-pad')
                step.copyToLayer(tmp_hj, size=152.4)
                step.unaffectAll()
                step.affect(tmp_hj)
                step.refSelectFilter(sgt_lay, mode='cover')
                if step.Selected_count():
                    step.selectDelete()
                step.selectReverse()
                if step.Selected_count():
                    ...
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, tmp_hj,
                                                                         sgt_lay, log, dic_zu_layer)
                    # step.selectResize(-152.4)
                    res_lays.append(tmp_hj)
                else:
                    rm_lays.append(tmp_hj)
                step.unaffectAll()
            if step.isLayer('osp-pad'):
                tmp_osp = 'osp-pad+++%s' % sgt_lay
                step.removeLayer(tmp_osp)
                step.affect('osp-pad')
                step.copyToLayer(tmp_osp, size=355.6)
                step.unaffectAll()
                step.affect(tmp_osp)
                ...
                step.selectReverse()
                if step.Selected_count():
                    log = u"检测到osp-pad开窗补偿不足7mil"
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, tmp_osp,
                                                                         sgt_lay, log, dic_zu_layer)
                    # step.selectResize(-355.6)
                    res_lays.append(tmp_osp)
                else:
                    rm_lays.append(tmp_osp)
                step.unaffectAll()
            step.removeLayer(tmp)
            for lay in rm_lays:
                step.removeLayer(lay)
            if res_lays:
                for res_lay in list(set(res_lays)):
                    size = -152.4 if res_lay.startswith('hj-pad') else -355.6
                    step.affect(res_lay)
                    step.selectResize(size)
                    step.unaffectAll()
        step.removeLayer(profile_fill)
        if step.isLayer('hj-pad') and step.isLayer('osp-pad'):
            tmp_hj = 'hj-pad+++'
            tmp_osp = 'osp-pad+++'
            step.affect('hj-pad')
            step.copyToLayer(tmp_hj, size=152.4)
            ...
            if step.Selected_count():
                log = u"检测到化金和osp pad间距不足10mil"
                arraylist.append(log)
                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, tmp_hj,
                                                                     'osp-pad', log, dic_zu_layer)
                step.selectResize(-152.4)
            else:
                step.unaffectAll()
                step.removeLayer(tmp_hj)
