...
class auto_check_rule(object):
    ...
    def check_feature_outside_of_pnlrout(self, autocheck=None):
        ...
        inns = list(filter(lambda drill_lay: re.match('inn(\d{2,}|-pp)?$', drill_lay), drill_lays))
        # print('aaa', pnlrout_dic)
        pnlrout_drls = {}
        if list(pnlrout_dic.keys()):
    ...
    def check_lot_rotation(self, autocheck=None):
                ...
                if s < e and s <= halfjob_numb or (s > e >= halfjob_numb):
                    if s < e and s < halfjob_numb:
                        ...
                        if s == lay_num:
                            r515CheckData[rlayer]['mir'] = mir_choose
                        if s > lay_num:
                            log = '%s镭射层钻带的起始层超出层别数量,请检查' % rlayer
                            arraylist.append(log)
                elif re.search('s(\d+)-(\d+)-(\d+)$', rlayer):
                    pass
                else:
                    log = '%s镭射层钻带的起始层定义不正确,请检查' % rlayer
                    arraylist.append(log)
            else:
                log = '%s镭射层别非最新命名，应为s开始层-结束层，中间有横杠,请检查' % rlayer
                arraylist.append(log)
        ...
        def checkSRtableContinue():
            ...
            if 'set' in job.getSteps():
                ...
                add_lot_step = 'edit'
            for i in range(info.get('gSRstep')):
        ...
        def judgeM97orM98Direct(chkLotLayer, getfirstSetInfo, getCheckSameCenter):
            ...
            if laserLotInfo[0].get('mirror') == 'yes':
                ...
                if pnlLotAngle == 180:
                    ...
                    log = '层别%s 在pnl中的lot角度为%s,仅支持180度90度' % (chkLotLayer, pnlLotAngle)
                    arraylist.append(log)
                    return -1
            else:
                ...
                if pnlLotAngle == 0:
                    ...
                    log = '层别%s 在pnl中的lot角度为%s,仅支持0度270度' % (chkLotLayer, pnlLotAngle)
                    arraylist.append(log)
                    return -1
            return lotCode
        ...
        def mainCheckLaserLot(chkLotLyr):
            ...
            if getLotDirec == 'laserNameError':
                log = '镭射层%s 命名不正确,Code:101,程序退出' % chkLotLyr
                arraylist.append(log)
                return -1
            return getLotDirec
        #
