...
class auto_check_rule(object):
    ...
    def check_silk_screen(self, autocheck=None):
        ...
        for ss in ss_layers:
            ...
            panel_step.removeLayer('%s+++_flat' % ss)
            panel_step.removeLayer('%s+++2_flat' % ss)
        for stepname in get_pnl_steps:
            ...
            for ss in ss_layers:
                ...
                step.resetFilter()
                if step.Selected_count():
                    log = u"检测到文字层{0}在{1}有开窗，请到{2}中的{3}查看位置！".format(ss, sm, stepname, tmp_ss)
                    arraylist.append(log)
                ...
                step.resetFilter()
                if step.Selected_count():
                    log = u"检测到文字层{0}距{1}有间距小于5mil，请到{2}中的{3}查看位置！".format(ss, sm, stepname,tmp_ss2)
                    arraylist.append(log)
        ...
        for stepname in ['panel']:
            ...
            for ss in ss_layers:
                ...
                step.resetFilter()
                if step.Selected_count():
                    log = u"检测到文字层{0}距{1}有间距小于5mil，请到{2}中的{3}查看位置！".format(ss, sm, stepname,
                                                                                               tmp_ss2)
                    arraylist.append(log)
