...
class auto_check_rule(object):
    ...
    def check_silk_screen(self, autocheck=None):
        ...
        for ss in ss_layers:
            ...
            tmp_sm = '%s+++' % sm
            tmp_sm2 = '%s+++2' % sm
            flat_signal = '%s+++flat' % signal
            step.removeLayer(show_ss)
            step.removeLayer(tmp_ss)
            step.removeLayer(flat_signal)
            step.Flatten(ss, tmp_ss)
            step.Flatten(sm, tmp_sm)
            step.Flatten(signal, flat_signal)
            #
            step.affect(tmp_sm)
            ...
            layerObj = gen.Layer(step, tmp_ss)
            ...
            if index_list1:
                for i in index_list1:
                    step.selectByIndex(tmp_ss, i)
                log = u"检测到字宽小于16mil,请检查{0}文字层{1}".format(stepname, ss)
                arraylist.append(log)
                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, tmp_ss, tmp_ss, log,
                                                                  dic_zu_layer)
                step.clearSel()
            if index_list2:
            ...
            if step.Selected_count():
                ...
                if index_list1:
                    ...
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, tmp_ss, ss, log,
                                                                         dic_zu_layer)
                    step.clearSel()
                if index_list2:
                    for i in index_list2:
