...
# 20241216 zl check_ks_pad_exists 检测控深pad是否添加
# 20250113 zl check_d_coupon_serial_number d-coupon序列号检测
# 20250123 zl 优化 check_d_coupon_serial_number
# 20250206 zl 优化 check_md_cover_info
# 20250210 zl 优化 check_offset_et_rj_fbk 把显示层改为加标记
# 20250227 zl check_ul_mark 检测ul标记是否添加
# 20250301 zl check_ccd_alignment 蚀刻引线层对位点设计检测
# 20250304 zl check_loss_test_pad 检测测试pad宽度
# 20250305 zl check_drill_through 检测钻带拉伸是否正确
# 20250317 zl 优化 check_silk_screen 在panel中flatten去检测
# 20250318 zl check_impedance_line_shielding LOSS条和阻抗条上阻抗线屏蔽层检查
# 20250326 zl check_customer_jobname 检测客户型号是否正确
# 20250402 zl check_panel_rout_size 检测panel锣边尺寸 cam和mi尺寸是否一致
# 20250415 zl check_hole_in_dq_layers  检测导气孔径是否正确
# 20250422 zl check_exist_d_coupon_and_test_coupon A86/D10客户要固定加D-COUPON和拉力测试coupon(不加）
# 20250423 zl check_serial_number 序号检测
# 20250424 zl check_bars_in_area 检测五代靶、DLD、靶标在板边有效范围内
# 20250505 zl check_open_window_of_secondary_sm 检测二次防焊资料针对动态变量（实际生产有变动）位置做开通窗
# 20250512 zl check_feature_outside_of_pnlrout 检测锣边层对应的钻孔层是否超出锣边层范围
# 20250521 zl check_lot_rotation 内检测：lot号旋转检测
# 20250526 zl check_optical_and_damaged_point 光学点、坏板点设计检测
# 20250609 zl check_npth_dot_2_cu 检测NPTH孔档点距离未开窗的铜的距离最小满足10mil，不足报警
# 20250616 zl check_distance_between_targethole_and_drillhole 检测靶孔距离其他孔间距
# 20250617 zl check_selective_window 选化开窗检测
"""
...
import json
import tarfile
from genesisPackages_zl import (get_mai_drill_hole, \
     ...
     tongkongDrillLayer, mai_drill_layers, ksz_layers, bd_drillLayer,\
     inner_sz_layers, outer_sz_layers, lp_sm_layers, get_profile_limits, \
     get_sr_area_for_step_include, signalLayers, get_sr_limits, \
     get_panel_features_in_set_position, calcExposeArea, \
     mai_man_drill_layers, get_symbol_area, get_layer_limits)
...
from get_erp_job_info import get_face_tech_params, \
     ...
     get_wip_jobs, getBackDrillDepth
...
class auto_check_rule(object):
    ...
    def check_compare_layers(self):
        """
        http://192.168.2.120:82/zentao/story-view-7443.html
        :return:
        """
        stepname = "edit"
        if stepname not in matrixinfo["gCOLstep_name"]:
    ...
    def check_hole_open_window(self, autocheck=None):
        """
        http://192.168.2.120:82/zentao/story-view-7706.html
        :param autocheck:
        :return:
        """
        ...
        if check_flag and len(sgt_layers) < 2:
            return u"检测到没有选化层sgt-c/sgt-s", None
        if 'drl' not in rows:
            return u"success 没有孔层drl", None
        arraylist = []
        dic_zu_layer = {}
        # 清理层
            ...
            if feat_indexes:
                ...
                for sgt in sgt_layers:
                    ...
                    if step.Selected_count():
                        ...
                        log = u"检测到大于等于1mm的pth孔在{0}没有开窗，请到{1}中的{2}查看位置！".format(sgt, stepname,
                                                                                      show_sgt)
                        arraylist.append(log)
                        dic_zu_layer = self.get_view_dic_layers(job.name, stepname, step,
    ...
    def check_silk_screen(self, autocheck=None):
        """
        http://192.168.2.120:82/zentao/story-view-7448.html
        ...
        if 'panel' not in job.steps.keys():
            return u"success 没有要检查的panel", None
        ss_layers = job.matrix.returnRows('board', 'silk_screen')
        if not ss_layers:
            return u"success 没有要检查的文字层", None
        get_pnl_steps = get_panelset_sr_step(jobname=job.name, panelset='panel')
        m1_list = list(filter(lambda l: re.match('m1(-\d)?$', l), solderMaskLayers))
        m2_list = list(filter(lambda l: re.match('m2(-\d)?$', l), solderMaskLayers))
        # ss_layers = job.matrix.returnRows('board', 'silk_screen')
        # if not ss_layers:
        ...
        result_lays = []
        dic_zu_layer = {}
        job.matrix.deleteRow('surface_fill')
        get_sr_area_flatten("surface_fill")
        # 清理层
        panel_step = gen.Step(job, 'panel')
        panel_step.initStep()
        for sm in m1_list + m2_list:
            panel_step.removeLayer('%s+++' % sm)
            panel_step.removeLayer('%s+++2' % sm)
        for ss in ss_layers:
            panel_step.removeLayer('%s+++' % ss)
            panel_step.removeLayer('%s+++2' % ss)
            panel_step.removeLayer('%s+++check_silk_screen' % ss)
            panel_step.removeLayer('%s+++2check_silk_screen' % ss)
            panel_step.removeLayer('%s+++check_silk_screen_flat' % ss)
            panel_step.removeLayer('%s+++2check_silk_screen_flat' % ss)
        for stepname in get_pnl_steps:
            step = gen.Step(job, stepname)
            step.initStep()
            for ss in ss_layers:
                layerObj = gen.Layer(step, ss)
                smlist = m1_list if ss[1] == '1' else m2_list
                ...
                for sm in smlist:
                    ...
                    if step.Selected_count():
                        step.selectDelete()
                    step.Contourize()
                    step.copyToLayer(tmp_sm2) # 254->228.6  20250417 5mil改为4.5mil 提出人：周涌
                    step.unaffectAll()
                    step.affect(tmp_sm2)
                    step.selectResize(228.6, 'yes') # 254->228.6  20250417 5mil改为4.5mil 提出人：周涌
                    step.unaffectAll()
                    step.affect(tmp_ss)
                    step.selectSymbol('wzfd-*')
                    step.resetFilter()
                    ...
                    step.setAttrFilter_pro('.geometry',text='wzfd-both1')
                    ...
                    if step.Selected_count():
                        step.selectDelete()
                    step.Contourize()
                    step.copySel(tmp_ss2)
                    step.refSelectFilter(tmp_sm)
                    step.resetFilter()
                    if step.Selected_count():
                        log = u"检测到文字层{0}在{1}有开窗，请到{2}查看位置！".format(ss, sm, tmp_ss)
                        arraylist.append(log)
                        result_lays.append(tmp_ss)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, tmp_ss, sm, log, dic_zu_layer)
                    step.unaffectAll()
                    step.removeLayer(tmp_sm)
                    # 20250320 zl 检测文字距防焊间距是否5mil以上
                    step.affect(tmp_ss2)
                    # step.setFilterTypes('text|line|arc')
                    step.refSelectFilter(tmp_sm2)
                    step.resetFilter()
                    if step.Selected_count():
                        log = u"检测到文字层{0}距{1}有间距小于4.5mil，请到{2}查看位置！".format(ss, sm, tmp_ss2)
                        arraylist.append(log)
                        result_lays.append(tmp_ss2)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, tmp_ss2, sm, log, dic_zu_layer)
                    step.unaffectAll()
                    step.removeLayer(tmp_sm2)
                    step.affect(ss)
                    ...
                    if step.Selected_count():
                        ...
                        if index_list2:
                            for i in index_list2:
                                step.selectByIndex(ss, i)
                            log = u"检测到字高小于25mil,请检查文字层{0}".format(ss)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, ss, ss, log,
                                                                                 dic_zu_layer)
                            step.clearSel()
                        if index_list3:
                            for i in index_list3:
                                step.selectByIndex(ss, i)
                            log = u"检测到文字线宽小于4mil,请检查文字层{0}".format(ss)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, ss, ss, log,
                                                                                 dic_zu_layer)
                            step.clearSel()
                ...
                if step.Selected_count():
                    # 线宽
                    index_list = []
                    feat_info = layerObj.featout_dic_Index(options="select+feat_index")['lines']
                    feat_info += layerObj.featout_dic_Index(options="select+feat_index")['arcs']
                    for feat in feat_info:
                        if float(feat.symbol[1:]) < 4:
                            index_list.append(feat.feat_index)
                    step.clearSel()
                    if index_list:
                        for i in index_list:
                            step.selectByIndex(ss, i)
                        log = u"检测到线宽小于4mil,请检查文字层{0}".format(ss)
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, ss, ss, log,
                                                                             dic_zu_layer)
                        step.clearSel()
                ...
                step.COM(
                    'set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=no,attribute=.pattern_fill,min_int_val=0,max_int_val=0,min_float_val=0,max_float_val=0,option=,text=')
                step.COM('set_filter_and_or_logic,filter_name=popup,criteria=inc_attr,logic=or')
                step.selectAll()
                step.resetFilter()
                if step.Selected_count():
                    ...
                    if step.Selected_count():
                        ...
                        if index_list1:
                            for i in index_list1:
                                step.selectByIndex(ss, i)
                            log = u"检测到线路网格铜位置字宽小于38mil,请检查文字层{0}".format(ss)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, ss, ss, log,
                                                                                 dic_zu_layer)
                            step.clearSel()
                        if index_list2:
                            for i in index_list2:
                                step.selectByIndex(ss, i)
                            log = u"检测到线路网格铜位置线宽小于6mil,请检查文字层{0}".format(ss)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, ss, ss, log,
                                                                                 dic_zu_layer)
                            step.clearSel()
                ...
                # step.PAUSE(ss)
        # 20250520 zl
        # 先去panel内的step中转铜再去panel flatten
        for stepname in get_pnl_steps:
            step = gen.Step(job, stepname)
            step.initStep()
            for sm in m1_list + m2_list:
                tmp_sm = '%s+++%s' % (sm, step.pid)
                step.affect(sm)
                step.copySel(tmp_sm)
                step.unaffectAll()
                step.affect(tmp_sm)
        ...
        for stepname in ['panel']:
            ...
            for ss in ss_layers:
                smlist = m1_list if ss[1] == '1' else m2_list
                signal = outsignalLayers[0] if ss[1] == '1' else outsignalLayers[-1]
                tmp_ss = '%s+++check_silk_screen' % ss
                tmp_ss2 = '%s+++2check_silk_screen' % ss
                for sm in smlist:
                    tmp_sm = '%s+++check_silk_screen' % sm
                    tmp_sm2 = '%s+++2check_silk_screen' % sm
                    # flatten 阻焊 检测
                    step.affect(ss)
                    step.copySel(tmp_ss)
                    step.unaffectAll()
                    ...
                    step.Contourize()
                    step.unaffectAll()
                    step.Flatten(sm_, tmp_sm)
                    # step.removeLayer('%s+++%s' % (sm, step.pid))
                    step.affect(tmp_sm)
                    # step.selectSymbol('wzfd-*')
                    # step.resetFilter()
                    # if step.Selected_count():
                    #     step.selectDelete()
                    # step.Contourize()
                    # step.copyToLayer(tmp_sm2, size=228.6)  # 254->228.6  20250417 5mil改为4.5mil 提出人：周涌
                    step.copyToLayer(tmp_sm2)
                    step.unaffectAll()
                    step.affect(tmp_sm2)
                    step.selectResize(228.6, 'yes')
                    step.unaffectAll()
                    step.affect(tmp_ss)
                    step.selectSymbol('wzfd-*')
                    step.resetFilter()
                ...
                if step.Selected_count():
                    ...
                    if index_list:
                        ...
                        log = u"检测到线宽小于4mil,请检查文字层{0}".format(ss)
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, ss, ss,
                                                                             log,
                                                                             dic_zu_layer)
                        step.clearSel()
                step.unaffectAll()
                # 线路网格铜
                tmp_signal = signal + '+++check_silk_screen'
                step.affect(signal)
                step.setFilterTypes('line')
                step.COM(
                    'set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=no,attribute=.pattern_fill,min_int_val=0,max_int_val=0,min_float_val=0,max_float_val=0,option=,text=')
                ...
                # flatten 文字 检测
                for sm in smlist:
                    tmp_ss = '%s_%s+++check_silk_screen_flat' % (ss, sm)
                    tmp_ss2 = '%s_%s+++2check_silk_screen_flat' % (ss, sm)
                    tmp_sm = '%s+++check_silk_screen' % sm
                    tmp_sm2 = '%s+++2check_silk_screen' % sm
                    step.affect(sm)
                    step.copySel(tmp_sm)
                    ...
                    step.Contourize()
                    step.unaffectAll()
                    step.Flatten(ss_, tmp_ss)
                    step.removeLayer(ss_)
                    step.affect(tmp_ss)
                    step.refSelectFilter('surface_fill')
                    if step.Selected_count():
                        step.selectReverse()
                    ...
                    if step.Selected_count():
                        ...
                        arraylist.append(log)
                        result_lays.append(tmp_ss)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, tmp_ss, sm, log,
                                                                             dic_zu_layer)
                    step.unaffectAll()
                    step.removeLayer(tmp_sm)
                    # 20250320 zl 检测文字距防焊间距是否5mil以上
                    ...
                    if step.Selected_count():
                        ...
                        arraylist.append(log)
                        result_lays.append(tmp_ss2)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, tmp_ss2, sm, log,
                                                                             dic_zu_layer)
                    step.unaffectAll()
                    step.removeLayer(tmp_sm2)
        # 清除空层
        ...
        for sm in m1_list + m2_list:
            tmp_sm = '%s+++%s' % (sm, step.pid)
            panel_step.removeLayer(tmp_sm)
        panel_step.removeLayer('surface_fill')
        for ss in ss_layers:
            all_tmp_lays.extend(['%s+++' % ss, '%s+++2' % ss, '%s+++check_silk_screen' % ss, '%s+++2check_silk_screen' % ss,
                      '%s+++check_silk_screen_flat' % ss,'%s+++2check_silk_screen_flat' % ss])
            for sm in m1_list + m2_list:
                all_tmp_lays.extend(['%s_%s+++check_silk_screen_flat' % (ss, sm), '%s_%s+++2check_silk_screen_flat' % (ss, sm)])
        result_lays = set(result_lays)
        other_lays = set(all_tmp_lays) - result_lays
        if other_lays:
            [panel_step.removeLayer(other_lay) for other_lay in other_lays]
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
                self.view_note_detail(dic_zu_layer)
    ...
    def check_steel_mesh_to_via_not_plugged(self, autocheck=None):
        """
        http://192.168.2.120:82/zentao/story-view-7816.html
        :param autocheck:
        :return:
        """
        p_layers = []
        ...
        lps = list(filter(lambda row: re.match('lp(\d+)?(-(c|s))?', str(row)), rows))
        if not lps:
            return u"没有铝片层", None
        stepname = 'edit'
        arraylist = []
        ...
        for p in p_layers:
            ...
            step.COM('set_filter_and_or_logic,filter_name=popup,criteria=inc_attr,logic=or')
            step.refSelectFilter(';'.join(lps), mode='disjoint')
            step.resetFilter()
            if step.Selected_count():
    ...
    def check_text_serial_number(self, autocheck=None):
        """
        http://192.168.2.120:82/zentao/story-view-7442.html
        :param autocheck:
        :return:
        """
        ...
        if not data_info:
            return  u"success 没有添加序号的数据", None
        data = data_info[0]
    ...
    # 20250423 zl 序号检测
    def check_serial_number(self, autocheck=None):
        """
        d-coupon序号检测
        http://192.168.2.120:82/zentao/story-view-8196.html
        """
        stepname = 'panel'
        arraylist = []
        dic_zu_layer = {}
        panel_step = gen.Step(job, stepname)
        steps = get_panelset_sr_step(jobname=job.name, panelset='panel', is_group_by=False)
        # steps = panel_step.info.get('gREPEATstep')
        d_coupon_list = list(filter(lambda step: 'd-coupon' in step or 'dcoupon' in step, steps))
        d_coupons = list(set(d_coupon_list))
        if d_coupons:
            # d_coupons_01 = filter(lambda coupon: coupon.endswith('-01') or coupon.endswith('-1'), d_coupons)
            # d_coupons_01 = filter(lambda coupon: '-01' in coupon or '-1' in coupon, d_coupons)
            d_coupons_01 = filter(lambda coupon: re.search('-01$|-01\D|-1$|-1\D', coupon), d_coupons)
            err_d_coupon = []
            for d_coupon_01 in d_coupons_01:
                if len(filter(lambda coupon: coupon.startswith(d_coupon_01), d_coupons_01)) > 1:
                    err_d_coupon.append(d_coupon_01)
            if err_d_coupon:
                arraylist.append(u'检测到有%s命名的d-coupon命名冲突，无法分组' % u'、'.join(err_d_coupon))
            else:
                check_layers = silkscreenLayers
                # 20250122 分组
                d_coupons_dict = {}
                for d_coupon in d_coupons_01:
                    key = d_coupon.rsplit('-01', 1)[0] if re.search('-01$|-01\D', d_coupon) else d_coupon.rsplit('-1', 1)[0]
                    d_coupons_dict[key] = []
                is_break = False
                # print(d_coupons_dict)
                # job.PAUSE('aaaaaaaaaaaaaaa')
                for v in d_coupon_list:
                    if list(d_coupons_dict.keys()):
                        keylist = list(filter(lambda key: v.startswith(key), d_coupons_dict.keys()))
                        # print(v, keylist)
                        if len(keylist) > 1:
                            arraylist.append(u'检测到有%s命名的d-coupon命名冲突，无法分组' % u'、'.join(keylist))
                            is_break = True
                            break
                        if keylist:
                            d_coupons_dict[keylist[0]].append(v)
                        else:
                            if v not in d_coupons_dict.keys():
                                d_coupons_dict[v] = []
                            d_coupons_dict[v].append(v)
                ...
                # panel_step.PAUSE('B')
                if not is_break:
                    for d_coupon in d_coupons:
                        step = gen.Step(job, d_coupon)
                        step.initStep()
                        fill_layer = '%s_fill+++' % d_coupon
                        # if not step.isLayer(fill_layer):
                        step.removeLayer(fill_layer)
                        step.createLayer(fill_layer)
                        step.srFill(fill_layer)
                    panel_step.initStep()
                    if check_layers:
                        for d_couponval, vals in d_coupons_dict.items():
                            val_list_01, val_list_other = [], []
                            for val in vals:
                                if val in d_coupons_01:
                                    val_list_01.append(val)
                                else:
                                    val_list_other.append(val)
                            if val_list_01:
                                val = val_list_01[0]
                                serial_layers = []
                                tmp_layer = '%s_fill+++' % val
                                flat_layer = 'flat_d_coupon+++01'
                                panel_step.Flatten(tmp_layer, flat_layer)
                                # 挑出每个物件去跟文字层touch
                                feat_indexes = []
                                feat_info = panel_step.Features_INFO(flat_layer, mode='feat_index')
                                for feat in feat_info:
                                    if feat and re.match('#\d+', feat[0]):
                                        feat_indexes.append(feat[0].replace('#', ''))
                                tmp_flat_layer = 'tmp_flat_d_coupon+++01'  # 依次挑出来判断接触的文字
                                # 找出接触d-coupon区域的文字层
                                for check_layer in check_layers:
                                    panel_step.affect(check_layer)
                                    panel_step.setFilterTypes('text', 'positive')
                                    panel_step.refSelectFilter(flat_layer)
                                    panel_step.resetFilter()
                                    if panel_step.Selected_count():
                                        serial_layers.append(check_layer)
                                    panel_step.unaffectAll()
                                result = {}
                                # 记录每层的异常信息 第一个值 d-coupon对应多个序号 第二个值 d-coupon没有对应的序号 20250123 补充第三个值 错误的序号个数
                                if serial_layers:
                                    for serial_layer in serial_layers:
                                        # log = u"检测到末尾是01的d_coupon在panel中%s层没有序列号" % serial_layer
                                        # arraylist.append(log)
                                        # dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step,serial_layer, serial_layer, log,dic_zu_layer)
                                        result[serial_layer] = [0, 0, 0]

                                    for index in feat_indexes:
                                        panel_step.VOF()
                                        panel_step.truncate(tmp_flat_layer)
                                        panel_step.VON()
                                        panel_step.affect(flat_layer)
                                        panel_step.selectByIndex(flat_layer, index)
                                        panel_step.copySel(tmp_flat_layer)
                                        panel_step.unaffectAll()
                                        for serial_layer in serial_layers:
                                            layer_cmd = gen.Layer(panel_step, serial_layer)
                                            panel_step.affect(serial_layer)
                                            panel_step.setFilterTypes('text', 'positive')
                                            panel_step.refSelectFilter(tmp_flat_layer)
                                            panel_step.resetFilter()
                                            feat_text = layer_cmd.featout_dic_Index(units="mm", options="select+feat_index")[
                                                "text"]
                                            if panel_step.Selected_count():
                                                num_text_list = []
                                                for obj in feat_text:
                                                    ...
                                                    if re.match('\d+', text):
                                                        num_text_list.append(int(text))
                                                if num_text_list:
                                                    if num_text_list.count(1) > 1:
                                                        # log = u"检测到末尾是01的d_coupon在panel中对应的序列号有多个01"
                                                        # arraylist.append(log)
                                                        # dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step,serial_layer, serial_layer, log, dic_zu_layer)
                                                        result[serial_layer][0] += 1
                                                    error_nums = list(filter(lambda num: num != 1, num_text_list))
                                                    if error_nums:
                                                        result[serial_layer][2] += len(error_nums)
                                                else:
                                                    # log = u"检测到末尾是01的d_coupon在panel中对应的序列号没有01"
                                                    # arraylist.append(log)
                                                    # dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step, serial_layer,serial_layer, log, dic_zu_layer)
                                                    result[serial_layer][1] += 1
                                            else:
                                                result[serial_layer][1] += 1
                                            panel_step.unaffectAll()
                                else:
                                    log = u"检测到末尾是01的%s在panel中对应的序列号没有01" % d_couponval
                                    arraylist.append(log)
                                panel_step.removeLayer(tmp_layer)
                                panel_step.removeLayer(flat_layer)
                                panel_step.removeLayer(tmp_flat_layer)
                                effect_layer = []  # 有01的层需要判断下是否有多个01的记录，有则要报出
                                empty_layer = []
                                for layer, vals in result.items():
                                    if vals[1] > 0:
                                        empty_layer.append(layer)
                                    else:
                                        effect_layer.append(layer)
                                if serial_layers and len(empty_layer) == len(serial_layers):  # 文字层都没有01
                                    for serial_layer in serial_layers:
                                        log = u"检测到末尾是01的%s在panel中对应的序列号没有01" % d_couponval
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname,
                                                                                             panel_step,
                                                                                             serial_layer,
                                                                                             serial_layer, log,
                                                                                             dic_zu_layer)
                                for layer in effect_layer:
                                    if result.get(layer)[0] > 0:
                                        log = u"检测到有%s个末尾是01的%s在panel中%s对应的序列号有多个01" % (
                                            result.get(layer)[0], d_couponval, layer)
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname,
                                                                                             panel_step, layer,
                                                                                             layer, log,
                                                                                             dic_zu_layer)
                                    if result.get(layer)[2] > 0:
                                        log = u"检测到末尾是01的%s在panel中%s对应的序列号有%s个非01序号" % (
                                            d_couponval, layer, result.get(layer)[2])
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname,
                                                                                             panel_step, layer,
                                                                                             layer, log,
                                                                                             dic_zu_layer)
                            num_text_dict = {}
                            # print(val_list_other)
                            # panel_step.PAUSE('b')
                            if val_list_other:
                                serial_layers = []
                                flat_layer = 'flat_d_coupon+++'
                                panel_step.removeLayer(flat_layer)
                                for val in set(val_list_other):
                                    tmp_layer = '%s_fill+++' % val
                                    tmp_flat_layer = 'flat_%s+++' % val
                                    panel_step.Flatten(tmp_layer, tmp_flat_layer)
                                    panel_step.affect(tmp_flat_layer)
                                    panel_step.moveSel(flat_layer)
                                    panel_step.unaffectAll()
                                    panel_step.removeLayer(tmp_layer)
                                    panel_step.removeLayer(tmp_flat_layer)
                                # 挑出每个物件去跟文字层touch
                                ...
                                for feat in feat_info:
                                    if feat and re.match('#\d+', feat[0]):
                                        feat_indexes.append(feat[0].replace('#', ''))
                                tmp_flat_layer = 'tmp_flat_d_coupon+++'  # 依次挑出来判断接触的文字
                                # 找出接触d-coupon区域的文字层
                                for check_layer in check_layers:
                                    panel_step.affect(check_layer)
                                    panel_step.setFilterTypes('text', 'positive')
                                ...
                                for index in feat_indexes:
                                    ...
                                    panel_step.unaffectAll()
                                    for serial_layer in serial_layers:
                                        if serial_layer not in num_text_dict.keys():
                                            num_text_dict[serial_layer] = []
                                        layer_cmd = gen.Layer(panel_step, serial_layer)
                                        panel_step.affect(serial_layer)
                                        panel_step.setFilterTypes('text', 'positive')
                                        panel_step.refSelectFilter(tmp_flat_layer)
                                        panel_step.resetFilter()
                                        if panel_step.Selected_count():
                                            feat_text = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")[
                                                "text"]
                                            for obj in feat_text:
                                                text = obj.text.replace("'", '')
                                                if re.match('\d+$', text):
                                                    num_text_dict[serial_layer].append(int(text))
                                        panel_step.unaffectAll()
                                panel_step.removeLayer(tmp_flat_layer)
                                # 缺少的，重复的，多出来的
                                err_num, err2_num, err3_num = {}, {}, {}
                                # 如果文字层都没有序列号
                                has_serial = False
                                for lay, nums in num_text_dict.items():
                                    err_num[lay], err2_num[lay], err3_num[lay] = [], [], []
                                    if nums:
                                        has_serial = True
                                if not has_serial:
                                    panel_step.removeLayer(flat_layer)
                                    arraylist.append(u"检测到%s未添加序列号" % d_couponval)
                                    continue
                                start_num = 2 if val_list_01 else 1
                                end_num = len(val_list_other) + 2 if val_list_01 else len(val_list_other) + 1
                                for lay, nums in num_text_dict.items():
                                    if nums:  # 可能这层文字层没有序列号
                                        for num in range(start_num, end_num):
                                            if nums.count(num) == 0:
                                                err_num[lay].append(str(num))  # 缺少的
                                            if nums.count(num) > 1:  # 重复的
                                                err2_num[lay].append(str(num))
                                        err3_num[lay].extend([str(n) for n in filter(
                                            lambda num: num not in range(start_num, end_num), nums)])
                                if err_num:
                                    for k, v in err_num.items():
                                        if v:
                                            # panel_step.affect(k)
                                            # panel_step.setFilterTypes('text', 'positive')
                                            # panel_step.refSelectFilter(flat_layer)
                                            # panel_step.resetFilter()
                                            # if panel_step.Selected_count():
                                            log = u"检测到%s在panel中%s层缺失序列号%s" % (d_couponval, k, u'、'.join(v))
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name,
                                                                                                 stepname,
                                                                                                 panel_step, k,
                                                                                                 k, log,
                                                                                                 dic_zu_layer)
                                ...
                                if err3_num:
                                    for k, v in err3_num.items():
                                        if v:
                                            ...
                                            panel_step.resetFilter()
                                            if panel_step.Selected_count():
                                                log = u"检测到%s在panel中%s层超出范围的异常序列号%s" % (
                                                    d_couponval, k, u'、'.join(v))
                                                arraylist.append(log)
                                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name,
                                                                                                     stepname,
                                                                                                     panel_step, k,
                                ...
                                if err2_num:
                                    for k, v in err2_num.items():
                                        if v:
                                            ...
                                            panel_step.setFilterTypes('text', 'positive')
                                            for n in v:
                                                panel_step.setTextFilter('*0%s' % n)
                                                panel_step.refSelectFilter(flat_layer)
                                                feat_info = panel_step.Features_INFO(k, mode='feat_index+select')
                                                for feat in feat_info:
                                                    if feat and re.match('#\d+', feat[0]):
                                                        repeat_pos.append(feat[0].replace('#', ''))
                                            panel_step.resetFilter()
                                            for i in repeat_pos:
                                                panel_step.selectByIndex(k, i)
                                            log = u"检测到%s在panel中%s层序列号%s重复" % (d_couponval, k, u'、'.join(v))
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname,
                                                                                                 panel_step, k, k,
                                                                                                 log, dic_zu_layer)
                    ...
                    for d_coupon in d_coupons:
                        fill_layer = '%s_fill+++' % d_coupon
                        # if not step.isLayer(fill_layer):
                        panel_step.removeLayer(fill_layer)
        """
        序号检测
        http://192.168.2.120:82/zentao/story-view-8549.html
        :param autocheck:
        :return:
        """
        # 检测mi指示栏位有pcs&Set序号,cam资料未添加时报警提示
        ...
        # print(data_info)
        if not data_info:
            arraylist.append(u"success MI没有添加set或pcs序号的指示")
        else:
            data = data_info[0]
            if data['CI_ADD_POSITION_CODE_'] == 0:
                arraylist.append(u"success MI指示不加set或pcs序号，不检测")
            else:
                # stepname = 'panel'
                # arraylist = []
                # dic_zu_layer = {}
                step = gen.Step(job, stepname)
                # set数量
                set_list = list(filter(lambda x: re.match('(set|set\d+)$', x), step.info.get('gREPEATstep')))
                set_series = list(set(set_list))  # 多种set的不检测set序号
                # 如果有多种set,判断其pcs数量是否一致，不一致时无法检测set序号
                pcs_numlist = []
                if len(set_series) > 0:
                    for setname in set_series:
                        set_step = gen.Step(job, setname)
                        pcs_repeat = set_step.info.get('gREPEATstep').count('edit')
                        pcs_numlist.append(pcs_repeat)
                    pcs_numlist = list(set(pcs_numlist))
                    set_repeat = set_list.count(set_series[0])
                    set_step = gen.Step(job, set_series[0])
                    pcs_repeat = set_step.info.get('gREPEATstep').count('edit')
                else:
                    set_list = ['panel']
                    set_series = ['panel']
                    set_step = gen.Step(job, set_series[0])
                    set_repeat = set_step.info.get('gREPEATstep').count('edit')
                    pcs_repeat = 1
                check_layers = outsignalLayers + silkscreenLayers + solderMaskLayers
                step.initStep()
                for check_layer in check_layers:
                    step.removeLayer('serial_%s+++' % check_layer)
                    step.removeLayer('serial_%s+++_flat' % check_layer)
                if data['CI_ADD_POSITION_CODE_'] == 3:
                    get_sr_area_flatten("surface_fill", include_sr_step=set_series)
                    get_sr_area_flatten("surface_fill_edit", set_step.name, include_sr_step='edit')
                    # 检测对应是否有加pcs序列号
                    pcs_serial_layers = []
                    for setname in set_series:
                        set_step = gen.Step(job, setname)
                        ...
                        same_serial_layers = []
                        for check_layer in check_layers:
                            num_count_sucess = False  # 标识该层是不是有序号且序号无缺失
                            serial_layer = 'serial_%s+++' % check_layer
                            set_step.affect(check_layer)
                            set_step.setFilterTypes('text', 'positive')
                            set_step.refSelectFilter('surface_fill_edit', mode='cover')
                            set_step.resetFilter()
                            if set_step.Selected_count():
                                layer_obj = gen.Layer(set_step, check_layer)
                                feats = layer_obj.featout_dic_Index(options="select+feat_index")['text']
                                set_step.clearSel()
                                nums = []
                                indexs = []
                                for feat in feats:
                                    ...
                                    if re.match('\d+', text):
                                        ...
                                        indexs.append(feat.feat_index)
                                # 有数字的text才可能是序号 才去进一步判断
                                if nums:
                                    num_count_sucess = True
                                    for i in range(1, pcs_repeat + 1):
                                        if nums.count(i) > 1:
                                            log = u"检测到%s多加pcs序列号%s" % (check_layer, i)
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                                 check_layer, check_layer, log,
                                                                                                 dic_zu_layer)
                                            num_count_sucess = False
                                        elif nums.count(i) < 1:
                                            log = u"检测到%s少加pcs序列号%s" % (check_layer, i)
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                                 check_layer, check_layer, log,
                                                                                                 dic_zu_layer)
                                            num_count_sucess = False
                                    pcs_serial_layers.append(check_layer)
                                    set_step.clearSel()
                                    for index in indexs:
                                        set_step.selectByIndex(check_layer, index)
                                    set_step.copySel(serial_layer)
                            set_step.unaffectAll()
                            if num_count_sucess:
                                same_serial_layers.append(serial_layer)
                        # 20250528 zl 检测多个序号层序号是否一致
                        if len(same_serial_layers) > 1:
                            layerobj = gen.Layer(set_step, 'surface_fill_edit')
                            feats = layerobj.featout_dic_Index(options='feat_index')['surface']
                            ids = [feat.feat_index for feat in feats]
                            tmp_surface = 'surface_fill_edit+++'
                            for id in ids:
                                set_step.affect('surface_fill_edit')
                                set_step.selectByIndex('surface_fill_edit', id)
                                set_step.copySel(tmp_surface)
                                set_step.unaffectAll()
                                same_serials = []
                                for same_serial_layer in same_serial_layers:
                                    set_step.affect(same_serial_layer)
                                    set_step.refSelectFilter(tmp_surface)
                                    if set_step.Selected_count():
                                        layerobj = gen.Layer(set_step, same_serial_layer)
                                        feats = layerobj.featout_dic_Index(options='feat_index+select')['text']
                                        for feat in feats:
                                            text = re.sub("[-']", '', feat.text)
                                            if re.match('\d+', text):
                                                same_serials.append(int(text))
                                    set_step.unaffectAll()
                                if len(set(same_serials)) > 1:
                                    log = u"检测到%s同一个pcs位置的序号不相同,请检查" % u'、'.join([re.sub('serial_|\+\+\+', '', i) for i in same_serial_layers])
                                    if log not in arraylist:
                                        arraylist.append(log)
                                set_step.unaffectAll()
                                set_step.truncate(tmp_surface)
                            set_step.removeLayer(tmp_surface)
                    pcs_serial_layers = list(set(pcs_serial_layers))
                    if not pcs_serial_layers:
                        log = u"检测到%s未添加pcs序列号" % set_step.name
                        arraylist.append(log)
                    if pcs_serial_layers and set_series != ['panel']:
                        if len(pcs_numlist) > 1:
                            for check_layer in pcs_serial_layers:
                                serial_layer = 'serial_%s+++' % check_layer
                                step.removeLayer(serial_layer)
                            arraylist.append(u'多个set的pcs数不一致,无法确定每个set序号个数,请自行检查set序号！！！')
                        else:
                            serial_layers = []
                            step.initStep()
                            same_serial_layers = []
                            for check_layer in pcs_serial_layers:
                                ...
                                serial_layer = 'serial_%s+++' % check_layer
                                step.affect(check_layer)
                                step.setFilterTypes('text', 'positive')
                                step.refSelectFilter('surface_fill', mode='cover')
                                step.resetFilter()
                                if step.Selected_count():
                                    layer_obj = gen.Layer(step, check_layer)
                                    feats = layer_obj.featout_dic_Index(options="select+feat_index")['text']
                                    step.clearSel()
                                    nums,indexs = [], []
                                    for feat in feats:
                                    ...
                                    if nums:
                                        num_count_sucess = True
                                        for i in range(1, set_repeat + 1):
                                            if nums.count(i) > pcs_repeat:
                                                log = u"检测到%s多加set序列号%s" % (check_layer, i)
                                                arraylist.append(log)
                                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, check_layer, check_layer, log, dic_zu_layer)
                                                num_count_sucess = False
                                            elif nums.count(i) < pcs_repeat:
                                                log = u"检测到%s少加set序列号%s" % (check_layer, i)
                                                arraylist.append(log)
                                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step, check_layer, check_layer, log, dic_zu_layer)
                                                num_count_sucess = False
                                        serial_layers.append(check_layer)
                                        # 检测与pcs序号是否重叠
                                        step.clearSel()
                                        step.Flatten(serial_layer, '%s_flat' % serial_layer)
                                        for index in indexs:
                                            step.selectByIndex(check_layer, index)
                                        step.copySel(serial_layer)
                                        step.unaffectAll()
                                        step.affect(serial_layer)
                                        step.refSelectFilter('%s_flat' % serial_layer)
                                        if step.Selected_count():
                                            log = u"检测到%s层set和pcs序号重叠" % check_layer
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                                 serial_layer, serial_layer, log,
                                                                                                 dic_zu_layer)

                                        step.unaffectAll()
                                        step.removeLayer('%s_flat' % serial_layer)
                                step.unaffectAll()
                                if num_count_sucess:
                                    same_serial_layers.append(serial_layer)
                            # 20250528 zl 检测多个序号层序号是否一致
                            if len(same_serial_layers) > 1:
                                feats = step.INFO('-t layer -e %s/%s/%s -d FEATURES -o feat_index' % (job.name, step.name, 'surface_fill'))
                                del feats[0]
                                ids = []
                                for feat in feats:
                                    feat = feat.strip()
                                ...
                                # ids = [feat.feat_index for feat in feats]
                                tmp_surface = 'surface_fill+++'
                                for id in ids:
                                    step.affect('surface_fill')
                                    step.selectByIndex('surface_fill', id)
                                    step.copySel(tmp_surface)
                                    step.unaffectAll()
                                    same_serials = []
                                    for same_serial_layer in same_serial_layers:
                                        step.affect(same_serial_layer)
                                        step.refSelectFilter(tmp_surface)
                                        if step.Selected_count():
                                            layerobj = gen.Layer(step, same_serial_layer)
                                            feats = layerobj.featout_dic_Index(options='feat_index+select')[
                                                'text']
                                            for feat in feats:
                                                text = re.sub("[-']", '', feat.text)
                                                if re.match('\d+', text):
                                        ...
                                        step.unaffectAll()
                                    if len(set(same_serials)) > 1:
                                        log = u"检测到%s同一个set位置的序号不相同，请检查" % u'、'.join(
                                            [re.sub('serial_|\+\+\+', '', i) for i in same_serial_layers])
                                        if log not in arraylist:
                                            arraylist.append(log)
                                    step.unaffectAll()
                                    step.truncate(tmp_surface)
                                step.removeLayer(tmp_surface)
                            for check_layer in pcs_serial_layers:
                                serial_layer = 'serial_%s+++' % check_layer
                                step.removeLayer(serial_layer)
                            if not serial_layers:
                                log = u"检测到%s未添加set序列号" % stepname
                                arraylist.append(log)
                elif data['CI_ADD_POSITION_CODE_'] == 2:
                    if len(pcs_numlist) > 1:
                        arraylist.append(u'多个set的pcs数不一致,无法确定每个set序号个数,请自行检查set序号！！！')
                    else:
                        # set_repeat = pcs_numlist[0]
                        if set_series == ['panel']:
                        ...
                        for check_layer in check_layers:
                            ...
                            step.unaffectAll()
                            if num_count_sucess:
                                same_serial_layers.append(check_layer)
                        # print(same_serial_layers)
                        # job.PAUSE('AAA')
                        # 20250528 zl 检测多个序号层序号是否一致
                        if len(same_serial_layers) > 1:
                            feats = step.INFO('-t layer -e %s/%s/%s -d FEATURES -o feat_index' % (job.name, step.name, 'surface_fill'))
                        ...
                        if not serial_layers:
                            log = u"检测到%s未添加set序列号" % stepname
                            arraylist.append(log)
                elif data['CI_ADD_POSITION_CODE_'] == 1:
                    get_sr_area_flatten("surface_fill_edit", include_sr_step='edit')
                    serial_layers = []
                    set_step = gen.Step(job, stepname)
                    set_step.initStep()
                    same_serial_layers = []
                    for check_layer in check_layers:
                        num_count_sucess = False  # 标识该层是不是有序号且序号无缺失
                            ...
                            if nums:
                                ...
                                for i in range(1, set_repeat + 1):
                                    if nums.count(i) > pcs_repeat:
                                        log = u"检测到%s多加pcs序列号%s" % (check_layer, i)
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, set_step.name, set_step, check_layer, check_layer, log, dic_zu_layer)
                                        num_count_sucess = False
                                    elif nums.count(i) < pcs_repeat:
                                        log = u"检测到%s少加pcs序列号%s" % (check_layer, i)
                                        arraylist.append(log)
                    ...
                    # 20250528 zl 检测多个序号层序号是否一致
                    if len(same_serial_layers) > 1:
                        feats = step.INFO('-t layer -e %s/%s/%s -d FEATURES -o feat_index' % (job.name, set_step.name, 'surface_fill_edit'))
                        del feats[0]
                        ids = []
                        for feat in feats:
                            feat = feat.strip()
                        ...
                        for id in ids:
                            ...
                            for same_serial_layer in same_serial_layers:
                                ...
                                set_step.unaffectAll()
                            if len(set(same_serials)) > 1:
                                log = u"检测到%s同一个pcs位置的序号不相同,请检查" % u'、'.join(
                                    [re.sub('serial_|\+\+\+', '', i) for i in same_serial_layers])
                                if log not in arraylist:
                                    arraylist.append(log)
                            set_step.unaffectAll()
                    ...
                    if not serial_layers:
                        ...
                        arraylist.append(log)
                step.removeLayer('surface_fill')
                step.removeLayer('surface_fill_edit')
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
                self.view_note_detail(dic_zu_layer)
    ...
    # 20241216 zl 检测控深钻外层有没有加PAD
    def check_ks_pad_exists(self, autocheck=None):
        """
        http://192.168.2.120:82/zentao/story-view-6927.html
        :param autocheck:
        :return:
        """
        if not get_led_board(job.name):
            return u"success 非灯板，不检测", None
        kz_drills = []
        rows = job.matrix.returnRows('board', 'drill')
        for name in rows:
            if name in ["cdc", "cds"] or re.match("^cd[0-9]{1,2}-[0-9]{1,2}[cs]$|^ks\d{1,2}-\d{1,2}$", name):
                kz_drills.append(name)
        if not kz_drills:
            return u"success 未找到控深钻带", None
        dic_zu = {}
        for layer in kz_drills:
            ...
            result = re.search('^ks(\d{1,2})-(\d{1,2})$', layer)
            if result:
                start, end = int(result.group(1)), int(result.group(2))
                if start < end:
                    dic_zu[layer] = "l1"
                else:
                    dic_zu[layer] = "l{0}".format(int(lay_num))
        ...
        if all_steps:
            ...
            for drill_layer, sig_layer in dic_zu.iteritems():
                tmp_drl_layer = '%s+++check_ks_pad_exists' % drill_layer
                tmp_sig_layer = '%s+++check_ks_pad_exists' % sig_layer
                show_drl = '%s+++check_ks_pad_exists_show' % drill_layer
                step.removeLayer(tmp_drl_layer)
                step.removeLayer(tmp_sig_layer)
                step.removeLayer(show_drl)
        arraylist = []
        dic_zu_layer = {}
        for stepname in all_steps:
            step = gClasses.Step(job, stepname)
            ...
            for drill_layer, sig_layer in dic_zu.iteritems():
                step.clearAll()
                # 线路层复制出来转surface，去掉负性
                tmp_drl_layer = '%s+++check_ks_pad_exists' % drill_layer
                tmp_sig_layer = '%s+++check_ks_pad_exists' % sig_layer
                show_drl = '%s+++check_ks_pad_exists_show' % drill_layer
                step.affect(sig_layer)
                step.copySel(tmp_sig_layer)
                step.clearAll()
                step.affect(tmp_sig_layer)
                ...
                step.resetFilter()
                if step.featureSelected():
                    step.copySel(tmp_drl_layer)
                    step.clearAll()
                    step.affect(tmp_drl_layer)
                    step.refSelectFilter(tmp_sig_layer, mode='include')
                    if step.featureSelected():
                        step.COM('sel_reverse')
                        if step.featureSelected():
                            step.copySel(show_drl)
                            log = u"检测到{2}中控深钻{0}外层线路{1}加PAD有遗漏！".format(drill_layer, sig_layer, stepname)
                            arraylist.append(log)
                            dic_zu_layer = self.get_view_dic_layers(job.name, stepname, step,
                                                                    worklayer="view_layer_" + show_drl,
                                                                    dic_layer_list={
                                                                        u"查看异常层：": [show_drl, sig_layer], },
                                                                    dic_zu_layer=dic_zu_layer)
                    else:
                        step.copySel(show_drl)
                        log = u"检测到{2}中控深钻{0}外层线路{1}没有加PAD！".format(drill_layer, sig_layer, stepname)
                        arraylist.append(log)
                        dic_zu_layer = self.get_view_dic_layers(job.name, stepname, step,
                                                                worklayer="view_layer_" + show_drl,
                                                                dic_layer_list={
    ...
    # 20250113 zl d-coupon序列号检测
    # 20250107 01的step命名 -1或-01结尾
    def check_d_coupon_serial_number(self, autocheck=None):
        """
        http://192.168.2.120:82/zentao/story-view-8196.html
        :return:
        """
            ...
            if d_coupons:
                ...
                for d_coupon_01 in d_coupons_01:
                    if len(filter(lambda coupon: coupon.startswith(d_coupon_01), d_coupons_01)) > 1:
                        err_d_coupon.append(d_coupon_01)
                if err_d_coupon:
                    return  u'检测到有%s命名的d-coupon命名冲突，无法分组' % u'、'.join(err_d_coupon), None
                check_layers = silkscreenLayers
                # 20250122 分组
                d_coupons_dict = {}
                for d_coupon in d_coupons_01:
                ...
                for v in d_coupon_list:
                    if d_coupons_dict:
                        keylist = filter(lambda key: v.startswith(key), d_coupons_dict.keys())
                        if keylist:
                            d_coupons_dict[keylist[0]].append(v)
                        else:
                            if v not in d_coupons_dict.keys():
                ...
                panel_step.initStep()
                if check_layers:
                    for vals in d_coupons_dict.values():
                        val_list_01, val_list_other = [], []
                        for val in vals:
                            if val in d_coupons_01:
                                val_list_01.append(val)
                        ...
                        if val_list_01:
                            ...
                            if serial_layers:
                                ...
                                for index in feat_indexes:
                                    ...
                                    for serial_layer in serial_layers:
                                        ...
                                        panel_step.refSelectFilter(tmp_flat_layer)
                                        panel_step.resetFilter()
                                        feat_text = layer_cmd.featout_dic_Index(units="mm", options="select+feat_index")["text"]
                                        if panel_step.Selected_count():
                                            num_text_list = []
                                            for obj in feat_text:
                                                text = obj.text.replace("'", '')
                            ...
                            for layer, vals in result.items():
                                if vals[1] > 0:
                                    ...
                                    effect_layer.append(layer)
                            if len(empty_layer) == len(serial_layers):  # 文字层都没有01
                                for serial_layer in serial_layers:
                                    log = u"检测到末尾是01的d_coupon在panel中对应的序列号没有01"
                                    arraylist.append(log)
                                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname,
                            ...
                            for layer in effect_layer:
                                if result.get(layer)[0] > 0:
                                    log = u"检测到有%s个末尾是01的d_coupon在panel中%s对应的序列号有多个01" % (
                                    result.get(layer)[0], layer)
                                    arraylist.append(log)
                                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname,
                                                                                         panel_step, layer,
                                                                                         layer, log,
                                                                                         dic_zu_layer)
                                if result.get(layer)[2] > 0:
                                    log = u"检测到末尾是01的d_coupon在panel中%s对应的序列号有%s个非01序号" % (
                        ...
                        if val_list_other:
                            ...
                            for lay, nums in num_text_dict.items():
                                ...
                                if nums:
                                    has_serial = True
                            if not has_serial:
                                return u"检测到d_coupon未添加序列号", None
                            ...
                            if err_num:
                                for k, v in err_num.items():
                                    if v:
                                        ...
                                        # panel_step.resetFilter()
                                        # if panel_step.Selected_count():
                                        log = u"检测到d_coupon在panel中%s层缺失序列号%s" % (k, u'、'.join(v))
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name,
                                                                                             stepname,
                                                                                             panel_step, k,
                            ...
                            if err3_num:
                                for k, v in err3_num.items():
                                    if v:
                                        ...
                                        panel_step.resetFilter()
                                        if panel_step.Selected_count():
                                            log = u"检测到d_coupon在panel中%s层超出范围的异常序列号%s" % (
                                            k, u'、'.join(v))
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name,
                                                                                                 stepname,
                            ...
                            if err2_num:
                                for k, v in err2_num.items():
                                    if v:
                                        ...
                                        for i in repeat_pos:
                                            panel_step.selectByIndex(k, i)
                                        log = u"检测到d_coupon在panel中%s层序列号%s重复" % (k, u'、'.join(v))
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname,
                                                                                             panel_step, k, k,
                                                                                             log, dic_zu_layer)
    ...
    # 20250227 zl 检测ul标记
    def check_ul_mark(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        conn_inplan = oracleConn("inmind")
        sql = u"""SELECT
        ...
        data_info = conn_inplan.executeSql(sql.format(jobname.split("-")[0].upper()))
        if not data_info:
            return u"未找到ul symbol", None
        ...
        ul_symbol = "wz-ul-sh%s" % sh_pattern
        # print(ul_symbol)
        sql = u"""SELECT
                b.VALUE
        FROM
        ...
        data_info = conn_inplan.executeSql(sql.format(jobname.split("-")[0].upper()))
        if not data_info:
            return u"未找到添加层", None
        period_tmp = data_info[0][0]
        if '文字' in period_tmp:
            period_layer_type = 'silk_screen'
        if '防焊' in period_tmp:
            period_layer_type = 'solder_mask'
        if '蚀刻' in period_tmp:
            period_layer_type = 'signal'
        if re.search('C', period_tmp, re.IGNORECASE):
            period_side = 'top'
        if re.search('S', period_tmp, re.IGNORECASE):
            period_side = 'bottom'
        if re.search('C.*S|S.*C', period_tmp, re.IGNORECASE):
            period_side = 'top'
        layers = job.matrix.returnRows('board', period_layer_type, side=period_side)
        check_layer = layers[0]
        get_pnl_steps = get_panelset_sr_step(jobname=job.name, panelset='panel')
        checked_ul = set()
        for stepname in get_pnl_steps:
            step = gen.Step(job, stepname)
            ...
            if step.Selected_count():
                ...
                for feat in feats:
                    feat = feat.strip()
                    if feat:
                        if re.search('\.string=ul_(.*)$', feat):
                            checked_ul.add(re.search('\.string=ul_(.*)$', feat).group(1))
            step.unaffectAll()
        checked_ul = list(checked_ul)
        if not checked_ul:
            log = u"检测到%s层未添加ul标记（%s）！" % (check_layer,ul_symbol)
            arraylist.append(log)
        else:
            # 是否有多种
            if len(checked_ul) > 1:
                log = u"检测到%s层有多种ul标记（%s）！" % (check_layer, u'、'.join(checked_ul))
                arraylist.append(log)
            else:
                if checked_ul[0] != ul_symbol:
                    log = u"检测到%s层添加的标记%s有误（应为%s）！" % (check_layer, checked_ul[0], ul_symbol)
                    arraylist.append(log)
        if arraylist:
            # 获取制作时间
            sql = """select outer_time
                    from project_status_jobmanage
                    WHERE job = '{0}'
                    order by id desc"""
            job_process_info = conn.SELECT_DIC(dbc_p, sql.format(jobname.split("-")[0]))
            if job_process_info:
                if job_process_info[0].get('outer_time'):
                    try:
                        if datetime.datetime.strptime(job_process_info[0].get('outer_time'), '%Y-%m-%d %H:%M:%S') > datetime.datetime.strptime('2025-02-27 15:00:00', '%Y-%m-%d %H:%M:%S'):
                            update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
        ...
        if arraylist:
            ...
            return arraylist, dic_zu_layer
        # return "success", None
        return u"success 检测到%s层已添加ul标记（%s）"% (check_layer,ul_symbol), None

    # 20250301 zl 蚀刻引线层对位点设计检测
    def check_ccd_alignment(self, autocheck=None):
        # 蚀刻引线 etch-c etch-s
        arraylist = []
        dic_zu_layer = {}
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            step.initStep()
            etch_layers = {}
            for layer in ('etch-c', 'etch-s'):
                if step.isLayer(layer):
                    etch_layers[layer] = 'l1' if layer == 'etch-c' else 'l%s' % lay_num
            if len(etch_layers.keys()) == 0:
                return u'success 没有etch-c/etch-s层，无需检测', None
            #
            inplan_all_flow = get_inplan_all_flow(job.name[:13].upper(), True)
            figer_dp_positive = 'no'
            for d in inplan_all_flow:
                if (d['PROCESS_DESCRIPTION'] == '碱性蚀刻') and (d['WORK_CENTER_CODE'] and '金手指去导线' in d['WORK_CENTER_CODE']):
                    figer_dp_positive = 'yes'
            for etch, signal in etch_layers.items():
                tmp_etch = '%s+++check_ccd_align' % etch
                tmp_signal = '%s+++' % signal
                step.removeLayer(tmp_etch)
                step.removeLayer(tmp_signal)
                step.affect(etch)
                step.setSymbolFilter('r0')
                step.setAttrFilter_pro('.fiducial_name', text='317.reg')
                step.selectAll()
                step.resetFilter()
                r0_count = step.Selected_count()
                if r0_count == 4:
                    step.copySel(tmp_etch)
                    step.unaffectAll()
                    step.affect(signal)
                    symbol = 's5082'
                    process = u'酸性'
                    ...
                    step.selectBreak()
                    step.unaffectAll()
                    step.affect(tmp_etch)
                    step.refSelectFilter(tmp_signal, mode='same_center')
                    if step.Selected_count() != 4:
                        step.selectReverse()
                        log = u"step:{0} 此板金手指蚀刻引线走{3}蚀刻流程，要求对位点使用{4}，检测到{1}对位点与{2}{5}未对齐，请检查！".format(step.name, etch, signal, process, target,target)
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step,tmp_etch, etch, log, dic_zu_layer)
                    step.unaffectAll()
                    step.removeLayer(tmp_signal)
                else:
                    step.copySel(tmp_etch)
                    step.unaffectAll()
                    log = u"step:{0} 检测到{1}层r0对位点数量异常（{2}），正常为4个！".format(step.name, etch, r0_count)
                    arraylist.append(log)
                    step.affect(tmp_etch)
                    step.selectReverse()
                    if step.Selected_count():
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, tmp_etch, etch,
                                                                             log, dic_zu_layer)
                    step.unaffectAll()
                step.unaffectAll()
        if arraylist:
    ...
    # 20250304 zl 检测loss testPAD宽度
    def check_loss_test_pad(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        if lay_num < 4:
            return u"success %s层板不需要检测" % lay_num, None
        if "loss" in matrixinfo["gCOLstep_name"]:
            step = gen.Step(job, 'loss')
            step.initStep()
            ...
            for outsignal, second_outsignal in check_layers.items():
                ...
                if ids:
                    ...
                    log = u"step:{0} 检测到{1}层测试pad未补偿！".format(step.name, outsignal)
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, outsignal, outsignal, log, dic_zu_layer)
                    step.unaffectAll()
                # 掏开
                ids = []
                ...
                if ids:
                    ...
                    log = u"step:{0} 检测到{1}层未补偿掏开！".format(step.name, second_outsignal)
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, second_outsignal,
                                                                         second_outsignal, log, dic_zu_layer)
                    step.unaffectAll()
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
    ...
    # 20250318 zl 阻抗线屏蔽层检测
    def check_impedance_line_shielding(self, autocheck=None):
        """ 增加LOSS条和阻抗条上阻抗线屏蔽层检查 http://192.168.2.120:82/zentao/story-view-8115.html"""
        arraylist = []
        dic_zu_layer = {}
        if lay_num < 4:
            return u"success %s层板不需要检测" % lay_num, None
        ...
        for stepname in list(set(setsteps)):
            if 'loss' in stepname or 'icg' in stepname:
                check_steps.append(stepname)
        if not check_steps:
            return u"success 没有需要检测的loss和icg", None
        check_layers = {}
        arraylist_imp_info = get_inplan_imp(job.name.upper())
        for dic_imp_info in arraylist_imp_info:
            ...
            trace_layer = dic_imp_info["TRACE_LAYER_"].lower()
            check_layers[trace_layer] = ref_layers
        # check_layers = {'l1': 'l2', 'l%s' % lay_num: 'l%s' % (lay_num - 1)}
        print(check_layers)
        rows = job.matrix.rowList()
        # 清除临时层
        # for outsignal, second_outsignal in check_layers.items():
        #     tmp_outsignal = '%s+++check_impedance_line_shielding' % outsignal
        #     if tmp_outsignal in rows:
        #         job.matrix.removeLayer(tmp_outsignal)
        #     for signal in second_outsignal:
        #         tmp_second_outsignal = '%s+++check_impedance_line_shielding' % second_outsignal
        #         if tmp_second_outsignal in rows:
        #             job.matrix.removeLayer(signal)
        for signal in outsignalLayers + innersignalLayers:
            tmp_signal = '%s+++check_impedance_line_shielding' % signal
            if tmp_signal in rows:
                job.matrix.deleteRow(tmp_signal)
        icgs = [name for name in check_steps if 'icg' in name]
        for icg in icgs:
            step = gen.Step(job, icg)
            step.initStep()
            for outsignal, second_outsignals in check_layers.items():
                for second_outsignal in second_outsignals:
                    tmp_outsignal = '%s_%s+++check_impedance_line_shielding' % (outsignal, second_outsignal)
                    tmp_second_outsignal = '%s+++check_impedance_line_shielding' % second_outsignal
                    step.affect(outsignal)
                    step.setFilterTypes('line|arc', 'positive')
                    step.setAttrFilter_pro('.imp_line')
                    step.setAttrLogic_pro()
                    step.selectAll()
                    step.resetFilter()
                    if step.Selected_count():
                        ...
                        step.unaffectAll()
                        step.affect(second_outsignal)
                        step.copySel(tmp_second_outsignal)
                        step.unaffectAll()
                        step.affect(second_outsignal)
                        step.setFilterTypes('pad', 'negative')
                        step.refSelectFilter(tmp_outsignal)
                        step.resetFilter()
                        if step.Selected_count():
                            step.copySel(tmp_second_outsignal, 'yes')
                        step.unaffectAll()
                        step.affect(tmp_outsignal)
                        step.refSelectFilter(tmp_second_outsignal, mode='cover')
                        if step.Selected_count():
                            step.selectDelete()
                        step.selectAll()
                        if step.Selected_count():
                            log = u"step:{0} 检测到{1}层阻抗线未完全被{2}屏蔽！".format(step.name, outsignal,second_outsignal)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, tmp_outsignal,
                                                                                     second_outsignal, log, dic_zu_layer)
                        else:
                            step.removeLayer(tmp_outsignal)
                        step.unaffectAll()
                        step.removeLayer(tmp_second_outsignal)
                    step.unaffectAll()
        losses = [name for name in check_steps if 'loss' in name]
        for loss in losses:
            step = gen.Step(job, loss)
            step.initStep()
            for outsignal, second_outsignals in check_layers.items():
                for second_outsignal in second_outsignals:
                    ...
                    step.affect(outsignal)
                    step.setFilterTypes('line|arc', 'positive')
                    step.setAttrFilter_pro('.string',text='loss_line')
                    step.setAttrLogic_pro()
                    step.selectAll()
                    step.resetFilter()
                    if step.Selected_count():
                        ...
                        step.selectAll()
                        if step.Selected_count():
                            log = u"step:{0} 检测到{1}层loss线未完全被{2}屏蔽！".format(step.name, outsignal,
                                                                                       second_outsignal)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step,
                                                                                 tmp_outsignal,
                                                                                 second_outsignal, log,
                                                                                 dic_zu_layer)
                        else:
                            step.removeLayer(tmp_outsignal)
                        step.unaffectAll()
    ...
    # 20250326 zl 检测客户型号是否正确
    def check_customer_jobname(self, autocheck=None):
        import connect_database_all as incam_tool
        arraylist = []
        dic_zu_layer = {}
        get_pnl_steps = get_panelset_sr_step(jobname=job.name, panelset='panel')
        #
        customer_jobname = incam_tool.InPlan(jobname).get_job_customer()
        for stepname in get_pnl_steps:
            step = gen.Step(job, stepname)
            step.initStep()
            for layer in outsignalLayers + solderMaskLayers + silkscreenLayers:
                feat_index = []
                step.affect(layer)
                step.setAttrFilter('.string','text=customer_jobname')
                step.selectAll()
                step.resetFilter()
                if step.Selected_count():
                    layerobj = gen.Layer(step, layer)
                    feats = layerobj.featout_dic_Index(options='feat_index+select')['text']
                    if not feats:
                        log = u"检测到{0}层添加的客户型号不是text属性！".format(layer)
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, layer, layer,
                                                                             log, dic_zu_layer)
                        step.unaffectAll()
                        continue
                    for feat in feats:
                        if feat.text != customer_jobname:
                            feat_index.append(feat.feat_index)
                step.unaffectAll()
                if feat_index:
                    step.affect(layer)
                    for index in feat_index:
                        step.selectByIndex(layer, index)
                    log = u"检测到{0}层添加的客户型号text跟erp的客户型号不一致！".format(layer)
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, layer, layer, log, dic_zu_layer)
                    step.unaffectAll()
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
    ...
    # 20250402 zl 检测panel尺寸  cam和mi尺寸是否一致
    def check_panel_rout_size(self, autocheck=None):
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            panel_step.initStep()
            if array_mrp_info:
                last_rout_layer = 'pnl_rout%s' % len(array_mrp_info) if len(array_mrp_info) else 'pnl_rout'
                if panel_step.isLayer(last_rout_layer):
                    tmp_last_rout = '%s++++' % last_rout_layer
                    panel_step.removeLayer(tmp_last_rout)
                    panel_step.affect(last_rout_layer)
                    panel_step.copySel(tmp_last_rout)
                    panel_step.unaffectAll()
                    panel_step.affect(tmp_last_rout)
                    panel_step.selectCutData()
                    panel_step.setFilterTypes('line')
                    panel_step.selectAll()
                    panel_step.resetFilter()
                    if panel_step.Selected_count():
                        panel_step.selectDelete()
                    panel_step.unaffectAll()
                    gLIMITSxmin, gLIMITSymin, gLIMITSxmax, gLIMITSymax = get_layer_limits(panel_step, tmp_last_rout)
                    xsize = gLIMITSxmax - gLIMITSxmin
                    ysize = gLIMITSymax - gLIMITSymin
                    if xsize != pnl_routx or ysize != pnl_routy:
                        log = u"检测到锣边尺寸（x:%s y:%s）和mi尺寸（x:%s y:%s）不一致！" % (xsize, ysize, pnl_routx, pnl_routy)
                        arraylist.append(log)
                    panel_step.removeLayer(tmp_last_rout)
                    panel_step.removeLayer(tmp_last_rout + '+++')
        if arraylist:
    ...
    # 20250415 zl  检测导气孔径是否正确
    def check_hole_in_dq_layers(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        get_pnl_steps = get_panelset_sr_step(jobname=job.name, panelset='panel')
        dqs = list(filter(lambda row: row.endswith('dq'), job.matrix.returnRows()))
        if not dqs:
            return u"success 没有导气不需要检测", None
        target_holes = set(['r2500','r1000'])
        for stepname in get_pnl_steps:
            step = gen.Step(job, stepname)
            step.initStep()
            for dq in dqs:
                step.affect(dq)
                step.setFilterTypes('pad|line')
                step.selectAll()
                step.resetFilter()
                if step.Selected_count():
                    layer_ = gen.Layer(step, dq)
                    pads = layer_.featSelOut(units='mm')['pads']
                    lines = layer_.featSelOut(units='mm')['lines']
                    if lines:
                        pads += lines
                    symbols = set([obj.symbol for obj in pads])
                    step.clearSel()
                    extra_holes = symbols - target_holes
                    if extra_holes:
                        step.selectSymbol(';'.join(extra_holes))
                        step.resetFilter()
                        log = u"检测到{0}层有其他孔径（{1}）！".format(dq, u'、'.join(extra_holes))
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, dq, dq,
                                                                             log, dic_zu_layer)
                    miss_holes = target_holes - symbols
                    if miss_holes:
                        log = u"检测到{0}层缺失孔径（{1}）！".format(dq, u'、'.join(miss_holes))
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, dq, dq,
                                                                             log, dic_zu_layer)
                # 判断除了pad|line之外的异常
                ...
                step.selectReverse()
                if step.Selected_count():
                    log = u"检测到{0}层有不是孔的异常物件！".format(dq)
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, dq, dq,
                                                                         log, dic_zu_layer)
                step.unaffectAll()
    ...
    # 20250422 zl A86/D10客户要固定加D-COUPON和拉力测试coupon
    def check_exist_d_coupon_and_test_coupon(self, autocheck=None):
        '''http://192.168.2.120:82/zentao/story-view-8799-2.html'''
        arraylist = []
        dic_zu_layer = {}
        if jobname[1:4] not in ["a86", "d10"]:
            return u"success非nv料号", None
        if 'panel' not in job.steps.keys():
            return u"success 没有panel", None
        get_pnl_steps = get_panelset_sr_step(job.name, 'panel')
        d_coupons = [step for step in get_pnl_steps if 'd-coupon' in str(step)]
        cu_coupons = [step for step in get_pnl_steps if 'cu-coupon' in str(step)]
        if not d_coupons:
            arraylist.append(u'panel拼版缺少d-coupon')
        if not cu_coupons:
            arraylist.append(u'panel拼版缺少拉力测试coupon')
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
                self.view_note_detail(dic_zu_layer)
    ...
    # 20250424 zl
    def check_bars_in_area(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        stepname = 'panel'
        step = gen.Step(job, stepname)
        ...
        os.system('python /incam/server/site_data/scripts/hdi_scr/Panel/check_bars_in_area/check_bars_in_area.py auto_check')
        if os.path.exists(tmp_file):
            with open(tmp_file) as reader:
                content = reader.read()
            os.unlink(tmp_file)
            if content:
                loads = json.loads(content)
                for dict_ in loads:
                    if dict_.get('type') == 'critical':
                        log = dict_.get('content').encode('utf8').decode('utf8')
                        show_layers = re.findall('(_\w+)', log)
                        arraylist.append(log)
                        if show_layers:
                            dic_zu_layer = self.get_view_dic_layers(job.name, stepname, step,
                                                                        worklayer="view_layer_" + show_layers[0],
                                                                        dic_layer_list={
                                                                        u"查看异常层：": show_layers,},
                                                                        dic_zu_layer=dic_zu_layer)
        if arraylist:
    ...
    # 检测二次防焊资料针对动态变量（实际生产有变动）位置做开通窗
    def check_open_window_of_secondary_sm(self, autocheck=None):
        '''http://192.168.2.120:82/zentao/story-view-8737.html'''
        arraylist = []
        dic_zu_layer = {}
        stepnames = ('edit', 'set')
        m1_list = []
        rows = job.matrix.returnRows()
        if 'm1-1' in rows and 'm1-2' in rows:
            m1_list.append('m1-1')
        if 'm2-1' in rows and 'm2-2' in rows:
            m1_list.append('m2-1')
        if not m1_list:
            return u"success没有二次防焊资料", None
        for stepname in stepnames:
            if stepname not in job.steps.keys():
                continue
            step = gen.Step(job, stepname)
            step.initStep()
            for m1 in m1_list:
                ...
                step.selectAll()
                areas, index_list = [], []
                m1_layer = gen.Layer(step, m1)
                step.resetFilter()
                if step.Selected_count():
                    feats = m1_layer.featout_dic_Index(options='select+feat_index')['text']
                    step.clearSel()
                    for feat in feats:
                        text = feat.text.replace("'", '')
                        if re.search('\$\$', text):
                            print text
                            index_list.append(feat.feat_index)
                    if index_list:
                        for index in index_list:
                            step.selectByIndex(m1, index)
                            xmin, ymin, xmax, ymax = get_layer_selected_limits(step, m1)
                            areas.append([xmin - 0.127, ymin - 0.127, xmax + 0.127, ymax + 0.127])  # 单边需要5mil以上开窗
                            step.clearSel()
                step.unaffectAll()
                m2 = m1.replace('-1', '-2')
                tmp_m2 = '%s+++' % m2
                ...
                if areas:
                    ...
                    for area in areas:
                        step.addRectangle(area[0], area[1], area[2], area[3])
                    step.refSelectFilter(tmp_m2, mode='cover')
                    if step.Selected_count():
                        step.selectDelete()
                    step.unaffectAll()
                    step.removeLayer(tmp_m2)
                    ...
                    if step.Selected_count():
                        ...
                        ids = []
                        for feat in feats:
                            if feat.feat_index in index_list:
                                ids.append(feat.feat_index)
                        if ids:
                            for id in ids:
                                step.selectByIndex(m1, id)
                            log = u"检测到{0}层TXT属性及动态变量字体位置未做开通窗".format(m1)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, m1, m2,
                                                                                 log, dic_zu_layer)
                    step.unaffectAll()
                    step.removeLayer(area_layer)
        if arraylist:
    ...
    # 20250513 zl
    def check_feature_outside_of_pnlrout2(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        if 'panel' not in job.steps.keys():
            return u"success 没有panel", None
        mrp_info = get_inplan_mrp_info(job.name.upper())
        if not mrp_info:
            return u"没有压合数据", None
        pnlrout_dic = {}
        for data in mrp_info:
            if data['PROCESS_NUM'] > 1:
                pnlrout = 'pnl_rout%s' % (data['PROCESS_NUM']  - 1)
                pnlrout_dic[pnlrout] = [data['FROMLAY'], data['TOLAY']]
        step = gen.Step(job, 'panel')
        drills = mai_drill_layers + laser_drill_layers
        print('aaa', pnlrout_dic)
        if list(pnlrout_dic.keys()):
            pnlrout_dic[list(pnlrout_dic.keys())[-1]] = tongkongDrillLayer + ksz_layers + bd_drillLayer
            for key, lays in pnlrout_dic.items()[:-1]:
                if step.isLayer(key):
                    start, end = int(lays[0].replace('L', '')), int(lays[1].replace('L', ''))
                    drls = []
                    for layer in drills:
                        layer_ = re.sub('[sb]', '', layer)
                        start_, end_ = layer_.split('-')
                        ...
                        if start_ < end_:
                            ...
                            if end_ == start and start_ <= end:
                                drls.append(layer)
                    pnlrout_dic[key] = drls
            #
            step.initStep()
            if step.isLayer('pnl_rout'):
                for pnlrout, drls in pnlrout_dic.items():
                    tmp_pnlrout = '%s+++' % pnlrout
                    step.affect('pnl_rout')
                    step.setAttrFilter_pro('.string', text=pnlrout)
                    step.setAttrLogic_pro()
                    step.selectAll()
                    step.resetFilter()
                    if step.Selected_count():
                        step.copySel(tmp_pnlrout)
                        step.unaffectAll()
                        step.affect(tmp_pnlrout)
                        step.selectCutData()
                        step.unaffectAll()
                        step.removeLayer('%s+++' % tmp_pnlrout)
                        for drl in drls:
                            step.affect(drl)
                            step.refSelectFilter(tmp_pnlrout, mode='cover')
                            step.selectReverse()
                            if step.Selected_count():
                                log = u"检测到{0}层有物件超出锣边层{1}！".format(drl, pnlrout)
                                arraylist.append(log)
                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, drl, pnlrout,
                                                                                     log, dic_zu_layer)
                            step.unaffectAll()
                        step.removeLayer(tmp_pnlrout)
                    step.unaffectAll()
        else:
            if step.isLayer('pnl_rout'):
                pnlrout_dic['pnl_rout'] = tongkongDrillLayer + ksz_layers + bd_drillLayer
            step.initStep()
            print(pnlrout_dic)
            for pnlrout, drls in pnlrout_dic.items():
                tmp_pnlrout = '%s+++' % pnlrout
                ...
                for drl in drls:
                    ...
                    if step.Selected_count():
                        log = u"检测到{0}层有物件超出锣边层{1}！".format(drl, pnlrout)
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, drl, pnlrout, log, dic_zu_layer)
                    step.unaffectAll()
                step.removeLayer(tmp_pnlrout)
        if arraylist:
            if autocheck == "show_ui":
    ...
    # 20250512 zl
    def check_feature_outside_of_pnlrout(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        if 'panel' not in job.steps.keys():
            return u"success 没有panel", None
        ...
        for data in mrp_info:
            if data['PROCESS_NUM'] > 1:
                pnlrout = 'pnl_rout%s' % (data['PROCESS_NUM']  - 1)
                pnlrout_dic[pnlrout] = [int(data['FROMLAY'].replace('L', '')), int(data['TOLAY'].replace('L', ''))]

        step = gen.Step(job, 'panel')
        drills = mai_drill_layers + laser_drill_layers
        drill_lays = job.matrix.returnRows('board', 'drill')
        inns = list(filter(lambda drill_lay: re.match('inn(\d{2,})?$', drill_lay), drill_lays))
        # print('aaa', pnlrout_dic)
        pnlrout_drls = {}
        if list(pnlrout_dic.keys()):
            keys = sorted(pnlrout_dic.keys(), key=lambda i: int(i.split('pnl_rout')[1]))
            pnlrout_drls = dict.fromkeys(keys)
            for key in pnlrout_drls.keys():
                pnlrout_drls[key] = []  # 设置初始为[]
            pnlrout_drls[keys[-1]] = tongkongDrillLayer
            # 盲孔和镭射钻
            for layer in drills:
                layer_ = re.sub('[sb]', '', layer)
                start_, end_ = layer_.split('-')
                start_, end_ = int(start_), int(end_)
                for key in keys:
                    lays = pnlrout_dic.get(key)
                    if step.isLayer(key):
                        start, end = lays[0], lays[1]
                        if start_ < end_:
                            if start_ == start and end_ <= end:
                                pnlrout_drls[key].append(layer)
                        else:
                            if start_ == end and end_ >= start:
                                pnlrout_drls[key].append(layer)
            # 备钻和控深钻
            for layer in bd_drillLayer + ksz_layers:
                    side = 'c' if 'c' in layer[2:] else 's'
                    if not re.search('-(\d+)', layer):
                        pnlrout_drls[keys[-1]].append(layer)
                        continue
                    laynum_ = int(re.search('-(\d+)', layer).group(1))
                    for key in keys[:-1]:
                        lays = pnlrout_dic.get(key)
                        if step.isLayer(key):
                            start, end = lays[0], lays[1]
                            if side == 'c':
                                ...
                                if laynum_ == end:
                                    pnlrout_drls[key].append(layer)
            # 树脂塞孔
            for layer in inner_sz_layers:
                layer_ = re.search('\d+-\d+', layer).group(0)
                start_, end_ = layer_.split('-')
                start_, end_ = int(start_), int(end_)
                for key in keys[:-1]:
                    lays = pnlrout_dic.get(key)
            ...
            # inn层
            for layer in inns:
                res = re.search('\d+', layer)
                if res:
                    layer_ = res.group(0)
                    if len(layer_) < 4:
                        start_, end_ = layer_[0], layer_[1:]
                    else:
                        start_, end_ = layer_[:2], layer_[2:]
                    start_, end_ = int(start_), int(end_)
                    for key in keys:
                        lays = pnlrout_dic.get(key)
                        if step.isLayer(key):
            ...
            for layer in outer_sz_layers + lp_sm_layers:
                pnlrout_drls[keys[-1]].append(layer)
        else:
            if step.isLayer('pnl_rout'):
                pnlrout_drls['pnl_rout'] = tongkongDrillLayer
                for layer in bd_drillLayer + ksz_layers:
                    if not re.search('-(\d+)', layer):
                        pnlrout_drls['pnl_rout'].append(layer)
                        continue
                    laynum_ = int(re.search('-(\d+)', layer).group(1))
                    if laynum_ == lay_num:
                        pnlrout_drls['pnl_rout'].append(layer)
                for layer in outer_sz_layers + lp_sm_layers:
                    pnlrout_drls['pnl_rout'].append(layer)
        step.initStep()
        # print('pnlrout_drls', pnlrout_drls)
        for pnlrout, drls in pnlrout_drls.items():
            tmp_pnlrout = '%s+++' % pnlrout
            if pnlrout == 'pnl_rout1' and not step.isLayer(pnlrout) and step.isLayer('pnl_rout'): # 如果没有pnl_rout1 找pnl_rout
                pnlrout = 'pnl_rout'
            step.affect(pnlrout)
            step.copySel(tmp_pnlrout)
            step.unaffectAll()
            step.affect(tmp_pnlrout)
            step.selectCutData()
            print pnlrout, step.COMANS
            if step.COMANS != '1 0 1 1':
                log = u"检测到锣边层%s无法填充！请修改资料" % pnlrout
                arraylist.append(log)
                step.unaffectAll()
                step.removeLayer(tmp_pnlrout)
                step.removeLayer('%s+++' % tmp_pnlrout)
            ...
            if step.Selected_count():
                ...
                if step.Selected_count():
                    step.selectDelete()
            step.COM('sel_delete_atr,mode=all')
            step.selectResize(-4000)
            step.unaffectAll()
            step.removeLayer('%s+++'% tmp_pnlrout)
            for drl in drls:
                step.affect(drl)
                ...
                step.selectReverse()
                if step.Selected_count():
                    log = u"检测到{0}层有物件超出锣边层{1}内2mm处！".format(drl, pnlrout)
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, drl, pnlrout, log, dic_zu_layer)
                step.unaffectAll()
            step.removeLayer(tmp_pnlrout)
    ...
    # 20250521 zl lot号旋转检测
    def check_lot_rotation(self, autocheck=None):
        stepname = 'panel'
        arraylist = []
        dic_zu_layer = {}
        rlayers = []
        halfjob_numb = lay_num/2
        cur_script_dir = '/incam/server/site_data/scripts/hdi_scr/Output/Laser'
        global special_laser_mode
        special_laser_mode = None
        lot_base_step = []
        r515CheckData = {}
        Merged_layer_top, Merged_layer_bot = [], []
        laser_resize_dict, merge_status = {}, {}
        panel_step = gen.Step(job, stepname)
        for i in range(len(matrixinfo["gROWname"])):
            if matrixinfo["gROWcontext"][i] == "board" and matrixinfo["gROWlayer_type"][i] == "drill" and re.search('^s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$', matrixinfo['gROWname'][i]):
                if re.search('sd1024e6t96a1|ba8622o6t97a1', jobname):
                    rlayers.append(matrixinfo['gROWname'][i])
                    continue
                first, end = int(re.search('^s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$', matrixinfo['gROWname'][i]).group(1)), int(re.match('s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$', matrixinfo['gROWname'][i]).group(2))
                laser_resize_dict[matrixinfo['gROWname'][i]] = abs(end - first) - 1
                if merge_status.get(matrixinfo['gROWname'][i]) == 'yes':
                    continue
                status = 'yes'
                merged_layer_t_index, merged_layer_b_index, top_merged_layer_list, bot_merged_layer_list = [],[],[],[]
                for h in range(len(matrixinfo["gROWname"])):
                    if matrixinfo["gROWcontext"][h] == "board" and re.search('^s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$', matrixinfo['gROWname'][h]) and \
                        matrixinfo['gROWname'][h] != matrixinfo['gROWname'][i]:
                        first_, end_ = int(re.search('^s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$', matrixinfo['gROWname'][h]).group(1)), int(re.search('^s(\d+)-(\d+)(-ls\d{6}\.\d{4})?$', matrixinfo['gROWname'][h]).group(2))
                        if first == first_:
                            if first < halfjob_numb:
                                if end < end_:
                                    if merged_layer_t_index:
                                        merged_layer_t_index.append(end_)
                                    else:
                                        merged_layer_t_index.append(first)
                                        merged_layer_t_index.append(end)
                                        merged_layer_t_index.append(end_)
                                    top_merged_layer_list.append(matrixinfo['gROWname'][h])
                                    top_merged_layer_list.append(matrixinfo['gROWname'][i])
                                status = 'no'
                            else:
                                if end < end_:
                                    if merged_layer_b_index:
                                        merged_layer_b_index.append(end_)
                                    else:
                                        merged_layer_b_index.append(first)
                                        merged_layer_b_index.append(end)
                                        merged_layer_b_index.append(end_)
                                    bot_merged_layer_list.append(matrixinfo['gROWname'][h])
                                    bot_merged_layer_list.append(matrixinfo['gROWname'][i])
                                status = 'no'
                merged_layer_t = ''
                if merged_layer_t_index:
                    new_merged_layer_t_index = sorted(merged_layer_t_index)
                    merged_layer_t = 's%s' % ('-'.join([str(index) for index in new_merged_layer_t_index]))
                    panel_step.removeLayer(merged_layer_t)
                    Merged_layer_top.append(merged_layer_t)
                merged_layer_b = ''
                if merged_layer_b_index:
                    new_merged_layer_b_index = sorted(merged_layer_b_index)
                    merged_layer_b = 's%s' % ('-'.join([str(index) for index in new_merged_layer_b_index]))
                    panel_step.removeLayer(merged_layer_b)
                    Merged_layer_bot.append(merged_layer_b)
                copy_pnl = 'yes'
                top_max_size = 0
                for m_layer in top_merged_layer_list:
                    if laser_resize_dict.get(m_layer):
                ...
                if merged_layer_t != '':
                    join_merge_layer = ' '.join(top_merged_layer_list)
                    os.system('python %s/merge_laser_layer.py %s %s' % (cur_script_dir, merged_layer_t, join_merge_layer))
                    for m_layer in top_merged_layer_list:
                        merge_status[m_layer] = 'yes'
                #
                bot_max_size = 0
                for m_layer in bot_merged_layer_list:
                    if laser_resize_dict.get(m_layer):
                ...
                if merged_layer_b != '':
                    join_merge_layer = ' '.join(bot_merged_layer_list)
                    os.system('python %s/merge_laser_layer.py %s %s' % (cur_script_dir, merged_layer_b, join_merge_layer))
                    for m_layer in bot_merged_layer_list:
                        merge_status[m_layer] = 'yes'
                if status == 'yes':
                    if merge_status.get(matrixinfo['gROWname'][i]) != 'yes':
                        rlayers.append(matrixinfo['gROWname'][i])
            elif 'sd1024e6t96a1' in jobname and matrixinfo["gROWcontext"][i] == "board" and matrixinfo["gROWlayer_type"][i] == "drill" and matrixinfo['gROWname'][i] == 's18-17-2':
                rlayers.append(matrixinfo['gROWname'][i])
        rlayers.extend(Merged_layer_top + Merged_layer_bot)
        print('rlayers', rlayers)
        ###
        for rlayer in rlayers:
            pnlspecial = ('r125', 'r19.724', 'r20.276', 'r20.472')
            r515num = None
            m = 0
            r515CheckData[rlayer] = {}
            info = job.DO_INFO('-t layer -e %s/%s/%s -d SYMS_HIST -o break_sr' % (jobname, stepname, rlayer))
            if re.match('(s\d+-\d+)(-ls\d{6}\.\d{4})?$', rlayer):
                show_rlayer = re.match('(s\d+-\d+)(-ls\d{6}\.\d{4})?$', rlayer).group(1)
            for _ in info.get('gSYMS_HISTsymbol'):
                if _ in pnlspecial:
                    if _ == pnlspecial[0]:
                        r3175num = info.get('gSYMS_HISTpad')[m]
                    if _ == pnlspecial[2]:
                        r515num = info.get('gSYMS_HISTpad')[m]
                else:
                    if _ == 'r1.772':
                        r45num = info.get('gSYMS_HISTpad')[m]
                    # symbol_size = _
                    # symbol_size = re.sub('r', '', symbol_size)
                    # drillSize = round(float(symbol_size) * 25.4)
                    # if 457 <= drillSize and drillSize != 515 and drillSize != 520:
                    #     log = '%s层中存在大于等于18mil的镭射孔，大小: %s 请检查！' % (rlayer, drillSize)
                    #     arraylist.append(log)
            if r515num:
                r515CheckData[rlayer]['lot_design'] = 'yes'
            res = re.search('s(\d+)-(\d+)-?(\d+)?.*$', rlayer)
            if res:
                s, e = int(res.group(1)), int(res.group(2))
                if s < e and s <= halfjob_numb or (s > e >= halfjob_numb):
                    if s < e and s < halfjob_numb:
                        mir_choose = 'no'
                        if s == 1:
                            r515CheckData[rlayer]['mir'] = mir_choose
                    elif s > e >= halfjob_numb:
                        mir_choose = 'yes'
                        if s == lay_num:
                            r515CheckData[rlayer]['mir'] = mir_choose
                        if s > lay_num:
                            log = u'%s镭射层钻带的起始层超出层别数量,请检查' % rlayer
                            arraylist.append(log)
                elif re.search('s(\d+)-(\d+)-(\d+)$', rlayer):
                    pass
                else:
                    log = u'%s镭射层钻带的起始层定义不正确,请检查' % rlayer
                    arraylist.append(log)
            else:
                log = u'%s镭射层别非最新命名，应为s开始层-结束层，中间有横杠,请检查' % rlayer
                arraylist.append(log)
        def check_if_special_laser():
            global special_laser_mode
            info = job.DO_INFO('-t step -e %s/%s -d SR,units=mm' % (jobname, stepname))
            for i in range(len(info.get('gSRstep'))):
                if info.get('gSRflip')[i] == 'yes':
                    FlipSetExist = 'yes'
                    special_laser_mode = 'yes'
                    return
            # === step list	去重 ===
            steplist = set(info.get('gSRstep'))
            for tmpLaser in r515CheckData.keys():
                info = job.DO_INFO('-t layer -e %s/%s/%s -d SYMS_HIST' % (jobname, stepname, tmpLaser))
                pnl_symbol_list = info['gSYMS_HISTsymbol']
                pnl_pad_num = info['gSYMS_HISTpad']
                # === symbbol 及数量的键值对 ===
                pnl_symbol_hash = {}
                # pnl_symbol_hash[pnl_symbol_list] = pnl_pad_num
                # pnl_symbol_hash = {key: value for key, value in zip(pnl_symbol_list, pnl_pad_num)}
                for key, value in zip(pnl_symbol_list, pnl_pad_num):
                    pnl_symbol_hash[key] = value
                if pnl_symbol_hash.get('r20.276'):
                    special_laser_mode = 'yes'
                    return
                for current_step in steplist:
                    info = job.DO_INFO('-t layer -e %s/%s/%s -d SYMS_HIST -o break_sr' % (jobname, current_step, tmpLaser))
                    symbol_list = info['gSYMS_HISTsymbol']
                    pad_num = info['gSYMS_HISTpad']
                    symbol_hash = {}
                    # symbol_hash[symbol_list] = pad_num
                    # symbol_hash = {key: value for key, value in zip(symbol_list, pad_num)}
                    for key, value in zip(symbol_list, pad_num):
                        symbol_hash[key] = value
                    if symbol_hash.get('r20.276'):
                        r515CheckData[tmpLaser]['lotCode'] = ''
                        lot_base_step.append(current_step)
                        if current_step != 'set':
                            special_laser_mode = 'yes'
                            r515CheckData[tmpLaser]['laser_base_step'] = current_step
                        else:
                            info = job.DO_INFO('-t layer -e %s/%s/%s -d SYMS_HIST' % (jobname, current_step, tmpLaser))
                            array_symbol_list = info.get('gSYMS_HISTsymbol')
                            array_pad_num = info.get('gSYMS_HISTpad')
                            array_symbol_hash = {}
                            # array_symbol_hash[array_symbol_list] = array_pad_num
                            # array_symbol_hash = {key: value for key, value in zip(array_symbol_list, array_pad_num)}
                            for key, value in zip(array_symbol_list, array_pad_num):
                                array_symbol_hash[key] = value
                            if array_symbol_hash.get('r20.276'):
                                if array_symbol_hash.get('r20.276') == symbol_hash.get('r20.276') and array_symbol_hash.get('r20.276') == 1:
                                    r515CheckData[tmpLaser]['laser_base_step'] = current_step
                                ...
                                special_laser_mode = 'yes'
            # job.PAUSE(special_laser_mode)
            if len(lot_base_step) == 0:
                special_laser_mode = 'no'

        def checkr515exist(dis_lay):
            r515exist = 'no'
            r515laser = 'r515%s' % dis_lay
            if 'set' in job.getSteps():
                lot_step = 'set'
            else:
                lot_step = 'edit'
            step = gen.Step(job, lot_step)
            step.initStep()
            step.removeLayer(r515laser)
            step.affect(dis_lay)
            step.selectSymbol('r515')
            step.resetFilter()
            if step.Selected_count():
                r515exist = 'yes'
                step.copySel(r515laser)
            if r515exist == 'yes':
                job.matrix.modifyRow(r515laser, type='drill')
            step.unaffectAll()
            return r515exist

        def checkSRtableContinue():
            # 用于检测是否set在panel中拼版为连续的
            info = job.DO_INFO('-t step -e %s/%s -d SR,units=mm' % (jobname, stepname))
            setNumList, firstSetInfo = [], {}
            if 'set' in job.getSteps():
                add_lot_step = 'set'
            else:
                add_lot_step = 'edit'
            for i in range(len(info.get('gSRstep'))):
                curStep = info.get('gSRstep')[i]
                if curStep == add_lot_step:
                    if info.get('gSRflip')[i] == 'yes':
                        return 'mirrorExist'
                    setNumList.append(i)
                    if len(setNumList) == 1:
                        laseNum = i
                        firstSetInfo = {
                            'stepName': info.get('gSRstep')[i],
                            'stepIndex': i,
                            'gSRxa': info.get('gSRxa')[i],
                            'gSRya': info.get('gSRya')[i],
                            'gSRdx': info.get('gSRdx')[i],
                            'gSRdy': info.get('gSRdy')[i],
                            'gSRnx': info.get('gSRnx')[i],
                            'gSRny': info.get('gSRny')[i],
                            'gSRangle': info.get('gSRangle')[i],
                            'gSRmirror': info.get('gSRmirror')[i],
                            'gSRxmin': info.get('gSRxmin')[i],
                            'gSRymin': info.get('gSRymin')[i],
                            'gSRxmax': info.get('gSRxmax')[i],
                            'gSRymax': info.get('gSRymax')[i],
                            'gSRflip': info.get('gSRflip')[i],
                        }
                    else:
                        tmp_chk_num = laseNum + 1
                        if i != tmp_chk_num:
                            return 'noContinue'
                        laseNum = i
            return firstSetInfo

        def checkLaserLotSameWithCopper(chkLaserLayer):
            # 用于检测工艺边上的lot号是否与底铜一致
            res = re.search('s([1-9][0-9]?)-([1-9][0-9]?)', chkLaserLayer)
            if res:
                lotAddlayer = 'l%s' % res.group(1)
            else:
                return 'laserNameError'
            chkLay = 'tmp_laser_lot+++'
            if 'set' in job.getSteps():
                set_step = 'set'
            else:
                set_step = 'edit'
            r515laser = 'r515%s' % chkLaserLayer
            # getlayerF = []
            step = gen.Step(job, set_step)
            step.initStep()
            step.affect(lotAddlayer)
            step.selectSymbol('laser-array-lot-cu')
            step.resetFilter()
            if step.Selected_count():
                # info = step.INFO('-t layer -e %s/%s/%s -d FEATURES -o select' % (jobname, set_step, lotAddlayer) )
                # del info[0]
                # getlayerF = info
                layerobj = gen.Layer(step, lotAddlayer)
                getlayerF = layerobj.featout_dic_Index(options='feat_index+select')['pads']
                step.copySel(chkLay)
            else:
            ...
            step.COM('sel_invert')
            step.selectBreak()
            step.refSelectFilter(r515laser, mode='cover')
            if step.Selected_count():
                step.removeLayer(chkLay)
                step.unaffectAll()
                return getlayerF
            else:
                step.unaffectAll()
        ...
        def judgeM97orM98Direct(chkLotLayer, getfirstSetInfo, getCheckSameCenter):
            # 通过底铜判断M97，M98方向
            firstSetInfo = getfirstSetInfo
            laserLotInfo = getCheckSameCenter
            # print('aaaaaaaaaa', laserLotInfo)
            # print(laserLotInfo[0].mirror, laserLotInfo[0].rotation)
            lotCode, chkMirRef = None, None
            if firstSetInfo['gSRmirror'] == 'yes':
                print(u'set在panel中为镜像Lot号特殊加法，请检查')
            # Top面镭射不允许添加镜像；
            # Bot面镭射不允许添加非镜像；
            if re.search('s([1-9][0-9]?)-([1-9][0-9]?)',chkLotLayer):
                if r515CheckData[chkLotLayer].get('mir'):
                    chkMirRef = r515CheckData[chkLotLayer].get('mir')
                else:
                    return 'laserNameError'
            else:
                return 'laserNameError'
            if laserLotInfo[0].mirror != chkMirRef:
                log = u'set中镭射%s 对应层别添加的laser-array-lot-cu镜像为%s, 实际应该添加镜像为%s' % (chkLotLayer, laserLotInfo[0].mirror, chkMirRef)
                arraylist.append(log)
                return -1
            if laserLotInfo[0].mirror == 'yes':
                pnlLotAngle = laserLotInfo[0].rotation - firstSetInfo['gSRangle'] + 360
                if pnlLotAngle >= 360:
                    pnlLotAngle -= 360
                if pnlLotAngle == 180:
                    lotCode = 'M97'
                    ...
                    log = u'层别%s 在pnl中的lot角度为%s,仅支持180度90度' % (chkLotLayer, pnlLotAngle)
                    arraylist.append(log)
                    return -1
            else:
                pnlLotAngle = laserLotInfo[0].rotation + firstSetInfo['gSRangle']
                if pnlLotAngle >= 360:
                    pnlLotAngle -= 360
                if pnlLotAngle == 0:
                    lotCode = 'M97'
                elif pnlLotAngle == 270:
                    lotCode = 'M98'
                else:
                    log = u'层别%s 在pnl中的lot角度为%s,仅支持0度270度' % (chkLotLayer, pnlLotAngle)
                    arraylist.append(log)
                    return -1
            return lotCode

        def mainCheckLaserLot(chkLotLyr):
            getfirstSetInfo = checkSRtableContinue()
            flipStepExist = 'no'
            if getfirstSetInfo == 'noContinue':
                log = u'set在panel的S&R Table中未设计连续'
                arraylist.append(log)
                return -1
            elif getfirstSetInfo == 'mirrorExist':
                print(u'set在panel的S&R Table中镜像拼板,程序特殊算法')
                if 's1-' not in chkLotLyr:
                    log = u'set中的r515必须加于Top面,程序退出'
                    arraylist.append(log)
                    return -1
                flipStepExist = 'yes'
            getCheckSameCenter = checkLaserLotSameWithCopper(chkLotLyr)
            if getCheckSameCenter == 'noOutSymDesign':
                log = u'外层未设计laser-array-lot-cu或symbol被打散,程序退出'
                arraylist.append(log)
                return -1
            elif getCheckSameCenter == 'noSameCenter':
                log = u'外层设计laser-array-lot-cu与r515中心不一致,程序退出'
                arraylist.append(log)
                return -1
            elif getCheckSameCenter == 'laserNameError':
                log = u'镭射层$chkLotLyr 命名不正确,Code:100,程序退出'
                arraylist.append(log)
                return -1
            elif getCheckSameCenter == 'numErrorDesign':
                log = u'镭射层%s 对应外层laser-array-lot-cu数量超过1,Code:103,程序退出' % chkLotLyr
                arraylist.append(log)
                return -1
            if flipStepExist == 'yes':
                return 'mirrorExist'
            getLotDirec = judgeM97orM98Direct(chkLotLyr, getfirstSetInfo, getCheckSameCenter)
            if getLotDirec == 'laserNameError':
                log = u'镭射层%s 命名不正确,Code:101,程序退出' % chkLotLyr
                arraylist.append(log)
                return -1
            return getLotDirec
        #
        if r515CheckData:
            ...
            special_laser_mode = 'no'
        print('AAA special_laser_mode', special_laser_mode)
        if special_laser_mode == 'no':
            if r515CheckData:
                for tmpLaser in r515CheckData.keys():
                    r515exist = checkr515exist(tmpLaser)
                    if r515exist == 'yes':
                        mainCheckLaserLot(tmpLaser)
    ...
    # 20250526 zl 光学点、坏板点设计检测
    def check_optical_and_damaged_point(self, autocheck=None):
        """http://192.168.2.120:82/zentao/story-view-9054.html"""
        arraylist = []
        dic_zu_layer = {}
        stepname = 'set'
        if stepname not in job.getSteps().keys():
            return u'success 没有set', None
        step = gen.Step(job, stepname)
        step.initStep()
        isForbid = False
        for layer in outsignalLayers:
            optical_layer = 'optical+++%s' % layer
            tmplayer = '%s+++%s' % (layer, step.pid)
            step.affect(layer)
            step.copySel(tmplayer)
            step.unaffectAll()
            step.affect(tmplayer)
            step.setAttrFilter_pro('.fiducial_name')
            step.setAttrLogic_pro()
            step.selectAll()
            step.resetFilter()
            if step.Selected_count():
                step.moveSel(optical_layer)
                step.Contourize(0)
                step.unaffectAll()
                sm_layer = 'm1' if layer == 'l1' else 'm2'
                optical_window_layer = '%s+++' % sm_layer
                step.affect(sm_layer)
                step.refSelectFilter(optical_layer, mode='same_center')
                if step.Selected_count():
                    step.copyToLayer(optical_window_layer, size=254)
                    step.unaffectAll()
                    step.affect(optical_window_layer)
                    step.refSelectFilter(tmplayer)
                    if step.Selected_count():
                        step.selectReverse()
                        if step.Selected_count():
                            step.selectDelete()
                        step.unaffectAll()
                        step.affect(sm_layer)
                        step.refSelectFilter(optical_window_layer, mode='cover')
                        if step.Selected_count():
                            log = u"检测到%s层光学点开窗距外层物件间距单边没有5mil以上" % sm_layer
                            arraylist.append(log)
                            isForbid = True
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, sm_layer,
                                                                                 layer, log, dic_zu_layer)
                        step.unaffectAll()
                    step.unaffectAll()
                step.unaffectAll()
                step.removeLayer(optical_window_layer)
                step.removeLayer(optical_layer)
            else:
                log = u'%s未识别到光学点（.fiducial_name属性）' % layer
                arraylist.append(log)
                isForbid = True
            step.unaffectAll()
            optical_layer = 'optical+++%s' % layer
            step.truncate(tmplayer)
            step.affect(layer)
            step.copySel(tmplayer)
            step.unaffectAll()
            step.affect(tmplayer)
            step.setAttrFilter_pro('.fiducial_name', text='badmark')
            step.setAttrLogic_pro()
            step.selectAll()
            step.resetFilter()
            if step.Selected_count():
                ...
                if step.Selected_count():
                    ...
                    if step.Selected_count():
                        ...
                        step.refSelectFilter(optical_window_layer, mode='cover')
                        if step.Selected_count():
                            log = u"检测到%s层坏板点开窗距外层物件间距单边没有5mil以上" % sm_layer
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, sm_layer,
                                                                                 layer, log, dic_zu_layer)
                        step.unaffectAll()
                ...
                log = u'%s未识别到坏板点（.fiducial_name=badmark属性）' % layer
                arraylist.append(log)
            step.unaffectAll()
            step.removeLayer(tmplayer)
        if arraylist:
            if isForbid:
                update_current_job_check_log(os.environ.get("CHECK_ID", "NULL"),
                                             u"forbid_or_information='{0}'".format(u"拦截"))
    ...
    # 20250609 zl
    def check_npth_dot_2_cu(self, autocheck=None):
        """检测NPTH孔档点距离未开窗的铜的距离最小满足10mil，不足报警
        http://192.168.2.120:82/zentao/story-view-9151.html
        """
        arraylist = []
        dic_zu_layer = {}
        check_step = list(filter(lambda s: s in ('edit', 'set'), job.steps.keys())) # 检测edit/set
        if not check_step:
            return u'success 没有set', None
        check_layers = {'md1': 'l1', 'md2': 'l%s' % jobname[4:6]}
        for stepname in check_step:
            step = gen.Step(job, stepname)
            step.initStep()
            for md, signal in check_layers.items():
                if step.isLayer(md):
                    tmp_md = '%s++++%s' % (md, step.pid)
                    step.affect(md)
                    step.setAttrFilter_pro('.bit', text='npth_dot')
                    step.setAttrLogic_pro()
                    step.selectAll()
                    step.resetFilter()
                    if step.Selected_count():
                        step.copyToLayer(tmp_md, size=508)
                        step.unaffectAll()
                        # 线路负片和铜转surface
                        tmp_signal = '%s++++%s' % (signal, step.pid)
                        step.affect(signal)
                        step.setFilterTypes('surface')
                        step.selectAll()
                        step.resetFilter()
                        if step.Selected_count():
                            step.copySel(tmp_signal)
                        step.setFilterTypes(polarity='negative')
                        step.selectAll()
                        step.resetFilter()
                        if step.Selected_count():
                            step.copySel(tmp_signal)
                        ...
                        if step.isLayer(tmp_signal):
                            ...
                            if step.Selected_count():
                                step.unaffectAll()
                                step.affect(md)
                                step.refSelectFilter(tmp_md, mode='same_center')
                                log = u"检测到%s层NPTH孔档点距离%s的铜距离不足10mil" % (md, signal)
                                arraylist.append(log)
                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, md,
                                                                                     signal, log, dic_zu_layer)
                                step.unaffectAll()
                            step.unaffectAll()
                            step.removeLayer(tmp_signal)
                        step.removeLayer(tmp_md)
                    step.unaffectAll()
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
    ...
    # 20250616 zl 检测靶孔距离其他孔间距
    def check_distance_between_targethole_and_drillhole(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        stepname = 'panel'
        del_drl_size = 'r2500'
        ...
        step = gen.Step(job, stepname)
        step.initStep()
        drillLayers = job.matrix.returnRows('board', 'drill')
        # print(list(filter(lambda drl: re.match('drl$|b(\d+)?-(\d+)?$|m(\d+)?-(\d+)?$|\dnd$|[b|c]d[c|s]$', drl), drillLayers)))
        for drl in drillLayers:
            sig_layer = 'l2'
            if re.match('drl$|b(\d+)?-(\d+)?$|m(\d+)?-(\d+)?$|\dnd$|[b|c]d[c|s]$', drl):
                if re.match('m(\d+)?-(\d+)?$', drl):
                    a,b = re.match('m(\d+)?-(\d+)?$', drl).groups()
                    inn = 'm%s-%s.inn' % (a, b)
                elif re.match('b(\d+)?-(\d+)?$', drl):
                    a, b = re.match('b(\d+)?-(\d+)?$', drl).groups()
                    inn = 'inn%s%s' % (a, b)
                    num_sig = int(a) + 1
                    sig_layer = 'l%s' % num_sig
                elif re.match('\dnd$|[b|c]d[c|s]$', drl):
                    inn = '%s.inn' % drl
                else:
                    inn = 'inn'
                inn_padup = '%s_padup_tl+++' % inn
                step.copyLayer(job.name, stepname, inn, inn_padup)
                step.affect(inn_padup)
                step.selectSymbol(del_drl_size)
                if step.Selected_count():
                    step.selectDelete()
                # step.selectResize(size * 2000)
                step.unaffectAll()
                drl_f = '%s_f_tl+++' % drl
                step.affect(sig_layer)
                step.setSymbolFilter('hdi1-by*')
                step.selectAll()
                step.resetFilter()
                if step.Selected_count():
                    step.copySel(inn_padup)
                step.unaffectAll()
                step.affect(inn_padup)
                step.selectChange('r%s' % (size * 2000))
                step.unaffectAll()
                step.Flatten(drl, drl_f)
                step.affect(inn_padup)
                step.refSelectFilter(drl_f)
                if step.Selected_count():
                    log = u"检测到%s层中心距%s层不足%sMM" % (inn, drl, size)
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, inn_padup,
                                                                         drl, log, dic_zu_layer)
                step.unaffectAll()
                step.removeLayer(drl_f)
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
                self.view_note_detail(dic_zu_layer)
    ...
    # 20250617 zl 选化开窗检测
    def check_selective_window(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        stepname = 'edit'
        step = gen.Step(job, stepname)
        step.initStep()
        if not step.isLayer('hj-pad') and not step.isLayer('osp-pad'):
            return u'success 没有层hj-pad/osp-pad', None
        if step.isLayer('hj-pad'):
            tmp_hj = 'hj-pad+++'
            step.removeLayer(tmp_hj)
            # 分析间距
            checklist = gen.Checklist(step, 'check_selective_window', 'valor_analysis_signal')
            checklist.action()
            # checklist.COM('chklist_cupd,chklist=valor_analysis_signal,nact=1,params=((pp_layer=hj-pad)(pp_spacing=20)(pp_r2c=10)(pp_d2c=10)(pp_sliver=4)(pp_min_pad_overlap=5)(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular')
            checklist.COM('chklist_cupd,chklist=valor_analysis_signal,nact=1,params=((pp_layer=hj-pad)(pp_spacing=254)(pp_r2c=254)(pp_d2c=254)(pp_sliver=101.6)(pp_min_pad_overlap=127)(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular')
            checklist.run()
            checklist.clear()
            # checklist.COM('chklist_cupd,chklist=valor_analysis_signal,nact=1,params=((pp_layer=hj-pad)(pp_spacing=20)(pp_r2c=10)(pp_d2c=10)(pp_sliver=4)(pp_min_pad_overlap=5)(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular')
            checklist.COM('chklist_cupd,chklist=valor_analysis_signal,nact=1,params=((pp_layer=hj-pad)(pp_spacing=254)(pp_r2c=254)(pp_d2c=254)(pp_sliver=101.6)(pp_min_pad_overlap=127)(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular')
            ...
            info = step.INFO('-t check -e %s/%s/%s -m script -d MEAS -o action=1+category=p2p,units=mm' % (
            jobname, stepname, checklist.name))
            coors = []
            if info:
                for line in info:
                    splits = line.split()
                    if float(splits[2]) < 177.8:  # 7mil
                        coors.append([float(splits[7]), float(splits[8]), float(splits[9]), float(splits[10])])
            if checklist.name in step.checks:
                step.deleteCheck(checklist.name)
            if coors:
                step.createLayer(tmp_hj)
                step.affect(tmp_hj)
                for coor in coors:
                    x1, y1, x2, y2 = coor
                    step.addLine(x1, y1, x2, y2, 'r1')
                step.unaffectAll()
                step.affect('hj-pad')
                step.refSelectFilter(tmp_hj)
                if step.Selected_count():
                    log = u"检测到hj-pad间距不足7mil"
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, 'hj-pad',
                                                                         'hj-pad', log, dic_zu_layer)
                step.unaffectAll()
            step.removeLayer(tmp_hj)
            if solderMaskLayers:
                tmp_hj = 'hj-pad+++'
                step.affect('hj-pad')
                step.copyToLayer(tmp_hj, size=152.4)
                step.unaffectAll()
                step.affect(tmp_hj)
                step.refSelectFilter('\;'.join(solderMaskLayers), mode='cover')
                if step.Selected_count():
                    step.selectDelete()
                step.selectReverse()
                if step.Selected_count():
                    log = u"检测到hj-pad开窗补偿不足3mil"
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, tmp_hj, solderMaskLayers[0], log, dic_zu_layer)
                    step.selectResize(-152.4)
                else:
                    step.removeLayer(tmp_hj)
                step.unaffectAll()
        sgt_lays = list(filter(lambda sgt: step.isLayer(sgt),('sgt-c', 'sgt-s')))
        if step.isLayer('osp-pad'):
            tmp_osp = 'osp-pad+++'
            step.removeLayer(tmp_osp)
            # 分析间距
            checklist = gen.Checklist(step,'check_selective_window', 'valor_analysis_signal')
            checklist.action()
            # checklist.COM('chklist_cupd,chklist=valor_analysis_signal,nact=1,params=((pp_layer=osp-pad)(pp_spacing=20)(pp_r2c=10)(pp_d2c=10)(pp_sliver=4)(pp_min_pad_overlap=5)(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular')
            checklist.COM('chklist_cupd,chklist=valor_analysis_signal,nact=1,params=((pp_layer=osp-pad)(pp_spacing=254)(pp_r2c=254)(pp_d2c=254)(pp_sliver=101.6)(pp_min_pad_overlap=127)(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular')
            checklist.run()
            checklist.clear()
            # checklist.COM('chklist_cupd,chklist=valor_analysis_signal,nact=1,params=((pp_layer=osp-pad)(pp_spacing=20)(pp_r2c=10)(pp_d2c=10)(pp_sliver=4)(pp_min_pad_overlap=5)(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular')
            checklist.COM('chklist_cupd,chklist=valor_analysis_signal,nact=1,params=((pp_layer=osp-pad)(pp_spacing=254)(pp_r2c=254)(pp_d2c=254)(pp_sliver=101.6)(pp_min_pad_overlap=127)(pp_tests=Spacing)(pp_selected=All)(pp_check_missing_pads_for_drills=Yes)(pp_use_compensated_rout=No)(pp_sm_spacing=No)),mode=regular')
            ...
            step.createCheck(checklist.name)
            step.pasteCheck(checklist.name)
            info = step.INFO('-t check -e %s/%s/%s -m script -d MEAS -o action=1+category=p2p,units=mm' % (jobname, stepname, checklist.name))
            coors = []
            if info:
                for line in info:
                    splits = line.split()
            ...
            if coors:
                ...
                step.unaffectAll()
                step.affect('osp-pad')
                step.refSelectFilter(tmp_osp)
                if step.Selected_count():
                    log = u"检测到osp-pad间距不足7mil"
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, 'osp-pad','osp-pad', log, dic_zu_layer)
                step.unaffectAll()
            step.removeLayer(tmp_osp)
            if sgt_lays:
                step.affect('osp-pad')
                step.copyToLayer(tmp_osp, size=152.4)
                step.unaffectAll()
                step.affect(tmp_osp)
                step.refSelectFilter('\;'.join(sgt_lays), mode='cover')
                if step.Selected_count():
                    step.selectDelete()
                step.selectReverse()
                if step.Selected_count():
                    log = u"检测到hj-pad开窗补偿不足3mil"
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step, tmp_osp,
                                                                         sgt_lays[0], log, dic_zu_layer)
                    step.selectResize(-152.4)
                else:
                    step.removeLayer(tmp_osp)
                step.unaffectAll()
    ...
    # 20250305 检测钻带拉伸是否正确
    def check_drill_through(self, autocheck=None):
        arraylist = []
        dic_zu_layer = {}
        matrix_ = job.matrix.getInfo()
        drill_dict = {}
        outsignal_rows = []
        ...
        for layer, start_end in drill_dict.items():
            ...
            if layer in tongkongDrillLayer:
                ...
                if (start_ in (start, end) or any([start_ + '-ls' in i for i in (start, end)])) and (
                        end_ in (start, end) or any([end_ + '-ls' in i for i in (start, end)])):
                    pass
                else:
                    err_layers.append(layer)
        if err_layers:
            for err_layer in err_layers:
                arraylist.append(u'检测到钻带%s贯穿层异常!' % err_layer)
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
                self.view_note_detail(dic_zu_layer)
    ...
    def check_md_cover_info(self, autocheck=None):
        """
        http://192.168.2.120:82/zentao/story-view-7552.html
        :return:
        """
        md_layers = filter(lambda row: row in ('md1', 'md2'), job.matrix.returnRows())
        arraylist = []
        dic_zu_layer = {}
        if "panel" in matrixinfo["gCOLstep_name"]:
            all_steps = get_panelset_sr_step(job.name, "panel")
            ...
            if 'tk.ykj' in rows:
                drl = 'tk.ykj'
            ksz_layers = filter(lambda row: re.match('cd(c|s)(\d+)?$', row),rows)
            if not drl and not ksz_layers:
                return u"没有钻孔层 请检查", None
            if not md_layers:
                return u"success 没有挡点层", None
            # 控深钻的逻辑
            # 构建字典 {'cds2': {'2': 'md2', '1.2': 'md2', '1.55': 'md2'}, 'cds': {'0.95': 'md2', '1.25': 'md2'}}
            ksz_map = {}
            results = getBackDrillDepth(jobname.upper())
            if results:
                for result in results:
                    if result.get('LAYER_NAME_') in ksz_layers:
                        if '控深平头' in result.get('ERP_FINISH_SIZE_'):
                            print(result.get('FINISHED_SIZE'))
                            # 判断挡点层
                            if int(result.get('START_INDEX')) < lay_num/2:
                                md_layer = 'md1'
                            else:
                                md_layer = 'md2'
                            if result.get('LAYER_NAME_') not in ksz_map.keys():
                                ksz_map[result.get('LAYER_NAME_')] = {}
                            ksz_map[result.get('LAYER_NAME_')].update({str(result.get('FINISHED_SIZE')): md_layer})
            # print(ksz_map)
            # 挡点层对应的钻孔
            md_drill = '%s+++md' % drl
            job.matrix.deleteRow(md_drill)
            for stepname in all_steps:
                step = gen.Step(job, stepname)
                step.initStep()
                step.setUnits('inch')
                ...
                for solderMaskLayer in solderMaskLayers:
                    ...
                    if step.isLayer(md_layer):
                        ...
                        if not step.Selected_count():
                            step.unaffectAll()
                            continue
                        step.copyToLayer(tmp_md, size=2)
                        step.clearAll()
                        ...
                        step.selectReverse()
                        if step.Selected_count():
                            log = u"{2}检测到挡点{0}不比防焊开窗{1}小单边1mil，请到{3}查看超出位置！".format(md_layer, solderMaskLayer, stepname, tmp_md)
                            arraylist.append(log)
                            dic_zu_layer = self.get_view_dic_layers(job.name, stepname, step, worklayer="view_layer_"+md_layer,dic_layer_list={u"挡点不比防焊单边小1mil，请查看异常层：": [
                ...
                if ksz_layers:
                    # TODO 检测是否存在挡点
                    # 钻孔表程序 是否有贯穿层 有贯穿层检测单面挡点  没有贯穿层的检测双面 单面挡点根据起始层判断是哪一层
                    for ksz in ksz_layers:
                        # 单面检测和双面检测的孔层
                        sigle_ksz = 'sigle_%s+++' % ksz
                        double_ksz = 'double_%s+++' % ksz
                        step.removeLayer(sigle_ksz)
                        step.removeLayer(double_ksz)
                        if ksz_map.get(ksz):
                            # 空层跳过
                            step.affect(ksz)
                            step.selectAll()
                            if not step.Selected_count():
                                ...
                                continue
                            step.clearSel()
                            # for size, md_layer in ksz_map.items():
                            sizes = map(lambda k:'r%s' % (float(k) * 1000), list(ksz_map.get(ksz).keys()))
                            md_layer = ksz_map.get(ksz).values()[0]
                            # step.affect(ksz)
                            step.selectSymbol('\;'.join(sizes))
                            if step.Selected_count():
                                step.copySel(sigle_ksz)
                                step.selectReverse()
                                step.resetFilter()
                                if step.Selected_count():
                                    step.copySel(double_ksz)
                            else:
                                step.resetFilter()
                                step.copySel(double_ksz)
                            step.unaffectAll()
                            if step.isLayer(sigle_ksz):
                                step.affect(sigle_ksz)
                                step.refSelectFilter(md_layer, mode='disjoint')
                                if step.Selected_count(): # 有未接触到的则有不存在的挡点 报出
                                    step.selectReverse()
                                    if step.Selected_count():
                                        step.selectDelete()
                                    step.unaffectAll()
                                    step.affect(ksz)
                                    step.refSelectFilter(sigle_ksz)
                                    log = u"step:{0} 控深钻{1}在挡点层{2}缺少挡点(单面挡点),请检查".format(stepname, ksz, md_layer)
                                    arraylist.append(log)
                                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                         ksz, md_layer, log,
                                                                                         dic_zu_layer)
                                ...
                                step.unaffectAll()
                            # 双面挡点
                            if step.isLayer(double_ksz):
                                for md_layer in md_layers:
                                    step.affect(double_ksz)
                                    step.refSelectFilter(md_layer, mode='disjoint')
                                    if step.Selected_count():  # 有未接触到的则有不存在的挡点 报出
                                        step.selectReverse()
                                        if step.Selected_count():
                                        ...
                                        step.unaffectAll()
                                        step.affect(ksz)
                                        step.refSelectFilter(double_ksz)
                                        log = u"step:{0} 控深钻{1}在挡点层{2}缺少挡点(双面挡点),请检查".format(stepname, ksz, md_layer)
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                             ksz, md_layer, log,
                                                                                             dic_zu_layer)
                    ...
                    for md in md_layers:
                        ...
                        step.affect(tmp_md)
                        step.Contourize(0)
                        step.refSelectFilter('\;'.join(ksz_layers), mode='same_center')
                        if step.Selected_count():
                            step.selectDelete()
                            step.selectReverse()
                            if step.Selected_count():
                                step.unaffectAll()
                                step.affect(md)
                                step.refSelectFilter(tmp_md)
                                log = u"step:{0} 挡点{1}与控深钻孔{2}位置不在同一位置,请检查".format(stepname, md,
                                                                                                 u'、'.join(ksz_layers))
                                arraylist.append(log)
                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                     md, md, log,
                    ...
                    #         step.affect(md_layer)
                    # step.affect(drl)
                    if len(solderMaskLayers) == 2:
                        # 双面开窗的孔
                        drill = '%s+++' % drl
                        step.affect(drl)
                        step.refSelectFilter(solderMaskLayers[0], mode='cover')
                        if step.Selected_count():
                            step.copySel(drill)
                            step.unaffectAll()
                            step.affect(drill)
                            step.refSelectFilter(solderMaskLayers[1], mode='cover')
                            step.selectReverse()
                            step.selectDelete()
                            step.unaffectAll()
                            step.affect(drill)
                            step.selectAll()
                            if step.Selected_count(): # 有双面开窗的孔
                                step.unaffectAll()
                                for md in md_layers:
                                   step.affect(drill)
                                   ...
                                   if step.Selected_count():
                                       step.selectReverse()
                                       if step.Selected_count():
                                           step.selectDelete()  # 留下未接触到的报出来
                                       step.unaffectAll()
                                       step.affect(drl)
                                       step.refSelectFilter(drill)
                                       log = u"双面开窗的孔必须存在挡点,请检查{0}中钻孔层{1}和挡点层{2}".format(
                                           stepname, drl, md)
                                       arraylist.append(log)
                                       dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                            drl, drl, log, dic_zu_layer)
                                   step.unaffectAll()
                            step.unaffectAll()
                        step.unaffectAll()
                        step.removeLayer(drill)
                    # 检测挡点与钻孔不在同一位置报出位置
                    for md in md_layers:
                        tmp_md = '%s+++' % md
                        step.affect(md)
                        ...
                        step.affect(tmp_md)
                        step.Contourize(0)
                        step.refSelectFilter(drl, mode='same_center')
                        if step.Selected_count():
                            step.selectDelete()
                            step.selectReverse()
                            if step.Selected_count():
                                ...
                                step.affect(md)
                                step.refSelectFilter(tmp_md)
                                log = u"step:{0} 挡点{1}与钻孔{2}位置不在同一位置,请检查".format(stepname, md,
                                                                                                 drl)
                                arraylist.append(log)
                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                     md, drl, log,
                ...
                if ksz_layers:
                    for ksz in ksz_layers:
                        layer_cmd = gen.Layer(step, ksz)
                        md_ksz = '%s+++md' % ksz
                        step.affect(ksz)
                        step.setFilterTypes('pad')
                        step.refSelectFilter('\;'.join(md_layers))
                        step.resetFilter()
                        if step.Selected_count():
                            step.copySel(md_ksz)
                        else:
                            # log = u"step:{0} 挡点层{1}没有和钻孔层{2}对应的孔".format(stepname, u'、'.join(md_layers),
                            #                                                           ksz)
                            # arraylist.append(log)
                            step.unaffectAll()
                            continue
                        ...
                        step.unaffectAll()
                        step.removeLayer(md_ksz)
                        # 单独挑出
                        tmp_ksz = '%s+++' % ksz
                        for obj in feat_out:
                            step.affect(ksz)
                            print(obj.symbol)
                            step.selectByIndex(ksz, obj.feat_index)
                            step.copySel(tmp_ksz)
                            step.unaffectAll()
                            # 计算大小
                            drill_type = obj.attrs.get('drill')
                            if not drill_type:
                                step.truncate(tmp_ksz)
                                continue
                            md_size = self.get_md_size(drill_type, float(obj.symbol.replace('r','')))
                            print('calculate size...')
                            for md in md_layers:
                                # 不符合大小的挡点
                                show_error_md = 'show_%s+++' % md
                                md_obj = gen.Layer(step, md)
                                step.affect(md)
                                step.refSelectFilter(tmp_ksz)
                                if step.Selected_count():
                                    target_feat = md_obj.featout_dic_Index(units='inch', options="feat_index+select")["pads"]
                                    for feat in target_feat:
                                        print(feat.symbol)
                                        # 是否符合计算的大小 不符合则copy出来
                                        if feat.symbol != 'r%s' % md_size:
                                            step.copySel(show_error_md)
                                step.unaffectAll()
                            step.truncate(tmp_ksz)
                        for md in md_layers:
                            show_error_md = 'show_%s+++' % md
                            if step.isLayer(show_error_md):
                                step.affect(show_error_md)
                                step.selectReverse()
                                if step.Selected_count():
                                    log = u"step:{0} 检测到{1}有挡点大小不符，请到{2}中查看！".format(
                                        stepname, md, show_error_md)
                                    arraylist.append(log)
                                    dic_zu_layer = self.get_view_dic_layers(job.name, stepname, step,
                                                                            worklayer="view_layer_" + md,
                                                                            dic_layer_list={u"挡点大小不符，请查看异常层：": [
                                                                                md, ksz, show_error_md], },
                                                                            dic_zu_layer=dic_zu_layer)
                else:
                    # 挡点对应的孔选出来
                    layer_cmd = gen.Layer(step, md_drill)
                    step.affect(drl)
                    step.setFilterTypes('pad')
                    step.refSelectFilter('\;'.join(md_layers))
                    step.resetFilter()
                    if step.Selected_count():
                        step.copySel(md_drill)
                    else:
                        # log = u"step:{0} 挡点层{1}没有和钻孔层{2}对应的孔".format(stepname, u'、'.join(md_layers),
                        #                                                                  drl)
                        # arraylist.append(log)
                    ...
                    for obj in feat_out:
                        step.affect(drl)
                        print(obj.symbol, obj.attrs.get('drill'))
                        step.selectByIndex(drl, obj.feat_index)
                        step.copySel(tmp_drl)
                        step.unaffectAll()
                        # 计算大小
                        print('calculate size...')
    ...
    def check_offset_et_rj_fbk(self, autocheck=None):
        """
        http://192.168.2.120:82/zentao/story-view-7449.html
        :return:
        """
        if "panel" not in matrixinfo["gCOLstep_name"]:
        ...
        check_drls = []
        if 'drl' in matrixinfo["gROWname"]:
            check_drls.append('drl')
        if 'tk.ykj' in matrixinfo["gROWname"]:
            check_drls.append('tk.ykj')
        check_drls += mai_drill_layers
        if not check_drls:
            return u"没有要检测的通孔和埋孔，请检查", None
        data_info = get_inplan_mrp_info(job.name.upper(), condtion="1=1")
        core_num = 0
        for dic_mrp_info in data_info:
            if dic_mrp_info["PROCESS_NUM"] == 1:
                core_num += 1
        if core_num == 1:
            return u'success 芯板数为1，无需检测', None
        arraylist = []
        dic_zu_layer = {}
        stepname = 'panel'
        step = gen.Step(job, stepname)
        ...
        for drl in check_drls:
            ...
            if step.Selected_count():
                ...
                log = u"step:{1} 检测到{0}没有防爆孔.".format(drl,stepname)
                arraylist.append(log)
                step.unaffectAll()
                continue
            step.unaffectAll()
                ...
                if step.Selected_count():
                    ...
                    if step.Selected_count():
                        ...
                        if step.Selected_count():
                            ...
                            step.unaffectAll()
                            step.affect(drl)
                            step.refSelectFilter(show_drl)
                            log = u"step:{0} 检测到{1}防爆孔与{2}融合块位置偏移，请检查！".format(stepname,drl, innersignalLayer)
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                 drl, innersignalLayer, log,
                                                                                 dic_zu_layer)
                            step.unaffectAll()
                    else:
                        step.copySel(show_drl)
                        ...
                        step.affect(drl)
                        step.refSelectFilter(show_drl)
                        log = u"step:{0} 检测到{1}防爆孔与{2}融合块位置偏移，请检查！".format(stepname, drl,
                                                                                            innersignalLayer)
                        arraylist.append(log)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                             drl, innersignalLayer, log,
    ...
    # 20250207 zl 根据孔类型和大小 按照挡点大小准则返回计算的挡点大小 单位inch
    def get_md_size(self, drill_type, drill_size):
        md_size = -1
        if drill_type == 'non_plated':
            if drill_size <= 39.37:
                md_size = drill_size + 6
            else:
                md_size = drill_size - 8
        else:
            if drill_size <= 15.748:
                md_size = drill_size + 10
            elif drill_size < 19.685:
                md_size = drill_size + 3
            elif drill_size < 35.433:
                md_size = drill_size + 4
            else:
                md_size = drill_size - 8
        return md_size
