            ...
            if d_coupons:
                ...
                if serial_layer:
                    ...
                    for d_coupon in d_coupons:
                        ...
                        step.initStep()
                        if d_coupon.endswith('01'):
                            fill_layer = 'd_coupon+++01'
                    ...
                    if panel_step.isLayer(tmp_layer):
                        ...
                        panel_step.resetFilter()
                        count = panel_step.Selected_count()
                        if count == 1:
                            feat_text = layer_cmd.featout_dic_Index(units="mm", options="feat_index+select")["text"]
                            for obj in feat_text:
                                # if obj.text != '01' or obj.text != '1':
                                if obj.text != '01':
                                    log = u"检测到末尾是01的d_coupon在panel中对应的序列号不是01"
                                    arraylist.append(log)
                            ...
                            log = u"检测到末尾是01的d_coupon在panel中有多个序列号"
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step, serial_layer, serial_layer, log, dic_zu_layer)
                    ...
                    if panel_step.isLayer(tmp_layer):
                        ...
                        if panel_step.Selected_count():
                            ...
                            num_text_list = []
                            for obj in feat_text:
                                if re.match('\d+$', obj.text):
                                    num_text_list.append(int(obj.text))
                            if len(num_text_list) == len(d_coupon_list):
                                # 序号顺序
                                err_num = []
                                for num in range(len(d_coupon_list)):
                                    if num not in num_text_list:
                                       err_num.append(num)
                                if err_num:
                                    log = u"检测到%s层缺少d_coupon序号%s" % (serial_layer, '、'.join([str(num) for num in err_num]))
                                    arraylist.append(log)
                                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step, serial_layer, serial_layer, log, dic_zu_layer)
                            else:
                                log = u"检测到d_coupon个数和序号个数不一致"
                                arraylist.append(log)
                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step, serial_layer, serial_layer, log, dic_zu_layer)
                        else:
                            log = u"检测d_coupon在panel上未添加文字"
                            arraylist.append(log)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step,serial_layer, serial_layer, log, dic_zu_layer)
                        panel_step.removeLayer(tmp_layer)
                        panel_step.removeLayer(flat_layer)
