...
class auto_check_rule(object):
    ...
    def check_ks_pad_exists(self, autocheck=None):
        ...
        """
        if job.name[1:4] != '631':
            return u"success 非灯板（631系列)，不检测", None
        kz_drills = []
        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            if d_coupons:
                ...
                if check_layers:
                    ...
                    panel_step.removeLayer(fill_layer + '01')
                    fill_layers = set()
                    for d_coupon in d_coupons:
                        if d_coupon.endswith('-1') or d_coupon.endswith('01'):
                            fill_layers.add(fill_layer + '01')
                            d_coupons_01.add(d_coupon)
                        else:
                            fill_layers.add(fill_layer)
                    for layer in fill_layers:
                        panel_step.createLayer(layer)
                    for d_coupon in d_coupons:
                        # 01的d_coupon填充到d_coupon+++01
                        step = gen.Step(job, d_coupon)
                        step.initStep()
                        fill_layer = 'd_coupon+++01' if d_coupon in d_coupons_01 else 'd_coupon+++'
                        step.srFill(fill_layer)
                    panel_step.open()
                    tmp_layer = 'd_coupon+++01'
                    serial_layers = []
                    # for check_layer in check_layers:
                    ...
                    if panel_step.isLayer(tmp_layer):
                        ...
                        for check_layer in check_layers:
                            ...
                            panel_step.unaffectAll()
                        result = {}
                        # 记录每层的异常信息 第一个值 d-coupon对应多个序号 第二个值 d-coupon没有对应的序号
                        if serial_layers:
                            for serial_layer in serial_layers:
                                ...
                                result[serial_layer] = [0, 0]
                        else:
                            return u"检测到d_coupon未添加序列号01", None
                        for index in feat_indexes:
                            panel_step.VOF()
                        ...
                        for layer in effect_layer:
                            if result.get(layer)[0] > 0:
                                log = u"检测到有%s个末尾是01的d_coupon在panel中%s对应的序列号有多个01" % (result.get(layer)[0], layer)
                                arraylist.append(log)
                                dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step,layer, layer, log,dic_zu_layer)
                    tmp_layer = 'd_coupon+++'
                    serial_layers = []
                    if panel_step.isLayer(tmp_layer):
                        ...
                        end_num = len(d_coupon_list) + 2 - len(d_coupons_01) if d_coupons_01 else len(d_coupon_list) + 1
                        for lay, nums in num_text_dict.items():
                            if nums: # 可能这层文字层没有序列号
                                ...
                                err3_num[lay].extend([str(n) for n in filter(lambda num: num not in range(start_num, end_num), nums)])
                        if err_num:
                            for k,v in err_num.items():
                                if v:
                                    ...
                                    if panel_step.Selected_count():
                                        ...
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step, k, k, log, dic_zu_layer)
                                    panel_step.unaffectAll()
                        if err3_num:
                            for k,v in err3_num.items():
                                if v:
                                    ...
                                    panel_step.resetFilter()
                                    if panel_step.Selected_count():
                                        log = u"检测到d_coupon在panel中%s层超出范围的异常序列号%s" % (k, u'、'.join(v))
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, panel_step, k, k, log, dic_zu_layer)
                                    panel_step.unaffectAll()
                        if err2_num:
