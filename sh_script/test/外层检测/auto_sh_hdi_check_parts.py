...
class CheckParts(object):
        ...
        if check_steps:
            ...
            for k, v in check_steps.items():
                ...
                if step.featureSelected():
                    ...
                    for cur_layer in baord_layers:
                        ...
                        if step.featureSelected():
                            ...
                            showstr = u'%s中层:%s,切铜不完整或有物件进入coupon' % (step.name, cur_layer)
                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step,
                                                                                 cur_layer, cur_layer, showstr,dic_zu_layer, add_note_layer=cur_layer)
                ...
                for cur_lay in baord_layers:
                    ...
                    if step.featureSelected():
                        log = u'%s中层:%s,切铜不完整或有物件进入coupon' % (step.name, cur_lay)
                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, step.name, step,
            ...
            if stepName == 'panel':
                ...
                if job_dict['laser_board']:
                    for step_i in range(1, job_dict['laser_board'] + 1):
                        ...
                        if plus_num != 8:
                            log = 'plus{0}-floujie'.format(step_i) + u'在%s中不是8个,当前数量为%s' % (stepName, plus_num)
                            arraylist.append(log)
                ...
                if job_dict['laser_board']:
                    for step_i in range(1, job_dict['laser_board'] + 1):
                        plus_num = len([i for i in pnl_sr_steps if i == 'plus%s-floujie' % step_i])
                        if plus_num < 1:
                            log = u'plus%s-floujie在%s中不是1个,当前数量为%s' % (step_i, stepName, plus_num)
                            arraylist.append(log)
        if arraylist:
            # showMessageInfo(*(arraylist))
    ...
    def check_set_slotMark(self):
        ...
        if not job_sql_data:
            arraylist.append(u"数据库未上传防呆半圆槽数据，请先运行程序上传")
            return arraylist, dic_zu_layer
        step.open()
        step.clearAll()
        step.resetFilter()
    ...
    def check_mi_cam_bar_design(self):
        ...
        for mrp in mrp_info:
            ...
            int(mrp['PROCESS_NUM']) - 1, str(mrp['FROMLAY']).lower(), str(mrp['TOLAY']).lower())
        ...
        if 'edit' in step_list:
            ...
            for layer in res_layers:
                ...
                if step.featureSelected():
                    ...
                    arraylist.append(log)
                    dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepName, step, tmp_layer['line'],layer,
                                                                         log, dic_zu_layer, add_note_layer=layer)
                step.clearAll()
            for tplayer in tmp_layer.values():
                step.removeLayer(tplayer)
            step.clearAll()
    ...
    def check_gk_film(self):
        ...
        for stepname in step_in_panels:
            ...
            for gklay, drill_list in gk_dict.items():
                ...
                step_cmd.filter_set(feat_types='pad', polarity='negative')
                step_cmd.selectAll()
                if step_cmd.featureSelected():
                ...
                for drl_name in drill_list:
                    if re.match('s\d+-\d+', drl_name):
                        ...
                        symbols = set([obj.symbol for obj in features_info if float(obj.symbol[1:]) >= 127 and obj.symbol not in ['r515']])
                        if symbols:
        ...
        for drlname in check_list:
            ...
            if int(jobname[4:6]) == 2:
                ...
                step.COM("sel_ref_feat,layers={0},use=filter,mode=cover,pads_as=limits,f_types="
                         "line\;pad\;surface\;arc\;text,polarity=positive\;negative,include_syms=sh-pindonut*,exclude_syms=".format(ref_signal))
