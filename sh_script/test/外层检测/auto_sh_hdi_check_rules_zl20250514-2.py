...
# 20250512 zl check_feature_outside_of_pnlrout
"""
...
class auto_check_rule(object):
    ...
    def check_serial_number(self, autocheck=None):
        ...
        if d_coupons:
            ...
            d_coupons_01 = filter(lambda coupon: re.search('-01|-1$|-1\D', coupon), d_coupons)
            err_d_coupon = []
            ...
            if err_d_coupon:
                ...
                for d_coupon in d_coupons_01:
                    key = d_coupon.rsplit('-01', 1)[0] if '-01' in d_coupon else d_coupon.rsplit('-1', 1)[0]
                    d_coupons_dict[key] = []
                ...
                if check_layers:
                    for d_couponval, vals in d_coupons_dict.items():
                        ...
                        if val_list_other:
                            ...
                            for check_layer in check_layers:
                                # flatten 文字层
                                tmp_check_layer = '%s+++' % check_layer
                                panel_step.Flatten(check_layer, tmp_check_layer)
                                panel_step.affect(tmp_check_layer)
                                panel_step.setFilterTypes('text', 'positive')
                                panel_step.refSelectFilter(flat_layer)
                                panel_step.resetFilter()
                                if panel_step.Selected_count():
                                    ...
                                    panel_step.removeLayer(tmp_check_layer)
                                panel_step.unaffectAll()
                            print(serial_layers)
                            print(feat_indexes)
                            panel_step.PAUSE('B')
                            for index in feat_indexes:
                                ...
                                for serial_layer in serial_layers:
                                    ...
                                    tmp_serial_layer = '%s+++' % serial_layer
                                    layer_cmd = gen.Layer(panel_step, tmp_serial_layer)
                                    panel_step.affect(tmp_serial_layer)
                                    panel_step.setFilterTypes('text', 'positive')
                                    panel_step.refSelectFilter(tmp_flat_layer)
                                    panel_step.resetFilter()
                                    if panel_step.Selected_count():
                                    ...
                                    # else:
                                    #     panel_step.removeLayer(tmp_serial_layer)
                                    panel_step.unaffectAll()
                            print(num_text_dict)
                            panel_step.PAUSE('C')
                            panel_step.removeLayer(tmp_flat_layer)
                                ...
                                for k, v in err3_num.items():
                                    if v:
                                        ...
                                        if panel_step.Selected_count():
                                            ...
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name,
                                                                                                 stepname,
                                                                                                 panel_step, tmp_serial_layer,
                                                                                                 tmp_serial_layer, log,
                                                                                                 dic_zu_layer)
                                        panel_step.unaffectAll()
                            if err2_num:
                                for k, v in err2_num.items():
                                    if v:
                                        ...
                                        for n in v:
                                            ...
                                            panel_step.refSelectFilter(flat_layer)
                                            feat_info = panel_step.Features_INFO(tmp_serial_layer, mode='feat_index+select')
                                            for feat in feat_info:
                                                if feat and re.match('#\d+', feat[0]):
                                                    repeat_pos.append(feat[0].replace('#', ''))
                                        panel_step.resetFilter()
                                        for i in repeat_pos:
                                            panel_step.selectByIndex(tmp_serial_layer, i)
                                        log = u"检测到%s在panel中%s层序列号%s重复" % (d_couponval, k, u'、'.join(v))
                                        arraylist.append(log)
                                        dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname,
                                                                                             panel_step, tmp_serial_layer, tmp_serial_layer,
                                                                                             log, dic_zu_layer)
                                        panel_step.unaffectAll()
                            panel_step.removeLayer(flat_layer)
                for d_coupon in d_coupons:
        ...
        # print(data_info)
        if not data_info:
            arraylist.append(u"success 没有添加set或pcs序号的数据")
        else:
            data = data_info[0]
            if data['CI_ADD_POSITION_CODE_'] == 0:
                arraylist.append(u"success 不加set或pcs序号，不检测")
            # stepname = 'panel'
            # arraylist = []
            # dic_zu_layer = {}
            step = gen.Step(job, stepname)
            # set数量
            set_list = list(filter(lambda x: 'set' in x, step.info.get('gREPEATstep')))
    ...
    def check_feature_outside_of_pnlrout(self, autocheck=None):
        ...
        if list(pnlrout_dic.keys()):
            pnlrout_dic[list(pnlrout_dic.keys())[-1]] = ['drl']
                ...
                for pnlrout, drls in pnlrout_dic.items():
                    ...
                    if step.Selected_count():
                        ...
                        for drl in drls:
                            ...
                            step.selectReverse()
                            if step.Selected_count():
                                log = u"检测到{0}层有异物超出锣边层{1}！".format(drl, pnlrout)
                                arraylist.append(log)
