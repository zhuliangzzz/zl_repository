...
class auto_check_rule(object):
    ...
    def check_silk_screen(self, autocheck=None):
        ...
        empty_lays = []
        for ss in ss_layers:
            isEmpty1, isEmpty2, isEmpty3, isEmpty4= True, True, True, True
            for stepname in get_pnl_steps + ['panel']:
                step = gen.Step(job, stepname)
                step.initStep()
                ss_ = '%s+++check_silk_screen' % ss
                if step.isLayer(ss_):
                    step.affect(ss_)
                    step.selectAll()
                    if step.Selected_count():
                ...
                if step.isLayer(ss_):
                    ...
                    if step.Selected_count():
                        isEmpty2 = False
                    step.unaffectAll()
                ss_ = '%s+++check_silk_screen_flat' % ss
                if step.isLayer(ss_):
                    step.affect(ss_)
                    step.selectAll()
                    if step.Selected_count():
                        isEmpty3 = False
                    step.unaffectAll()
                ss_ = '%s+++2check_silk_screen_flat' % ss
                if step.isLayer(ss_):
                    step.affect(ss_)
                    step.selectAll()
                    if step.Selected_count():
                    ...
                    step.unaffectAll()
            if isEmpty1:
                empty_lays.append('%s+++check_silk_screen' % ss)
            if isEmpty2:
                empty_lays.append('%s+++2check_silk_screen' % ss)
            if isEmpty3:
                empty_lays.append('%s+++check_silk_screen_flat' % ss)
            if isEmpty4:
                empty_lays.append('%s+++2check_silk_screen_flat' % ss)
        if empty_lays:
            [panel_step.removeLayer(empty_lay) for empty_lay in empty_lays]
        if arraylist:
            if autocheck == "show_ui":
                showMessageInfo(*arraylist)
                self.view_note_detail(dic_zu_layer)
