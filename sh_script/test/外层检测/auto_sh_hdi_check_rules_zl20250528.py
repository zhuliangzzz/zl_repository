...
# 20250422 zl check_exist_d_coupon_and_test_coupon A86/D10客户要固定加D-COUPON和拉力测试coupon
...
class auto_check_rule(object):
                            ...
                            if step.Selected_count():
                                ...
                                if nums:
                                    for i in range(1, len(set_list) + 1):
                                        if nums.count(i) > set_repeat:
                                            log = u"检测到%s多加set序列号%s" % (check_layer, i)
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                                 check_layer, check_layer, log,
                                                                                                 dic_zu_layer)
                                        elif nums.count(i) < set_repeat:
                                            log = u"检测到%s少加set序列号%s" % (check_layer, i)
                                            arraylist.append(log)
                                            dic_zu_layer = self.get_selected_dic_note_coordinate(job.name, stepname, step,
                                                                                                 check_layer, check_layer, log,
    ...
    def check_optical_and_damaged_point(self, autocheck=None):
        ...
        for layer in outsignalLayers:
            ...
            step.unaffectAll()
            step.affect(tmplayer)
            step.setAttrFilter_pro('.string', text='badmark')
            step.setAttrLogic_pro()
            step.selectAll()
            step.resetFilter()
            if step.Selected_count():
                ...
                log = u'%s未识别到坏板点（.string=badmark属性）' % layer
                arraylist.append(log)
            step.unaffectAll()
