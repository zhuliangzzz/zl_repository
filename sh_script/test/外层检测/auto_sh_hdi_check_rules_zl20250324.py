...
class auto_check_rule(object):
    ...
    def check_impedance_line_shielding(self, autocheck=None):
        ...
        check_steps = []
        if "loss" in matrixinfo["gCOLstep_name"]:
            check_steps.append('loss')
        if "icg" in matrixinfo["gCOLstep_name"]:
            check_steps.append('icg')
        if not check_steps:
        ...
        if "loss" in matrixinfo["gCOLstep_name"]:
            step = gen.Step(job, 'loss')
            step.initStep()
            for outsignal in innersignalLayers:
                tmp_outsignal = '%s+++check_impedance_line_shielding' % outsignal
                second_outsignals = list(filter(lambda l: l in rows , ('l%s' % (int(outsignal.replace('l','')) -1), 'l%s' % (int(outsignal.replace('l','')) + 1))))
                for second_outsignal in second_outsignals:
                    tmp_second_outsignal = '%s+++check_impedance_line_shielding' % second_outsignal
                    step.affect(outsignal)
