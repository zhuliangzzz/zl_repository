        ...
        if "panel" in matrixinfo["gCOLstep_name"]:
            ...
            for etch, signal in etch_layers.items():
                ...
                if r0_count == 4:
                    ...
                    step.unaffectAll()
                    step.affect(signal)
                    symbol = 'sh-dwsig2014' if figer_dp_positive == 'yes' else 's5082'
                    ...
                    if step.Selected_count() != 4:
                        step.selectReverse()
                        log = u"step:{0} 检测到{1}对位点与{2}未对齐，请检查！".format(step.name, etch, signal)
                        arraylist.append(log)
                    ...
                    step.copySel(tmp_etch)
                    step.unaffectAll()
                    log = u"检测到{0}中{1}层r0对位点数量异常（{2}）！".format(step.name, etch, r0_count)
                    arraylist.append(log)
                    step.affect(tmp_etch)
                    step.selectReverse()
                    if step.Selected_count():
