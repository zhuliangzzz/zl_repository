            ...
            if key == "week_table":                
                for info in value:
                    order, name, layer1, num1, layer2, num2,layer3, num3,status = info
                       ...
                       #("solder_mask" in args and layer2 != "/" and num2 != "0") or \
                       #("silk_screen" in args and layer3 != "/" and num3 != "0"):
                    for check_type in ["signal", "solder_mask", "silk_screen"]:
                        if check_type in args:
                            ...
                            if calc_num != int(num):
                                ...
                                arraylist_log.append(log.format(array_layers, calc_num, num, name))
                ...
                #dic_check_type_layers = {"signal": outsignalLayers,
                    ...
                    #for check_type in ["signal", "solder_mask", "silk_screen"]:
                            ...
                            #for key, vaules in dic_tgz_week_layers.iteritems():
                                    ...
                                    #continue
                                #num += len(vaules)
                            #if ("signal" == check_type and int(value[0][3]) != num) or\
                               #("solder_mask" == check_type and int(value[0][5]) != num) or \
                               #("silk_screen" == check_type and int(value[0][7]) != num):
                                #log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                    ...
                    #for key, vaules in dic_tgz_barcode_week_layers.iteritems():
                       ...
                       #("solder_mask" in args and int(value[1][5]) != num) or \
                       #("silk_screen" in args and int(value[1][7]) != num):
                        #log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                    ...
                    #for key, vaules in dic_tgz_picihao_layers.iteritems():
                       ...
                       #("solder_mask" in args and int(value[2][5]) != num) or \
                       #("silk_screen" in args and int(value[2][7]) != num):
                        #log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
                    ...
                    #for key, vaules in dic_tgz_banci_layers.iteritems():
                        #num += len(vaules)
                    #if ("signal" in args and int(value[3][3]) != num) or\
                       #("solder_mask" in args and int(value[3][5]) != num) or \
                       #("silk_screen" in args and int(value[3][7]) != num):
                        #log = u"获取{0}层板内{3}个数{1}跟登记数量{2}不一致，请检查板内{3}情况！"
