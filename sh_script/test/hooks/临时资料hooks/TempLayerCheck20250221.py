...
class TempLayerCheck(object):
    ...
    def check_restore_normal(self):
        ...
        if temp_layers:
            confirm = QtGui.QMessageBox.information(None, u"提示", u'检测到有临时层，是否还原成正式层', QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
            if confirm == QtGui.QMessageBox.Yes:
                dic_drill_start_end = self.get_drill_start_end_layers()
                for temp_layer in temp_layers:
                    normal_layer = temp_layer.split("-ls")[0]
                    self.job.COM("matrix_layer_context,job={0},matrix=matrix,layer={1},context=misc".format(self.job.name, temp_layer))
    ...
    def restore_normal(self):
        ...
        if temp_layers:
                ...
                print(self.job.COMANS)
                # 等于no则没有checkout   否则就是被checkout了，这时候不保存
                if self.job.COMANS == 'no':
                    self.job.save()
