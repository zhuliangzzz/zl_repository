...
import MySQL_DB
import Oracle_DB
os.environ['JOB'] = sys.argv[1]
from get_erp_job_info import get_core_inner_layers
from genesisPackages import laser_drill_layers, get_drill_start_end_layers, matrixInfo
...
class CheckUserOpenJob(object):
    ...
    def auto_lock_condition(self):
        # 20250317 zl 锣带用户打开料号不重新刷锁层信息 把非rout属性的层锁住即可
        DB_M = MySQL_DB.MySQL()
    ...
    def rout_user_lock_layers(self):
        ...
        lock_layers = [lay for i, lay in enumerate(matrixInfo["gROWname"])
                       if matrixInfo["gROWlayer_type"][i] != "rout"]
