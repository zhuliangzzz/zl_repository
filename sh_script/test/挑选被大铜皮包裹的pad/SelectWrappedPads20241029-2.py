...
def SelectWrappedPads():
    ...
    for signal in ['l1', bot_signal]:
        step.VOF()
        step.truncate(cover_layer)
        step.truncate(surface_layer)
        step.VON()
        step.affect(signal)
        ...
        if step.Selected_count():
            ...
            if step.Selected_count():
                ...
                step.unaffectAll()
                step.affect(cover_layer)
                step.refSelectFilter(surface_layer, mode='disjoint')  # 保留未接触到的
                if step.Selected_count():
                    step.selectReverse()
                    if step.Selected_count():
                        step.selectDelete()
                ...
                step.surf_outline(1)
                step.PAUSE('signal')
                step.refSelectFilter(cover_layer, mode='cover')
                if step.Selected_count():
                    step.selectReverse()
                    if step.Selected_count():
                        step.selectDelete()
