...
# 20241104 zl 检测孤立的smd/bga补偿是否一致 check_smd_bga_compensate_is_right
...
class OutComp(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        if "autocheck" in sys.argv[1:]:
            ...
            self.slayers = ['%s_result' % outsignalLayer for outsignalLayer in outsignalLayers]
    ...
    def check_isolated_smd_bga_compensate_is_right(self):
        ...
        for layer in self.slayers:
            ...
            # step.PAUSE('A')
            base_compensate_not_same_indexes = []
            step.COM('truncate_layer,layer=' + surface)
            step.affect(bga)
                ...
                for obj in feat_out:
                        ...
                        if orig_size:
                            ...
                            if obj.symbol.startswith('rect'):
                                ...
                                orig_size = orig_size.replace('rect', '')
                                lenth, width = float(symbol.split('x')[0]), float(symbol.split('x')[1])
                                orig_lenth, orig_width = float(orig_size.split('x')[0]), float(orig_size.split('x')[1])
