...
# 20241104 zl 增加检测smd/bga补偿是否一致 check_smd_bga_compensate_is_right
...
class OutComp(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        if "autocheck" in sys.argv[1:]:
            if 'smd_bga' in sys.argv[1:]:
                self.slayers = ['%s_reslut' % outsignalLayer for outsignalLayer in outsignalLayers]
            else:
    ...
    # 20241104 检查smd/bga补偿
    def check_smd_bga_compensate_is_right(self):
        """检测补偿是否一致"""
        job = gClasses.Job(self.job_name)
        step = gClasses.Step(job, self.step_name)
        step.open()
        ...
        for layer in self.slayers:
            ...
            dic_size = {"smdpad": small_smd, "bgapad": small_bga}
            base_compensate_not_same_indexes = []
            # for pad_type in ["small_smd", 'big_smd', 'small_bga', 'big_bga']:
            for pad_type in ["smdpad", 'bgapad']:
                step.selectNone()
                self.Adv_filter(select_type=pad_type)
                if step.featureSelected():
                    ...
                    for obj in feat_out:
                        ...
                        if orig_size:
                            # print(obj.symbol , orig_size , dic_size[line_type] , 111111111111111)
                            if abs(float(obj.symbol[1:]) - float(orig_size[1:]) - float(dic_size[pad_type])) > 0.05:
                                base_compensate_not_same_indexes.append(
                                    [obj.feat_index, orig_size, dic_size[pad_type]])
                        else:
                            base_compensate_not_same_indexes.append([obj.feat_index, orig_size, dic_size[pad_type]])
            ...
            if base_compensate_not_same_indexes:
                ...
                for index, orig_size, bc_value in base_compensate_not_same_indexes:
                    ...
                    if step.featureSelected():
                        step.copyToLayer(layer + "_base_compensate_error_+++")
                self.warn_list.append(u"检测到{0}层线路基础补偿跟程序自动补偿有差异，请到{1}层比对检查异常位置(每个pad中的属性orig_size为原稿大小，bc_vaule为自动补偿值)！！"
                                      .format(layer, layer + "_base_compensate_error_+++"))
...
if __name__ == "__main__":
    ...
    myapp = OutComp()
    if "autocheck" in sys.argv[1:]:
        if 'smd_bga' in sys.argv[1:]:            # smd_bga_autocheck
            myapp.check_smd_bga_compensate_is_right()
        else:
