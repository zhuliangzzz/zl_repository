    ...
    def check_wrapped_smd_bga_compensate_is_right(self):
        ...
        for slayer in self.slayers:
            ...
            if not step.isLayer(layer):
                continue
            step.removeLayer(layer + "_wrapped_smd_bga_compensate_error_+++")
            step.clearAll()
            step.affect(layer)
            base_compensate_not_same_indexes = []
            layer_cmd = gClasses.Layer(step, layer)
            ...
            if base_compensate_not_same_indexes:
                ...
                for index, orig_size, bc_value, dif in base_compensate_not_same_indexes:
                    ...
                    step.selectFeatureIndex(layer, index)
                    if step.featureSelected():
                        step.copyToLayer(layer + "_wrapped_smd_bga_compensate_error_+++")
                self.warn_list.append(u"检测到{0}层线路基础补偿跟程序自动补偿有差异，请到{1}层比对检查异常位置(每个pad中的属性orig_size为原稿大小，dif_value为偏差值，bc_vaule为自动补偿值)！！"
                                      .format(layer, layer + "_wrapped_smd_bga_compensate_error_+++"))
            step.clearAll()
