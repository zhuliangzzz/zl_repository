...
if ($softName eq 'incam' and $hostnameTmp eq 'MLB'){
	$JobPath = "/opt/db_management";
	$DBPath = 'linux1';
	$logPath = "/incam/server/site_data/scripts/Logs";
}elsif ($softName eq 'incam' and $hostnameTmp eq 'HDI'){
	$JobPath = "/data3/db_management";
	$DBPath = 'db1';
	$logPath = "/incam/server/site_data/scripts/Logs";
}elsif($softName eq 'incamp' and $hostnameTmp eq 'HDI'){
	$JobPath = "/data3/incampro/db_management";
	$DBPath = 'db1';
	$logPath = "/incampro/server/site_data/scripts/Logs";
}elsif($softName eq 'incamp' and $hostnameTmp eq 'MLB'){
	$JobPath = "/opt/incampro/db_management";
	$DBPath = 'db1';
	$logPath = "/incampro/server/site_data/scripts/Logs";
}
...
my $sel_job_lb = $select_lay_list->Scrolled('Listbox',-scrollbars=>'oeos',-selectbackground=>'blue',-selectforeground=>'white',-selectmode=>'single',-height => 16, -background=>'white',-foreground=>'black',-font=> 'charter 12 bold', -width=>30)->pack(-side=>'top',-fill=>'x');
...
sub run_import_tgz{
	...
	my $localPathJob = '/tmp';
	my $jobpath = &GetJobBackPtah("$selectJobImp[0]");
	#$f->PAUSE ("$jobpath");
	&ftpJob("$selectJobImp[0].tgz","$localPathJob","$jobpath");
	if ( $importOption == 1 ){
		...
		if ( $rv == 0 ){
			...
			&deleteServerJob($selectJobImp[0],$localPathJob,$jobpath);
...
sub get_jobslist{
	...
	my $sql;
	if ($softName eq 'incam' and $hostnameTmp eq 'MLB'){
		$sql = "SELECT job FROM abnormal_jobs WHERE DB_Source='incam' AND Site='MLB'";	
	}elsif($softName eq 'incamp' and $hostnameTmp eq 'MLB'){
		$sql = "SELECT job FROM abnormal_jobs WHERE DB_Source='incamp' AND Site='MLB'";	
	}elsif($softName eq 'incam' and $hostnameTmp eq 'HDI'){
		$sql = "SELECT job FROM abnormal_jobs WHERE DB_Source='incam' AND Site='HDI'";	
	}elsif($softName eq 'incamp' and $hostnameTmp eq 'HDI'){
		$sql = "SELECT job FROM abnormal_jobs WHERE DB_Source='incamp' AND Site='HDI'";	
	}
...
sub ftpJob{
	...
	if ($hostnameTmp eq 'MLB'){
		...
		$ftp -> cwd("$jobpath") or die "Cannot change working directory ", $ftp -> message;
	}else{
		# -->HDI ftp
	...
	$ftp -> binary();
	$ftp -> get("$ftpJob","$localPath/$ftpJob","$jobpath") or die "get failed ", $ftp -> message;
	$ftp -> quit;
}
...
sub deleteMysqlLog{
	...
	my $sql;
	if ($softName eq 'incam' and $hostnameTmp eq 'MLB'){
		$sql = "DELETE FROM abnormal_jobs WHERE Job = \'$delJob\' AND DB_Source='incam' AND Site='MLB'";
	}elsif($softName eq 'incamp' and $hostnameTmp eq 'MLB'){
		$sql = "DELETE FROM abnormal_jobs WHERE Job = \'$delJob\' AND DB_Source='incamp' AND Site='MLB'";	
	}elsif($softName eq 'incam' and $hostnameTmp eq 'HDI'){
		$sql = "DELETE FROM abnormal_jobs WHERE Job = \'$delJob\' AND DB_Source='incam' AND Site='HDI'";	
	}elsif($softName eq 'incamp' and $hostnameTmp eq 'HDI'){
		$sql = "DELETE FROM abnormal_jobs WHERE Job = \'$delJob\' AND DB_Source='incamp' AND Site='HDI'";	
	}
...
sub GetJobBackPtah{
	...
	my $sql;
	if($softName eq 'incam' and $hostnameTmp eq 'MLB'){
		$sql = "SELECT `Backup Path` FROM abnormal_jobs WHERE Job = \'$delJob\' AND DB_Source='incam' AND Site='MLB'";
	}elsif($softName eq 'incamp' and $hostnameTmp eq 'MLB'){
		$sql = "SELECT `Backup Path` FROM abnormal_jobs WHERE Job = \'$delJob\' AND DB_Source='incamp' AND Site='MLB'";
	}elsif($softName eq 'incam' and $hostnameTmp eq 'HDI'){
		$sql = "SELECT `Backup Path` FROM abnormal_jobs WHERE Job = \'$delJob\' AND DB_Source='incam' AND Site='HDI'";
	}elsif($softName eq 'incamp' and $hostnameTmp eq 'HDI'){
		$sql = "SELECT `Backup Path` FROM abnormal_jobs WHERE Job = \'$delJob\' AND DB_Source='incamp' AND Site='HDI'";
	}
	
	$sth = $dbh->prepare($sql);
	$sth->execute() or die "无法执行SQL语句:$dbh->errstr";
	
	my @ptah = $sth->fetchrow_array;
	$backpath = ($ptah[0]);
