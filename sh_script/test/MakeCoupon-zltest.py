...
# --------------------------------------------------------- #
'''
    20241017 通过inplan表去找添加周期的层和周期格式，并添加
'''
import re
...
class Coupon():
    ...
    def AddDC(self):
        ...
        if job_data:
            ...
            if dc_layers:
                self.addCodeWindow(dc_layers, dc_type, '程序自动获取')
            else:
                # 找出相关层
                if work_center_code == '防焊':
                    ...
                    type_ = 'silk_screen'
                    layers = self.job.matrix.returnRows('board', type_)
                elif work_center_code == '蚀刻':  # 外层线路
                    layers = genesisPackages.outsignalLayers
                self.addCodeWindow(layers, dc_type,'手动选择')  # 显示
        else:
            sql = """
                    SELECT 
                    RJTSL.JOB_NAME,
                ...
                for key, val_list in note_map.items():
                    ...
                    if '周期格式' in val_string and dc_type in val_string:
                        for layer in layers:
                            ...
                            if re.search('使用曝光资料.*?%s[^-\d]' % layer, val_string):  # 考虑带-的（c2-3) 或者c后面的数字超过一位数的文字层
                                dc_layers.append(layer)
            if dc_layers:
                dc_layers = set(dc_layers)
                self.addCodeWindow(dc_layers, dc_type, '程序自动获取') # 显示
            else:
                self.addCodeWindow(layers, dc_type, '手动选择')  # 显示

    # 设计界面去选择层添加周期
    def addCodeWindow(self, layers, dc_type, title):
        dialog = MyDialog(layers, dc_type, title)
        dialog.exec_()
...
class MyDialog(QtGui.QDialog):
    ...
    def __init__(self, *args):
        ...
        self.dc_type = args[1]
        self.title = args[-1]
        self.setWindowTitle(u'确认添加周期(%s)' % self.title.decode('utf-8'))
        self.ui()
    ...
    def ui(self):
        ...
        q_label = QtGui.QLabel(u'识别到需要添加的层：')
