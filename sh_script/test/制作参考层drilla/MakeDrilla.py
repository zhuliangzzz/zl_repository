...
class MakeDrilla(object):
    ...
    def make(self):
        ...
        for signal in outsignalLayers:
            tmp_signal = '%s++++' % signal
            sm = sm_list[0] if signal == 'l1' else sm_list[-1]
            self.step.affect(signal)
            self.step.setFilterTypes('surface')
            self.step.selectAll()
            ...
            if self.step.Selected_count():
                ...
                self.step.refSelectFilter(tmp_signal)
                if self.step.Selected_count():
                    self.step.selectDelete()
                self.step.unaffectAll()
                self.step.affect(tmp_signal)
                self.step.copySel(make_layer)
                self.step.truncate(tmp_signal)
            self.step.unaffectAll()
            self.step.affect(signal)
        ...
        if len(light_pad_lays) == 2:
            ...
            if self.step.Selected_count():
                ...
                if self.step.Selected_count():
                    ...
                    for feat in feats:
                        x, y = feat.x, feat.y
                        pad_radius = float(re.sub('r|s', '', feat.symbol))/2000
                        add_success = False
                        # for x_ in range(10 * (x - pad_radius - 3.3), 10 * (x - pad_radius - 1.84), 1):
                        for x_ in range(int(10 * (x - pad_radius - 1.2)), int(10 * (x - pad_radius - 3.3)), -1):
                            self.step.addText(float(x_)/10, y - 0.7, 'M', 1.2, 1.4, 0.65616798)
                            self.step.refSelectFilter(make_layer)
                        ...
                        if not add_success:
                            for x_ in range(int(10 * (x + pad_radius + 0.3)), int(10 * (x + pad_radius + 3.3)), 1):
                                self.step.addText(float(x_)/10, y - 0.7, 'M', 1.2, 1.4, 0.65616798)
                                self.step.refSelectFilter(make_layer)
                                if not self.step.Selected_count():
                                    add_success = True
                    ...
                    self.step.setFilterTypes('pad')
                    # self.step.refSelectFilter(same_pad_lay, mode='same_center')
                    self.step.refSelectFilter(same_pad_lay, mode='disjoint')
                    # self.step.selectReverse()
                    self.step.resetFilter()
                    if self.step.Selected_count():
                        feats = layerObj.featout_dic_Index(units='mm', options='select+feat_index')['pads']
                        ...
                        for feat in feats:
                            ...
                            add_success = False
                            # for x_ in range(10 * (x - pad_radius - 3.3), 10 * (x - pad_radius - 1.84), 1):
                            for x_ in range(int(10 * (x - pad_radius - 1.84)), int(10 * (x - pad_radius - 3.3)), -1):
                                self.step.addText(float(x_)/10, y - 0.7, 'MC', 1.2, 1.4, 0.65616798)
                                self.step.refSelectFilter(make_layer)
                                if not self.step.Selected_count():
                                    add_success = True
                                    break
                ...
                if self.step.Selected_count():
                    ...
                    for feat in feats:
                        ...
                        # for x_ in range(10 * (x - pad_radius - 3.3), 10 * (x - pad_radius - 1.84), 1):
                        for x_ in range(int(10 * (x - pad_radius - 1.84)), int(10 * (x - pad_radius - 3.3)), -1):
                            self.step.addText(float(x_)/10, y - 0.7, 'MS', 1.2, 1.4, 0.65616798)
                            self.step.refSelectFilter(make_layer)
                            if not self.step.Selected_count():
                                add_success = True
                                break
            ...
            if self.step.Selected_count():
                ...
                for feat in feats:
                    ...
                    # for x_ in range(10 * (x - pad_radius - 3.3), 10 * (x - pad_radius - 1.84), 1):
                    for x_ in range(int(10 * (x - pad_radius - 1.84)), int(10 * (x - pad_radius - 3.3)), -1):
                        self.step.addText(float(x_)/10, y - 0.7, text, 1.2, 1.4, 0.65616798)
                        self.step.refSelectFilter(make_layer)
                        if not self.step.Selected_count():
                            add_success = True
                            break
        ...
        for signal in outsignalLayers:
            tmp_signal = '%s++++' % signal
            self.step.affect(signal)
            self.step.setAttrFilter_pro('.pattern_fill')
            self.step.selectAll()
            self.step.resetFilter()
            if self.step.Selected_count():
                self.step.copySel(tmp_signal)
                # self.step.affect(signal)
                self.step.setFilterTypes('surface')
                self.step.refSelectFilter(tmp_signal, mode='disjoint')
                self.step.truncate(tmp_signal)
                if self.step.Selected_count():
                    self.step.copySel(tmp_signal)
                    self.step.unaffectAll()
                    ...
                    self.step.copySel(make_layer)
                self.step.unaffectAll()
                self.step.removeLayer(tmp_signal)
            self.step.unaffectAll()
