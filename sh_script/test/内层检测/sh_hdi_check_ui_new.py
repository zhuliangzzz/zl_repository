...
class workThread(QtCore.QThread):
    ...
    def set_params(self, *args, **kwargs):
        self.PID = kwargs["pid"]
        self.script_path = kwargs["script_path"].strip()
    ...
    def run(self):
        ...
        try:            
            if sys.platform == "win32":
                ...
                cmd = ("COM script_run,name=%s,"
                "env1=JOB=%s,env2=STEP=%s,env3=CHECK_ID=%s,env4=INNER_OUTER=%s,params=%s" % (
                script_path, jobname, stepname,self.check_id, self.check_inner_or_outer, function))
            ...
            if script_path.endswith(".py"):
                os.environ["JOB"] = str(jobname)
                os.environ["STEP"] = str(stepname)
                os.environ["CHECK_ID"] = str(self.check_id)
                os.environ["INNER_OUTER"] = str(self.check_inner_or_outer)
                os.system("python %s %s" % (script_path, function))
            else:
                os.system("csh /incam/release/app_data/scr_start.csh %s" % csh_script_path)
            try:
                os.unlink(csh_script_path)
            except:
                pass
...
class show_check_ui(QtGui.QWidget):
    def __init__(self,parent = None):
        ...
        self.info_label.setGeometry(100, 5, 500, 40)
        ...
        self.setWindowTitle(u"{0} HDI自动检测项目({2}) {1}".format(jobname, __version__, u"内层" if (inn_or_out=="inner" or inn_or_out=="coreinner") else u"外层"))
    ...
    def view_note_detail(self, index, button):
        ...
        if button.text() == QtCore.QString(u"异常已查"):
            check_id = self.model.item(row, 9).text()
            index = self.model.index(row,4, QtCore.QModelIndex())                
            self.model.setData(index, u"检测异常(已查看)")        
            update_current_job_check_log(check_id, u"check_process='{0}'".format(u"检测异常(已查看)"))            
            return
    ...
    def update_check_log_view(self, row,check_id, args):
        self.showNormal()
        #if "-lyh" in jobname:
            #QtGui.QMessageBox.information(
                #self, u'警告', "".join(args), 1)
        if "check_over" in args:            
            index = self.model.index(row,4, QtCore.QModelIndex())                
            self.model.setData(index, u"检测异常(已查看)")        
            update_current_job_check_log(check_id, u"check_process='{0}'".format(u"检测异常(已查看)"))
    ...
    def run_script_in_thread(self, manual_run_row=None):
        ...
        for row in range(self.model.rowCount()):
            ...
            if "(coreinner)" in self.model.item(row, 1).text():
                check_inner_or_outer ="coreinner"
    ...
    def get_check_data(self):
        ...
        if self.test_check_btn.isChecked():
            ...
            if inn_or_out == "coreinner":
                data_info = get_check_function_list(user, jobname,
                                                    "check_status = 'test' and check_inn_or_out in ('all','{0}') and {1} = '是'".format("inner", persona))
            else:
                data_info = get_check_function_list(user, jobname,
            ...
            if inn_or_out == "coreinner":
                data_info = get_check_function_list(user, jobname,
                                                    "check_status = 'normal' and check_inn_or_out in ('all','{0}') and {1} = '是'".format("inner", persona))
            else:                
                data_info = get_check_function_list(user, jobname,
        ...
        if inn_or_out == "outer":
            data_info = [dic_info for dic_info in data_info
                         ...
                         and "(coreinner)" not in dic_info["function_description"]]
        ...
        if inn_or_out == "coreinner":
            data_info = [dic_info for dic_info in data_info
                         if "(coreinner)" in dic_info["function_description"]]            
