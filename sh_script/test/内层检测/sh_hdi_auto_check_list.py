...
def set_check_function_name_from_erp(check_status="('normal')"):
    ...
    if inn_or_out == "coreinner":
        condition = u"{0} = '是' and check_inn_or_out in ('all','{1}')".format(persona, "inner")
    else:        
    ...
    for info in sorted(data_info, key=lambda x: x["id"]):
        ...
        if inn_or_out == "coreinner":
            dic_value['inner_or_out'] = "({0})".format(inn_or_out)
...
def get_check_function_list(user,jobname, condition="1=1"):
    ...
    and {2}"""
    if "id =" in condition:
        sql = """select * from pdm_job_check_log_list where jobname = '{1}'
        and {2}"""
    print("----------------->", sql)
...
def get_sys_user_info():
    sql = """select * from sys_user"""
    data_info = ikm_fun.PG.SELECT_DIC(ikm_fun.dbc_p, sql)
    return data_info    
...
def get_process_is_running():
    """检测该项目是否运行过"""
    if inn_or_out == "coreinner":
        sql = """select * from pdm_job_check_log_list where job_id = {0} and check_inn_or_out = '{1}' """
        data_info = ikm_fun.PG.SELECT_DIC(ikm_fun.dbc_p, sql.format(jobID, "inner"))
    else:        
        sql = """select * from pdm_job_check_log_list where job_id = {0} and check_inn_or_out = '{1}' """
        data_info = ikm_fun.PG.SELECT_DIC(ikm_fun.dbc_p, sql.format(jobID, inn_or_out))
    # print data_info
    if not data_info:
        return False
