...
class Main:
    def __init__(self, debug=False):
        ...
        gClasses.Job(self.origin_name).open(1)
    ...
    def get_layers(self):
        ...
        for i, name in enumerate(matrix_info['gROWname']):
            ...
            if type == 'drill' and context == 'board':
                if re.match('s1-\d+$|s%s-\d+$|2nd$' % lay_num, name):
                    self.tgz_layers.append(name)
    ...
    def export_tgz(self):
        ...
        if platform.system() == "Windows":
            ...
            tmp_path = '/tmp/tmp'
            tgz_path = os.path.join(r'/id/workfile/film', self.origin_name)
        ...
        reVal = self.GEN.COM('export_job,job=%s,path=%s,mode=tar_gzip,submode=partial,'
                             'lyrs_mode=include,lyrs=%s,overwrite=yes,output_name=%s' % (self.job_name, tmp_path, include_lyrs, self.origin_name))
        self.GEN.VON()
    ...
    def export_pretreatment(self):
        ...
        # if len(process_) == 2:
        #     # 找镭射
        #     laser_drills = filter(lambda l: re.match('s1-\d+$|s(%s)-\d+$' % lay_num, l), self.job.matrix.returnRows('board', 'drill'))
        #     # 树脂塞孔层
        #     rows = self.job.matrix.returnRows()
        #     for row in rows:
        #         if re.match('szsk-(c|s)$', row):
        #             # TODO 找外层树脂塞孔层
        #             pass
        #     # TODO drl和镭射层去掉树脂塞孔
        # 去掉板外物件
        for stepname in self.job.stepsList:
            ...
            step.selectCutData()
            step.unaffectAll()
            for layer in self.out_signals + self.solder_masks:
                step.affect(layer)
