...
class Main:
    def __init__(self, debug=False):
        ...
        for stepname in current_job.stepsList:
            ...
            for layer in self.tgz_layers:
                ...
                layer_obj = step.layers.get(layer)
                barcodes = layer_obj.featout_dic_Index(units="mm", options='feat_index')["barcodes"]
                if barcodes:
                    step.affect(layer)
                    ...
                    for barcode in barcodes:
                        ...
                        new_text = barcode.text.replace('$$job', '$#JOB')
                        if '$$job' in text:
