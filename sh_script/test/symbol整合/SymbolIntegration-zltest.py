...
"""
   ...
   @desc:
   # 用两个料号各自加symbol复制过来
   # 20250303 add_pad break 然后再add_pad
"""
import copy
import os
import re
    ...
    def symbol_compare(self):
        ...
        if self.compare_symbols:
            ...
            for hdi_symbol in self.compare_symbols:
                ...
                self.step.selectBreak()
                self.step.unaffectAll()
                self.step.COM('delete_unused_sym,job=%s' % self.test_jobname)
            ...
            if inconsistent_symbols:
                ...
                for symbol in inconsistent_symbols:
                    change_text = {}
                    self.step_import.COM('open_entity,job=%s,type=symbol,name=%s,iconic=no' % (self.test_job_import.name, symbol))
                    self.step_import.group = self.step_import.COMANS
                    self.step_import.setGroup()
                    self.step_import.setFilterTypes('text')
                    self.step_import.selectAll()
                    self.step_import.resetFilter()
                    if self.step_import.Selected_count():
                        feats = self.step_import.INFO(
                            '-t symbol -e %s/%s -d FEATURES -o feat_index+select' % (self.test_job_import.name, symbol))
                        del feats[0]
                        for feat in feats:
                            ...
                            if '$$time' in text.lower():
                                ...
                                break
                    self.step_import.clearSel()
                    if change_text:
                        for k, v in change_text.items():
                            if symbol not in dynamic_symbols:
                                dynamic_symbols.append(symbol)
                            self.step_import.selectByIndex(symbol, k)
                            self.step_import.changeText(v)
            self.step.initStep()
            # 重新加 重新比对不一致的
            if dynamic_symbols:
                for hdi_symbol in dynamic_symbols:
                    ...
                    self.step.removeLayer(map_layer)
                    #
                    self.step.truncate(hdi_symbol)
                    self.step.truncate(mlb_symbol)
                    self.step.copyLayer(self.test_job_import.name, self.step_import.name, hdi_symbol, mlb_symbol)
                    self.step.affect(mlb_symbol)
                    self.step.selectBreak()
                    self.step.unaffectAll()
                    ...
                    # 当前symbol也去掉动态
                    change_text = {}
                    self.step.COM('open_entity,job=%s,type=symbol,name=%s,iconic=no' % (self.test_job.name, hdi_symbol))
                    ...
                    if self.step.Selected_count():
                        feats = self.step.INFO(
                            '-t symbol -e %s/%s -d FEATURES -o feat_index+select' % (self.test_job.name, hdi_symbol))
                        del feats[0]
                        for feat in feats:
                            text = re.search("'(.*)'", feat).group(1)
                            idx = feat.split()[0].replace('#', '')
                            if '$$layer' in text.lower():
                                ...
                                change_text[idx] = new_text
                            if '$$time' in text.lower():
                                time_symbols.append(hdi_symbol)
                                break
                    self.step.clearSel()
                    if change_text:
                        for k, v in change_text.items():
                            self.step.selectByIndex(hdi_symbol, k)
