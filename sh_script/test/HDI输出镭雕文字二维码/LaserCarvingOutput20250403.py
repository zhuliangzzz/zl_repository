...
"""
   ...
   @software:PyCharm
   @desc:
   20241030 回读进来如果时反面 以0点x镜像  提出人：翟鸣
   20241121 没有set序号层也需要输出 提出人：周涌
            没有序号层 按panel内获取set时的默认顺序
"""
import json
import os
import re
...
class LaserCarvingOutput(QtGui.QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        if platform.system() == "Windows":
            ...
            self.output_dir = '/id/workfile/hdi_film/' + self.job_name
        self.pre_show_ui()
        if self.dbc_o:
            self.DB_O.DB_CLOSE(self.dbc_o)
        self.label_job.setText('Job:' + self.job_name)
        symbols = sorted(list(filter(lambda symbol: symbol.startswith('2dbc'), os.listdir('/incam/server/site_data/library/symbols'))))
        ...
        self.widget_bot.hide()
        self.checkBox.clicked.connect(self.toggle_show)
        self.checkBox_2.clicked.connect(self.toggle_show)
    ...
    def toggle_show(self):
        if self.checkBox.isChecked():
            self.widget_top.show()
        else:
            self.widget_top.hide()
        if self.checkBox_2.isChecked():
            self.widget_bot.show()
        else:
            self.widget_bot.hide()
    ...
    def pre_show_ui(self):
        ...
        WHERE RJTSL.JOB_NAME = '%s' and RJTSL.work_center_code = '激光打标' 
        ORDER BY RJTSL.ORDER_NUM, RJTSL.TRAVELER_ORDERING_INDEX""" % self.job_name.upper()[:13]  # --截取前十三位料号名
        data_info = conn.select_dict(sql, is_arraysize=False)
        if data_info:
            resText = []
            for data in data_info:
                if 'C' in data.get('VALUE_AS_STRING'):
                    if 'C面' not in resText:
                        resText.append('C面')
                    self.checkBox.setChecked(True)
                if 'S' in data.get('VALUE_AS_STRING'):
                    if 'S面' not in resText:
                        resText.append('S面')
                    self.checkBox_2.setChecked(True)
            self.lineEdit.setText('+'.join(resText).decode('utf-8'))
    ...
    def addCode(self):
        if not self.checkBox.isChecked() and not self.checkBox_2.isChecked():
            M = M_Box()
            showText = M.msgText(u'提示', u"请勾选添加的面次", showcolor='green')
            M.msgBox(showText)
            return
        self.hide()
        ...
        exist_layer, target_layer = [], []
        if self.job.steps.get(step).isLayer(layer_c):
            exist_layer.append(layer_c)
        if self.job.steps.get(step).isLayer(layer_s):
            exist_layer.append(layer_s)
        if self.checkBox.isChecked():
            target_layer.append(layer_c)
        if self.checkBox_2.isChecked():
            target_layer.append(layer_s)
        if exist_layer:
            resp = QtGui.QMessageBox.information(None, u'tips', u'已存在%s，是否删除重做' % u"、".join(exist_layer),
        ...
        for layer in target_layer:
            GEN.CREATE_LAYER(layer)
        x = self.job.steps.get(step).profile.xmin * 25.4
        y = self.job.steps.get(step).profile.ymin * 25.4
        if GEN.LAYER_EXISTS(layer_c, self.job_name, self.pnl_step) and GEN.LAYER_EXISTS(layer_s, self.job_name, self.pnl_step):
            ...
            GEN.COM('sel_transform,oper=rotate;mirror,x_anchor=0,y_anchor=0,angle=0,direction=ccw,x_scale=1,y_scale=1,x_offset=0,y_offset=0,mode=axis,duplicate=no')
            GEN.COM('cur_atr_reset')
            GEN.CLEAR_LAYER()
        elif GEN.LAYER_EXISTS(layer_c, self.job_name, self.pnl_step):
            GEN.WORK_LAYER(layer_c)
            GEN.MOUSE('点击选择添加位置')
            x, y = GEN.MOUSEANS.split()
            x, y = float(x), float(y)
            ...
            GEN.AFFECTED_LAYER(layer_s, 'yes')
            GEN.COM('cur_atr_set,attribute=.string,text=2dmark')
            GEN.ADD_PAD(x, y, bot_symbol, angle=bot_angle, attr='yes')
            GEN.COM('cur_atr_reset')
            GEN.CLEAR_LAYER()
        number = 1
        for layer in target_layer:
            GEN.WORK_LAYER(layer, number)
    ...
    def get_add_2dbc(self):
        ...
        for slayer in all_silk_layers:
            # === 获取文字层面次，加到不同的列表里 ===
            if GEN.DO_INFO("-t layer -e %s/%s/%s -d SIDE" % (self.job_name, self.pnl_step, slayer))['gSIDE'] == 'top':
                top_silk_layer.append(slayer)
            elif GEN.DO_INFO("-t layer -e %s/%s/%s -d SIDE" % (self.job_name, self.pnl_step, slayer))[
                'gSIDE'] == 'bottom':
    ...
    def reread_2dbc_file(self, file_name, side):
        ...
        if side == 'bot':
            GEN.COM('sel_transform,oper=mirror,x_anchor=0,y_anchor=0,angle=0,direction=ccw,x_scale=1,y_scale=1,x_offset=0,y_offset=0,mode=anchor,duplicate=no')
    ...
    def get_all_cor(self, side):
        ...
        # cu_index_reg = re.compile (r'^#(\d+)\s+#S P 0;.pattern_fill,.string=set\n')
        for line in info:
        ...
        for search_layer in search_layers:
            ...
            GEN.CLEAR_LAYER()
        if not number_layer:
            QtGui.QMessageBox.information(self, 'tips', u'未识别到set序号层，程序按默认顺序输出' + side)
            for cu_i, cur_index in enumerate(index_list):
                ...
                if sel_num > 0:
                    ...
                    if set_sr_exist == 'yes':
                        ...
                        GEN.SEL_REVERSE()
                    if int(GEN.GET_SELECT_COUNT()) > 0:
                        get_pnl_co = GEN.INFO('-t layer -e %s/%s/%s -d FEATURES -o select,units=mm' % (
                        self.job_name, self.pnl_step, tmp_layer))
                        dc_cor = get_pnl_co[1:]
                        # 以set跳码
                        pp = 0  # 二维码跳码序号
                        dd = 0  # 明码跳码序号
                        for i in dc_cor:
                            ...
                            if m:
                                ...
                                M = M_Box()
                                showText = M.msgText(u'提示', u"层别:%s中的symbol:%s不符合二维码及明码命名" % (
                                tmp_layer, cur_sym_name), showcolor='red')
                                M.msgBox(showText)
                            ...
                            if cu_step == 'ewm-coupon':
                                ...
                                if bc_type == 1:
                                    ...
                                    crs_ind = dd
                            al_cor[jj] = dict(x=cur_x_cor, y=cur_y_cor, angle=sym_agl, set_index=crs_ss,
                                              unit_index=crs_ind,
                                              bc_type=bc_type, use_bc_size=use_barcode_size,
                                              use_text_size=use_text_size)
                            jj += 1
                        # job.PAUSE(str(jj))
                        GEN.COM('sel_delete')
                    get_pnl_co = GEN.INFO(
                        '-t layer -e %s/%s/%s  -d FEATURES ,units=mm' % (self.job_name, self.pnl_step, tmp_layer))
                    #  === 存在set上的母码 ===
                    if len(get_pnl_co) > 0:
                        dc_cor = get_pnl_co[1:]
                        # 以set跳码
