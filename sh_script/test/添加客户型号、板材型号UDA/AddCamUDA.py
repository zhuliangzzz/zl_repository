...
class AddCamUDA(QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        self.add_layers = job.matrix.returnRows('board', 'signal|silk_screen|solder_mask')
        self.comboBox.addItems(self.add_layers)
        self.comboBox_2.addItems(self.add_layers)
    ...
    def run(self):
        ...
        if self.checkBox.isChecked():  # 客户型号
            add_layer = self.comboBox.currentText()
        ...
        if self.checkBox_2.isChecked():  # 板材型号
            add_layer = self.comboBox_2.currentText()
            step.display(add_layer)
