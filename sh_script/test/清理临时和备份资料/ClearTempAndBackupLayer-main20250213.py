    ...
    def run(self):
        ...
        for jobname in os.listdir(self.db1):
            ...
            if len(jobname) != 13:
                continue
            # 测试zltest料号
            # if not jobname.endswith('zltest'):
            #     continue
            if os.path.isdir(os.path.join(self.db1, jobname)):
                ...
                for lyr in os.listdir(os.path.join(self.db1, jobname, 'steps', os.listdir(os.path.join(self.db1, jobname, 'steps'))[0], 'layers')):
                    ...
                    # 杂层
                    if lyr in ('rout_drc', 'symbol_tmp') or re.match('-+$', lyr) or lyr.count('+') > 1:
                        if jobname not in clear_dict.keys():
                            clear_dict[jobname] = []
                        clear_dict[jobname].append(lyr)
        ...
        if clear_dict:
            with open(self.log_file, 'a') as writer:
                writer.write('[%s]The number of cleaning this time: %s.\n\n' % (run_time, len(clear_dict.keys())))
        # jobname = 'sa1504of021b5'
        # lyrs_str = 'm1-ls250116.0204 m2-ls250116.0204'
        # os.system('python /frontline/incam/server/site_data/scripts/sh_script/zl/crontab/CheckLossTestPadComp.py %s %s' % (jobname, lyrs_str))
