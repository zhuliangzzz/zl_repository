                ...
                for name in set(all_ouput_jobs):            
                    ...
                    if "not_import_tgz" in args:
                        ...
                        if sys.platform <> "win32":
                            ...
                            if os.path.exists(db_job_path):
                                print("----------->", db_job_path)
                                res = self.import_tgz(name, dic_job_id[name], "no_gui_login_incam", tgz_import_path=db_job_path)
                            else:                                
                            ...
                            if os.path.exists(error_file):
                                ...
                                if os.path.exists(success_file):
                                    ...
                                    if success_log:
                                        ...
                                        shutil.copy(tgz_path, dest_net_path+"/"+jobname+".tgz")
                                    else:
            ...
            for info in data_info:
                ...
                if exist_uploading_data_info:
                    ...
                    if is_continue:
                        ...
                        and data_type='正业坐标量测资料'\
                        and isoutput = 'Y'\
                        and remark <> 'update_zy_MI_date_{1}'".format(jobname.upper(), current_day)
                        exist_data_info = conn.SELECT_DIC(dbc_m, sql)
                        # print("----->", exist_data_info[0]["id"])
                        if exist_data_info:                        
                            sql = """update hdi_electron_tools.hdi_job_tools
...
if __name__ == "__main__":
    ...
    # 测试    
    if func is None:
        func = "uploading_laser_barcode_info"
        # args = ["26953","gui_run"]
    ####
