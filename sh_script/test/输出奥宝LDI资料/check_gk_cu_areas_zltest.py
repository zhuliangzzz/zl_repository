...
class Main:
    ...
    def re_count_area(self):
        ...
        # 20241225 阴阳拼版改matrix
        self.flipStepName('panel')
        # tool_step = job.steps[0]  # python2 字典不能用索引取值。。。
        ...
        for i, dict_sel in enumerate(dict_dklayer.values()):
            ...
            if self.flipList:
                tool_step.open()
                tool_step.clearAndReset()
                tool_step.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=yes' % job_name)
                tool_step.COM("matrix_layer_type,job={0},matrix=matrix,layer={1},type=signal".format(job_name,t_layer))
                tool_step.COM("matrix_layer_type,job={0},matrix=matrix,layer={1},type=signal".format(job_name, b_layer))
                tool_step.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=no' % job_name)
            else:
            ...
            if sel_drill:
                for dr in sel_drill:
                    ...
                    if self.flipList:
                        ...
                        tool_step.clearAndReset()
                        tool_step.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=yes' % job_name)
                        tool_step.removeLayer(back_layer)
                        tool_step.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=no' % job_name)
                        step.open()
                    else:
                    ...
                    if self.flipList:
                        ...
                        tool_step.clearAndReset()
                        tool_step.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=yes' % job_name)
                        tool_step.COM("matrix_layer_type,job={0},matrix=matrix,layer={1},type=drill".format(job_name, back_layer))
                        tool_step.COM("matrix_layer_context,job={0},matrix=matrix,layer={1},context=board".format(job_name,back_layer))
                        tool_step.COM("matrix_layer_drill,job={0},matrix=matrix,layer={1},start={2},end={3}".format(job_name,back_layer,t_layer,b_layer))
                        tool_step.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=no' % job_name)
                    else:
            ...
            if self.flipList:
                ...
                tool_step.clearAndReset()
                tool_step.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=yes' % job_name)
                tool_step.COM("matrix_layer_type,job={0},matrix=matrix,layer={1},type=solder_paste".format(job_name, t_layer))
                tool_step.COM("matrix_layer_type,job={0},matrix=matrix,layer={1},type=solder_paste".format(job_name, b_layer))
                if back_drl_list:
                    for dr in back_drl_list:
                        tool_step.removeLayer(dr)
                tool_step.COM('matrix_suspend_symmetry_check,job=%s,matrix=matrix,suspend=no' % job_name)
            else:
    ...
    def flipStepName(self, step):
        ...
        :return: None
        """
        info = job.DO_INFO('-t step -e %s/%s -m script -d SR -p flip+step' % (job_name, step))
