...
{	
	...
	if (scalar(@selGkLayer) > 0){
		...
		my $res = system("python /incam/server/site_data/scripts/sh_script/ldi_orbotech/check_gk_cu_areas_zltest.py $JOB $string");
		if($res != 0){
			exit(0);
		}
	}	
    ...
    for my $lay (@outLayer)
    {
		...
		if ($fg_num != 0) {     
			...
			if ($c->GET_SELECT_COUNT() != 0) {
				&Messages('warning', "分割设计,图形中属性(.fiducial_name=317.reg)不应存在，请确认资料，程序退出");
				exit(1);
			}
			if ($fg_num % 4 != 0) {
				&Messages('warning', "分割图形中属性(.fiducial_name=300.measure)部件数量不是4的整数倍，请确认资料，程序退出");
				exit(1);
			}
			$c->CLEAR_FEAT();
			...
			if (($fg_vnum + $fg_hnum) == 0) {
				&Messages('warning', "分割图形中属性(.fiducial_name=300.trmv或.fiducial_name=300.trmh)不存在，请确认资料，程序退出");
				exit(1);
			}
			my $fg_times = $fg_num / 4;
			# === 二分割应该 分割线或者横线或者竖线
			if ($fg_times == 1) {
				&Messages('warning', "分割光点数量为4，不合理，请确认资料，程序退出");
				exit(1);
			} elsif  ($fg_times == 2) {
				if (($fg_vnum + $fg_hnum) != 1) {
					&Messages('warning', "光点数量为8应为2分割,分割线r0数量不正确,请确认资料，程序退出");
					exit(1);
				}
			} elsif ($fg_times == 3) {
				if ($fg_vnum == 2 || $fg_hnum == 2 ) {
				} else {
					&Messages('warning', "光点数量为12应为3分割,分割线r0数量应横向2或纵向2,\n目前数量不正确,请确认资料，程序退出");
					exit(1);
				}
			} elsif ($fg_times == 4) {
				if ($fg_vnum == 1 && $fg_hnum == 1 ) {
				} else {
					&Messages('warning', "光点数量为16应为4分割,分割线r0数量应横向1+纵向1,\n目前数量不正确,请确认资料，程序退出");
					exit(1);
				}
			} else {
				&Messages('warning', "目前仅支持2分割及4分割,光点数量 $fg_times\n数量不正确,请确认资料，程序退出");
				exit(1);
			}
		} else {
			# --循环四个坐标添加r0对位靶点
...
sub outputOpfx
{
	...
	if ($out_Lay =~ /gk/){
		...
		my $renameFile = `ls $dirPath/$JOB | grep ${JOB}\@${out_Lay}_-`;
		chomp $renameFile;
		if (-e "$dirPath/$JOB/$renameFile")
		{
...
sub check_Align_ro_shdwsd{
    ...
    for my $sm_layer(@check_sm_layers){
        ...
        my @mask_r0_array;
        my @mask_dwsd_array;
        $f->COM("info,args=-t layer -e $JOB/$sel_step/$sm_layer -m script -d FEATURES,out_file=$feature_file,write_mode=replace,units=inch");
        if(-e $feature_file){
            open(MYFL,$feature_file) or die "can't open file $feature_file: $!";
            while(<MYFL>){
                 my @data_array = split(" ",$_);
				 # === 排除分割线属性.fiducial_name=300.trmh .fiducial_name=300.trmv 以及300.measure
                 if($data_array[0] eq "#P" and $data_array[3] eq "r0" and $data_array[7] !~ /300.trm|300.measure/ ){
