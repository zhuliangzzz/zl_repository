...
class UpadateIdbEffectiveSchedule(object):
    ...
    def UpdateIdbEffective(self):
        # jobdict
        jobdict = {}
        sql = "select id,jobname,odb_md5_key from hdi_engineering.incam_output_tgz_info order by id desc"
        results = self.my_sql.SELECT_DIC(self.my_sql.dbc, sql)
        for result in results:
            jobname = result['jobname']
            if jobname not in jobdict.keys():
                jobdict[jobname] = {'id': result['id'], 'odb_md5_key': result['odb_md5_key']}
        jobs, ids = [], []
        start_time = datetime.datetime.now()
        # 找每个jobname的odb_md5_key是否在tgz有对应的tgz
        for jobname, data in jobdict.items():
            id = data.get('id')
