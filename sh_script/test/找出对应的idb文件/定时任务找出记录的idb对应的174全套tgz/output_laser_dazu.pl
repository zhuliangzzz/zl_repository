...
for (my $i = 0; $i <= $#rlayers; $i++) {
    ...
    foreach (@{$f->{doinfo}{gSYMS_HISTsymbol}}) {
        if ($_ eq $pnlspecial[0] || $_ eq $pnlspecial[1] || $_ eq $pnlspecial[2] || $_ eq $pnlspecial[3] || $_ eq $pnlspecial[4]) {
			...
			if (406 <= $drillSize and $drillSize != 515 and $drillSize != 520){
				$c->Messages("info", "$rlayers[$i] 层中存在大于等于16mil的镭射孔，大小: $drillSize 请检查！");
				exit();
			}			
...
my $job_output_path = "/id/workfile/hdi_film/$JOB";
if (!-d "$job_output_path") {
    mkdir("$job_output_path");
}
my $output_path_des = "/id/workfile/hdi_film/$JOB/大族uv镭射";
            ...
            for (my $n = 1; $n <= $nLoop; $n++) {
                ...
                if ($clip_mode eq "不分割") {
                    ...
                    if (${$hash{$rlayers[$i]}}[4] eq 'yes') {
                        ...
                        $f->COM("sel_transform,oper=rotate\;mirror,x_anchor=0,y_anchor=0,angle=0,direction=ccw,x_scale=1,y_scale=1,x_offset=0,y_offset=0,mode=anchor,duplicate=no");
                        $f->COM("affected_layer,mode=all,affected=no");
                    }
                }else{
                    # --分割资料回读
				...
				# 此处输出钻带文件加入参数
                #system("python $cur_script_dir/reWriteLaserFiles.py $output_path $JOB.$rlayers[$i].write");
				...
				mv("$output_path/$JOB.$rlayers[$i].write","$output_path_des/$JOB.$rlayers[$i]");
    ...
    if ($t21exist ne "no") {
        ...
        foreach $tps (@{$position{$search_key}}) {
            ...
            if ($tps =~ /X-?(\d+)Y-?(\d+)/) {
                ...
                elsif ($showx > $srx_max && $sry_min < $showy && $showy < $sry_max) {
                    $t21_cor{4} = $tps . "\n";
                }
                elsif ($showy < $sry_min) {
                    $t21_cor{3} = $tps . "\n";
