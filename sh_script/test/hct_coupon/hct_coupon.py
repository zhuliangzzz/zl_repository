...
from genesisPackages import get_layer_selected_limits, \
     signalLayers, innersignalLayers, laser_drill_layers, \
     get_drill_start_end_layers, matrixInfo, solderMaskLayers, \
     getSmallestHole
from get_erp_job_info import get_drill_information,\
     get_barcode_mianci, get_inplan_mrp_info, \
     get_cu_weight
...
class MAIN(object):
    def __init__(self):
        ...
        self.array_mrp_info = get_inplan_mrp_info(str(self.job_name).upper(), condtion="1=1")        
    ...
    def check_copper_thick(self):
        ...
        array_cu_weight = get_cu_weight(self.job_name.upper(), True)
        # print array_mrp_info
        for dic_mrp_info in self.array_mrp_info:
            ...
            if process_num == 1:
                for dic_cu_info in array_cu_weight:
                    ...
                    if dic_cu_info["LAYER_NAME"].lower() in [layer1, layer2]:
                        ...
                        if weight >= 1.5:
                            self.add_hct_type = "big"
                        # print weight, "----->2"
                        if weight >= 1.98:
                            """检测到内层L2、L3铜厚大于2 OZ，请与主管确认线宽、孔ring设计！"""
                            return u"检测到内层{0} {1}铜厚大于等于2OZ，请与主管确认线宽、孔ring设计！".format(layer1, layer2)
            else:
                for dic_cu_info in array_cu_weight:
                    if dic_cu_info["LAYER_NAME"].lower() in [layer1, layer2]:
                        ...
                        # print weight, "----->2"
                        if weight >= 2.99:
                            return u"检测到次外层或外层{0} {1}铜厚大于等于3mil，请与主管确认线宽、孔ring设计！".format(layer1, layer2)
        ...
        for time in self.laser_dict:
            ...
            for side in ['top_laser','bot_laser']:
                if side in self.laser_dict[time]:
                    ...
                    # self.GEN.PAUSE(str([self.laser_dict[time], laser_num_list]))
                    if side == 'top_laser':
        ...
        for time in self.laser_dict:
            ...
            if 'top_laser' not in self.laser_dict[time]:
                ...
                if self.modeUse == 'normal':
                    ...
                    des_mk_layer1 = 'b%s-%s' % (self.laser_dict[time]['top_laser_end_layer'] - 1, self.laser_dict[time]['bot_laser_end_layer'] + 1)
                    # self.GEN.PAUSE(str([self.modeUse, des_mk_layer, des_mk_layer1, self.mk_layers]))
                    if des_mk_layer in self.mk_layers:
                        self.mk_layer = des_mk_layer
                    elif des_mk_layer1 in self.mk_layers:
                        self.mk_layer = des_mk_layer1
                    else:
            ...
            # elif time == 'twice':
                ...
                if time != "twice":
                    des_coupon = 'hct-coupon-2_{0}'.format(time.replace("twice_", ""))
                throughmode = 'twice'
            ...
            if getattr(self, "add_hct_type", None) == "big":
                big_hole_mode = True
                lib_coupon = 'hct-bighole'
                if laser_mode == 'oneside':
            ...
            array_log = self.modify_line_pad_size(lib_coupon, des_coupon)
            if array_log:
                self.Message("\n".join(array_log))
            
            if self.oldStepExist == 'yes':
                self.reUseBackupCoupon(des_coupon)
    ...
    def get_top_bot_laser(self, ip_laser_layers):
        ...
        for layer in ip_laser_layers:
            ...
            if abs(int(digt_s) - int(digt_e)) == 1:
                ...
                key_word = "twice_%.0f"%round(abs((int(digt_s) + int(digt_e)) *0.5 - (self.layer_num+1)*0.5), 0)
            #elif abs(int(digt_s) - int(digt_e)) == 2:
                ...
                ##exit(0)
                #key_word = 'twice_{0}'.format(abs(int(digt_s) - int(digt_e)) - 2)
    ...
    def getAddType(self):
        ...
        if self.GEN.STEP_EXISTS(job=self.job_name, step="edit") == 'yes':
            ...
            edit_step.VOF()
            edit_step.COM("sel_delete_atr,mode=list,attributes=.rout_chain")
            edit_step.VON()            
            ...
            # self.GEN.PAUSE(str([hole_size, self.modeUse]))
    ...
    def get_inplan_information(self, inplan_job):
        ...
        for info in drill_info:
            ...
            if laserReg.match(info['DRILL_LAYER_']):
                if info['ERP_FINISH_SIZE_'] and "绕烧" in info['ERP_FINISH_SIZE_']:
        ...
        if len(info['gTOOLdrill_size']) != 0:
            ...
            for i in range(len(info['gTOOLdrill_size'])):
                if info['gTOOLtype'][i] in ['via', 'plated']:
                    pth_tool.append(int(info['gTOOLdrill_size'][i]))
            if len(pth_tool):
            ...
            mintool = getSmallestHole(self.job_name, "", layer)
    ...
    def modify_line_pad_size(self, lib_stepname, stepname):
        
        # 翟鸣通知 正常的coupon孔环不变
        if "big" in lib_stepname:
            ring_size = 7            
        else:
            ring_size = 0
        # http://192.168.2.120:82/zentao/story-view-5622.html
        # 厚铜板会把线蚀刻断 故加大线跟pad 20230607 by lyh
        ...
        for worklayer in signalLayers:
            if step.isLayer(worklayer):
                ...
                drill_size = 4.016# 默认取0.1mm的镭射孔径
                max_hole_size = 100
                for drill_layer in laser_drill_layers:
                    pre, suffix = int(drill_layer.split("-")[0][1:]), int(drill_layer.split("-")[1])
                    if min([pre, suffix])<= index <= max([pre, suffix]):
                        layer_cmd = gClasses.Layer(step, drill_layer)
                        feat_out = layer_cmd.featCurrent_LayerOut(units="inch")["pads"]
                        if feat_out:                                
                            drill_size = float(feat_out[0].symbol[1:])
                            max_hole_size = drill_size * 25.4
                            if drill_layer in self.big_laser_holes:
                                max_hole_size = 350                            
                            break
                ...
                layer_type = ""
                for dic_mrp_info in self.array_mrp_info:
                    if '光板' in dic_mrp_info['MRPNAME']:
                        continue
                    layer1 = dic_mrp_info["FROMLAY"].lower()
                    layer2 = dic_mrp_info["TOLAY"].lower()
                    process_num = dic_mrp_info["PROCESS_NUM"]
                    mrp_name = dic_mrp_info["MRPNAME"]
                    
                    if worklayer in [layer1, layer2]:                            
                        if "-" in mrp_name:
                            if process_num == 1:
                                layer_type = "inn"
                                break                        
                ...
                # step.PAUSE(str([worklayer, layer_type]))                            
                if layer_type == "out":
                    step.resetFilter()
                    step.setAttrFilter(".bit,text=pos_laser_pad")
                    step.selectAll()
                    if max_hole_size <= 300:
                        if step.featureSelected():
                            # step.COM("sel_resize,size={0}".format((7-(25.5-drill_size) / 2) *25.4*2))
                            if ring_size == 7:                                
                                step.changeSymbol("r{0}".format((ring_size*2+drill_size)*25.4))
                            #else:
                                ## 翟鸣通知改成固定大小12.5mil
                                #step.changeSymbol("r{0}".format(12.5*25.4))                            
                    else:
                        arraylist_log.append(u"检测到{0}层对应的镭射孔径大于300，请检查ring环是否满足铜厚要求！".format(worklayer))
                     
                    step.selectNone()       
                    step.resetFilter()
                    step.setAttrFilter(".bit,text=pos_line")
                    step.selectAll()
                    if step.featureSelected():
                        step.COM("sel_resize,size={0}".format((12-10) *25.4))
                        
                    step.resetFilter()
                    step.setAttrFilter(".bit,text=pos_line_outline")
                    step.selectAll()
                    if step.featureSelected():
                        step.COM("sel_resize,size={0}".format((12-10) *25.4))
                else:
                    step.resetFilter()
                    step.setAttrFilter(".bit,text=neg_laser_pad")
                    step.setAttrFilter(".bit,text=pos_laser_pad")
                    ...
                    step.selectNone()
                    step.resetFilter()
                    step.setAttrFilter(".bit,text=neg_line")
                    step.setAttrFilter(".bit,text=pos_line")
                    step.COM("filter_atr_logic,filter_name=popup,logic=or")
                    step.selectAll()
                    if step.featureSelected():
                        if layer_type == "inn":
                            # step.COM("sel_resize,size={0}".format((10-10) *25.4))
                            pass
                        else:
                            # 次外层
                            step.COM("sel_resize,size={0}".format((12-10) *25.4))
        ...
        #翟鸣通知 取消链路上的开窗 20230904 by lyh
        # http://192.168.2.120:82/zentao/story-view-5945.html
        for layer in solderMaskLayers:
            step.clearAll()
            step.affect(layer)
            step.addRectangle(7.4, -0.1, 26.5, 7.2, polarity='negative')
            step.contourize()
            ...
            step.clearAll()
            step.affect(self.drlLay)
            step.resetFilter()
            step.setAttrFilter(".drill,option=non_plated")
            step.selectAll()
            if step.featureSelected():
                step.copyToLayer(layer, size=20*25.4)
            ...
            step.selectAll()
            if step.featureSelected():
                step.copySel("tmp_hole")
                step.clearAll()
                step.affect("tmp_hole")
                step.changeSymbol("r1000")                
                step.copyToLayer(layer)
                step.removeLayer("tmp_hole")
    ...
    def addLayerDetail(self, couStep, throughmode='once', twicemode='one',big_hole=None,laser_mode=None):
        ...
        # self.GEN.PAUSE(str([self.top_signal, couStep, self.bot_signal]))
        self.GEN.CLEAR_LAYER()
        ...
        if self.modeUse == 'normal' or laser_mode == 'oneside':
            ...
            # 无最小通孔时 取0.452的孔20230606 by lyh
            self.filterFeature('r250', 'r%s' % self.allDrlInfo[self.drlLay].get('min_pth', 452))
            self.GEN.CLEAR_FEAT()
        ...
        # 拷贝镭射层模板至正式镭射层
        # self.GEN.PAUSE(str([self.laser_dict[twicemode], throughmode]))
        ...
        #厚铜板 这里不能按大孔逻辑来 否则有层会漏掉 20231110 by lyh
        if big_hole and getattr(self, "add_hct_type", None) != "big":
            currentLaserThroughLayers = [2, self.layer_num - 1]
        
        inner_signal_layers = []
        is_inner_sigle_laser = False    #定义仅内层镭射的情况 
        if 'twice' in throughmode and 'twice' in twicemode:
            currentLaserThroughLayers = []            
            for layer in currentLaserLayers:
                pre,suffix = layer.split("-")
                pre = pre[1:]
                for i in range(self.layer_num+1):
                    if (i > int(pre) and i < int(suffix)) or \
                       (i > int(suffix) and i < int(pre)):
                        currentLaserThroughLayers.append(i)
                if not ("l{0}".format(pre) in self.top_signal + self.bot_signal or \
                   "l{0}".format(suffix) in self.top_signal + self.bot_signal):
                    is_inner_sigle_laser = True
                    inner_signal_layers.append("l{0}".format(pre))
                    inner_signal_layers.append("l{0}".format(suffix))
                        
        # self.GEN.PAUSE(str(currentLaserThroughLayers))
        ...
        # 镭射贯穿层
        for inum in currentLaserThroughLayers + inner_signal_layers:
        ...
        if 'twice' in throughmode and 'twice' in twicemode:
            ...
            if is_inner_sigle_laser:
                inner_top_signal_layer = sorted(inner_signal_layers, key=lambda x: int(x[1:]))[0]
                self.GEN.CLEAR_LAYER()
                self.GEN.AFFECTED_LAYER('l1','yes')
                self.GEN.FILTER_RESET()
                self.GEN.FILTER_SET_TYP('pad;line')
                self.GEN.FILTER_SELECT()
                if int(self.GEN.GET_SELECT_COUNT()) > 0:
                    self.GEN.SEL_COPY(inner_top_signal_layer+"_tmp")
                    self.GEN.FILTER_RESET()
                    self.GEN.FILTER_SET_TYP('pad;line')
                    self.GEN.COM('filter_set,filter_name=popup,update_popup=no,exclude_syms=s1524')
                    self.GEN.FILTER_SELECT()
                    if int(self.GEN.GET_SELECT_COUNT()) > 0:
                        self.GEN.SEL_DELETE()
                    
                    self.GEN.CLEAR_LAYER()
                    self.GEN.AFFECTED_LAYER(inner_top_signal_layer+"_tmp",'yes')
                    self.GEN.FILTER_RESET()
                    self.GEN.FILTER_SET_INCLUDE_SYMS("s1524")
                    self.GEN.FILTER_SELECT()
                    if int(self.GEN.GET_SELECT_COUNT()) > 0:                    
                        self.GEN.SEL_CHANEG_SYM("r650")
                    
                    self.GEN.CLEAR_LAYER()
                    self.GEN.AFFECTED_LAYER(inner_top_signal_layer,'yes')                        
                    self.GEN.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                             "area=reference,area_type=rectangle,inout=inside,"
                             "contour_cut=no,margin=254,ref_layer={0}_tmp,"
                             "feat_types=line\;pad\;surface\;arc\;text,".format(inner_top_signal_layer))
                    
                    self.GEN.CLEAR_LAYER()
                    self.GEN.AFFECTED_LAYER(inner_top_signal_layer+"_tmp",'yes')
                    self.GEN.SEL_COPY(inner_top_signal_layer)                    
                    
                if len(inner_signal_layers) > 2:                    
                    inner_bot_signal_layer = sorted(inner_signal_layers, key=lambda x: int(x[1:]) * -1)[0]
                    self.GEN.CLEAR_LAYER()
                    self.GEN.AFFECTED_LAYER('l{0}'.format(self.layer_num),'yes')
                    self.GEN.FILTER_RESET()
                    self.GEN.FILTER_SET_TYP('pad;line')
                    self.GEN.FILTER_SELECT()
                    if int(self.GEN.GET_SELECT_COUNT()) > 0:
                        self.GEN.SEL_COPY(inner_bot_signal_layer+"_tmp")
                        self.GEN.FILTER_RESET()
                        self.GEN.FILTER_SET_TYP('pad;line')
                        self.GEN.COM('filter_set,filter_name=popup,update_popup=no,exclude_syms=r762')
                        self.GEN.FILTER_SELECT()
                        if int(self.GEN.GET_SELECT_COUNT()) > 0:
                            self.GEN.SEL_DELETE()
                        
                        self.GEN.CLEAR_LAYER()
                        self.GEN.AFFECTED_LAYER(inner_bot_signal_layer+"_tmp",'yes')
                        self.GEN.FILTER_RESET()
                        ...
                        self.GEN.CLEAR_LAYER()
                        self.GEN.AFFECTED_LAYER(inner_bot_signal_layer,'yes')                        
                        self.GEN.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                                 "area=reference,area_type=rectangle,inout=inside,"
                                 "contour_cut=no,margin=254,ref_layer={0}_tmp,"
                                 "feat_types=line\;pad\;surface\;arc\;text,".format(inner_bot_signal_layer))
                        
                        self.GEN.CLEAR_LAYER()
                        self.GEN.AFFECTED_LAYER(inner_bot_signal_layer+"_tmp",'yes')
                        self.GEN.SEL_COPY(inner_bot_signal_layer)                     
        ...
        #埋孔对应的线路层也要加pad 20230803 by lyh
        if getattr(self, "mk_layer", None) is not None:            
            top_mai_sig_layer,bot_mai_sig_layer =  get_drill_start_end_layers(matrixInfo, self.mk_layer)
            self.GEN.CLEAR_LAYER()
            self.GEN.WORK_LAYER("mk")
            self.GEN.SEL_CHANEG_SYM('r762')
            self.GEN.SEL_COPY(top_mai_sig_layer)
            self.GEN.SEL_COPY(bot_mai_sig_layer)        
            self.GEN.CLEAR_LAYER()
        ...
        if is_inner_sigle_laser:
            ...
            self.GEN.FILTER_SET_TYP('pad;line')
            self.GEN.FILTER_SET_INCLUDE_SYMS("s1524")
            self.GEN.SEL_REF_FEAT(self.drlLay, "disjoint")
            if int(self.GEN.GET_SELECT_COUNT()) > 0:
                self.GEN.DELETE_LAYER("hole_tmp")
                self.GEN.SEL_COPY("hole_tmp")
                self.GEN.CLEAR_LAYER()
                self.GEN.AFFECTED_LAYER('hole_tmp','yes')
                self.GEN.SEL_CHANEG_SYM('r%s' % self.allDrlInfo[self.drlLay].get('min_pth', 452))
                self.GEN.COM('cur_atr_reset')
                self.GEN.COM('cur_atr_set,attribute=.drill,option=plated')
                self.GEN.COM('sel_change_atr,mode=add')
                self.GEN.COM('cur_atr_reset')                
                self.GEN.SEL_COPY(self.drlLay)
                
                for inum in range(2, self.layer_num):
                    self.GEN.CLEAR_LAYER()
                    self.GEN.AFFECTED_LAYER('l{0}'.format(inum),'yes')            
                    self.GEN.COM("clip_area_end,layers_mode=layer_name,layer={0},"
                             "area=reference,area_type=rectangle,inout=inside,"
                             "contour_cut=no,margin={1},ref_layer=hole_tmp,"
                             "feat_types=surface".format('l{0}'.format(inum), 15*25.4))
                self.GEN.DELETE_LAYER("hole_tmp")                
                
            self.GEN.CLEAR_LAYER()
            self.GEN.AFFECTED_LAYER(self.drlLay,'yes')
            self.GEN.FILTER_RESET()
            self.GEN.FILTER_SET_INCLUDE_SYMS('r%s' % self.allDrlInfo[self.drlLay].get('min_pth', 452))
            self.GEN.FILTER_SELECT()
            if int(self.GEN.GET_SELECT_COUNT()) > 0:
                self.GEN.SEL_COPY("hole_tmp")
                self.GEN.CLEAR_LAYER()
                self.GEN.AFFECTED_LAYER('hole_tmp','yes')
                self.GEN.SEL_CHANEG_SYM('r762')
                self.GEN.SEL_COPY("l1")
                self.GEN.SEL_COPY("l{0}".format(self.layer_num))
                self.GEN.DELETE_LAYER("hole_tmp") 
            
        self.GEN.CLEAR_LAYER()

    def reUseBackupCoupon(self, des_coupon):
        """
        ...
        # 拼入新跑完的hctcoupon
        self.GEN.COM("sr_tab_add, line=1, step=%s, x=0, y=0, nx=1, ny=1" % des_coupon)
        # 打散拼版，重建profile
        self.GEN.COM("sredit_reduce_nesting, mode = one_highest")
        self.GEN.COM('profile_rect,x1=0,y1=0,x2=%s,y2=%s' % (prof_xmax, prof_ymax))
        # 删除跑好的hct-coupon，重命名后旧的hct-coupon__backup__
        self.GEN.DELETE_ENTITY('step', des_coupon, job=self.job_name)
        self.GEN.COM('rename_entity,job=%s,name=%s,new_name=%s,is_fw=no,type=step,fw_type=form' %
                     (self.job_name, self.bakupStepName, des_coupon))
