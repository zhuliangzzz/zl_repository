...
class MAIN(object):
    ...
    def get_raoshao_laser_hole(self, drill_layer):
        ...
        if self.GEN.STEP_EXISTS(job=self.job_name, step="edit") == 'yes':
            ...
            edit_step.resetFilter()
            edit_step.selectAll()
            count = edit_step.featureSelected()

            edit_step.copyLayer(gjob.name, edit_step.name, drill_layer + "_tmp1", drill_layer + "_tmp3")
            edit_step.clearAll()
            edit_step.affect(drill_layer + "_tmp3")            
            
            for index in range(1, count):
                edit_step.VOF()
                edit_step.COM("sel_layer_feat,operation=select,layer={0},index={1}".format(drill_layer + "_tmp3", index))                
                edit_step.VON()
            ...
            if edit_step.featureSelected() != 1:
                edit_step.removeLayer(drill_layer + "_tmp1")
                edit_step.removeLayer(drill_layer + "_tmp2")
                edit_step.removeLayer(drill_layer + "_tmp3")
                return None
            ...
            edit_step.resetFilter()
            edit_step.refSelectFilter(drill_layer + "_tmp3")
            if edit_step.featureSelected():
