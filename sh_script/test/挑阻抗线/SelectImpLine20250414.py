...
class SelectImpLine(QMainWindow, ui.Ui_MainWindow):
    ...
    def run(self):
        ...
        try:
            ...
            if return_code == -1:
                return
            self.statusbar.showMessage(u'挑阻抗线完成（特性:_tx_；差分:_cd_)，请检查~')
    ...
    def getImpInfo(self, step):
        ...
        for worklayer in signal_layers:
            ...
            for orig_width, array_ref_layers in dic_ref_layer.iteritems():
                ...
                step.clearSel()
                if ids:
                    tx_layer = worklayer + "_tx_{0}".format(round(orig_width, 2))
            ...
            for orig_width, array_ref_layers in dic2_ref_layer.iteritems():
                ...
                step.clearSel()
                if ids:
                    cd_layer = worklayer + "_cd_{0}".format(round(orig_width, 2))
