...
class SelectImpLine(QMainWindow, ui.Ui_MainWindow):
    ...
    def render(self):
        ...
        self.lineEdit_step.setText('net')
    ...
    def run(self):
        ...
        try:
            ...
            if not self.step:
                ...
                return
            # 20250418 需要复制到edit，先判断是否有edit
            if 'edit' not in job.stepsList:
                QMessageBox.warning(self, 'error', u'阻抗线需要从net复制到edit，未检测到edit!')
                return
            self.editStep = gen.Step(job, 'edit')
        ...
        for worklayer in signal_layers:
            ...
            for orig_width, array_ref_layers in dic_ref_layer.iteritems():
                ...
                if ids:
                    ...
                    # 20250611 加属性
                    step.unaffectAll()
                    step.affect(tx_layer)
                    step.addAttr('.imp_line', attrVal='')
                    self.editStep.open()
                    self.editStep.copyLayer(jobname, step.name, tx_layer, tx_layer)
                    step.open()
                step.unaffectAll()
            ...
            for orig_width, array_ref_layers in dic2_ref_layer.iteritems():
                ...
                if step.isLayer(cd_layer):
                    self.editStep.open()
                    self.editStep.copyLayer(jobname, step.name, cd_layer, cd_layer)
                    step.open()
