...
class TemporaryLayerComparsion(QWidget, ui.Ui_Form):
    ...
    def run(self):
        ...
        for layer in layers:
            ...
            if step.Selected_count():
                ...
                for feat in feat_out:
                    ...
                    for week_text in ["$$yy", "$${yy}", "$$ww",
                                      ...
                                      "$${mm}", "$$dd", "$${dd}","$$year",
                                      "$${year}"]:
                        if week_text in text.lower():
                            index_list.append(index)
                            break
                if "ea5706gb123e4" in jobname:
                    step.PAUSE(str([index_list]))
                if index_list:
            ...
            if areas and step.isLayer(area_layer):
            ...
            if step.Selected_count():
                ...
                step.removeLayer(map_layer)
            step.unaffectAll()
            if areas:step.removeLayer(area_layer)
