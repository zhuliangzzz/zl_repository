...
class TemporaryLayerComparsion(QWidget, ui.Ui_Form):
    ...
    def run(self):
        ...
        for layer in layers:
            ...
            # 获取区域
            areas = []
            layer_cmd = gen.Layer(step, tmp_layer)
            ...
            if step.Selected_count():
                ...
                for obj in feat_out:
                    step.clearSel()
                    step.selectByIndex(tmp_layer, obj.feat_index)
                    xmin, ymin, xmax, ymax = get_layer_selected_limits(step, tmp_layer)
            ...
            step.resetFilter()
            if step.Selected_count():
                feat_out = step.Features_INFO(tmp_layer, mode='feat_index+select')
                index, text = feat_out[0], feat_out[11]
                index_list = []
                if text in ["$$yy", "$${yy}", "$$ww", "$${ww}",
                                             "$$month", "$${month}", "$$day", "$${day}",
                                             "$$banci", "$${banci}", "$$xingqi", "$${xingqi}",
                                             "$$picihao", "$${picihao}", "$$mm", "$${mm}",
                                             "$$dd", "$${dd}","$$year", "$${year}"]:
                    index_list.append(index)
                if index_list:
                    for feat_index in index_list:
                        step.clearSel()
                        step.selectByIndex(tmp_layer, feat_index)
                        xmin, ymin, xmax, ymax = get_layer_selected_limits(step, tmp_layer)
                        areas.append([xmin - 0.5, ymin - 0.5, xmax + 0.5, ymax + 0.5])
                        step.clearSel()
            step.unaffectAll()
