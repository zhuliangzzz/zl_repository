...
class AddGoldPlatedDryFilm(QtGui.QWidget, ui.Ui_Form):
    ...
    def make(self):
        ...
        drl_slot = 'drl_slot' # 槽孔层
        step.removeLayer(drl_slot)
        if self.comboBox.currentText() == u'无蚀刻引线':
            ...
            for xlym, signal in xlym_list.items():
                ...
                if self.checkBox.isChecked():
                    ...
                    step.refSelectFilter(tmp_el, mode='disjoint')
                    step.resetFilter()
                    if step.Selected_count():
                ...
                # step.PAUSE('D')
                if step.isLayer(drl_slot):
                    step.affect(drl_slot)
        ...
        if cur_stepname == 'panel':
            ...
            if not repeatxmin:
                QtGui.QMessageBox.warning(self, u'警告', 'panel中没有set!')
                sys.exit()
            ...
            for xlym, signal in xlym_list.items():
                ...
                step.addRectangle(set_xmin - 1.5, set_ymin - 1.5, set_xmax + 1.5, set_ymax + 1.5, polarity='negative')
                step.Contourize(0)
                step.unaffectAll()
                if step.isLayer(drl_slot):
                    step.Flatten(drl_slot, drl_slot + '+++')
                    step.affect(drl_slot + '+++')
                    step.refSelectFilter(tmp_linek_area)
                    if step.Selected_count():
                        step.selectReverse()
                        if step.Selected_count():
                            step.selectDelete()
                    ...
                    step.unaffectAll()
                step.affect('drl')
                step.copyToLayer(linek, 'yes', size=762)
                step.unaffectAll()
                step.affect(signal)
                step.selectSymbol('sh-dw*2013;sh-dw*2014;')
                step.resetFilter()
                if step.Selected_count():
                    step.removeLayer(signal + '+++')
                    step.copySel(signal + '+++')
                    step.unaffectAll()
                    step.affect(signal + '+++')
                    step.selectChange('s4512')
                    step.selectPolarity()
                    step.copySel(linek, 'yes')
                    step.unaffectAll()
                    step.removeLayer(signal + '+++')
                step.unaffectAll()
                tmp_drl = 'drl+++tmp'
                step.removeLayer(tmp_drl)
                step.Flatten('drl', tmp_drl)
                step.affect(tmp_drl)
                # step.setAttrFilter('.string = dw_hole ; .drill = non_plated')
                step.COM('set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=yes,attribute=.string,min_int_val=0,max_int_val=0,min_float_val=0,max_float_val=0,option=,text=dw_hole')
                step.COM('set_filter_attributes,filter_name=popup,exclude_attributes=no,condition=yes,attribute=.drill,min_int_val=0,max_int_val=0,min_float_val=0,max_float_val=0,option=non_plated,text=')
                ...
                if step.Selected_count():
                    ...
                    if step.Selected_count():
                        step.selectDelete()
                    step.selectRectangle(set_xmin - 1.5, set_ymin - 1.5, set_xmax + 1.5, set_ymax + 1.5)
