...
class Export_Job(QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        self.comboBox_step.addItems(mode)
        ...
        self.setStyleSheet(
            ...
            'QToolTip{background-color:#cecece;}#pushButton_ok:hover,#pushButton_cancel:hover{background:#882F09;}QMessageBox{font:12pt;}')
    ...
    def export(self):
        if self.output_type == 'full':
            job.COM('export_job,job=%s,path=%s,mode=tar_gzip,submode=full,overwrite=yes' % (jobname, export_path))
        else:
            ...
            if lyrs:
                ...
                if err_lyr:
                    ...
                    return
            if steps:
                err_step = filter(lambda x: x not in job.stepsList, steps.split(';'))
                if err_step:
            ...
            job.COM('export_job,job=%s,path=%s,mode=tar_gzip,submode=partial,steps_mode=%s,steps=%s,lyrs_mode=%s,lyrs=%s,overwrite=yes'
                    % (jobname, export_path, steps_mode, steps, layer_mode, lyrs))
        sys.exit()
