...
class SelectImpLine(QMainWindow, ui.Ui_MainWindow):
    ...
    def run(self):
        ...
        select_confirm = 'no' if self.radioButton_confirm_no.isChecked() else 'yes'
        delete_imp_tmp = 'no' if self.radioButton_delete_imp_no.isChecked() else 'yes'
        # stepfilter
        ...
        try:
            ...
            if 'auto_comp' in self.result_mode_items.get(result_mode):
                for lyr_imp in imp_layers:
                    ...
                    ans = self.compensate_imp(lyr_imp)
                    if ans:
                        return ans
        ...
        for line_width in sorted(imp_lines.keys()):
            ...
            for imp in imp_lines.get(line_width).values():
                ...
                if re.match('diff', imp.get('type'), re.I):
                    ...
                    polylines = layer_obj.featout_dic_Index()['lines']
                    self.statusbar.showMessage('正在通过计算查找线宽:%s mil,间距:%s mil的差分阻抗' % (line_width, imp.get("diff_spacing")))
                    # my $record;
                    # my @tmp_record;
                    # my $index = 1;
                            ...
                            for mea_line in spacing_length.get('spacing').values():
                                ...
                                if lower <= imp_width <= upper:
                                    ...
                                    for line in polylines:
                                        ...
                                        l_centerx = line.xs + line.xe / 2
                                        l_centery = line.ys + line.ye / 2
                                        if ((centerx < line.xs and centerx < line.xe) or (centerx > line.xs and centerx > line.xe)) \
                                            and ((centery < line.ys and centery < line.ye) or (centery > line.ys and centery > line.ye)):
                                            continue
                    ...
                    for key in record.keys():
                        ...
                        for line in polylines.get(polyline1)
                            pass
    ...
    def point2line_dist(self, x, y, line):
        # 如果线段长度为0，则无法计算垂直距离
        xs, ys, xe, ye = line.xs, line.ys, line.xe, line.ye
        if xs == xe and ys == ye:
            return None
