...
class AddDcouponSerialNumber(QtGui.QWidget, ui.Ui_Form):
    ...
    def render(self):
        all_steps = get_panelset_sr_step(jobname, "panel", is_group_by=False)
        d_coupons = list(filter(lambda step: 'd-coupon' in step or 'dcoupon' in step, set(all_steps)))
    ...
    def run(self):
        ...
        if not number_mirror:
            ...
            return
        # 检测coupons是否有01冲突的命名
        self.d_coupons_01 = filter(lambda coupon: re.search('-01$|-01\D|-1$|-1\D', coupon), coupons)
        err_d_coupon = []
        ...
        if err_d_coupon:
            ...
            d_coupons_dict = {}
            for d_coupon in self.d_coupons_01:
            ...
            for v in coupons:
                if list(d_coupons_dict.keys()):
                    ...
                    # print(v, keylist)
                    if len(keylist) > 1:
                        QtGui.QMessageBox.warning(self, u'检测到有%s命名的d-coupon命名冲突，无法分组' % u'、'.join(keylist))
                        return
                    if keylist:
        ...
        if len(xmin_set) > 1 or len(ymin_set) > 1 or len(xmax_set) > 1 or len(ymax_set) > 1:
            ...
            for coupon in coupons[1:]:
                self.editAddText(layer, font_width, font_height, line_width, angle,
                                 ...
                                 coupon, number_postion, number_mirror)
        # coupon_center_layer
        panel_step.removeLayer("dcoupon_center+++")
        ...
        panel_step.createLayer('dcoupon_center+++')
        ...
        for stepname in all_steps:
            ...
            step.initStep()
            if stepname in coupons:
                step.prof_to_rout('dcoupon_center+++')
                step.affect('dcoupon_center+++')
                step.selectCutData()  # 填充coupon
                step.selectReverse()
                step.addAttr('.string', attrVal='text=%s' % stepname)
                step.unaffectAll()
                step.removeLayer('dcoupon_center++++++')
            step.COM("profile_to_rout,layer=draw_set_profile,width=10")
        panel_step.initStep()
        panel_step.Flatten("draw_set_profile", "draw_set_profile_tmp")
        panel_step.Flatten("dcoupon_center+++", "dcoupon_center+++tmp")
        # panel_step.PAUSE('A')
        panel_step.clearAll()
        # 20250122 在每个d-coupon中心加个标识
        label_text_list, infos = [], []
        layercmd = gen.Layer(panel_step, 'dcoupon_center+++tmp')
        feats = panel_step.INFO('-t layer -e %s/%s/%s -d FEATURES -o feat_index' % (jobname, panel_step.name, layercmd.name))
        del feats[0]
        ids = []
        for feat in feats:
            feat = feat.strip()
        ...
        panel_step.affect(layercmd.name)
        for id in ids:
            panel_step.selectByIndex(layercmd.name, id)
            xmin, ymin, xmax, ymax = get_layer_selected_limits(panel_step, layercmd.name)
            feats = panel_step.INFO('-t layer -e %s/%s/%s -d FEATURES -o select+feat_index' % (jobname, panel_step.name, layercmd.name))
            atr = ''
            del feats[0]
            for feat in feats:
                feat = feat.strip()
                if feat:
                    if re.match('#\d+', feat):
                        atr = feat.split(';')[1].split('=')[1]
                        break
            infos.append((atr, xmin, ymin, xmax, ymax))
            panel_step.clearSel()
        ...
        if panel_step.name == "set":
            ...
            for info in infos:
                ...
                arraylist_text_info.append((tuple(indexes), round(((xs + xe) * 0.5 - xmin) * x_resize, 0),
                                            round(((ys + ye) * 0.5 - ymax) * y_resize, 0), atr, label))
            # for i in xrange(len(gREPEATstep)):
            #     if gREPEATstep[i] in coupons:
            #         xs = gREPEATxmin[i]
            #         ys = gREPEATymin[i]
...
class show_set_pnl_order(QtGui.QDialog):
    ...
    def set_ui_widget(self):
        ...
        self.conform_btn.setGeometry(QtCore.QRect(self.width - 180, self.height - 40, 80, 40))
