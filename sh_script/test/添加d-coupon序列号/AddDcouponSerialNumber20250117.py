    ...
    def set_ui_widget(self):
        ...
        for key in sorted(dic_text_info.keys(), key=lambda x: x * -1):
            for index, x, y, coupon_name in dic_text_info[key]:
                ...
                # 带-1的序列号默认为1
                if coupon_name.endswith('-1'):
                    pos = self.dic_combox[index].findText(str(1), QtCore.Qt.MatchExactly)
                    self.dic_combox[index].setCurrentIndex(pos)
                # d-coupon name
                label = QtGui.QLabel(self)
                label.setGeometry(QtCore.QRect(x + 50, y * -1, len(coupon_name)*8, 30))
                # self.dic_combox[index].setObjectName("combox%s"%index)
                label.setText(coupon_name)
