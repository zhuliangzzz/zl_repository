...
class RoutInput(QWidget, ui.Ui_Form):
        ...
        for stepname in steplist:
            ...
            for layer in self.rout_layers:
                ...
                if self.step.isLayer(layer):
                    self.step.removeLayer(bk_layer)
                    self.step.createLayer(bk_layer, 'board', 'rout')
                    self.step.affect(layer)
                    self.step.copySel(bk_layer)
                    self.step.unaffectAll()
                    self.job.matrix.modifyRow(layer, context='board', type='rout')
                else:
                    self.step.createLayer(layer, 'board', 'rout')
    ...
    def update_lock_info(self):
        ...
        #     conn.SQL_EXECUTE(dbc_m, sql, (json.dumps(self.job_make_info), json.dumps(self.pre_lock)))
