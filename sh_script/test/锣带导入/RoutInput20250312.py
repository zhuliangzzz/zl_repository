...
class RoutInput(QWidget, ui.Ui_Form):
    ...
    def run(self):
        ...
        for layer in layers:
            ...
            x_mirror = "no"
            y_mirror = "no"
            # lines = str(file(rout_temp_filepath).readlines())
            # pattern = re.compile(';|DRL|BC.*|M48,.*')
            # lines = re.sub(pattern, '', lines)
            # pattern = re.compile('%\nM252(.*)C(.*)')
            # lines = re.sub(pattern, 'T21C\2\n%\nT21\n\1', lines)
            # pattern = re.compile('M252(.*)C(.*)')
            # lines = re.sub(pattern, '\1', lines)
            ...
            self.job.matrix.modifyRow(layername, type='rout')
