...
class RoutInput(QWidget, ui.Ui_Form):
    ...
    def render(self):
        ...
        if stepname:
            self.comboBox.setCurrentIndex(list(filter(lambda i: steps[i] == stepname, range(len(steps))))[0])
        self.label_title.setText('Job:%s' % self.jobname)
        self.lineEdit.setValidator(QIntValidator(self.lineEdit))
        self.lineEdit_2.setValidator(QIntValidator(self.lineEdit_2))
    ...
    def run(self):
        ...
        if not format1:
            QMessageBox.warning(None, u'警告', u'请输出fommat1！')
            return
        if not format2:
            QMessageBox.warning(None, u'警告', u'请输出fommat2！')
            return
        ...
        for layer in layers:
            ...
            self.step.COM(
                'input_manual_set,path=%s,job=%s,step=%s,format=Excellon2,data_type=ascii,units=mm,coordinates=absolute,zeroes=trailing,nf1=%s,nf2=%s,decimal=no,separator=nl,tool_units=mm,layer=%s,wheel=,wheel_template=,nf_comp=0,multiplier=1,text_line_width=0.0024,signed_coords=no,break_sr=yes,drill_only=no,merge_by_rule=no,threshold=200,resolution=3' % (
                    os.path.join(str(self.lineEdit_path.text()), layer), self.jobname, self.step.name, format1, format2, layer))
